...
suite('Notebook builtin output renderer', () => {
	...
	test('Append streaming output', async () => {
		...
		assert.ok(
			inserted.innerHTML.indexOf('>content</') === inserted.innerHTML.lastIndexOf('>content</'),
	...
	test(`Render with wordwrap and scrolling for error output`, async () => {
		...
		assert.ok(
			outputElement.classList.contains('word-wrap') && inserted.classList.contains('scrollable'),
			`output content classList should contain word-wrap and scrollable ${inserted.classList}`,
		)
		assert.ok(
			inserted.innerHTML.indexOf('>Expected type `str`, but received type') > -1,
			`Content was not added to output element:\n ${outputElement.innerHTML}`,
		)
		assert.ok(
			inserted.textContent!.indexOf("Expected type `str`, but received type `<class 'int'>`") > -1,
