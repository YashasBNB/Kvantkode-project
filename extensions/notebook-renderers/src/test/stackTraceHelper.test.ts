...
suite('StackTraceHelper', () => {
	...
	test('IPython stack line numbers are linkified', () => {
		...
		assert.ok(
			cleanStack.indexOf("<a href='vscode-notebook-cell:?execution_count=3&line=2'>2</a>") > 0,
	...
	test('IPython stack line numbers are linkified for IPython 8.3', () => {
		...
		assert.ok(
			formatted.indexOf("<a href='vscode-notebook-cell:?execution_count=2&line=5'>5</a>") > 0,
	...
	test('IPython stack without line numbers are not linkified', () => {
		...
		formattedLines
			.slice(1)
			.forEach((line) =>
				assert.ok(!/<a href=.*>/.test(line), 'line should not contain a link: ' + line),
			)
	})
})
