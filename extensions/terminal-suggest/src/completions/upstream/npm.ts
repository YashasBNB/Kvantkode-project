...
export const createNpmSearchHandler =
	...
	async (
		...
		const queryPackages = ['-s', '-H', 'Accept: application/json', queryPackagesUrl]
		...
		try {
			...
			return results.map((item: { package: { name: string; description: string } }) => ({
		...
		try {
			...
			if (scripts) {
				return Object.entries(scripts).map(([scriptName, scriptContents]) => {
					const icon = npmClient === 'yarn' ? 'fig://icon?type=yarn' : 'fig://icon?type=npm'
		...
		},
		{
			...
			options: [
				...workSpaceOptions,
				{
					...
					args: {
						name: 'audit',
						suggestions: ['info', 'low', 'moderate', 'high', 'critical', 'none'],
					},
				},
				{
					name: '--package-lock-only',
			...
			subcommands: [
				{
					...
					options: [{ name: ['-g', '--global'], description: 'Sets it globally' }],
				},
				{
					name: 'get',
				...
				},
				{
					...
					options: [{ name: '--global', description: 'Edits the global config' }],
				},
			],
		},
					...
					args: [
						{
							name: 'property',
							description: 'Property name',
							suggestions: ['email', 'fullname', 'homepage', 'freenode', 'twitter', 'github'],
						},
						{
							name: 'value',
