...
const postProcessTrackedFiles: Fig.Generator['postProcess'] = (out, context) => {
...
export const gitGenerators: Record<string, Fig.Generator> = {
			...
			if (tokens.includes('-r')) {
				return pp?.(
					(
						await executeShellCommand({
							command: 'git',
							args: ['--no-optional-locks', '-r', '--no-color', '--sort=-committerdate'],
						})
					).stdout,
					tokens,
				)
				...
				return pp?.(
					(
						await executeShellCommand({
							command: 'git',
							args: ['--no-optional-locks', 'branch', '--no-color', '--sort=-committerdate'],
						})
					).stdout,
					tokens,
				)
	...
	getStagedFiles: {
		script: ['bash', '-c', "git --no-optional-locks status --short | sed -ne '/^M /p' -e '/A /p'"],
	...
	getChangedTrackedFiles: {
		script: function (context) {
			if (context.includes('--staged') || context.includes('--cached')) {
				...
				return ['bash', '-c', `git --no-optional-locks status --short | sed -ne '/M /p' -e '/A /p'`]
			}
		},
		postProcess: postProcessTrackedFiles,
	},
	...
	subcommands: [
		...
		},
		{
			...
			args: {
				...
				suggestions: [{ name: '-', description: 'Shorthand for the previous branch' }],
			},
			options: [
				{
					name: '--commit',
