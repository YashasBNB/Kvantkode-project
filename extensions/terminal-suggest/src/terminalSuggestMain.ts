		...
		vscode.window.registerTerminalCompletionProvider(
			{
				...
				async provideTerminalCompletions(
					...
					const shellGlobals =
						(await getShellGlobals(terminalShellType, commandsInPath?.labels)) ?? []
		...
		for (const command of availableCommands) {
			...
			if (!labels.has(commandTextLabel)) {
				...
				const existingItem = items.find(
					(i) => (typeof i.label === 'string' ? i.label : i.label.label) === commandTextLabel,
				)
				if (!existingItem) {
					continue
				}
...
function compareItems(
	existingItem: vscode.TerminalCompletionItem,
	command: ICompletionResource,
): vscode.TerminalCompletionItem | undefined {
	...
	if (score > 0) {
		score -=
			typeof existingItem.label === 'object' ? (existingItem.label.detail !== undefined ? 1 : 0) : 0
		score -=
			typeof existingItem.label === 'object'
				? existingItem.label.description !== undefined
					? 2
					: 0
				: 0
		score -= existingItem.documentation
			? typeof existingItem.documentation === 'string'
				? 2
				: 3
			: 0
		if (score >= 0) {
			return {
				...command,
				replacementIndex: existingItem.replacementIndex,
				replacementLength: existingItem.replacementLength,
			}
		}
	}
}
