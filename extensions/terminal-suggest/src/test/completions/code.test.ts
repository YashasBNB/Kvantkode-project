...
export function createCodeTestSpecs(executable: string): ITestSpec[] {
		...
		const expectedCompletions = [
			{
				...
				description:
					executable === codeCompletionSpec.name
						? (codeCompletionSpec as any).description
						: (codeInsidersCompletionSpec as any).description,
			},
		]
	...
	return [
		...
		},
		{
			...
			expectedCompletions: codeSpecOptionsAndSubcommands.filter((c) => c !== '--list-extensions'),
			expectedResourceRequests: { type: 'both', cwd: testPaths.cwd },
		},
		{
			input: `${executable} --show-versions |`,
			expectedCompletions: codeSpecOptionsAndSubcommands.filter((c) => c !== '--show-versions'),
			expectedResourceRequests: { type: 'both', cwd: testPaths.cwd },
		},
