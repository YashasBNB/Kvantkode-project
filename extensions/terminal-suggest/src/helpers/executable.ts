...
export function isExecutable(
	filePath: string,
	configuredWindowsExecutableExtensions?: { [key: string]: boolean | undefined } | undefined,
): Promise<boolean> | boolean {
	if (osIsWindows()) {
		const resolvedWindowsExecutableExtensions = resolveWindowsExecutableExtensions(
...
function resolveWindowsExecutableExtensions(configuredWindowsExecutableExtensions?: {
	...
	return Array.from(new Set(resolvedWindowsExecutableExtensions)).filter(
