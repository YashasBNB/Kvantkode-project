...
suite('HTML Embedded Formatting', () => {
	async function assertFormat(
		...
		const result = await format(languageModes, document, range, formatOptions, undefined, {
			css: true,
			javascript: true,
		})
	...
	async function assertFormatWithFixture(
		...
		const input = fs
			.readFileSync(
				path.join(__dirname, '..', '..', 'src', 'test', 'fixtures', 'inputs', fixtureName),
			)
			.toString()
			.replace(/\r\n/gm, '\n')
		const expected = fs
			.readFileSync(
				path.join(__dirname, '..', '..', 'src', 'test', 'fixtures', 'expected', expectedPath),
			)
			.toString()
			.replace(/\r\n/gm, '\n')
