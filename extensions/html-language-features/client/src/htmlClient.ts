...
async function startClientWithParticipants(
	...
	toDispose.push(
		workspace.onDidChangeConfiguration(
			(e) => e.affectsConfiguration(SettingIds.formatEnable) && updateFormatterRegistration(),
		),
	)
	...
	toDispose.push(
		languages.registerCompletionItemProvider(documentSelector, {
			provideCompletionItems(doc, pos) {
				...
				if (
					match2 &&
					doc.getText(new Range(new Position(0, 0), pos)).match(htmlSnippetCompletionRegExpr)
				) {
