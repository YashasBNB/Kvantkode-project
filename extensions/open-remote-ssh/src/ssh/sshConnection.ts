...
export default class SSHConnection extends EventEmitter {
	...
	getSocksPort(localPort: number): Promise<number> {
		return this.addTunnel({ name: '__socksServer', socks: true, localPort: localPort }).then(
			(tunnel) => {
	...
	addTunnel(SSHTunnelConfig: SSHTunnelConfig): Promise<SSHTunnelConfig & { server: Server }> {
		...
		this.emit(SSHConstants.CHANNEL.TUNNEL, SSHConstants.STATUS.BEFORECONNECT, {
			...
			return new Promise((resolve, reject) => {
				...
				if (SSHTunnelConfig.socks) {
					server = createSocksServer({
						connectionFilter: (
							destination: SocksConnectionInfo,
				...
				server
					.on('listening', () => {
						...
						this.activeTunnels[SSHTunnelConfig.name!] = Object.assign(
							{},
							{ server },
							SSHTunnelConfig,
						)
