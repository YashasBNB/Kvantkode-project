...
export class RemoteSSHResolver implements vscode.RemoteAuthorityResolver, vscode.Disposable {
	...
	resolve(
		...
		this.logger.info(
			`Resolving ssh remote authority '${authority}' (attemp #${context.resolveAttempt})`,
		)
		...
		return vscode.window.withProgress(
			...
			async () => {
				try {
					...
					const agentForward =
						enableAgentForwarding && (sshHostConfig['ForwardAgent'] || 'no').toLowerCase() === 'yes'
					const agent =
						agentForward && this.sshAgentSock ? new ssh2.OpenSSHAgent(this.sshAgentSock) : undefined
					...
					if (sshHostConfig['ProxyJump']) {
						...
						for (let i = 0; i < proxyJumps.length; i++) {
							...
							const proxyAgentForward =
								...
								(proxyHostConfig['ForwardAgent'] || 'no').toLowerCase() === 'yes'
							const proxyAgent =
							...
							const proxyIdentitiesOnly =
								(proxyHostConfig['IdentitiesOnly'] || 'no').toLowerCase() === 'yes'
							...
							const destPort = nextProxyJump
								? (nextProxyJump[1]['Port'] && parseInt(nextProxyJump[1]['Port'], 10)) ||
						...
						let proxyArgs = (sshHostConfig['ProxyCommand'] as unknown as string[]).map((arg) =>
							arg
					...
					this.labelFormatterDisposable = vscode.workspace.registerResourceLabelFormatter({
						...
						formatting: {
							...
							workspaceSuffix:
								...
								(sshDest.port && sshDest.port !== 22 ? `:${sshDest.port}` : ''),
						},
					})
	...
	private async openTunnel(localPort: number, remotePortOrSocketPath: number | string) {
		...
		const remotePort =
			typeof remotePortOrSocketPath === 'number' ? remotePortOrSocketPath : undefined
		const remoteSocketPath =
			typeof remotePortOrSocketPath === 'string' ? remotePortOrSocketPath : undefined
