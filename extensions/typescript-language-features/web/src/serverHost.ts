...
function createServerHost(
	...
	// Internals
	const combinePaths: (path: string, ...paths: (string | undefined)[]) => string = (ts as any)
		.combinePaths
	...
	const getExecutingDirectoryPath = memoize(() =>
		memoize(() => ensureTrailingDirectorySeparator(getDirectoryPath(executingFilePath))),
	)
	...
	return {
		...
		importPlugin: async (root, moduleName) => {
			...
			try {
				...
				return {
					module: undefined,
					error: new Error(`Could not load plugin. Could not load 'package.json'.`),
				}
			}
			...
			if (!browser) {
				return {
					module: undefined,
					error: new Error(`Could not load plugin. No 'browser' field found in package.json.`),
				}
			}
		...
		writeFile(path, data, writeByteOrderMark) {
			...
			try {
				...
				if (
					...
					(name === 'package.json' || name === 'package-lock.json' || name === 'package-lock.kdl')
				) {
...
export async function createSys(
	...
	if (hasArgument(args, '--enableProjectWideIntelliSenseOnWeb')) {
		...
		return {
			sys: createServerHost(ts, logger, undefined, args, watchManager, pathMapper, false, onExit),
		}
	}
}
