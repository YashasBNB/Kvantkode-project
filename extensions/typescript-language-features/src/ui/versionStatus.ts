...
export class VersionStatus extends Disposable {
	...
	constructor(private readonly _client: ITypeScriptServiceClient) {
		...
		this._statusItem = this._register(
			vscode.languages.createLanguageStatusItem('typescript.version', jsTsLanguageModes),
		)
		...
		this._register(
			this._client.onTsServerStarted(({ version }) => this.onDidChangeTypeScriptVersion(version)),
		)
	}
