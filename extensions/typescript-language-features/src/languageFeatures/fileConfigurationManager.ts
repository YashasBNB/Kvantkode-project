...
export default class FileConfigurationManager extends Disposable {
	...
	private getFormattingOptions(document: vscode.TextDocument): FormattingOptions | undefined {
		...
		return {
			...
			insertSpaces:
				typeof editor.options.insertSpaces === 'boolean' ? editor.options.insertSpaces : undefined,
		}
	}
	...
	private getPreferences(document: vscode.TextDocument): Proto.UserPreferences {
		...
		const preferences: Proto.UserPreferences = {
			...
			autoImportFileExcludePatterns: this.getAutoImportFileExcludePatternsPreference(
				preferencesConfig,
			...
			config.get<string[]>('autoImportFileExcludePatterns')?.map((p) => {
				...
				return path.isAbsolute(p)
					...
					: p.startsWith('*')
						? wildcardPrefix + p
						: isRelative
							? this.client.toTsFilePath(vscode.Uri.joinPath(workspaceFolder, p))!
	...
	private getOrganizeImportsPreferences(
		...
		const organizeImportsCaseSensitivity = config.get<'auto' | 'caseInsensitive' | 'caseSensitive'>(
						...
						organizeImportsCaseFirst:
							organizeImportsCaseSensitivity === 'caseInsensitive'
								...
								: withDefaultAsUndefined(
										config.get<'default' | 'upper' | 'lower' | false>(
