	...
	public async execute(args: SelectRefactorCommand.Args): Promise<void> {
		...
		const selected = await vscode.window.showQuickPick(
			args.refactor.actions.map(
				(action): vscode.QuickPickItem & { action: Proto.RefactorActionInfo } => ({
					action,
					label: action.name,
...
class TypeScriptRefactorProvider implements vscode.CodeActionProvider<TsCodeAction> {
					...
					if (
						...
						if (
							...
							if (
								...
								!navTree.body ||
								!TypeScriptRefactorProvider.isOnSignatureName(navTree.body, rangeOrSelection)
							) {
								return undefined
							}
						}
	...
	private refactorActionToCodeActions(
		...
		if (action.name === 'Move to file') {
			codeActions.push(
				new MoveToFileCodeAction(document, action, rangeOrSelection, context.triggerKind),
			)
		} else {
			codeActions.push(
	...
	private pruneInvalidActions(
		...
		for (const action of actions) {
			...
			if (
				action.kind &&
				(Extract_Constant.kind.contains(action.kind) || Extract_Function.kind.contains(action.kind))
			) {
