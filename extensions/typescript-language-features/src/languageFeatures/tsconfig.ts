...
class TsconfigLinkProvider implements vscode.DocumentLinkProvider {
	...
	private isPathValue(node: jsonc.Node | undefined): node is jsonc.Node {
		return node && node.type === 'string' && node.value && !(node.value as string).includes('*') // don't treat globs as links.
	}
...
async function getTsconfigPath(
	...
	return resolveNodeModulesPath(baseDirUri, [
		pathValue,
		...(pathValue.endsWith('.json') ? [] : [`${pathValue}.json`, `${pathValue}/tsconfig.json`]),
	])
}
...
export function register() {
	...
	const selector: vscode.DocumentSelector = languages
		.map((language) => patterns.map((pattern): vscode.DocumentFilter => ({ language, pattern })))
		.flat()

	return vscode.Disposable.from(
		vscode.commands.registerCommand(
			openExtendsLinkCommandId,
			async ({ resourceUri, extendsValue, linkType }: OpenExtendsLinkCommandArgs) => {
