...
export async function activate(context: vscode.ExtensionContext): Promise<Api> {
	...
	context.subscriptions.push(
		lazilyActivateClient(lazyClientHost, pluginManager, activeJsTsEditorTracker, async () => {
...
class RemoteWorkspaceContentsPreloader extends Disposable {
	...
	constructor(
		...
		const fsWatcher = this._register(
			vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(workspaceUri, '*')),
		)
		this._register(
