	...
	public collectPropertySuggestions(
		...
		if (location.matches(['dependencies']) || location.matches(['devDependencies'])) {
			if (currentWord.length > 0) {
				const queryUrl =
					'https://registry.bower.io/packages/search/' + encodeURIComponent(currentWord)
				...
				return this.xhr({
					...
					(success) => {
						if (success.status === 200) {
							...
							collector.error(
								l10n.t('Request to the bower repository failed: {0}', success.responseText),
							)
							return 0
						}
						return undefined
					...
					(error) => {
						collector.error(
							l10n.t('Request to the bower repository failed: {0}', error.responseText),
						)
						return 0
					},
				)
