...
export function getCellMetadata(
	options: { cell: NotebookCell | NotebookCellData } | { metadata?: { [key: string]: any } },
): CellMetadata {
	if ('cell' in options) {
	...
	switch (outputType) {
		...
		default: {
			...
			const isStream = output.items.every(
				(item) =>
					item.mime === CellOutputMimeTypes.stderr || item.mime === CellOutputMimeTypes.stdout,
			)
...
function convertStreamOutput(output: NotebookCellOutput): JupyterOutput {
	...
	output.items
		.filter(
			(opit) =>
				opit.mime === CellOutputMimeTypes.stderr || opit.mime === CellOutputMimeTypes.stdout,
		)
