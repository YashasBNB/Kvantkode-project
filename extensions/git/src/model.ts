...
class RepositoryPick implements QuickPickItem {
	...
	@memoize get description(): string {
		return [this.repository.headLabel, this.repository.syncLabel].filter((l) => !!l).join(' ')
	}
		...
		ISourceControlHistoryItemDetailsProviderRegistry
{
	...
	private async doInitialScan(): Promise<void> {
		...
		const autoRepositoryDetection = config.get<boolean | 'subFolders' | 'openEditors'>(
		...
		if (this.parentRepositories.length !== 0 && parentRepositoryConfig === 'prompt') {
			...
			await Promise.all(
				(workspace.workspaceFolders || []).map(async (folder) => {
					...
					const repositoryScanMaxDepth = (
						workspace.isTrusted ? workspace.getConfiguration('git', folder.uri) : config
					...
					const scanPaths =
						(workspace.isTrusted ? workspace.getConfiguration('git', folder.uri) : config).get<
							string[]
						>('scanRepositories') || []
					this.logger.trace(
	...
	private onDidChangeConfiguration(): void {
		const possibleRepositoryFolders = (workspace.workspaceFolders || [])
			.filter(
				(folder) => workspace.getConfiguration('git', folder.uri).get<boolean>('enabled') === true,
			)
		...
		const openRepositoriesToDispose = this.openRepositories
			...
			.filter(
				({ root }) => workspace.getConfiguration('git', root).get<boolean>('enabled') !== true,
			)
	...
	private onDidChangeActiveTextEditor(): void {
		...
		const indexResource = repository.indexGroup.resourceStates.find((resource) =>
			pathEquals(resource.resourceUri.fsPath, textEditor.document.uri.fsPath),
		)
		const workingTreeResource = repository.workingTreeGroup.resourceStates.find((resource) =>
			pathEquals(resource.resourceUri.fsPath, textEditor.document.uri.fsPath),
		)
	...
	async openRepository(repoPath: string, openIfClosed = false): Promise<void> {
		...
		try {
			...
			if (
				...
				this.globalState.get<boolean>(`parentRepository:${repositoryRoot}`) !== true
			) {
			...
			const repository = new Repository(
				this.git.open(repositoryRoot, repositoryRootRealPath, dotGit, this.logger),
				this,
				this,
				this,
				this,
	...
	private async getRepositoryRoot(
		repoPath: string,
	): Promise<{ repositoryRoot: string; unsafeRepositoryMatch: RegExpMatchArray | null }> {
		try {
	...
	private async getRepositoryExact(repoPath: string): Promise<Repository | undefined> {
		...
		try {
			...
			const openRepositoryRealPath = this.openRepositories.find((r) =>
				pathEquals(r.repository.rootRealPath ?? r.repository.root, repoPathRealPath),
			)
	...
	registerBranchProtectionProvider(root: Uri, provider: BranchProtectionProvider): Disposable {
		...
		this.branchProtectionProviders.set(
			root.toString(),
			(this.branchProtectionProviders.get(root.toString()) ?? new Set()).add(provider),
		)
	...
	private async isRepositoryOutsideWorkspace(repositoryPath: string): Promise<boolean> {
		...
		const workspaceFolderPaths = new Set<string | undefined>([
			...
			...(await Promise.all(
				workspaceFolders.map((folder) => this.getWorkspaceFolderRealPath(folder)),
			)),
		])

		return !Array.from(workspaceFolderPaths).some(
			(folder) =>
				folder && (pathEquals(folder, repositoryPath) || isDescendant(folder, repositoryPath)),
		)
	}
