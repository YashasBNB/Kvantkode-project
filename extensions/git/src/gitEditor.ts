...
export class GitEditor implements IIPCHandler, ITerminalEnvironmentProvider {
		...
		if (commitMessagePath) {
			...
			return new Promise((c) => {
				const onDidClose = window.tabGroups.onDidChangeTabs(async (tabs) => {
					if (
						tabs.closed.some(
							(t) => t.input instanceof TabInputText && t.input.uri.toString() === uri.toString(),
						)
					) {
						onDidClose.dispose()
						return c(true)
	...
	getTerminalEnv(): { [key: string]: string } {
		const config = workspace.getConfiguration('git')
		return config.get<boolean>('useEditorAsCommitInput') && config.get<boolean>('terminalGitEditor')
			? this.env
			: {}
	}
