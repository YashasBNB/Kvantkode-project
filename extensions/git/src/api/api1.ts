...
export class ApiRepositoryState implements RepositoryState {
	...
	get mergeChanges(): Change[] {
		return this.#repository.mergeGroup.resourceStates.map((r) => new ApiChange(r))
	}
	get indexChanges(): Change[] {
		return this.#repository.indexGroup.resourceStates.map((r) => new ApiChange(r))
	}
	get workingTreeChanges(): Change[] {
		return this.#repository.workingTreeGroup.resourceStates.map((r) => new ApiChange(r))
	}
	get untrackedChanges(): Change[] {
		return this.#repository.untrackedGroup.resourceStates.map((r) => new ApiChange(r))
	}
}
...
export function registerAPICommands(extension: GitExtensionImpl): Disposable {
	...
	disposables.push(
		commands.registerCommand('git.api.getRemoteSources', (opts?: PickRemoteSourceOptions) => {
