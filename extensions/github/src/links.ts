...
function getRangeOrSelection(lineNumber: number | undefined) {
	return lineNumber !== undefined &&
		(!vscode.window.activeTextEditor ||
			...
			!vscode.window.activeTextEditor.selection.contains(new vscode.Position(lineNumber - 1, 0)))
...
export async function getLink(
	...
	const gitRepo =
		(fileUri ? getRepositoryForFile(gitAPI, fileUri) : githubRepository) ?? githubRepository
	if (!gitRepo) {
		return
	}
...
export async function ensurePublished(repository: Repository, file: vscode.Uri) {
	...
	if (
		(repository.state.HEAD?.type === RefType.Head || repository.state.HEAD?.type === RefType.Tag) &&
