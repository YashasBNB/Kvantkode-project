	...
	const operationRanges = Array.from(editor.selections)
		...
		.map((selection) => {
			...
			if (
				...
				firstLineOfRange.firstNonWhitespaceCharacterIndex > rangeToReplace.start.character
			) {
				...
				for (let i = 0; i < rangesToReplace.length; i++) {
					...
					if (i > 0 && newPreviewLineEnd === lastNewPreviewRange.end.line) {
						...
						newPreviewStart =
							...
							(oldPreviewRange.start.character - lastOldPreviewRange.end.character)
...
export function expandEmmetAbbreviation(args: any): Thenable<boolean | undefined> {
	...
	function getRootNode() {
		...
		const usePartialParsing =
			vscode.workspace.getConfiguration('emmet')['optimizeStylesheetParsing'] === true
		if (
			...
			rootNode = parsePartialStylesheet(
				editor.document,
				editor.selection.isReversed ? editor.selection.anchor : editor.selection.active,
			)
		} else {
	...
	return expandAbbreviationInRange(editor, abbreviationList, allAbbreviationsSame).then(
		(success) => {
...
export function isValidLocationForEmmetAbbreviation(
	...
	if (isStyleSheet(syntax)) {
		...
		if (
			stylesheet &&
			(stylesheet.comments || []).some((x) => offset >= x.start && offset <= x.end)
		) {
			return false
		}
	...
	if (currentHtmlNode) {
		if (currentHtmlNode.name === 'script') {
			...
			const isScriptJavascriptType =
				!typeValue || typeValue === 'application/javascript' || typeValue === 'text/javascript'
