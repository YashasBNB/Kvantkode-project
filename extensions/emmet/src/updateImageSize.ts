...
function updateHTMLTag(
	...
	if (!widthAttr) {
		...
		edits.push(
			new TextEdit(
				offsetRangeToVsRange(document, widthAttr.value.start, widthAttr.value.end),
				String(width),
			),
		)
	}
	if (!heightAttr) {
		...
		edits.push(
			new TextEdit(
				offsetRangeToVsRange(document, heightAttr.value.start, heightAttr.value.end),
				String(height),
			),
		)
	}
	if (textToAdd) {
		edits.push(
			new TextEdit(offsetRangeToVsRange(document, endOfAttributes, endOfAttributes), textToAdd),
		)
	}
...
function updateCSSNode(
	...
	if (!widthProp) {
		...
		edits.push(
			new TextEdit(
				offsetRangeToVsRange(document, widthProp.valueToken.start, widthProp.valueToken.end),
				`${width}px`,
			),
		)
	}
	if (!heightProp) {
		...
		edits.push(
			new TextEdit(
				offsetRangeToVsRange(document, heightProp.valueToken.start, heightProp.valueToken.end),
				`${height}px`,
			),
		)
	}
