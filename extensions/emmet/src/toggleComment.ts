		...
		Array.from(editor.selections)
			...
			.forEach((selection) => {
				const edits = isStyleSheet(editor.document.languageId)
					? toggleCommentStylesheet(editor.document, selection, <Stylesheet>rootNode)
...
function getRangesToUnCommentHTML(node: Node, document: vscode.TextDocument): vscode.TextEdit[] {
	...
	if (node.type === 'comment') {
		unCommentTextEdits.push(
			new vscode.TextEdit(
				offsetRangeToVsRange(document, node.start, node.start + startCommentHTML.length),
				'',
			),
		)
		unCommentTextEdits.push(
			new vscode.TextEdit(
				offsetRangeToVsRange(document, node.end - endCommentHTML.length, node.end),
				'',
			),
		)
...
function toggleCommentStylesheet(
	...
	rootNode.comments.forEach((comment) => {
		...
		if (selection.intersection(commentRange)) {
			...
			edits.push(
				new vscode.TextEdit(
					offsetRangeToVsRange(document, comment.end - endCommentStylesheet.length, comment.end),
					'',
				),
			)
		}
