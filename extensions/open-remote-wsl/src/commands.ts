...
async function showDistrosPicker(
	wslManager: WSLManager,
	placeHolder: string,
): Promise<WSLDistro | undefined> {
	const pickItemsPromise = wslManager.listDistros().then((distros) =>
		distros.map((distroData) => {
			return {
				...distroData,
	...
	const picked = await vscode.window.showQuickPick(pickItemsPromise, {
		canPickMany: false,
		placeHolder,
	})
	return picked
...
async function showOnlineDistrosPicker(
	wslManager: WSLManager,
	placeHolder: string,
): Promise<WSLOnlineDistro | undefined> {
	const pickItemsPromise = Promise.all([
		wslManager.listOnlineDistros(),
		wslManager.listDistros(),
	]).then(([onlineDistros, localDistros]) => {
		const distroToInstall = onlineDistros.filter(
			(d) => !localDistros.some((l) => l.name === d.name),
		)
...
export async function promptOpenRemoteWSLWindow(
	wslManager: WSLManager,
	useDefault: boolean,
	reuseWindow: boolean,
) {
...
export async function promptInstallNewWSLDistro(wslManager: WSLManager) {
	const distroName = (await showOnlineDistrosPicker(wslManager, 'Select the WSL distro to install'))
		?.name
	if (!distroName) {
		return
	}
...
export function openRemoteWSLWindow(distro: string, reuseWindow: boolean) {
	vscode.commands.executeCommand('vscode.newWindow', {
		remoteAuthority: getRemoteAuthority(distro),
		reuseWindow,
	})
}
...
export function openRemoteWSLLocationWindow(distro: string, path: string, reuseWindow: boolean) {
	vscode.commands.executeCommand(
		'vscode.openFolder',
		vscode.Uri.from({ scheme: 'vscode-remote', authority: getRemoteAuthority(distro), path }),
...
export async function deleteWSLDistro(wslManager: WSLManager, distroName: string) {
	...
	const resp = await vscode.window.showInformationMessage(
		`Are you sure you want to permanently delete the distro "${distroName}" including all its data?`,
		{ modal: true },
		deleteAction,
	)
