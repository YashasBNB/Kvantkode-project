...
export async function installCodeServer(
	...
	if (exitCode !== 0) {
		throw new ServerInstallError(
			`Couldn't install void server on remote server, install script returned non-zero exit status`,
		)
	}
