...
async function initMicrosoftSovereignCloudAuthProvider(
	...
	if (environment === 'custom') {
		...
		try {
			...
			const res = await vscode.window.showErrorMessage(
				vscode.l10n.t('Error validating custom environment setting: {0}', e.message),
				vscode.l10n.t('Open settings'),
			)
			if (res) {
				await vscode.commands.executeCommand(
	...
	if (!env) {
		const res = await vscode.window.showErrorMessage(
			vscode.l10n.t('The environment `{0}` is not a valid environment.', authProviderName),
			vscode.l10n.t('Open settings'),
		)
		return undefined
	}
	...
	const aadService = new AzureActiveDirectoryService(
		vscode.window.createOutputChannel(vscode.l10n.t('Microsoft Sovereign Cloud Authentication'), {
			log: true,
		}),
		context,
		uriHandler,
...
export async function activate(
	...
	context.subscriptions.push(
		vscode.authentication.registerAuthenticationProvider(
			...
			'Microsoft',
			{
				...
				getSessions: (scopes: string[], options?: vscode.AuthenticationProviderSessionOptions) =>
	...
	let microsoftSovereignCloudAuthProviderDisposable = await initMicrosoftSovereignCloudAuthProvider(
		context,
		telemetryReporter,
		uriHandler,
		betterSecretStorage,
