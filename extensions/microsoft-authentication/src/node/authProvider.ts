...
export class MsalAuthProvider implements AuthenticationProvider {
	...
	private constructor(
		...
		const accountChangeEvent = this._eventBufferer.wrapEvent(
			...
			(last, newEvent) => {
				...
				const dedupedEvent = {
					added: Array.from(
						new Map(mergedEvent.added.map((item) => [item.username, item])).values(),
					),
					deleted: Array.from(
						new Map(mergedEvent.deleted.map((item) => [item.username, item])).values(),
					),
					changed: Array.from(
						new Map(mergedEvent.changed.map((item) => [item.username, item])).values(),
					),
				}
	...
	static async create(
		...
		const publicClientManager = await CachedPublicClientApplicationManager.create(
			context.secrets,
			logger,
			env.name,
		)
		...
		for (const flow of flows) {
			...
			try {
				...
				if (
					...
					(e as ClientAuthError)?.errorCode === ClientAuthErrorCodes.userCanceled
				) {
					this._telemetryReporter.sendLoginFailedEvent()
					throw e
				}
