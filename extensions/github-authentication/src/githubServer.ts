...
export class GitHubServer implements IGitHubServer {
	...
	private async isNoCorsEnvironment(): Promise<boolean> {
		...
		const uri = await vscode.env.asExternalUri(
			vscode.Uri.parse(`${vscode.env.uriScheme}://vscode.github-authentication/dummy`),
		)
		this._isNoCorsEnvironment =
			(uri.scheme === 'https' && /^((insiders\.)?vscode|github)\./.test(uri.authority)) ||
	...
	public async logout(session: vscode.AuthenticationSession): Promise<void> {
		...
		const authHeader =
			'Basic ' + base64Encode(`${Config.gitHubClientId}:${Config.gitHubClientSecret}`)
	...
	private async checkEnterpriseVersion(token: string): Promise<void> {
		try {
			...
			if (!isSupportedTarget(this._type, this._ghesUri)) {
				...
				const json: { verifiable_password_authentication: boolean; installed_version: string } =
					await result.json()
