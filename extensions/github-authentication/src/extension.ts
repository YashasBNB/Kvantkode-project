...
function initGHES(context: vscode.ExtensionContext, uriHandler: UriEventHandler) {
	...
	try {
		...
		vscode.window.showErrorMessage(
			vscode.l10n.t('GitHub Enterprise Server URI is not a valid URI: {0}', e.message ?? e),
		)
		return
	}
...
export function activate(context: vscode.ExtensionContext) {
	...
	let githubEnterpriseAuthProvider: GitHubAuthenticationProvider | undefined = initGHES(
		context,
		uriHandler,
	)
