		...
		if (fileRegex.test(original)) {
			...
			fileOrCell = {
				...
				path: stripFormatting(
					original.replace(cellRegex, 'vscode-notebook-cell:?execution_count=$<executionCount>'),
				),
			}
			...
			fileOrCell = {
				kind: 'cell',
				path: stripFormatting(
					original.replace(inputRegex, 'vscode-notebook-cell:?execution_count=$<executionCount>'),
				),
			}
