...
export function activate(context: vscode.ExtensionContext) {
	...
	function doResolve(
		...
		const serverPromise = new Promise<vscode.ResolvedAuthority>(async (res, rej) => {
				...
				for (let i = 0; i < output.length; i++) {
					...
					if (chr === CharCode.LineFeed) {
						...
						if (match) {
							isResolved = true
							res(
								new vscode.ResolvedAuthority('127.0.0.1', parseInt(match[1], 10), connectionToken),
							) // success!
						}
						lastProgressLine = ''
			...
			const remoteDataDir =
				...
				path.join(os.homedir(), `${serverDataFolderName || dataFolderName}-testresolver`)
			...
			if (!commit) {
				...
				const shell = process.platform === 'win32'
				extHostProcess = cp.spawn(path.join(serverLocation, 'bin', serverCommand), commandArgs, {
					env,
					cwd: serverLocation,
					shell,
				})
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('vscode-testresolver.currentWindowManaged', () => {
			return vscode.commands.executeCommand('vscode.newWindow', {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('vscode-testresolver.toggleConnectionPause', () => {
			if (!connectionPaused) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('vscode-testresolver.toggleConnectionSlowdown', () => {
			if (!connectionSlowedDown) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('vscode-testresolver.openTunnel', async () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('vscode-testresolver.startRemoteServer', async () => {
			const result = await vscode.window.showInputBox({
...
async function tunnelFactory(
	...
	function newTunnel(localAddress: { host: string; port: number }): vscode.Tunnel {
		...
		return {
			...
			public:
				!!vscode.workspace.getConfiguration('testresolver').get('supportPublicPorts') &&
				tunnelOptions.public,
