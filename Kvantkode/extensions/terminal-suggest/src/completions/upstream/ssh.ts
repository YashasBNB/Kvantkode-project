...
const resolveAbsolutePath = (path: string, basePath: string, home: string): string => {
	...
	if (basePath.startsWith('/') || basePath.startsWith('~/') || basePath === '~') {
		return (
			basePath.replace('~', home) + (basePath.replace('~', home).endsWith('/') ? '' : '/') + path
		)
	}
...
const getConfigLines = async (
	...
	const includeLines: string[][] = await Promise.all(
		includes.map((file) => getConfigLines(file, executeShellCommand, home, basePath)),
	)
