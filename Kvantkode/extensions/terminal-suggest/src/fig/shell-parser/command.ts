...
const descendantAtIndex = (node: BaseNode, index: number, type?: NodeType): BaseNode | null => {
...
const convertCommandNodeToCommand = (tree: BaseNode): Command => {
	...
	if (
		+(children.length === 0 || children[children.length - 1].endIndex) < endIndex &&
		text.endsWith(' ')
	) {
...
export const substituteAlias = (command: Command, token: Token, alias: string): Command => {
	...
	const postAliasText = postAliasChars ? `${tree.text.slice(postAliasChars)}` : ''
...
export const getAllCommandsWithAlias = (buffer: string, aliases: AliasMap): Command[] => {
	const parseTree = parse(buffer)
