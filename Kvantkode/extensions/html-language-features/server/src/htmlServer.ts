...
namespace SemanticTokenRequest {
	export const type: RequestType<SemanticTokenParams, number[] | null, any> = new RequestType(
		'html/semanticTokens',
	)
}
namespace SemanticTokenLegendRequest {
	export const type: RequestType0<{ types: string[]; modifiers: string[] } | null, any> =
...
export function startServer(connection: Connection, runtime: RuntimeEnvironment) {
		...
		if (scopedSettingsSupport && needsDocumentSettings()) {
			...
			if (!promise) {
				...
				promise = connection
					...
					.then((s) => ({ css: s[0], html: s[1], javascript: s[2], 'js/ts': s[3] }))
	...
	async function onFormat(
		...
		if (document) {
			...
			const unformattedTags: string =
				(settings && settings.html && settings.html.format && settings.html.format.unformatted) ||
				''
			const enabledModes = {
	...
	connection.languages.onLinkedEditingRange((params, token) => {
		return <any>/* todo remove when microsoft/vscode-languageserver-node#700 fixed */ runSafe(
			runtime,
			async () => {
				const document = documents.get(params.textDocument.uri)
				if (document) {
