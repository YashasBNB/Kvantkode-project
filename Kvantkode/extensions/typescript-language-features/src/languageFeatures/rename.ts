...
class TypeScriptRenameProvider implements vscode.RenameProvider {
	...
	private async renameFile(
		...
		if (!path.extname(newName)) {
			...
			newName =
				newName.slice(0, newName.length - path.extname(newName).length) + path.extname(fileToRename)
		}
