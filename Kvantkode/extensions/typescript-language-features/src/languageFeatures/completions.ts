...
class MyCompletionItem extends vscode.CompletionItem {
	...
	constructor(
		...
		this.insertText =
			isSnippet && tsEntry.insertText
				...
				: tsEntry.insertText
		this.filterText =
			tsEntry.filterText || this.getFilterText(completionContext.line, tsEntry.insertText)
	...
	private static getCommitCharacters(
		...
		let commitCharacters =
			...
			(defaultCommitCharacters ? Array.from(defaultCommitCharacters) : undefined)
		if (commitCharacters) {
			if (
		...
		if (entry.kind === PConst.Kind.warning || entry.kind === PConst.Kind.string) {
...
class TypeScriptCompletionItemProvider implements vscode.CompletionItemProvider<MyCompletionItem> {
	...
	constructor(
		...
		commandManager.register(
			new CompletionAcceptedCommand(onCompletionAccepted, this.telemetryReporter),
		)
		...
		for (const entry of entries) {
			if (!shouldExcludeCompletionEntry(entry, completionConfiguration)) {
				...
				includesImportStatementCompletion =
					includesImportStatementCompletion || !!entry.isImportStatementCompletion
			}
		}
	...
	private logCompletionsTelemetry(
		...
		*/
		this.telemetryReporter.logTelemetry('completions.execute', {
			...
			flags:
				response?.type === 'response' && typeof response.body?.flags === 'number'
					? String(response.body.flags)
					: undefined,
			count: String(
				response?.type === 'response' && response.body ? response.body.entries.length : 0,
			),
			...
			createAutoImportProviderProgramDurationMs:
				response?.type === 'response' &&
				typeof response.performanceData?.createAutoImportProviderProgramDurationMs === 'number'
