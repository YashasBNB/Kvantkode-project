	...
	implements ITypeScriptServiceClient
{
	...
	private readonly _onTsServerStarted = this._register(
		new vscode.EventEmitter<{ version: TypeScriptVersion; usedApiVersion: API }>(),
	)
				...
				if (item === reportIssueItem) {
					...
					// Don't allow reporting issues with old TS versions
						...
						vscode.env.openExternal(
							vscode.Uri.parse('https://github.com/microsoft/vscode/wiki/TypeScript-Issues'),
						)
					}
				}
			})
	...
	public toTsFilePath(resource: vscode.Uri): string | undefined {
		...
		return (
			(this.isProjectWideIntellisenseOnWebEnabled() ? '' : inMemoryResourcePrefix) +
			'/' +
			resource.scheme +
			'/' +
		...
		if (isWeb()) {
			...
			if (parts) {
				const resource = vscode.Uri.parse(
					parts[1] + '://' + (parts[2] === emptyAuthority ? '' : parts[2]) + '/' + parts[3],
				)
	...
	private dispatchEvent(event: Proto.Event) {
		switch (event.event) {
			...
			case EventName.requestCompleted: {
				const diagnosticsDuration = (event.body as Proto.RequestCompletedEventBody).performanceData
					?.diagnosticsDuration
	...
	private createFileSystemWatcher(
		...
		const watcher = disposable.add(
			vscode.workspace.createFileSystemWatcher(pattern, undefined, ignoreChangeEvents),
		)
		disposable.add(
			watcher.onDidChange((changeFile) => this.addWatchEvent(id, 'updated', changeFile.fsPath)),
		)
		disposable.add(
			watcher.onDidCreate((createFile) => this.addWatchEvent(id, 'created', createFile.fsPath)),
		)
		disposable.add(
			watcher.onDidDelete((deletedFile) => this.addWatchEvent(id, 'deleted', deletedFile.fsPath)),
		)
		disposable.add({
			dispose: () => {
					...
					Object.keys(payload).forEach((key) => {
						try {
							if (payload.hasOwnProperty(key)) {
								properties[key] =
									typeof payload[key] === 'string' ? payload[key] : JSON.stringify(payload[key])
							}
						} catch (e) {
							// noop
						}
