...
export default class TypeScriptServiceClientHost extends Disposable {
	...
	constructor(
		...
		this.client = this._register(
			new TypeScriptServiceClient(context, onCaseInsensitiveFileSystem, services, allModeIds),
		)
		...
		import('./languageFeatures/updatePathsOnRename').then((module) =>
			this._register(
				module.register(this.client, this.fileConfigurationManager, (uri) => this.handles(uri)),
			),
		)
