...
class BufferSynchronizer {
	...
	private flush() {
		if (this._pending.size > 0) {
			...
			this.client.execute('updateOpen', { changedFiles, closedFiles, openFiles }, nulToken, {
				nonRecoverable: true,
			})
			this._pending.clear()
		}
...
class SyncedBuffer {
	...
	private getProjectRootPath(resource: vscode.Uri): string | undefined {
		...
		return fileSchemes.isOfScheme(resource, fileSchemes.officeScript, fileSchemes.chatCodeBlock)
			? '/'
			: undefined
	}
	...
	private sendPendingDiagnostics(): void {
		...
		for (const buffer of this.syncedBuffers.values()) {
			const editors = vscode.window.visibleTextEditors.filter(
				(editor) => editor.document.uri.toString() === buffer.resource.toString(),
			)
