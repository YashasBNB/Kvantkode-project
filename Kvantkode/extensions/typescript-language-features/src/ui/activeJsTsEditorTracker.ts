...
export class ActiveJsTsEditorTracker extends Disposable {
	...
	private getEditorCandidatesForActiveTab(): vscode.TextEditor[] {
		...
		if (tab.input instanceof vscode.TabInputTextDiff) {
			...
			return coalesce([vscode.window.activeTextEditor, ...vscode.window.visibleTextEditors]).filter(
				(editor) => {
					return (
		...
		if (tab.input instanceof vscode.TabInputNotebook) {
			...
			return notebook?.notebook
				.getCells()
				.some((cell) => cell.document.uri.toString() === activeEditor.document.uri.toString())
				? [activeEditor]
				: []
		}
