...
function trackAndUpdateCellMetadata(
	...
	updates: { cell: NotebookCell; metadata: CellMetadata & { vscode?: { languageId: string } } }[],
) {
	...
	updates.forEach(({ cell, metadata }) => {
		...
		edit.set(cell.notebook.uri, [
			NotebookEdit.updateCellMetadata(cell.index, sortObjectPropertiesRecursively(newMetadata)),
		])
	})
