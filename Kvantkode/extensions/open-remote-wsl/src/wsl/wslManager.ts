...
export class WSLManager {
	...
	async listDistros() {
		...
		if (exitCode) {
			this.logger.trace(
				`Command wsl listDistros exited with code ${exitCode}`,
				stdout + '\n\n' + stderr,
			)
	...
	async listOnlineDistros() {
		...
		if (exitCode) {
			this.logger.trace(
				`Command wsl listOnlineDistros exited with code ${exitCode}`,
				stdout + '\n\n' + stderr,
			)
	...
	async setDefaultDistro(distroName: string) {
		...
		if (exitCode) {
			this.logger.trace(
				`Command wsl setDefaultDistro exited with code ${exitCode}`,
				stdout + '\n\n' + stderr,
			)
	...
	async deleteDistro(distroName: string) {
		...
		if (exitCode) {
			this.logger.trace(
				`Command wsl deleteDistro exited with code ${exitCode}`,
				stdout + '\n\n' + stderr,
			)
