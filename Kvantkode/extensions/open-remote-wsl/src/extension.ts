...
export async function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.workspace.registerRemoteAuthorityResolver(REMOTE_WSL_AUTHORITY, remoteWSLResolver),
	)
	...
	context.subscriptions.push(
		vscode.window.createTreeView('wslTargets', { treeDataProvider: distroTreeDataProvider }),
	)
	context.subscriptions.push(distroTreeDataProvider)

	context.subscriptions.push(
		vscode.commands.registerCommand('openremotewsl.connect', () =>
			promptOpenRemoteWSLWindow(wslManager, true, true),
		),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand('openremotewsl.connectInNewWindow', () =>
			promptOpenRemoteWSLWindow(wslManager, true, false),
		),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand('openremotewsl.connectUsingDistro', () =>
			promptOpenRemoteWSLWindow(wslManager, false, true),
		),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand('openremotewsl.connectUsingDistroInNewWindow', () =>
			promptOpenRemoteWSLWindow(wslManager, false, false),
		),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand('openremotewsl.showLog', () => logger.show()),
	)
}
