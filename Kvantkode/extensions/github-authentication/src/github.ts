	...
	public async waitForCode(
		...
		try {
			return await Promise.race([
				...
				promiseFromEvent<void, string>(token.onCancellationRequested, (_, __, reject) => {
	...
	implements vscode.AuthenticationProvider, vscode.Disposable
{
	...
	private async readSessions(): Promise<vscode.AuthenticationSession[]> {
		...
		const sessionPromises = sessionData.map(
			async (session: SessionData): Promise<vscode.AuthenticationSession | undefined> => {
				// For GitHub scope list, order doesn't matter so we immediately sort the scopes
		...
		try {
			...
			*/
			...
			const sessionIndex = sessions.findIndex(
				(s) =>
					s.account.id === session.account.id && arrayEquals([...s.scopes].sort(), sortedScopes),
			)
