...
async function openOnGitHub(repository: Repository, commit: string): Promise<void> {
	...
	const remoteNames = new Set(
		branches.filter((b) => b.type === RefType.RemoteHead && b.remote).map((b) => b.remote!),
	)

	// GitHub remotes that contain the commit
	const remotes = repository.state.remotes.filter(
		(r) => remoteNames.has(r.name) && r.fetchUrl && getRepositoryFromUrl(r.fetchUrl),
	)
...
export function registerCommands(gitAPI: GitAPI): vscode.Disposable {
	...
	disposables.add(
		vscode.commands.registerCommand('github.copyVscodeDevLink', async (context: LinkContext) => {
	...
	disposables.add(
		vscode.commands.registerCommand(
			...
			async (repository: vscode.SourceControl, historyItem: vscode.SourceControlHistoryItem) => {
