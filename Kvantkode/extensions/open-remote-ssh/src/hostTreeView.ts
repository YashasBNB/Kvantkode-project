	...
	implements vscode.TreeDataProvider<DataTreeItem>
{
	...
	constructor(private locationHistory: RemoteLocationHistory) {
		...
		this._register(
			vscode.commands.registerCommand('openremotessh.explorer.add', () => addNewHost()),
		)
		this._register(
			vscode.commands.registerCommand('openremotessh.explorer.configure', () =>
				openSSHConfigFile(),
			),
		)
		this._register(
			vscode.commands.registerCommand('openremotessh.explorer.refresh', () => this.refresh()),
		)
		this._register(
			vscode.commands.registerCommand('openremotessh.explorer.emptyWindowInNewWindow', (e) =>
				this.openRemoteSSHWindow(e, false),
			),
		)
		this._register(
			vscode.commands.registerCommand('openremotessh.explorer.emptyWindowInCurrentWindow', (e) =>
				this.openRemoteSSHWindow(e, true),
			),
		)
		this._register(
			vscode.commands.registerCommand('openremotessh.explorer.reopenFolderInNewWindow', (e) =>
				this.openRemoteSSHLocationWindow(e, false),
			),
		)
		this._register(
			vscode.commands.registerCommand('openremotessh.explorer.reopenFolderInCurrentWindow', (e) =>
				this.openRemoteSSHLocationWindow(e, true),
			),
		)
		this._register(
			vscode.commands.registerCommand('openremotessh.explorer.deleteFolderHistoryItem', (e) =>
	...
	async getChildren(element?: HostItem): Promise<DataTreeItem[]> {
		if (!element) {
			...
			return hosts.map(
				(hostname) => new HostItem(hostname, this.locationHistory.getHistory(hostname)),
			)
		}
