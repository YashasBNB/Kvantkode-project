...
class RemoteSourceProviderQuickPick implements Disposable {
		...
		if (!this.quickpick) {
			...
			if (this.provider.supportsQuery) {
				this.quickpick.placeholder =
					this.provider.placeholder ?? l10n.t('Repository name (type to search)')
	...
	private async query(): Promise<void> {
		try {
			...
			this.quickpick!.items = [
				{ label: l10n.t('{0} Error: {1}', '$(error)', err.message), alwaysShow: true },
			]
			console.error(err)
		} finally {
			if (!this.isDisposed) {
...
export async function pickRemoteSource(
	...
	const updatePicks = (value?: string) => {
		if (value) {
			const label =
				(typeof options.urlLabel === 'string' ? options.urlLabel : options.urlLabel?.(value)) ??
				l10n.t('URL')
			quickpick.items = [
				{
					label: label,
