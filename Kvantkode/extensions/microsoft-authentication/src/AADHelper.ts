...
export class AzureActiveDirectoryService {
	...
	constructor(
		...
		_context.subscriptions.push(
			this._tokenStorage.onDidChangeInOtherWindow((e) => this.checkForUpdates(e)),
		)
		_context.subscriptions.push(
			...
			*/
			this._telemetryReporter.sendTelemetryEvent('account', {
				...
				scopes: JSON.stringify(
					token.scope
						.replace(/[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}/i, '{guid}')
						.split(' '),
				),
	...
	public getSessions(
		...
		this._logger.trace(
			`[${scopeData.scopeStr}] Queued getting sessions` + account ? ` for ${account?.label}` : '',
		)
	...
	private async doGetSessions(
		...
		this._logger.info(
			`[${scopeData.scopeStr}] Getting sessions` + account ? ` for ${account?.label}` : '',
		)
		...
		const results = await Promise.allSettled(
			matchingTokens.map((token) => this.convertToSession(token, scopeData)),
		)
		return results
	...
	private async doCreateSession(
		...
		this._logger.info(
			`[${scopeData.scopeStr}] Creating session` + account ? ` for ${account?.label}` : '',
		)

		const runsRemote = vscode.env.remoteName !== undefined
		const runsServerless =
			vscode.env.remoteName === undefined && vscode.env.uiKind === vscode.UIKind.Web

		if (
			runsServerless &&
			this._env.activeDirectoryEndpointUrl !== defaultActiveDirectoryEndpointUrl
		) {
	...
	private async createSessionWithoutLocalServer(
		...
		let callbackUri = await vscode.env.asExternalUri(
			vscode.Uri.parse(`${vscode.env.uriScheme}://vscode.microsoft-authentication`),
		)
	...
	private convertToTokenSync(
		...
		return {
			...
			account: {
				label:
					claims.preferred_username ?? claims.email ?? claims.unique_name ?? 'user@example.com',
				id,
				type:
		...
		try {
			...
			if (token.expiresIn) {
				this.setSessionTimeout(
					...
					token.refreshToken,
					scopeData,
					token.expiresIn * AzureActiveDirectoryService.REFRESH_TIMEOUT_MODIFIER,
				)
			}
			...
			uriEventListener = this._uriHandler.event(async (uri: vscode.Uri) => {
				try {
					...
					if (
						...
						!acceptedStates.includes(decodeURIComponent(nonce))
					) {
						throw new Error('Nonce does not match.')
					}

					const verifier =
						this._codeVerfifiers.get(nonce) ?? this._codeVerfifiers.get(decodeURIComponent(nonce))
					if (!verifier) {
	...
	private async fetchTokenResponse(
		...
		if (this._env.activeDirectoryEndpointUrl !== defaultActiveDirectoryEndpointUrl) {
			...
			const proxyEndpoints: { [providerId: string]: string } | undefined =
				await vscode.commands.executeCommand('workbench.getCodeExchangeProxyEndpoints')
