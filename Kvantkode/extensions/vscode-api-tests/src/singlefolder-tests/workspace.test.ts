...
suite('vscode API - workspace', () => {
		...
		return vscode.workspace
			.openTextDocument(
				vscode.workspace.workspaceFolders![0].uri.with({
					scheme: 'untitled',
					path: posix.join(vscode.workspace.workspaceFolders![0].uri.path, 'newfile.txt'),
				}),
			)
			.then((doc) => {
	...
	test('events: onDidOpenTextDocument, onDidChangeTextDocument, onDidSaveTextDocument', async () => {
		...
		assert.ok(
			Array.from(onDidOpenTextDocument).find((e) => e.uri.toString() === file.toString()),
			'did Open: ' + file.toString(),
		)
		assert.ok(
			Array.from(onDidChangeTextDocument).find((e) => e.uri.toString() === file.toString()),
			'did Change: ' + file.toString(),
		)
		assert.ok(
			Array.from(onDidSaveTextDocument).find((e) => e.uri.toString() === file.toString()),
	...
	test('openTextDocument, with selection', function () {
		return createRandomFile('foo\nbar\nbar').then((file) => {
			return vscode.workspace.openTextDocument(file).then((doc) => {
				return vscode.window
					.showTextDocument(doc, {
						selection: new vscode.Range(new vscode.Position(1, 1), new vscode.Position(1, 2)),
					})
					.then((editor) => {
	...
	test('registerTextDocumentContentProvider, multiple', function () {
		...
		return Promise.all([
			vscode.workspace.openTextDocument(vscode.Uri.parse('foo://foo/bla')).then((doc) => {
				assert.strictEqual(doc.getText(), '1')
			}),
			vscode.workspace.openTextDocument(vscode.Uri.parse('foo://bar/bla')).then((doc) => {
	...
	test('findFiles2 - null exclude', async () => {
		...
		await vscode.workspace
			.findFiles2(['**/file.txt'], { useExcludeSettings: vscode.ExcludeSettingOptions.None })
			.then((res) => {
				// search.exclude and files.exclude folders are both searched
				assert.strictEqual(res.length, 2)
				assert.strictEqual(basename(vscode.workspace.asRelativePath(res[0])), 'file.txt')
	...
	test('applyEdit', async () => {
		const doc = await vscode.workspace.openTextDocument(
			vscode.Uri.parse('untitled:' + join(vscode.workspace.rootPath || '', './new2.txt')),
		)
	...
	test('encoding: decode', async function () {
		...
		assert.strictEqual(
			await vscode.workspace.decode(
				new Uint8Array([0xef, 0xbb, 0xbf, 72, 101, 108, 108, 111, 32, 87, 111, 114, 108, 100]),
				uri,
			),
			'Hello World',
		) // UTF-8 with BOM
		assert.strictEqual(
			await vscode.workspace.decode(
				new Uint8Array([
					0xfe, 0xff, 0, 72, 0, 101, 0, 108, 0, 108, 0, 111, 0, 32, 0, 87, 0, 111, 0, 114, 0, 108,
					0, 100,
				]),
				uri,
			),
		...
		assert.strictEqual(
			await vscode.workspace.decode(
				new Uint8Array([
					0xff, 0xfe, 72, 0, 101, 0, 108, 0, 108, 0, 111, 0, 32, 0, 87, 0, 111, 0, 114, 0, 108, 0,
					100, 0,
				]),
				uri,
			),
		...
		assert.strictEqual(
			await vscode.workspace.decode(
				new Uint8Array([
					0, 72, 0, 101, 0, 108, 0, 108, 0, 111, 0, 32, 0, 87, 0, 111, 0, 114, 0, 108, 0, 100,
				]),
				uri,
			),
			'Hello World',
		...
		assert.strictEqual(
			await vscode.workspace.decode(Buffer.from('Hello World'), uri, { encoding: 'foobar123' }),
			'Hello World',
		)
	...
	test('encoding: encode', async function () {
		...
		assert.ok(
			equalsUint8Array(
				await vscode.workspace.encode('Hello World', uri, { encoding: 'utf8' }),
		...
		assert.ok(
			equalsUint8Array(
				await vscode.workspace.encode('Hello World', uri, { encoding: 'utf16le' }),
				new Uint8Array([
					0xff, 0xfe, 72, 0, 101, 0, 108, 0, 108, 0, 111, 0, 32, 0, 87, 0, 111, 0, 114, 0, 108, 0,
					100, 0,
				]),
		...
		assert.ok(
			equalsUint8Array(
				await vscode.workspace.encode('Hello World', uri, { encoding: 'utf16be' }),
				new Uint8Array([
					0xfe, 0xff, 0, 72, 0, 101, 0, 108, 0, 108, 0, 111, 0, 32, 0, 87, 0, 111, 0, 114, 0, 108,
					0, 100,
				]),
		...
		assert.ok(
			equalsUint8Array(
				await vscode.workspace.encode('Hellö Wörld', uri, { encoding: 'cp1252' }),
				new Uint8Array([72, 101, 108, 108, 0xf6, 32, 87, 0xf6, 114, 108, 100]),
			),
		)
		assert.ok(
			equalsUint8Array(
				await vscode.workspace.encode('Hello World', uri, { encoding: 'foobar123' }),
				new Uint8Array([72, 101, 108, 108, 111, 32, 87, 111, 114, 108, 100]),
			),
		)
	})
	...
	test('encoding: utf8bom does not explode (https://github.com/microsoft/vscode/issues/242132)', async function () {
		...
		const decoded = await vscode.workspace.decode(new Uint8Array(buffer), uri, {
			encoding: 'utf8bom',
		})
