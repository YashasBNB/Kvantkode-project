...
suite.skip('vscode API - Copy Paste', function () {
	...
	test('Copy should be able to overwrite text/plain', async () => {
		...
		testDisposables.push(
			vscode.languages.registerDocumentPasteEditProvider(
				...
				{
					providedPasteEditKinds: [vscode.DocumentDropOrPasteEditKind.Empty.append('test')],
	...
	test('Copy with multiple selections should get all selections', async () => {
		...
		editor.selections = [new vscode.Selection(0, 0, 0, 3), new vscode.Selection(2, 0, 2, 3)]
				...
				new (class implements vscode.DocumentPasteEditProvider {
					async prepareDocumentPaste(
						...
						if (existing) {
							...
							dataTransfer.set(
								textPlain,
								new vscode.DataTransferItem(`(${ranges.length})${selections.join(' ')}`),
							)
						}
					}
				})(),
