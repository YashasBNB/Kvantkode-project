	...
	function () {
		...
		suiteSetup(() => {
			suiteDisposables.push(
				vscode.workspace.registerNotebookSerializer('notebookCoreTest', apiTestSerializer),
			)
		})
					...
					try {
						...
						if (eventCount === 0) {
							...
							assert.strictEqual(
								cell.outputs.length,
								0,
								'no outputs yet: ' + JSON.stringify(cell.outputs[0]),
							)
		...
		test('Output changes are applied once the promise resolves', async function () {
			...
			const verifyOutputSyncKernel = new (class extends Kernel {
				...
				override async _execute(cells: vscode.NotebookCell[]) {
					...
					assert.deepStrictEqual(
						new TextDecoder().decode(cell.notebook.cellAt(0).outputs[0].items[0].data),
						'Some output',
					)
					task.end(undefined)
					called = true
			...
			const nextCellKernel = new (class extends Kernel {
				...
				override async _runCell(cell: vscode.NotebookCell) {
					...
					await task.appendOutput(
						[new vscode.NotebookCellOutput([vscode.NotebookCellOutputItem.text('my output')])],
						cell1,
					)
					await task.appendOutput(
						[new vscode.NotebookCellOutput([vscode.NotebookCellOutputItem.text('my output 2')])],
						cell1,
					)
					task.end(true)
				}
