	...
	() => {
		...
		test('basic auth', async () => {
			...
			try {
				...
				await vscode.workspace
					.getConfiguration()
					.update('http.proxy', `http://127.0.0.1:${proxyPort}`, vscode.ConfigurationTarget.Global)
				await change
				await new Promise<void>((resolve, reject) => {
					https
						.get(url, (res) => {
				...
				authOpts.realm = Buffer.from(JSON.stringify({ username: user, password: pass })).toString(
					'base64',
				)
				await new Promise<void>((resolve, reject) => {
					https
				...
				await vscode.workspace
					.getConfiguration()
					.update('integration-test.http.proxyAuth', undefined, vscode.ConfigurationTarget.Global)
			}
		})
		;(vscode.env.remoteName ? test : test.skip)(
			...
			async () => {
				...
				await vscode.workspace
					.getConfiguration()
					.update('http.useLocalProxyConfiguration', false, vscode.ConfigurationTarget.Global)
				await p2
				...
				await vscode.workspace
					.getConfiguration()
					.update('http.useLocalProxyConfiguration', true, vscode.ConfigurationTarget.Global)
				await p5
...
export const middlewareAuth =
	(
		opts: AuthOpts,
	): Middleware<RequestContext<RequestAdditionsAuth> | ConnectContext<RequestAdditionsAuth>> =>
	async (ctx, next) => {
