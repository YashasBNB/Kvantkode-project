...
;(vscode.env.uiKind === vscode.UIKind.Web ? suite.skip : suite)('Notebook API tests', function () {
	...
	suiteSetup(function () {
		suiteDisposables.push(
			vscode.workspace.registerNotebookSerializer(notebookType, apiTestSerializer),
		)
	})
	...
	test('multiple tabs: different editors with same document', async function () {
		const notebook = await openRandomNotebookDocument()
		const firstNotebookEditor = await vscode.window.showNotebookDocument(notebook, {
			viewColumn: vscode.ViewColumn.One,
		})
		const secondNotebookEditor = await vscode.window.showNotebookDocument(notebook, {
	...
	test.skip('#115855 onDidSaveNotebookDocument', async function () {
		...
		const notebookEdit = new vscode.NotebookEdit(new vscode.NotebookRange(1, 1), [
			new vscode.NotebookCellData(vscode.NotebookCellKind.Code, 'test 2', 'javascript'),
		])
		const edit = new vscode.WorkspaceEdit()
...
suite('Notebook & LiveShare', function () {
			...
			vscode.workspace.registerNotebookSerializer(
				...
				new (class implements vscode.NotebookSerializer {
					deserializeNotebook(
						...
						cell1.outputs = [
							new vscode.NotebookCellOutput([vscode.NotebookCellOutputItem.stderr(value)]),
						]
	...
	test('command: vscode.executeNotebookToData', async function () {
		...
		const notebook = new vscode.NotebookData([
			new vscode.NotebookCellData(vscode.NotebookCellKind.Code, value, 'fooLang'),
		])
