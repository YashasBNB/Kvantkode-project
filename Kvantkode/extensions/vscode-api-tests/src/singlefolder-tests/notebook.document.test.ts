...
suite('Notebook Document', function () {
	...
	suiteSetup(function () {
		disposables.push(
			vscode.workspace.registerNotebookSerializer('notebook.nbdtest', simpleContentProvider),
		)
	})
	...
	test('workspace edit API (replaceCells)', async function () {
		...
		// remove all cells
		{
			const edit = new vscode.WorkspaceEdit()
			edit.set(document.uri, [
				vscode.NotebookEdit.replaceCells(new vscode.NotebookRange(0, document.cellCount), []),
			])
			const success = await vscode.workspace.applyEdit(edit)
			assert.strictEqual(success, true)
		}
