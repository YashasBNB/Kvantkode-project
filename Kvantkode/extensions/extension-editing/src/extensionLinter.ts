...
const product = JSON.parse(
	fs.readFileSync(path.join(env.appRoot, 'product.json'), { encoding: 'utf-8' }),
)
const allowedBadgeProviders: string[] = (product.extensionAllowedBadgeProviders || []).map(
...
export class ExtensionLinter {
	...
	private queueReadme(document: TextDocument) {
		...
		if (
			...
			(p.toLowerCase().endsWith('/readme.md') || p.toLowerCase().endsWith('/changelog.md'))
		) {
	...
	private async lintWhenClauses(
		...
		>(
			...
			whenClauses.map(
				(w) => w.value as string /* we make sure to capture only if `w.value` is string above */,
			),
		)
	...
	private async lintReadme() {
		for (const document of this.readmeQ) {
			...
			const tokensAndPositions: TokenAndPosition[] = function toTokensAndPositions(
	...
	private readPackageJsonInfo(folder: Uri, tree: JsonNode | undefined) {
		...
		const parsedEngineVersion =
			engine?.type === 'string' ? normalizeVersion(parseVersion(engine.value)) : null
	...
	private packageJsonChanged(folder: Uri) {
		...
		workspace.textDocuments
			.filter((document) => this.getUriFolder(document.uri).toString().toLowerCase() === str)
