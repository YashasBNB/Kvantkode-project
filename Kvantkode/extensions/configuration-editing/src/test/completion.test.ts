...
suite('Completions in settings.json', () => {
	...
	test('window.title', async () => {
		...
		}
		{
			...
			const content = ['{', '  "window.title": "|${activeEditorShort}"', '}'].join('\n')
		...
		}
		{
			...
			const content = ['{', '  "window.title": "${a|ctiveEditorShort}"', '}'].join('\n')
	...
	test('files.associations', async () => {
		{
			const content = ['{', '  "files.associations": {', '    |', '  }', '}'].join('\n')
			const resultText = [
				'{',
				'  "files.associations": {',
	...
	test('files.exclude', async () => {
		...
		{
			const content = ['{', '  "files.exclude": {', '    "**/*.extension": |true', '  }', '}'].join(
				'\n',
			)
			const resultText = [
				'{',
	...
	test('remote.extensionKind', async () => {
		{
			const content = ['{', '\t"remote.extensionKind": {', '\t\t|', '\t}', '}'].join('\n')
	...
	test('remote.portsAttributes', async () => {
		{
			const content = ['{', '  "remote.portsAttributes": {', '    |', '  }', '}'].join('\n')
...
suite('Completions in extensions.json', () => {
	...
	test('change recommendation', async () => {
		{
			const content = ['{', '  "recommendations": [', '    "|a.b"', '  ]', '}'].join('\n')
			const resultText = [
				'{',
				'  "recommendations": [',
				'    "ms-vscode.js-debug"',
	...
	if (expected.notAvailable) {
		...
		if (expected.resultText) {
			...
			if (match.range && match.insertText) {
				...
				const text =
					typeof match.insertText === 'string' ? match.insertText : match.insertText.value
