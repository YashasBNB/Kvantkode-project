			...
			provideCompletionItems(document, position, _token) {
				...
				if (isCompletingInsidePropertyStringValue(document, location, position)) {
					...
					if (
						...
						(range.end.isEqual(position) && document.getText(range).endsWith('}'))
					) {
					...
					return [
						...
						{
							label: 'relativeFile',
							detail: vscode.l10n.t('The current opened file relative to ${workspaceFolder}'),
						},
						{
						...
						{
							label: 'cwd',
							detail: vscode.l10n.t("The task runner's current working directory on startup"),
						},
						{
							label: 'lineNumber',
							detail: vscode.l10n.t('The current selected line number in the active file'),
						},
						{
							label: 'selectedText',
							detail: vscode.l10n.t('The current selected text in the active file'),
						},
						...
						{ label: 'fileBasename', detail: vscode.l10n.t("The current opened file's basename") },
						{
							...
							detail: vscode.l10n.t("The current opened file's basename with no file extension"),
						},
						{
						...
						},
						{
							...
							detail: vscode.l10n.t('The path where an extension is installed.'),
					...
					if (name && depthInObjects === 2) {
						result.push(
							new vscode.SymbolInformation(
								...
								new vscode.Range(document.positionAt(startOffset), document.positionAt(offset)),
							),
						)
					}
					depthInObjects--
