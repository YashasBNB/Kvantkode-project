	...
	private async provideWindowTitleCompletionItems(
		...
		completions.push(
			this.newSimpleCompletionItem(
				...
				range,
				vscode.l10n.t('the name of the folder the file is contained in (e.g. myFileFolder)'),
			),
		)
		completions.push(
			this.newSimpleCompletionItem(
		...
		completions.push(
			this.newSimpleCompletionItem(
				getText('rootPath'),
				range,
				vscode.l10n.t('file path of the workspace (e.g. /Users/Development/myWorkspace)'),
			),
		)
		completions.push(
			this.newSimpleCompletionItem(
				getText('folderName'),
				range,
				vscode.l10n.t('name of the workspace folder the file is contained in (e.g. myFolder)'),
			),
		)
		completions.push(
			this.newSimpleCompletionItem(
		...
		completions.push(
			this.newSimpleCompletionItem(getText('appName'), range, vscode.l10n.t('e.g. VS Code')),
		)
		completions.push(
			this.newSimpleCompletionItem(getText('remoteName'), range, vscode.l10n.t('e.g. SSH')),
		)
		completions.push(
			this.newSimpleCompletionItem(
				getText('dirty'),
				range,
				vscode.l10n.t('an indicator for when the active editor has unsaved changes'),
			),
		)
		completions.push(
			this.newSimpleCompletionItem(
		...
		completions.push(
			this.newSimpleCompletionItem(
				...
				range,
				vscode.l10n.t('the name of the active branch in the active repository (e.g. main)'),
			),
		)
		completions.push(
			this.newSimpleCompletionItem(
	...
	private async provideEditorLabelCompletionItems(
		...
		completions.push(
			this.newSimpleCompletionItem(
				getText('dirname'),
				range,
				vscode.l10n.t('The parent folder name of the editor (e.g. myFileFolder)'),
			),
		)
		completions.push(
			this.newSimpleCompletionItem(
		...
		if (location.path.length === 1 && this.isCompletingPropertyValue(location, position)) {
			...
			return [
				this.newSimpleCompletionItem(
					...
					range,
					vscode.l10n.t('Use the language of the currently active text editor if any'),
				),
		...
		if (
			...
			while (matches?.length) {
				lastLanguageOverrideRange = new vscode.Range(
					this.document.positionAt(location.previousNode.offset + 1 + matches.index),
					this.document.positionAt(
						location.previousNode.offset + 1 + matches.index + matches[0].length,
					),
				)
