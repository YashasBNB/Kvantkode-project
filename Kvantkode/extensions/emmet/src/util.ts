...
export function getHtmlFlatNode(
	...
	if (currentNode.name === 'script' && currentNode.children.length === 0) {
		...
		if (scriptNodeBody) {
			currentNode =
				getHtmlFlatNode(scriptNodeBody, currentNode, offset, includeNodeBoundary) ?? currentNode
		}
	} else if (currentNode.type === 'cdata') {
		const cdataBody = setupCdataNodeSubtree(documentText, currentNode)
		currentNode =
			getHtmlFlatNode(cdataBody, currentNode, offset, includeNodeBoundary) ?? currentNode
	}
	return currentNode
}
...
export function setupScriptNodeSubtree(documentText: string, scriptNode: HtmlFlatNode): string {
	const isTemplateScript =
		...
		scriptNode.attributes.some(
			(x) =>
				x.name.toString() === 'type' && allowedMimeTypesInScriptTag.includes(x.value.toString()),
		)
...
export function isOffsetInsideOpenOrCloseTag(node: FlatNode, offset: number): boolean {
	...
	if (
		(htmlNode.open && offset > htmlNode.open.start && offset < htmlNode.open.end) ||
...
export function getCssPropertyFromDocument(
	...
	if (
		...
		const buffer =
			' '.repeat(htmlNode.start) + document.getText().substring(htmlNode.start, htmlNode.end)
