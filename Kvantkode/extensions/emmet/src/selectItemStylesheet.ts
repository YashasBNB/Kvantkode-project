...
export function nextItemStylesheet(
	...
	if (
		(currentNode.type === 'rule' && endOffset < (<Rule>currentNode).selectorToken.end) ||
...
function getSelectionFromProperty(
	...
	if (direction === 'prev') {
		...
		const tokenStartChar = document.positionAt(propertyNode.valueToken.start).character
		pos = selectionEnd === propertyNode.valueToken.end ? -1 : selectionEndChar - tokenStartChar - 1
	}

	const [newSelectionStartOffset, newSelectionEndOffset] =
		direction === 'prev' ? findPrevWord(propertyValue, pos) : findNextWord(propertyValue, pos)
