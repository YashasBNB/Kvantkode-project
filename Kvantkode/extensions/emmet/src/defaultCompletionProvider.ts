...
export class DefaultCompletionItemProvider implements vscode.CompletionItemProvider {
	...
	private provideCompletionItemsInternal(
		...
		const emmetMode = getEmmetMode(
			isSyntaxMapped ? mappedLanguages[document.languageId] : document.languageId,
			mappedLanguages,
			excludedLanguages,
		)
		...
		if (
			...
			!helper.isAbbreviationValid(syntax, extractAbbreviationResults.abbreviation)
		) {
			return
		}
					...
					.then((symbols) => {
						return (
							...
							symbols.some(
								(x) =>
									abbreviation === x.name ||
									(abbreviation.startsWith(x.name + '.') && !/>|\*|\+/.test(abbreviation)),
							)
						)
					})
			}
