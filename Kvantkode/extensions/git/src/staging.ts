...
export function applyLineChanges(
	...
	for (const diff of diffs) {
		...
		if (!isDeletion) {
			...
			result.push(
				modified.getText(new Range(fromLine, fromCharacter, diff.modifiedEndLineNumber, 0)),
			)
		}
...
function getDiffInformation(
	...
	return textEditor.diffInformation?.find(
		(diff) => diff.original && isGitUri(diff.original) && fromGitUri(diff.original).ref === ref,
	)
}
