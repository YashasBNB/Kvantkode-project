...
function toSourceControlHistoryItemRef(
	...
	switch (ref.type) {
		case RefType.RemoteHead:
			return {
				...
				description: ref.commit
					? l10n.t('Remote branch at {0}', getCommitShortHash(rootUri, ref.commit))
					: undefined,
				revision: ref.commit,
		...
		case RefType.Tag:
			return {
				...
				name: ref.name ?? '',
				description: ref.commit
					? l10n.t('Tag at {0}', getCommitShortHash(rootUri, ref.commit))
					: undefined,
				revision: ref.commit,
	...
	implements SourceControlHistoryProvider, FileDecorationProvider, IDisposable
{
	...
	private async onDidRunWriteOperation(result: OperationResult): Promise<void> {
		...
		// Auto-fetch
		const silent =
			result.operation.kind === OperationKind.Fetch && result.operation.showProgress === false
		...
		try {
			if (options.limit === undefined || typeof options.limit === 'number') {
				...
				const commitParentId =
					commit.parents.length > 0 ? commit.parents[0] : await this.repository.getEmptyTree()
			...
			for (const commit of commits) {
				...
				const subject =
					newLineIndex !== -1 ? `${message.substring(0, newLineIndex)}\u2026` : message
