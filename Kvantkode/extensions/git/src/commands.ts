	...
	constructor(
		...
		const remote =
			repository.remotes.find(
				(r) => r.pushUrl === repository.HEAD?.remote || r.fetchUrl === repository.HEAD?.remote,
...
async function evaluateDiagnosticsCommitHook(
	...
	const changesDiagnostics = diagnostics.filter(([uri, diags]) => {
		...
		return diags.find((d) => {
			...
			if (
				!d.source ||
				(Object.keys(sourceSeverity).includes(d.source) && sourceSeverity[d.source] === 'none')
			) {
				return false
			}
...
export class CommandCenter {
	...
	constructor(
		...
		this.disposables.push(
			workspace.registerTextDocumentContentProvider('git-output', this.commandErrors),
		)
	}
	...
	async cloneRepository(
		...
		try {
			...
			const repositoryPath = await window.withProgress(opts, (progress, token) =>
				this.git.clone(
					url!,
					{ parentPath: parentPath!, progress, recursive: options.recursive, ref: options.ref },
					token,
				),
			)
			...
			*/
			this.telemetryReporter.sendTelemetryEvent(
				...
				{
					openFolder:
						action === PostCloneAction.Open || action === PostCloneAction.OpenNewWindow ? 1 : 0,
				},
			)
	...
	async continueInLocalClone(): Promise<Uri | void> {
		...
		const items = this.model.repositories.reduce<(QuickPickItem & { repository: Repository })[]>(
			(items, repository) => {
	...
	async init(skipFolderPrompt = false): Promise<void> {
		...
		if (!repositoryPath) {
			...
			if (
				...
				workspace.workspaceFolders.some((w) => w.uri.toString() === uri.toString())
			) {
				askToOpen = false
			}
		}
		...
		await this.git.init(repositoryPath, {
			defaultBranch: sanitizeBranchName(defaultBranchName, branchWhitespaceChar),
		})
			...
			if (previousURI.path === uri.path && document) {
				...
				if (previousVisibleRanges && previousVisibleRanges.length > 0) {
					...
					if (previousSelection && previousVisibleRanges.length > 1) {
						...
						rangeToReveal =
							previousVisibleRanges.find((r) => r.intersection(previousSelection)) ?? rangeToReveal
					}
	...
	async rename(repository: Repository, fromUri: Uri | undefined): Promise<void> {
		...
		await commands.executeCommand('vscode.open', Uri.file(path.join(repository.root, to)), {
			viewColumn: ViewColumn.Active,
		})
	}
		...
		const selectedChanges = workingTreeLineChanges
			.map((change) =>
				selectedLines.reduce<LineChange | null>(
					(result, range) => result || intersectDiffWithRange(modifiedDocument, change, range),
					null,
				),
			)
	...
	async unstage(...resourceStates: SourceControlResourceState[]): Promise<void> {
		...
		const scmResources = resourceStates.filter(
			(s) => s instanceof Resource && s.resourceGroupType === ResourceGroupType.Index,
		) as Resource[]
	...
	async unstageSelectedRanges(): Promise<void> {
		...
		this.logger.trace(
			`[CommandCenter][unstageSelectedRanges] invertedDiffs: ${JSON.stringify(invertedDiffs)}`,
		)
	...
	private getDiscardUntrackedChangesDialogDetails(resources: Resource[]): [string, string, string] {
		...
		const discardUntrackedChangesToTrash =
			config.get<boolean>('discardUntrackedChangesToTrash', true) && !isRemote && !isLinuxSnap
	...
	private async smartCommit(
		...
		const config = workspace.getConfiguration('git', Uri.file(repository.root))
		let promptToSaveFilesBeforeCommit = config.get<'always' | 'staged' | 'never'>(
		...
		if (!opts.empty) {
			if (promptToSaveFilesBeforeCommit !== 'never') {
				let documents = workspace.textDocuments.filter(
					(d) => !d.isUntitled && d.isDirty && isDescendant(repository.root, d.uri.fsPath),
				)
	...
	async commitMessageDiscard(arg?: Uri): Promise<void> {
		...
		const editors = window.visibleTextEditors.filter(
			(e) =>
				e.document.languageId === 'git-commit' && e.document.uri.toString() === arg!.toString(),
		)
	...
	async undoCommit(repository: Repository): Promise<void> {
		...
		if (commit.parents.length > 1) {
			...
			const result = await window.showWarningMessage(
				l10n.t('The last commit was a merge commit. Are you sure you want to undo it?'),
				{ modal: true },
				yes,
			)
			...
			disposables.push(
				quickPick.onDidTriggerItemButton((e) => {
					...
					if (button.actual && item.refName) {
						button.actual.run(
							item.refRemote ? item.refName.substring(item.refRemote.length + 1) : item.refName,
						)
					}
	...
	private async promptForBranchName(
		...
		const getBranchName = async (): Promise<string> => {
			const branchName = branchRandomNameEnabled
				? await this.generateRandomBranchName(repository, branchWhitespaceChar)
				: ''
	...
	async deleteRemoteTag(repository: Repository): Promise<void> {
		...
		const remoteTagPick = await window.showQuickPick<RemoteTagDeleteItem | QuickPickItem>(
			remoteTagPicks(),
			{ placeHolder: tagPickPlaceholder },
		)
	...
	async fetch(repository: Repository): Promise<void> {
		...
		quickpick.show()
		const remoteItem = await new Promise<RemoteItem | FetchAllRemotesItem | undefined>(
			(resolve) => {
	...
	async publish(repository: Repository): Promise<void> {
		...
		if (remotes.length === 0) {
			...
			if (publishers.length === 1) {
				...
				const picks = publishers.map((provider) => ({
					label:
						(provider.icon ? `$(${provider.icon}) ` : '') + l10n.t('Publish to {0}', provider.name),
					alwaysShow: true,
					provider,
				}))
		...
		const addRemote = new AddRemoteItem(this)
		const picks = [
			...repository.remotes.map((r) => ({ label: r.name, description: r.pushUrl })),
			addRemote,
		]
		const placeHolder = l10n.t('Pick a remote to publish the branch "{0}" to:', branchName)
		const choice = await window.showQuickPick(picks, { placeHolder })
	...
	async ignore(...resourceStates: SourceControlResourceState[]): Promise<void> {
		...
		const resources = resourceStates.filter((s) => s instanceof Resource).map((r) => r.resourceUri)
	...
	private async _stash(
		...
		const config = workspace.getConfiguration('git', Uri.file(repository.root))
		const promptToSaveFilesBeforeStashing = config.get<'always' | 'staged' | 'never'>(
	...
	async stashView(repository: Repository): Promise<void> {
		...
		const multiDiffSourceUri = toGitUri(Uri.file(repository.root), `stash@{${stash.index}}`, {
			scheme: 'git-stash',
		})
	...
	private async pickStash(repository: Repository, placeHolder: string): Promise<Stash | undefined> {
		...
		const result = await window.showQuickPick<StashItem | QuickPickItem>(getStashQuickPickItems(), {
			placeHolder,
		})
	...
	private async _resolveTimelineOpenCommitCommand(
		...
		const commitParentId =
			commit.parents.length > 0 ? commit.parents[0] : await repository.getEmptyTree()
				...
				switch (err.gitErrorCode) {
					...
					case GitErrorCodes.NoUserEmailConfigured:
						...
						choices.set(l10n.t('Learn More'), () =>
							commands.executeCommand('vscode.open', Uri.parse('https://aka.ms/vscode-setup-git')),
						)
						break
					...
					default: {
						...
						message = hint ? l10n.t('Git: {0}', hint) : l10n.t('Git error')
