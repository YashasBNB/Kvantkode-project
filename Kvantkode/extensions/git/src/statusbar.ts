...
class CheckoutStatusBar {
	...
	get command(): Command | undefined {
		...
		const command =
			this.state.isCheckoutRunning || this.state.isCommitRunning || this.state.isSyncRunning
				? ''
				: 'git.checkout'
...
class SyncStatusBar {
	...
	constructor(
		...
		const onEnablementChange = filterEvent(workspace.onDidChangeConfiguration, (e) =>
			e.affectsConfiguration('git.enableStatusBarSync'),
		)
	...
	get command(): Command | undefined {
		...
		if (!this.state.hasRemotes) {
			...
			const command =
				this.state.isCheckoutRunning || this.state.isCommitRunning ? '' : 'git.publish'
