...
async function createModel(
	...
	const excludes = isTrusted
		? []
		: workspaceFolders.map((f) => path.normalize(f.uri.fsPath).replace(/[\r\n]+$/, ''))
...
export async function _activate(context: ExtensionContext): Promise<GitExtensionImpl> {
	...
	if (!enabled) {
		const onConfigChange = filterEvent(workspace.onDidChangeConfiguration, (e) =>
		...
		eventToPromise(onEnabled).then(
			async () =>
				(result.model = await createModel(context, logger, telemetryReporter, disposables)),
		)
		return result
	}
