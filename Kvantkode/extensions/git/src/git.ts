...
function getGitErrorCode(stderr: string): string | undefined {
	if (
		...
		/cannot make pipe for command substitution|cannot create standard input pipe/.test(stderr)
	) {
		...
		switch (change[0]) {
			...
			case 'R': {
				...
				uri = renameUri = Uri.file(
					path.isAbsolute(newPath) ? newPath : path.join(repositoryRoot, newPath),
				)
				break
			}
			default:
...
export class Repository {
	...
	async add(paths: string[], opts?: { update?: boolean }): Promise<void> {
		...
		if (paths && paths.length) {
			for (const chunk of splitInChunks(
				paths.map((p) => sanitizeRelativePath(this.repositoryRoot, p)),
				MAX_CLI_LENGTH,
			)) {
	...
	async stage(path: string, data: Uint8Array): Promise<void> {
		const relativePath = sanitizeRelativePath(this.repositoryRoot, path)
		const child = this.stream(['hash-object', '--stdin', '-w', '--path', relativePath], {
		...
		try {
			...
			if (/^error: failed to push some refs to\b/m.test(err.stderr || '')) {
				if (
					...
					forcePushMode === ForcePushMode.ForceWithLeaseIfIncludes &&
					/! \[rejected\].*\(remote ref updated since checkout\)/m.test(err.stderr || '')
				) {
	...
	async getStatus(opts?: {
		...
		let result = new Promise<{ status: IFileStatus[]; statusLength: number; didHitLimit: boolean }>(
			(c, e) => {
	...
	async getRemoteRefs(
		remote: string,
		opts?: { heads?: boolean; tags?: boolean; cancellationToken?: CancellationToken },
	): Promise<Ref[]> {
		if (opts?.cancellationToken && opts?.cancellationToken.isCancellationRequested) {
			throw new CancellationError()
		}
	...
	async getCommitTemplate(): Promise<string> {
		try {
			...
			let templatePath = result.stdout
				...
				.replace(
					/^~([^\/]*)\//,
					(_, user) => `${user ? path.join(path.dirname(homedir), user) : homedir}/`,
				)
