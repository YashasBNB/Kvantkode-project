...
class ProgressManager {
	...
	constructor(private repository: Repository) {
		...
		this.repository.onDidChangeOperations(() => {
			...
			this.repository.sourceControl.inputBox.enabled = !this.repository.operations.isRunning(
	...
	private onDidChangeLogLevel(logLevel: LogLevel): void {
		...
		this.eventDisposable = combinedDisposable([
			this.onWorkspaceWorkingTreeFileChange((uri) =>
				this.logger.debug(`[FileEventLogger][onWorkspaceWorkingTreeFileChange] ${uri.fsPath}`),
			),
			this.onDotGitFileChange((uri) =>
				this.logger.debug(`[FileEventLogger][onDotGitFileChange] ${uri.fsPath}`),
			),
		])
	}
...
class DotGitWatcher implements IFileWatcher {
	...
	constructor(
		...
		const filteredRootWatcher = filterEvent(
			rootWatcher.event,
			(uri) =>
				uri.scheme === 'file' && !/\/\.git(\/index\.lock)?$|\/\.watchman-cookie-/.test(uri.path),
		)
...
export class Repository implements Disposable {
	...
	constructor(
		...
		const onRepositoryWorkingTreeFileChange = filterEvent(
			...
			(uri) => !/\.git($|\\|\/)/.test(relativePath(repository.root, uri.fsPath)),
		)
		...
		try {
			...
			onRepositoryDotGitFileChange = filterEvent(onRepositoryFileChange, (uri) =>
				/\.git($|\\|\/)/.test(uri.path),
			)
		}
		...
		this.disposables.push(
			new FileEventLogger(onRepositoryWorkingTreeFileChange, onRepositoryDotGitFileChange, logger),
		)
		...
		const onConfigListener = filterEvent(workspace.onDidChangeConfiguration, (e) =>
			e.affectsConfiguration('git.alwaysShowStagedChangesResourceGroup', root),
		)
		...
		const onBranchProtectionProviderChanged = filterEvent(
			this.branchProtectionProviderRegistry.onDidChangeBranchProtectionProviders,
			(e) => pathEquals(e.fsPath, root.fsPath),
		)
		this.disposables.push(
			onBranchProtectionProviderChanged((root) => this.updateBranchProtectionMatchers(root)),
		)
		this.disposables.push(
		...
		const onDidChangeCountBadge = filterEvent(workspace.onDidChangeConfiguration, (e) =>
			e.affectsConfiguration('git.countBadge', root),
		)
	...
	provideOriginalResource(uri: Uri): Uri | undefined {
		...
		if (
			this.untrackedGroup.resourceStates.some((r) => pathEquals(r.resourceUri.path, uri.path)) ||
			this.workingTreeGroup.resourceStates.some(
				(r) => pathEquals(r.resourceUri.path, uri.path) && r.type === Status.UNTRACKED,
			)
		) {
			this.logger.trace(
		...
		if (
			activeTabInput instanceof TabInputTextDiff &&
			pathEquals(activeTabInput.modified.fsPath, uri.fsPath)
		) {
			this.logger.trace(
				`[Repository][provideOriginalResource] Resource is on the right-hand side of a diff editor: ${uri.toString()}`,
			)
			return undefined
		}
		...
		if (
			activeTabInput instanceof TabInputTextMultiDiff &&
			activeTabInput.textDiffs.some((diff) => pathEquals(diff.modified.fsPath, uri.fsPath))
		) {
			this.logger.trace(
				`[Repository][provideOriginalResource] Resource is on the right-hand side of a multi-file diff editor: ${uri.toString()}`,
			)
			return undefined
		}
	...
	async getInputTemplate(): Promise<string> {
		const commitMessage = (
			await Promise.all([this.repository.getMergeMessage(), this.repository.getSquashMessage()])
		).find((msg) => !!msg)
		...
		await this.run(
			...
			() => {
				...
				const untrackedChangesResourceGroupType =
					untrackedChanges === 'mixed' ? ResourceGroupType.WorkingTree : ResourceGroupType.Untracked
		...
		function checkTabShouldClose(input: TabInputTextDiff | TabInputNotebookDiff) {
			...
			if (
				...
				(workingTreeResources === undefined ||
					workingTreeResources.some((r) => pathEquals(r, input.modified.fsPath)))
			) {
				// Working Tree
				return true
			}
	...
	async getBranches(
		...
		return await this.run(Operation.GetBranches, async () => {
			...
			return refs.filter(
				(value) =>
					value.type === RefType.Head || (value.type === RefType.RemoteHead && query.remote),
			)
		})
	}
		...
		await this.run(Operation.Sync, async () => {
			await this.maybeAutoStash(async () => {
				...
				const fn = async (cancellationToken?: CancellationToken) => {
					...
					if (await this.checkIfMaybeRebased(this.HEAD?.name)) {
						await this._pullAndHandleTagConflict(rebase, remoteName, pullBranch, {
							tags,
							cancellationToken,
							autoStash,
						})
				...
				const shouldPush =
					this.HEAD && (typeof this.HEAD.ahead === 'number' ? this.HEAD.ahead > 0 : true)
	...
	async ignore(files: Uri[]): Promise<void> {
		return await this.run(Operation.Ignore, async () => {
			...
			const textToAppend = files
				.map((uri) =>
					relativePath(this.repository.root, uri.fsPath).replace(/\\|\[/g, (match) =>
						match === '\\' ? '/' : `\\${match}`,
					),
				)
				.join('\n')
	...
	checkIgnore(filePaths: string[]): Promise<Set<string>> {
		return this.run(Operation.CheckIgnore, () => {
			return new Promise<Set<string>>((resolve, reject) => {
				...
				const child = this.repository.stream(['check-ignore', '-v', '-z', '--stdin'], {
		...
		try {
			...
			if (!operation.readOnly) {
				await this.updateModelState(
					this.optimisticUpdateEnabled() ? getOptimisticResourceGroups() : undefined,
				)
			}
	...
	private async getStatus(cancellationToken?: CancellationToken): Promise<GitResourceGroups> {
		...
		const { status, statusLength, didHitLimit } = await this.repository.getStatus({
			limit,
			ignoreSubmodules,
			similarityThreshold,
			untrackedChanges,
		...
		for (const provider of this.branchProtectionProviderRegistry.getBranchProtectionProviders(
			...
			for (const { remote, rules } of provider.provideBranchProtection()) {
				...
				for (const rule of rules) {
					const include =
						rule.include && rule.include.length !== 0 ? picomatch(rule.include) : undefined
					const exclude =
						rule.exclude && rule.exclude.length !== 0 ? picomatch(rule.exclude) : undefined
