...
export class GitTimelineItem extends TimelineItem {
	...
	setItemDetails(
		...
		if (hash) {
			...
			this.tooltip.appendMarkdown(
				`[\`$(git-commit) ${getCommitShortHash(uri, hash)} \`](command:git.viewCommit?${encodeURIComponent(JSON.stringify([uri, hash]))} "${l10n.t('Open Commit')}")`,
			)
			this.tooltip.appendMarkdown('&nbsp;')
			this.tooltip.appendMarkdown(
	...
	async provideTimeline(
		...
		for (let index = 0; index < commits.length; index++) {
			...
			const messageWithLinks =
				(await provideSourceControlHistoryItemMessageLinks(this.model, repo, message)) ?? message
