...
export = new (class implements eslint.Rule.RuleModule {
	...
	private _processOptions(options: RawOption[]): ImportPatternsConfig[] {
		...
		const layerRules: ILayerRule[] = [
			...
			{
				layer: 'electron-main',
				deps: orSegment(['common', 'node', 'electron-utility', 'electron-main']),
				isNode: true,
				isElectron: true,
			},
		]
		...
		for (const option of options) {
			...
			const restrictions = (
				typeof option.restrictions === 'string' ? [option.restrictions] : option.restrictions
			).slice(0)
			...
			if (targetIsVS && option.layer) {
				...
				configs.push({
					target,
					restrictions: <string[]>restrictions.filter((r) => typeof r === 'string'),
				})
			}
		}
