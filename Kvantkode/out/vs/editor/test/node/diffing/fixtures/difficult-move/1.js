		...
		case 'linux':
			return (
				product.nodejsRepository !== 'https://nodejs.org'
					...
					: fetchUrls(`/dist/v${nodeVersion}/node-v${nodeVersion}-${platform}-${arch}.tar.gz`, {
							base: 'https://nodejs.org',
							checksumSha256,
						})
			)
...
function packageTask(type, platform, arch, sourceFolderName, destinationFolderName) {
	...
	return () => {
		...
		const src = gulp
			...
			.pipe(
				rename(function (path) {
					path.dirname = path.dirname.replace(new RegExp('^' + sourceFolderName), 'out')
				}),
			)
		...
		const extensionPaths = [...localWorkspaceExtensions, ...marketplaceExtensions].map(
			(name) => `.build/extensions/${name}/**`,
		)

		const extensions = gulp.src(extensionPaths, { base: '.build', dot: true })
		const extensionsCommonDependencies = gulp.src('.build/extensions/node_modules/**', {
			base: '.build',
			dot: true,
		})
		const sources = es
