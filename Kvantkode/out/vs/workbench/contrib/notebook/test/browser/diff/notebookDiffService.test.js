...
suite('NotebookDiff Diff Service', () => {
    ...
    test('diff test small source', async () => {
        const { mapping, diff } = await mapCells([[['123456789'], 'javascript', CellKind.Code, [], {}]], [[['987654321'], 'javascript', CellKind.Code, [], {}]]);
        assert.deepStrictEqual(mapping, [{ modified: 0, original: 0 }]);
        assert.deepStrictEqual(diff.cellsDiff.changes, [
            {
                originalStart: 0,
    ...
    test('No cells and add 1 cell', async () => {
        const { mapping, diff } = await mapCells([], [[['print("Hello world")'], 'python', CellKind.Code, [], undefined]]);
    ...
    test('No changes with 1 cell', async () => {
        const { mapping, diff } = await mapCells([[['print("Hello world")'], 'python', CellKind.Code, [], undefined]], [[['print("Hello world")'], 'python', CellKind.Code, [], undefined]]);
        assert.deepStrictEqual(mapping, [{ modified: 0, original: 0 }]);
        assert.deepStrictEqual(diff.cellsDiff.changes, [
    ...
    test('One changes with 1 cell', async () => {
        const { mapping, diff } = await mapCells([[['print("Hello world")'], 'python', CellKind.Code, [], undefined]], [[['print("Foo Bar")'], 'python', CellKind.Code, [], undefined]]);
        assert.deepStrictEqual(mapping, [{ modified: 0, original: 0 }]);
        assert.deepStrictEqual(diff.cellsDiff.changes, [
            {
                originalStart: 0,
    ...
    test('Update code cell 1', async () => {
        const { mapping, diff } = await mapCells([
            ...
            [['# Updated markdown cell'], 'markdown', CellKind.Markup, [], undefined],
            [
                [
                    'from sklearn.preprocessing import LabelEncoder\n',
                    'import ast\n',
    ...
    test('Modification with Insertion, detected as deletion of a cell', async () => {
        const { mapping, diff } = await mapCells([
            ...
            {
                cell_type: 'code',
                source: ['warnings.filterwarnings("ignore", category=torch.jit.TracerWarning)'],
            },
            {
                cell_type: 'code',
                source: [
            ...
            {
                cell_type: 'markdown',
                source: ['## Inference with OpenVINO\n', '[back to top ⬆️](#Table-of-contents:)\n'],
            },
            {
                cell_type: 'code',
                source: ['core = ov.Core()'],
            ...
            {
                cell_type: 'code',
                source: ['frames = ov_pipe(prompt, num_inference_steps=25)["frames"]'],
            },
            {
                cell_type: 'code',
                source: [
    ...
    test('Modification of multiple cells', async () => {
        const { mapping, diff } = await mapCells([
            ...
            },
            {
                ...
                source: ['res = seg_model(IMAGE_PATH)\n', 'Image.fromarray(res[0].plot()[:, :, ::-1])'],
            },
            {
                cell_type: 'markdown',
                id: '7e918cf6-cdce-4e90-a6d7-c435a3c08d93',
                source: ['Great! The result is the same, as produced by original models.'],
            },
            {
                cell_type: 'markdown',
    ...
    test('Modification of a large cell and insert a new cell', async () => {
        const { mapping, diff } = await mapCells([
            ...
            },
            {
                ...
                source: ['ov_yolo_model = YOLOv10(ov_model_path.parent, task="detect")'],
            },
            {
                cell_type: 'code',
            ...
            },
            {
                ...
                source: ['ov_yolo_model.predictor.model.ov_compiled_model = det_compiled_model'],
            },
            {
                cell_type: 'code',
            ...
            },
            {
                ...
                source: ['### Gradio Interactive Demo\n', '[back to top ⬆️](#Table-of-contents:)'],
            },
            {
                cell_type: 'code',
    ...
    test('Misalignment due to 2 deleted cells', async () => {
        const { mapping, diff } = await mapCells([
            ...
            },
            {
                ...
                source: ['**NOTE:** `prompt_process.box_prompt` returns `numy.ndarray`'],
            },
            {
                cell_type: 'code',
                execution_count: null,
            ...
            },
            {
                ...
                source: ['**NOTE:** `prompt_process.point_prompt` returns `numy.ndarray`'],
            },
            {
                cell_type: 'code',
                execution_count: null,
            ...
            },
            {
                ...
                source: ['**NOTE:** `prompt_process.text_prompt` returns `numy.ndarray`'],
            },
            {
                cell_type: 'code',
                execution_count: null,
