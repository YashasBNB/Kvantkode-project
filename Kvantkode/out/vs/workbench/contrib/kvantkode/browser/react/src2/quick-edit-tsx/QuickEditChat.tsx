...
import { isFeatureNameDisabled } from '../../../../../../../workbench/contrib/kvantkode/common/voidSettingsTypes.js';
...
export const QuickEditChat = ({
  ...
  const [isStreamingRef, setIsStreamingRef] = useRefState(
    editCodeService.isCtrlKZoneStreaming({ diffareaid })
  );
  useCtrlKZoneStreamingState(
    useCallback(
      (diffareaid2, isStreaming) => {
  ...
  const loadingIcon =
  <div className="codicon codicon-loading codicon-modifier-spin codicon-no-default-spin void-text-void-fg-3" />;
  ...
  lookupKeybinding(VOID_CTRL_K_ACTION_ID)?.
  getLabel();

  const chatAreaRef = useRef<HTMLDivElement | null>(null);
  return (
    <div ref={sizerRef} style={{ maxWidth: 450 }} className={`void-py-2 void-w-full`}>
			<VoidChatArea
        featureName="Ctrl+K"
        divRef={chatAreaRef}
        onSubmit={onSubmit}
