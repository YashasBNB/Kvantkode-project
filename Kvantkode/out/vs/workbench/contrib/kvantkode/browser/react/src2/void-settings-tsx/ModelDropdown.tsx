...
import {
  VOID_OPEN_SETTINGS_ACTION_ID,
  VOID_TOGGLE_SETTINGS_ACTION_ID } from
'../../../voidSettingsPane.js';
import {
  modelFilterOfFeatureName,
  ModelOption } from
'../../../../../../../workbench/contrib/kvantkode/common/voidSettingsService.js';
...
}: {options: ModelOption[];featureName: FeatureName;className: string;}) => {
...
const MemoizedModelDropdown = ({
...
}: {featureName: FeatureName;className: string;}) => {
  ...
  useEffect(() => {
    ...
    const newOptionsAll = settingsState._modelOptions.filter((o) =>
    filter(o.selection, {
      chatMode: settingsState.globalSettings.chatMode,
      overridesOfModel: settingsState.overridesOfModel
    })
    );
    // Only show built-in backend provider
    const newOptions = newOptionsAll.filter((o) => o.selection.providerName === 'openAICompatible');
  ...
  return (
    <ModelSelectBox featureName={featureName} options={memoizedOptions} className={className} />);
