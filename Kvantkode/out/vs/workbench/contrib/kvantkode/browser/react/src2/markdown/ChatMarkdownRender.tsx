...
}: {text: string;className?: string;onClick?: () => void;tooltip?: string;}) => {
...
const CodespanWithLink = ({
...
}: {text: string;rawText: string;chatMessageLocation: ChatMessageLocation;}) => {
  ...
  if (rawText.endsWith('`')) {
    ...
    if (link === undefined) {
      ...
      chatThreadService.generateCodespanLink({ codespanStr: text, threadId }).then((link) => {
        chatThreadService.addCodespanLink({
          newLinkText: text,
          newLinkLocation: link,
          messageIdx,
          threadId
        });
  ...
  const onClick = () => {
    ...
    link.selection.startLineNumber,
    link.selection.endLineNumber]
    );else
    voidOpenFileFn(link.uri, accessor);
  };
  ...
  return (
    <Codespan
      ...
      onClick={onClick}
      className={
      link ? "void-underline hover:void-brightness-90 void-transition-all void-duration-200 void-cursor-pointer" : ""}
    ...
    if (displayMathRegex.test(rawText) || inlineMathRegex.test(rawText)) {
      ...
      while ((match = displayMathRegex.exec(rawText)) !== null) {
        ...
        if (matchIndex > lastIndex) {
          ...
          segments.push(<span key={`text-${segmentId++}`}>{textBefore}</span>);
        }

        // Add the LaTeX expression
        segments.push(<LatexRender key={`latex-${segmentId++}`} latex={fullMatch} />);
      ...
      if (lastIndex < rawText.length) {
        ...
        while ((match = inlineMathRegex.exec(remainingText)) !== null) {
          ...
          if (matchIndex > lastIndex) {
            ...
            inlineSegments.push(<span key={`inline-text-${segmentId++}`}>{textBefore}</span>);
          }

          // Add the inline LaTeX
          inlineSegments.push(<LatexRender key={`inline-latex-${segmentId++}`} latex={fullMatch} />);
        ...
        if (lastIndex < remainingText.length) {
          inlineSegments.push(
            <span key={`inline-final-${segmentId++}`}>{remainingText.substring(lastIndex)}</span>
          );
        }
...
const RenderToken = ({
...
}: {token: Token | string;inPTag?: boolean;codeURI?: URI;chatMessageLocation?: ChatMessageLocation;tokenIdx: string;} & RenderTokenOptions): React.ReactNode => {
  const accessor = useAccessor();
  ...
  if (t.type === 'code') {
    ...
    if (codeURI) {
      ...
      // get lang from the uri in the first line of the markdown
    ...
    if (t.lang) {
      // a language was provided. empty string is common so check truthy, not just undefined
  ...
  if (t.type === 'codespan') {
    if (options.isLinkDetectionEnabled && chatMessageLocation) {
      return (
        <CodespanWithLink text={t.text} rawText={t.raw} chatMessageLocation={chatMessageLocation} />);
...
}: {string: string;inPTag?: boolean;codeURI?: URI;chatMessageLocation: ChatMessageLocation | undefined;} & RenderTokenOptions) => {
