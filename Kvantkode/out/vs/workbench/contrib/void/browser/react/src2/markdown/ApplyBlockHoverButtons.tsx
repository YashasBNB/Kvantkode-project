...
import {
  ...
  ListableToolItem,
  voidOpenFileFn,
  ToolChildrenWrapper } from
'../sidebar-tsx/SidebarChat.js';
...
}: {codeStr: string | (() => Promise<string> | string);toolTipName: string;}) => {
...
export const JumpToFileButton = ({
  uri,
  ...props
}: {uri: URI | 'current';} & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
  const accessor = useAccessor();
...
const _applyingURIOfApplyBoxIdRef: {current: {[applyBoxId: string]: URI | undefined;};} = {
  current: {}
};
...
}: {indicatorColor: IndicatorColor;title?: React.ReactNode;className?: string;} & React.HTMLAttributes<HTMLDivElement>) => {
  return (
    <div
      className={`void-flex void-flex-row void-text-void-fg-3 void-text-xs void-items-center void-gap-1.5 ${className}`}
      {...props}>
...
}: {tooltipName: string;color?: IndicatorColor;position?: PlacesType;offset?: number;}) => ({
  'data-tooltip-id':
  color === 'orange' ?
  `void-tooltip-orange` :
  color === 'green' ?
...
export const StatusIndicatorForApplyButton = ({
  applyBoxId,
  uri
}: {applyBoxId: string;uri: URI | 'current';} & React.HTMLAttributes<HTMLDivElement>) => {
  ...
  const onClickSubmit = useCallback(async () => {
    ...
    if (!applyDonePromise) {
      notificationService.info(
        `KvantKode Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`
      );
    }
    ...
    applyDonePromise?.catch((e) => {
      ...
      notificationService.info(`KvantKode Error: There was a problem running Apply: ${e}.`);
    });
...
export const BlockCodeApplyWrapper = ({
  ...
  return (
    <div className="void-border void-border-void-border-3 void-rounded void-overflow-hidden void-bg-void-bg-3 void-my-1">
			{/* header */}
			<div className=" void-select-none void-flex void-justify-between void-items-center void-py-1 void-px-2 void-border-b void-border-void-border-3 void-cursor-default">
				<div className="void-flex void-items-center">
					...
					<span className="void-text-[13px] void-font-light void-text-void-fg-3">{name}</span>
				</div>
