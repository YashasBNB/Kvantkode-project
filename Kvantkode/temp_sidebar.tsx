...
import {
  ...
  useActiveURI,
  useCommandBarState,
  useFullChatThreadsStreamState } from
'../util/services.js';
...
import {
  ...
  getIsReasoningEnabledState } from
...
import {
  ...
  isABuiltinToolName,
  MAX_FILE_CHARS_PAGE,
  MAX_TERMINAL_INACTIVE_TIME } from
...
const BrokerPanel = ({ userId, onChanged }: {userId: string;onChanged: () => void;}) => {
  ...
  const readBalance = useCallback(async () => {
    ...
    try {
      ...
      setBalance({ cashBalance: data.cashBalance, currency: data.currency || null });else
      setBalance(null);
    } catch (e: any) {
  ...
  const deleteBroker = async (brokerId: string) => {
    ...
    try {
      ...
      let friendly = raw;
      if (raw === 'user_exists') friendly = 'An account with this email or phone already exists.';else
      if (raw === 'invalid_otp') friendly = 'The OTP is incorrect or has expired.';else
      if (raw === 'phone_in_use') friendly = 'This phone number is already registered.';else
      if (raw === 'invalid_credentials') friendly = 'Invalid email/phone or password.';
      setError(friendly);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="void-px-4 void-py-3 void-border-t void-border-void-border-2 void-h-full void-overflow-y-auto">
            ...
            'No active subscription'}
					...
					{subError ?
          <div className="void-text-[11px] void-text-red-500 void-truncate" title={subError}>
							{subError}
						</div> :
          null}
				</div>
			...
			<div className="void-flex void-items-center void-justify-between void-mb-2">
				<div className="void-text-sm">Broker</div>
				<div className="void-text-xs void-text-void-fg-3 void-flex void-items-center void-gap-2">
					{loadingBalance ?
          ...
          <span className="void-opacity-70">Balance unavailable</span>
          }
				</div>
			</div>
			...
			<div className="void-grid void-gap-2 void-mb-2">
				<select
        ...
        <input
          ...
          onChange={(e) => setForm({ ...form, tokenId: e.target.value })} />
				...
				<label className="void-flex void-items-center void-gap-2 void-text-xs">
					<input
            type="checkbox"
          ...
          {' '}
				...
				{form.broker === 'Dhan' &&
        <div className="void-flex void-items-center void-gap-2 void-text-xs">
						<button
            className="void-px-2 void-py-1 void-rounded void-border void-border-void-border-2 void-bg-void-bg-2 disabled:void-opacity-50"
            ...
            onClick={() => generateDhanToken()}>
						...
						{dhanConsentError &&
          <span className="void-text-void-warning">{dhanConsentError}</span>
          }
						{dhanTokenError &&
          <span className="void-text-void-warning">{dhanTokenError}</span>
          }
					</div>
        }
				{error && <div className="void-text-void-warning void-text-xs">{error}</div>}
				{balanceError && <div className="void-text-void-warning void-text-xs">{balanceError}</div>}
				<button
          className="void-bg-white void-text-black void-rounded void-p-2 void-text-sm disabled:void-opacity-50"
          disabled={loading}
          onClick={onSubmit}>
			...
			<div className="void-text-xs void-text-void-fg-3 void-mb-1">Connected brokers</div>
			<div className="void-flex void-flex-col void-gap-2">
				{brokers?.length ?
        brokers.map((b: any) =>
        <div
          key={b._id}
          className="void-text-xs void-p-2 void-rounded void-border void-border-void-border-2 void-flex void-flex-col void-gap-2">
          ...
          <>
              ...
              <>
                  ...
                  onChange={(e) => setEditForm({ ...editForm, mpin: e.target.value })} />
									...
									<div className="void-flex void-items-center void-gap-2 void-justify-end">
										<button
                className="void-px-2 void-py-1 void-rounded void-border void-border-void-border-2"
                onClick={cancelEdit}
                disabled={loading}>
										...
										<button
                className="void-px-2 void-py-1 void-rounded void-bg-white void-text-black"
          ...
          <div className="void-flex void-items-center void-justify-between void-gap-2">
									<div>
										<div className="void-font-mono">{b.broker || 'Broker'}</div>
										<div className={`void-opacity-70 ${b.isActive ? "void-text-green-400" : ""}`}>
                ...
                disabled={loading}>
										...
										<button
                disabled={loading}
                className="void-px-2 void-py-1 void-rounded void-border void-border-void-border-2 void-text-void-warning"
        ...
        <div className="void-text-xs void-opacity-70">No brokers yet</div>
        }
			</div>
			<div className="void-mt-3">
				<div className="void-text-xs void-text-void-fg-3 void-mb-1">Positions</div>
				{poError && <div className="void-text-void-warning void-text-xs void-mb-1">{poError}</div>}
				<div className="void-flex void-flex-col void-gap-1">
					{loadingPo ?
          <div className="void-text-xs void-opacity-70">Loading…</div> :
          positions.length ?
          positions.map((p, i) =>
          <div
            key={i}
            className="void-text-xs void-p-2 void-rounded void-border void-border-void-border-2 void-flex void-justify-between">

								<div className="void-font-mono">{p.symbol}</div>
								<div className="void-opacity-80">Qty: {p.qty}</div>
							</div>
          ) :

          <div className="void-text-xs void-opacity-70">No positions</div>
          }
				</div>
			</div>
			<div className="void-mt-3">
				<div className="void-text-xs void-text-void-fg-3 void-mb-1">Recent orders</div>
				<div className="void-flex void-flex-col void-gap-1">
					{loadingPo ?
          <div className="void-text-xs void-opacity-70">Loading…</div> :
          orders.length ?
          ...
          <div
            ...
            className="void-text-xs void-p-2 void-rounded void-border void-border-void-border-2 void-flex void-justify-between">
								...
								<div className="void-font-mono">{o.symbol}</div>
								<div className="void-opacity-80">
									{o.side} {o.qty} · {o.status}
								</div>
							</div>
          ) :

          <div className="void-text-xs void-opacity-70">No orders</div>
          }
				</div>
			</div>
		</div>);
...
const AuthPanel = ({
  ...
  return (
    <div className="void-w-full void-h-full void-flex void-items-center void-justify-center void-p-4">
			<div className="void-bg-void-bg-1 void-border void-border-void-border-2 void-rounded void-p-4 void-w-full void-max-w-sm void-flex void-flex-col void-gap-2">
				{subscription && !subscription.isActive && subscription.paymentLink ?
        <div className="void-flex void-flex-col void-gap-3">
						<div className="void-text-lg">
          ...
          <div className="void-text-xs void-text-void-fg-3">
						...
						<button
            className="void-px-4 void-py-2 void-rounded void-bg-white void-text-black void-font-semibold"
            onClick={() => {
            ...
            disabled={pollingPayment}>
						...
						<button
            className="void-text-xs void-underline void-text-void-fg-3"
            onClick={async () => {
              setError(null);
              const token = localStorage.getItem('void_jwt');
              if (token) {
        ...
        <div className="void-text-center void-text-void-fg-3">Checking subscription…</div> :

        <>
						<div className="void-text-lg">{mode === 'login' ? 'Login' : 'Sign up'}</div>
						{mode === 'signup' &&
          <input
            className="void-bg-void-bg-2 void-border void-border-void-border-2 void-rounded void-p-2"
          ...
          <input
            ...
            onChange={(e) => setConfirmPassword(e.target.value)} />
						...
						<label className="void-text-[11px] void-text-void-fg-3 void-leading-snug void-flex void-items-start void-gap-2">
							<button
              type="button"
              className="void-mt-[2px] void-w-3 void-h-3 void-border void-border-void-border-2 void-rounded void-flex void-items-center void-justify-center void-bg-void-bg-1"
              onClick={() => setAcceptedTerms((v) => !v)}>

								{acceptedTerms &&
              <svg className="void-w-2 void-h-2 void-text-void-fg-2" fill="currentColor" viewBox="0 0 20 20">
							...
							<span>
								...
								<button
                type="button"
                className="void-underline void-text-void-fg-3 hover:void-text-void-fg-2"
            ...
            onClick={doSubmit}>
				...
				<button
            className="void-text-void-fg-3 void-text-xs void-underline"
  ...
  if (canTurnOffReasoning && !reasoningBudgetSlider) {
    ...
    return (
      <div className="void-flex void-items-center void-gap-x-2">
				<span className="void-text-void-fg-3 void-text-xs void-pointer-events-none void-inline-block void-w-10 void-pr-1">
					Thinking
				</span>
				<VoidSwitch
          size="xxs"
    ...
    <div className="">
        ...
        className={`void-w-full void-border void-border-void-border-3 void-rounded void-px-2 void-py-1 void-bg-void-bg-3 void-overflow-hidden ${className}`}>
				...
				<div className={`void-select-none void-flex void-items-center void-min-h-[24px]`}>
					<div
            className={`void-flex void-items-center void-w-full void-gap-x-2 void-overflow-hidden void-justify-between ${isRejected ? "void-line-through" : ""}`}>
    ...
    if (canInitialize && shouldInitialize) {
      setStagingSelections(
        (chatMessage.selections || []).map((s) => {
          ...
          if (s.type === 'File')
          return { ...s, state: { ...s.state, wasAddedAsCurrentFile: false } };else
          return s;
        })
      );
  ...
  return (
    <div
      ...
      <div className="void-flex-shrink-0">
					<svg className="void-w-1 void-h-1 void-opacity-60 void-mr-1.5 void-fill-current" viewBox="0 0 100 40">
...
const CommandTool = ({
  ...
  if (toolMessage.type === 'success') {
    ...
    let msg: string;
    if (type === 'run_command')
    msg = toolsService.stringOfResult['run_command'](toolMessage.params, result);else
...
const CommandBarInChat = () => {
  ...
  <div className="void-px-2 void-gap-1 void-w-full void-overflow-y-auto">
...
export const SidebarChat = () => {
  ...
  const isRunning = currThreadStreamState?.isRunning === 'LLM' || currThreadStreamState?.isRunning === 'tool' || currThreadStreamState?.isRunning === 'awaiting_user';
  ...
  <ScrollToBottomContainer
    ...
    className={` void-flex void-flex-col void-px-4 void-py-4 void-space-y-4 void-w-full void-flex-1 void-min-h-0 void-overflow-x-hidden void-overflow-y-auto ${
    ...
    <ProseWrapper>{<IconLoading className="void-opacity-50 void-text-sm" />}</ProseWrapper> :
    null}
  ...
  <div
    ref={sidebarRef}
    className="void-w-full void-flex-1 void-min-h-0 void-max-h-full void-flex void-flex-col void-overflow-auto void-px-4">
  ...
  <div ref={sidebarRef} className="void-w-full void-flex-1 void-min-h-0 void-flex void-flex-col void-overflow-hidden">
  ...
  return (
    <div className="void-relative void-w-full void-h-full void-min-h-0 void-flex void-flex-col">
			{authChecked && authed &&
      <button
        className="void-absolute void-top-2 void-right-2 void-z-40 void-px-2 void-py-1 void-rounded void-bg-white void-text-black void-text-xs"
        onClick={() => {
          try {
            localStorage.removeItem('void_jwt');
          } catch {}
          ...
          setAuthChecked(true);
			...
			<div
        className={`${!authChecked || !authed ? "void-pointer-events-none" : ""} void-w-full void-h-full void-min-h-0 void-flex void-flex-col`}>

				<div className="void-flex void-items-center void-gap-4 void-px-4 void-py-2 void-border-b void-border-void-border-2 void-select-none">
					<button
            className={`void-text-xs void-tracking-wide ${activeTab === 'chat' ? "void-text-white void-underline" : "void-text-void-fg-3"}`}
            onClick={() => setActiveTab('chat')}>

						CHAT
					</button>
					<button
            className={`void-text-xs void-tracking-wide ${activeTab === 'broker' ? "void-text-white void-underline" : "void-text-void-fg-3"}`}
          ...
          null}
			...
			{(!authChecked || !authed) &&
      <div
        className="void-fixed void-inset-0 void-bg-black void-flex void-items-center void-justify-center"
        style={{ zIndex: 2147483647, pointerEvents: 'auto' }}>

					<div className="void-w-full void-h-full void-absolute void-inset-0" />
					<div className="void-relative void-z-10 void-w-full void-h-full">
						<AuthPanel
            onAuthed={(token, user) => {
