	...
	private addCustomViewContainers(
		...
		let activityBarOrder =
			...
			viewContainersRegistry.all.filter(
				(v) =>
					!!v.extensionId &&
					viewContainersRegistry.getViewContainerLocation(v) === ViewContainerLocation.Sidebar,
			).length
		let panelOrder =
			5 +
			viewContainersRegistry.all.filter(
				(v) =>
					!!v.extensionId &&
					viewContainersRegistry.getViewContainerLocation(v) === ViewContainerLocation.Panel,
			).length +
			1
					...
					if (viewContainer !== existingViewContainer) {
						viewsToMove.push(
							...this.viewsRegistry
								...
								.filter(
									(view) => (view as ICustomViewDescriptor).originalContainerId === descriptor.id,
								),
						)
					}
				}
			...
			Object.entries(value).forEach(([key, value]) => {
				...
				for (let index = 0; index < value.length; index++) {
					...
					if (this.viewsRegistry.getView(item.id) !== null) {
						collector.error(
							localize('duplicateView2', 'A view with id `{0}` is already registered.', item.id),
						)
						continue
					}
					...
					const viewDescriptor: ICustomViewDescriptor = {
						...
						treeView:
							type === ViewType.Tree
								...
								: undefined,
						collapsed:
							this.showCollapsed(container) || initialVisibility === InitialVisibility.Collapsed,
						order: order,
