			...
			return session.customRequest(request, args).then((response) => {
				if (response && response.success) {
					...
					return Promise.reject(
						new ErrorNoTelemetry(response ? response.message : 'custom request failed'),
					)
				}
			})
		}
			...
			.map((bp) => {
				if ('name' in bp) {
					...
					return {
						...
						id: dbp.getId(),
						dataId:
							dbp.src.type === DataBreakpointSetType.Variable ? dbp.src.dataId : dbp.src.address,
						enabled: dbp.enabled,
...
class ExtensionHostDebugAdapter extends AbstractDebugAdapter {
	...
	startSession(): Promise<void> {
		return Promise.resolve(
			this._proxy.$startDASession(this._handle, this._ds.getSessionDto(this.session)),
		)
	}
