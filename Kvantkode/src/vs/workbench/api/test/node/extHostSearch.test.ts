		...
		test('basic sibling exclude clause', async () => {
			...
			await registerTestFileSearchProvider({
				provideFileSearchResults(
					...
					return Promise.resolve(
						reportedResults.map((relativePath) => joinPath(options.folder, relativePath)),
					)
				},
			})
			...
			await registerTestFileSearchProvider({
				provideFileSearchResults(
					...
					if (options.folder.fsPath === rootFolderA.fsPath) {
						reportedResults = ['folder/fileA.scss', 'folder/fileA.css', 'folder/file2.css'].map(
							(relativePath) => joinPath(rootFolderA, relativePath),
						)
					} else {
						reportedResults = ['fileB.ts', 'fileB.js', 'file3.js'].map((relativePath) =>
		...
		function assertResults(actual: IFileMatch[], expected: vscode.TextSearchResult[]) {
			...
			const makeComparable = (results: vscode.TextSearchResult[]) =>
				results
					.sort((a, b) => {
						const compareKeyA =
							a.uri.toString() + ': ' + (extensionResultIsMatch(a) ? a.preview.text : a.text)
						const compareKeyB =
							b.uri.toString() + ': ' + (extensionResultIsMatch(b) ? b.preview.text : b.text)
