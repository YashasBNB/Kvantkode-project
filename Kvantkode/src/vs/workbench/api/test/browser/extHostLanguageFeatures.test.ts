...
suite('ExtHostLanguageFeatures', function () {
	...
	setup(() => {
		...
		mainThread = rpcProtocol.set(
			...
			disposables.add(inst.createInstance(MainThreadLanguageFeatures, rpcProtocol)),
		)
	})
	...
	test('Quick Outline uses a not ideal sorting, #138502', async function () {
		...
		assert.deepStrictEqual(
			value.map((s) => s.name),
			['containers', 'container 0', 'name', 'ports', 'ports 0', 'containerPort'],
		)
	})
	...
	test('CodeLens, do not resolve a resolved lens', async () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const symbol = await Promise.resolve(
				data.provider.resolveCodeLens!(model, data.symbol, CancellationToken.None),
			)
	...
	test('Parameter Hints, order', async () => {
		...
		const value = await provideSignatureHelp(
			...
			model,
			new EditorPosition(1, 1),
			{ triggerKind: languages.SignatureHelpTriggerKind.Invoke, isRetrigger: false },
			CancellationToken.None,
		)
		assert.ok(value)
	})
		...
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const value = await provideSuggestionItems(
				...
				new CompletionOptions(
					undefined,
					new Set<languages.CompletionItemKind>().add(languages.CompletionItemKind.Snippet),
				),
			)
	...
	test('Links, data conversion', async () => {
		...
		const { links } = disposables.add(
			await getLinks(languageFeaturesService.linkProvider, model, CancellationToken.None),
		)
