	...
	implements ExtHostExtensionServiceShape
{
	...
	private _loadExtensionContext(
		...
		const languageModelAccessInformation =
			this._extHostLanguageModels.createLanguageModelAccessInformation(extensionDescription)
	...
	private static _callActivateOptional(
		...
		if (typeof extensionModule.activate === 'function') {
			try {
				...
				const activateResult: Promise<IExtensionAPI> = extensionModule.activate.apply(scope, [
					context,
				])
	...
	private async _doHandleExtensionTests(): Promise<number> {
		...
		const testRunner = await this._loadCommonJSModule<ITestRunner | INewTestRunner | undefined>(
			null,
		...
		const tunnelInformation: TunnelInformation = {
			...
			features: result.tunnelFeatures
				? {
						...
						protocol:
							result.tunnelFeatures.protocol === undefined ? true : result.tunnelFeatures.protocol,
					}
				: undefined,
		}
