...
suite('EditorPane', () => {
	...
	test('Editor Pane Registration', function () {
		...
		disposables.add(
			editorRegistry.registerEditorPane(editorDescriptor1, [new SyncDescriptor(TestInput)]),
		)
		disposables.add(
	...
	test('Editor Pane Lookup favors specific class over superclass (match on specific class)', function () {
		...
		disposables.add(
			editorRegistry.registerEditorPane(d1, [new SyncDescriptor(TestResourceEditorInput)]),
		)
	...
	test('Editor Input Serializer', function () {
		...
		workbenchInstantiationService(undefined, disposables).invokeFunction((accessor) =>
			editorInputRegistry.start(accessor),
		)
		disposables.add(
			editorInputRegistry.registerEditorSerializer(testInput.typeId, TestInputSerializer),
		)
	...
	test('EditorMemento - basics', function () {
		...
		memento = disposables.add(
			new EditorMemento('id', 'key', rawMemento, 3, editorGroupService, configurationService),
		)
		assert.ok(memento.loadEditorState(testGroup0, URI.file('/C')))
		assert.ok(memento.loadEditorState(testGroup0, URI.file('/D')))
		assert.ok(memento.loadEditorState(testGroup0, URI.file('/E')))
