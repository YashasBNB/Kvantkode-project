...
suite('Tree Sitter TokenizationFeature', function () {
	...
	setup(async () => {
		...
		instantiationService.set(
			ITreeSitterImporter,
			instantiationService.createInstance(TreeSitterImporter),
		)
		instantiationService.set(
			ICodeEditorService,
			instantiationService.createInstance(TestCodeEditorService),
		)
		...
		treeSitterParserService = disposables.add(
			instantiationService.createInstance(TreeSitterTextModelService),
		)
		...
		treeSitterTokenizationSupport = disposables.add(
			(await TreeSitterTokenizationRegistry.getOrCreate(
				'typescript',
			)) as ITreeSitterTokenizationSupport & IDisposable,
		)
	})
...
class y {
	...
	test('File single line file', async () => {
		...
		const model = await getModelAndPrepTree(content)
		const tokens = treeSitterTokenizationSupport.getTokensInRange(
			model,
			new Range(1, 1, 1, 18),
			0,
			17,
