...
export class NativeExtensionService extends AbstractExtensionService implements IExtensionService {
	...
	private async _getCanonicalURI(remoteAuthority: string, uri: URI): Promise<URI> {
		...
		const results = await Promise.all(
			localProcessExtensionHosts.map((extHost) => extHost.getCanonicalURI(remoteAuthority, uri)),
		)
	...
	private async _handleNoResolverFound(remoteAuthority: string): Promise<boolean> {
		...
		if (extension) {
			if (
				!extensionIsEnabled(this._logService, this._extensionEnablementService, extension, false)
			) {
						...
						run: async () => {
							...
							if (galleryExtension) {
								...
								this._notificationService.error(
									nls.localize('resolverExtensionNotFound', '`{0}` not found on marketplace'),
								)
							}
						},
					},
		...
		switch (runningLocation.kind) {
			...
			case ExtensionHostKind.LocalWebWorker: {
				if (this._webWorkerExtHostEnablement !== LocalWebWorkerExtHostEnablement.Disabled) {
					const startup = isInitialStart
						? this._webWorkerExtHostEnablement === LocalWebWorkerExtHostEnablement.Lazy
...
function determineLocalWebWorkerExtHostEnablement(
	...
	if (
		...
		const config =
			configurationService.getValue<WebWorkerExtHostConfigValue>(webWorkerExtHostConfig)
		if (config === true) {
			return LocalWebWorkerExtHostEnablement.Eager
		} else if (config === 'auto') {
