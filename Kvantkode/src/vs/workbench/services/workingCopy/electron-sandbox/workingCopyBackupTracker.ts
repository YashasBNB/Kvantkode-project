	...
	implements IWorkbenchContribution
{
	...
	protected async onBeforeShutdownWithModified(
		...
		const workingCopiesToAutoSave = modifiedWorkingCopies.filter(
			(wc) =>
				...
				this.filesConfigurationService.getAutoSaveMode(wc.resource).mode !== AutoSaveMode.OFF,
		)
				...
				if (isMacintosh || (await this.nativeHostService.getWindowCount()) > 1) {
					if (this.contextService.getWorkbenchState() !== WorkbenchState.EMPTY) {
						return modifiedWorkingCopies.filter(
							(modifiedWorkingCopy) =>
								modifiedWorkingCopy.capabilities & WorkingCopyCapabilities.Scratchpad,
				...
				if (result !== false) {
					await Promises.settled(
						workingCopies.map((workingCopy) =>
							workingCopy.isModified() ? workingCopy.save(saveOptions) : Promise.resolve(true),
						),
					)
				}
			},
			...
			async () => {
				...
				await Promises.settled(
					modifiedWorkingCopies.map((workingCopy) =>
						workingCopy.isModified() ? workingCopy.revert(revertOptions) : Promise.resolve(),
					),
				)
			},
			localize(
	...
	private discardBackupsBeforeShutdown(
		backupsToDiscardOrKeep: IWorkingCopyIdentifier[] | { except: IWorkingCopyIdentifier[] },
	): Promise<void>
			...
			async () => {
				...
				try {
					if (Array.isArray(arg1)) {
						await Promises.settled(
							arg1.map((workingCopy) => this.workingCopyBackupService.discardBackup(workingCopy)),
						)
					} else {
