...
suite('WorkspaceContextService - Folder', () => {
	...
	setup(async () => {
		...
		const userDataProfilesService = disposables.add(
			new UserDataProfilesService(environmentService, fileService, uriIdentityService, logService),
		)
		disposables.add(
...
suite('WorkspaceContextService - Workspace Editing', () => {
	...
	setup(async () => {
		...
		instantiationService.stub(
			ITextModelService,
			disposables.add(instantiationService.createInstance(TextModelResolverService)),
		)
		instantiationService.stub(
			IJSONEditingService,
	...
	;(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[3][0]).added.map((f) => f.uri.toString()),
					[folder.toString()],
				)
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				await fileService.writeFile(
					...
					VSBuffer.fromString('{ "initialization.testSetting1": "workspaceValue" }'),
				)
				await testObject.initialize(convertToWorkspacePayload(folder))
				...
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[4][0]).added.map((f) => f.uri.toString()),
					[folder.toString()],
				)
	...
	;(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(target.args[1], [undefined])
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[3][0]).added.map((folder) =>
						folder.uri.toString(),
					),
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				await fileService.writeFile(
					...
					VSBuffer.fromString('{ "initialization.testSetting1": "workspaceValue1" }'),
				)
				await fileService.writeFile(
					joinPath(ROOT, 'b', '.vscode', 'settings.json'),
					VSBuffer.fromString('{ "initialization.testSetting2": "workspaceValue2" }'),
				)
				await testObject.initialize(getWorkspaceIdentifier(configResource))
				...
				assert.deepStrictEqual(target.args[2], [undefined])
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[4][0]).added.map((folder) =>
						folder.uri.toString(),
					),
					[joinPath(ROOT, 'a').toString(), joinPath(ROOT, 'b').toString()],
				)
	...
	;(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[1][0]).added.map((folder_1) =>
						folder_1.uri.toString(),
					),
					[joinPath(ROOT, 'b').toString()],
				)
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[1][0]).removed.map((folder_2) =>
						folder_2.uri.toString(),
					),
	...
	;(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				await fileService.writeFile(
					joinPath(ROOT, 'b', '.vscode', 'settings.json'),
					VSBuffer.fromString('{ "initialization.testSetting1": "workspaceValue2" }'),
				)
				await testObject.initialize(convertToWorkspacePayload(joinPath(ROOT, 'b')))
				...
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[2][0]).added.map((folder_1) =>
						folder_1.uri.toString(),
					),
					[joinPath(ROOT, 'b').toString()],
				)
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[2][0]).removed.map((folder_2) =>
						folder_2.uri.toString(),
					),
					[joinPath(ROOT, 'a').toString()],
				)
	...
	;(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(target.args[2], [undefined])
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>target.args[4][0]).added.map((folder_1) =>
						folder_1.uri.toString(),
					),
					[joinPath(ROOT, 'b').toString()],
				)
...
suite('WorkspaceConfigurationService - Folder', () => {
	...
	setup(async () => {
		...
		instantiationService.stub(
			IKeybindingEditingService,
			disposables.add(instantiationService.createInstance(KeybindingsEditingService)),
		)
		instantiationService.stub(
			ITextFileService,
			disposables.add(instantiationService.createInstance(TestTextFileService)),
	...
	test('globals override defaults', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.testSetting": "userValue" }'),
			)
			await testObject.reloadConfiguration()
			assert.strictEqual(
	...
	test('workspace settings', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
	...
	test('workspace settings override user settings', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString('{ "configurationService.folder.testSetting": "workspaceValue" }'),
			)
			await testObject.reloadConfiguration()
			assert.strictEqual(
				testObject.getValue('configurationService.folder.testSetting'),
	...
	test('machine overridable settings override user Settings', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await testObject.reloadConfiguration()
			assert.strictEqual(
				testObject.getValue('configurationService.folder.machineOverridableSetting'),
				'workspaceValue',
			)
		}))
	...
	test('workspace settings override user settings after defaults are registered ', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.newSetting": "userValue" }'),
			)
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString('{ "configurationService.folder.newSetting": "workspaceValue" }'),
			)
			await testObject.reloadConfiguration()
			configurationRegistry.registerConfiguration({
				id: '_test',
	...
	test('machine overridable settings override user settings after defaults are registered ', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue('configurationService.folder.newMachineOverridableSetting'),
				'workspaceValue',
			)
		}))
	...
	test('application settings are not read from workspace', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.applicationSetting": "userValue" }'),
			)
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString(
	...
	test('machine settings are not read from workspace', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.machineSetting": "userValue" }'),
			)
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString('{ "configurationService.folder.machineSetting": "workspaceValue" }'),
			)
	...
	test('application machine overridable settings are not read from workspace', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue('configurationService.folder.applicationMachineSetting'),
				'userValue',
			)
		}))
	...
	test('get application scope settings are loaded after defaults are registered', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.applicationSetting-2": "userValue" }'),
			)
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString(
	...
	test('get application scope settings are not loaded after defaults are registered when workspace folder uri is passed', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.applicationSetting-3": "userValue" }'),
			)
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString(
			...
			await testObject.reloadConfiguration()
			assert.strictEqual(
				testObject.getValue('configurationService.folder.applicationSetting-3', {
					resource: workspaceService.getWorkspace().folders[0].uri,
				}),
				'workspaceValue',
			)
	...
	test('get application machine overridable scope settings are loaded after defaults are registered', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await testObject.reloadConfiguration()
			assert.strictEqual(
				testObject.getValue('configurationService.folder.applicationMachineSetting-2'),
				'workspaceValue',
			)
	...
	test('get application machine overridable scope settings are loaded after defaults are registered when workspace folder uri is passed', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await testObject.reloadConfiguration()
			assert.strictEqual(
				testObject.getValue('configurationService.folder.applicationMachineSetting-3', {
					resource: workspaceService.getWorkspace().folders[0].uri,
				}),
				'workspaceValue',
			)
	...
	test('get machine scope settings are not loaded after defaults are registered', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.machineSetting-2": "userValue" }'),
			)
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString('{ "configurationService.folder.machineSetting-2": "workspaceValue" }'),
			)
	...
	test('get machine scope settings are not loaded after defaults are registered when workspace folder uri is passed', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.machineSetting-3": "userValue" }'),
			)
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString('{ "configurationService.folder.machineSetting-3": "workspaceValue" }'),
			)
		...
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			const result = await runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				await fileService.writeFile(
					...
					VSBuffer.fromString('{ "configurationService.folder.policySetting": "policyValue" }'),
				)
				return promise
			})
			assert.deepStrictEqual(
	...
	test('policy settings when policy value is not set', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.policySetting": "userValue" }'),
			)
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString('{ "configurationService.folder.policySetting": "workspaceValue" }'),
			)
			await testObject.reloadConfiguration()
			assert.strictEqual(
				testObject.getValue('configurationService.folder.policySetting'),
	...
	test('policy value override all for object type setting', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				await fileService.writeFile(
					environmentService.policyFile!,
					VSBuffer.fromString('{ "configurationService.folder.policyObjectSetting": {"a": true} }'),
				)
				return promise
			})
			...
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.policyObjectSetting": {"b": true} }'),
			)
			await testObject.reloadConfiguration()
	...
	test('inspect', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'tasks.json'),
				VSBuffer.fromString('{ "configurationService.tasks.testSetting": "tasksValue" }'),
			)
			await testObject.reloadConfiguration()
	...
	test('inspect restricted settings after change', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(actual.value, 'userRestrictedValue')
			assert.strictEqual(
				event.affectsConfiguration('configurationService.folder.restrictedSetting'),
				true,
			)
		}))
	...
	test('update user configuration', () => {
		return testObject
			.updateValue('configurationService.folder.testSetting', 'value', ConfigurationTarget.USER)
			.then(() =>
				assert.strictEqual(testObject.getValue('configurationService.folder.testSetting'), 'value'),
			)
	})

	test('update workspace configuration', () => {
		return testObject
			.updateValue('tasks.service.testSetting', 'value', ConfigurationTarget.WORKSPACE)
			.then(() =>
				assert.strictEqual(testObject.getValue(TasksSchemaProperties.ServiceTestSetting), 'value'),
			)
	})
	...
	test('update language configuration for multiple languages when already set', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const actualContent = (
				await fileService.readFile(userDataProfileService.currentProfile.settingsResource)
			).value.toString()
	...
	test('update resource language configuration for a language using configuration overrides', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await testObject.updateValue(
				...
				{ resource: workspaceService.getWorkspace().folders[0].uri, overrideIdentifier: 'jsonc' },
				ConfigurationTarget.WORKSPACE_FOLDER,
			)
			assert.strictEqual(
				testObject.getValue('configurationService.folder.languageSetting', {
	...
	test('update tasks configuration', () => {
		return testObject
			...
			.then(() =>
				assert.deepStrictEqual(testObject.getValue(TasksSchemaProperties.Tasks), {
					version: '1.0.0',
	...
	test('change event when there are global tasks', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				...
				VSBuffer.fromString('{ "version": "1.0.0", "tasks": [{ "taskName": "myTask" }'),
			)
			const promise = Event.toPromise(testObject.onDidChangeConfiguration)
	...
	test('restricted setting is read from workspace when workspace is trusted', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.restrictedSetting": "userValue" }'),
			)
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString(
	...
	test('remove an unregistered setting', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.folder.unknownSetting": "userValue" }'),
			)
			await fileService.writeFile(
				joinPath(workspaceService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString('{ "configurationService.folder.unknownSetting": "workspaceValue" }'),
			)
...
suite('WorkspaceConfigurationService - Profiles', () => {
			...
			disposables.add(
				new UserDataProfileService(
					toUserDataProfile(
						'custom',
						'custom',
						joinPath(environmentService.userRoamingDataHome, 'profiles', 'temp'),
	...
	test('initialize', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			assert.strictEqual(
				testObject.getValue('configurationService.profiles.applicationSetting2'),
				'applicationValue',
			)
			assert.strictEqual(
	...
	test('inspect', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.profiles.testSetting": "applicationValue" }'),
			)
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.profiles.testSetting": "profileValue" }'),
			)
			await testObject.reloadConfiguration()
	...
	test('update application machine setting', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue('configurationService.profiles.applicationMachineSetting'),
				'applicationValue',
			)
		}))
	...
	test('registering normal setting after init', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.profiles.testSetting3": "defaultProfile" }'),
			)
			await testObject.reloadConfiguration()
	...
	test('registering application scope setting after init', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue('configurationService.profiles.applicationSetting3'),
				'defaultProfile',
			)
		}))
	...
	test('non registering setting should not be read from default profile', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.profiles.nonregistered": "defaultProfile" }'),
			)
			await testObject.reloadConfiguration()
			assert.strictEqual(
	...
	test('setting applied to all profiles is registered later', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.profiles.testSetting4": "userValue" }'),
			)
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.profiles.testSetting4": "profileValue" }'),
			)
			await testObject.updateValue(
				APPLY_ALL_PROFILES_SETTING,
	...
	test('test isSettingAppliedToAllProfiles', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.isSettingAppliedForAllProfiles('configurationService.profiles.testSetting2'),
				false,
			)
...
suite('WorkspaceConfigurationService-Multiroot', () => {
	...
	test('application settings are not read from workspace', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.workspace.applicationSetting": "userValue" }'),
			)
			...
			assert.strictEqual(
				testObject.getValue('configurationService.workspace.applicationSetting'),
				'userValue',
			)
		}))
	...
	test('get application scope settings are not loaded after defaults are registered', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.workspace.newSetting": "userValue" }'),
			)
			await jsonEditingServce.write(
				workspaceContextService.getWorkspace().configuration!,
				[
	...
	test('get application scope settings are not loaded after defaults are registered when workspace folder is passed', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.workspace.newSetting-2": "userValue" }'),
			)
			await jsonEditingServce.write(
				workspaceContextService.getWorkspace().configuration!,
				[
	...
	test('workspace settings override user settings after defaults are registered for machine overridable settings ', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await testObject.reloadConfiguration()
			assert.strictEqual(
				testObject.getValue('configurationService.workspace.newMachineOverridableSetting'),
				'workspaceValue',
			)
	...
	test('machine settings are not read from workspace folder', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.workspace.machineSetting": "userValue" }'),
			)
			await fileService.writeFile(
				workspaceContextService.getWorkspace().folders[0].toResource('.vscode/settings.json'),
				VSBuffer.fromString(
	...
	test('application settings are not read from workspace folder after defaults are registered', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await testObject.reloadConfiguration()
			assert.strictEqual(
				testObject.getValue('configurationService.workspace.testNewApplicationSetting', {
					resource: workspaceContextService.getWorkspace().folders[0].uri,
				}),
	...
	test('machine settings are not read from workspace folder after defaults are registered', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue('configurationService.workspace.testNewMachineSetting', {
					resource: workspaceContextService.getWorkspace().folders[0].uri,
				}),
				'workspaceFolderValue',
			)
	...
	test('resource language setting in folder is read after it is registered later', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue('configurationService.workspace.testNewResourceLanguageSetting2', {
					resource: workspaceContextService.getWorkspace().folders[0].uri,
				}),
				'workspaceFolderValue',
			)
	...
	test('machine overridable setting in folder is read after it is registered later', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue('configurationService.workspace.testNewMachineOverridableSetting2', {
					resource: workspaceContextService.getWorkspace().folders[0].uri,
				}),
				'workspaceFolderValue',
			)
	...
	test('inspect restricted settings after change', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(actual.value, 'userRestrictedValue')
			assert.strictEqual(
				event.affectsConfiguration('configurationService.workspace.testRestrictedSetting1'),
				true,
			)
	...
	test('update workspace folder configuration', () => {
		...
		return testObject
			...
			.then(() =>
				assert.strictEqual(
					testObject.getValue('configurationService.workspace.testResourceSetting', {
						resource: workspace.folders[0].uri,
					}),
					'workspaceFolderValue',
				),
	...
	test('update resource language configuration in workspace folder', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue('configurationService.workspace.testLanguageSetting', {
					resource: workspace.folders[0].uri,
				}),
				'workspaceFolderValue',
			)
	...
	test('update machine overridable setting in folder', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue('configurationService.workspace.machineOverridableSetting', {
					resource: workspace.folders[0].uri,
				}),
				'workspaceFolderValue',
			)
	...
	test('update tasks configuration in a folder', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.deepStrictEqual(
				testObject.getValue(TasksSchemaProperties.Tasks, { resource: workspace.folders[0].uri }),
				{ version: '1.0.0', tasks: [{ taskName: 'myTask' }] },
			)
		}))
	...
	test('restricted setting is read from workspace folders when workspace is trusted', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				joinPath(testObject.getWorkspace().folders[1].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString(
			...
			assert.strictEqual(
				testObject.getValue('configurationService.workspace.testRestrictedSetting1', {
					resource: testObject.getWorkspace().folders[0].uri,
				}),
				'workspaceValue',
			)
			assert.strictEqual(
				testObject.getValue('configurationService.workspace.testRestrictedSetting2', {
	...
	test('remove an unregistered setting', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.workspace.unknownSetting": "userValue" }'),
			)
			await jsonEditingServce.write(
				workspaceContextService.getWorkspace().configuration!,
				[
			...
			await fileService.writeFile(
				joinPath(workspaceContextService.getWorkspace().folders[0].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString(
					'{ "configurationService.workspace.unknownSetting": "workspaceFolderValue1" }',
				),
			)
			await fileService.writeFile(
				joinPath(workspaceContextService.getWorkspace().folders[1].uri, '.vscode', 'settings.json'),
				VSBuffer.fromString(
...
suite('WorkspaceConfigurationService - Remote Folder', () => {
	...
	setup(async () => {
		...
		instantiationService = workbenchInstantiationService(undefined, disposables)
		environmentService = TestEnvironmentService
		const remoteEnvironmentPromise = new Promise<Partial<IRemoteAgentEnvironment>>(
	...
	test('remote machine settings override globals', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				...
				VSBuffer.fromString('{ "configurationService.remote.machineSetting": "remoteValue" }'),
			)
	...
	test('machine settings in local user settings does not override defaults', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.remote.machineSetting": "globalValue" }'),
			)
			registerRemoteFileSystemProvider()
			resolveRemoteEnvironment()
			await initialize()
	...
	test('remote application machine settings override globals', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await initialize()
			assert.strictEqual(
				testObject.getValue('configurationService.remote.applicationMachineSetting'),
				'remoteValue',
			)
		}))
	...
	test('machine overridable settings in local user settings does not override defaults', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await initialize()
			assert.strictEqual(
				testObject.getValue('configurationService.remote.machineOverridableSetting'),
				'isSet',
			)
		}))
	...
	test('non machine setting is written in local settings', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await testObject.reloadConfiguration()
			assert.strictEqual(
				testObject.inspect('configurationService.remote.applicationSetting').userLocalValue,
				'applicationValue',
			)
		}))
	...
	test('machine overridable setting is written in remote settings', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await testObject.reloadConfiguration()
			assert.strictEqual(
				testObject.inspect('configurationService.remote.machineOverridableSetting').userRemoteValue,
				'machineValue',
			)
		}))
	...
	test('machine settings in local user settings does not override defaults after defalts are registered ', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				userDataProfileService.currentProfile.settingsResource,
				VSBuffer.fromString('{ "configurationService.remote.newMachineSetting": "userValue" }'),
			)
			registerRemoteFileSystemProvider()
			resolveRemoteEnvironment()
			await initialize()
	...
	test('machine overridable settings in local user settings does not override defaults after defaults are registered ', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue('configurationService.remote.newMachineOverridableSetting'),
				'isSet',
			)
		}))
})
