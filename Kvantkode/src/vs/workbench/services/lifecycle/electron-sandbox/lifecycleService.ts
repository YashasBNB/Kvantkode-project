...
export class NativeLifecycleService extends AbstractLifecycleService {
	...
	private registerListeners(): void {
		...
		ipcRenderer.on(
			...
			async (
				event: unknown,
				reply: { okChannel: string; cancelChannel: string; reason: ShutdownReason },
			) => {
		...
		ipcRenderer.on(
			'vscode:onWillUnload',
			async (event: unknown, reply: { replyChannel: string; reason: ShutdownReason }) => {
		...
		this._onWillShutdown.fire({
			...
			join(promiseOrPromiseFn, joiner) {
				...
				if (joiner.order === WillShutdownJoinerOrder.Last) {
					const promiseFn =
						typeof promiseOrPromiseFn === 'function' ? promiseOrPromiseFn : () => promiseOrPromiseFn
