...
export function createValidator(prop: IConfigurationPropertySchema): (value: any) => string | null {
	...
	return (value) => {
		...
		if (prop.type === 'boolean' && value !== true && value !== false) {
			errors.push(
				nls.localize('validations.booleanIncorrectType', 'Incorrect type. Expected "boolean".'),
			)
		}
		...
		if (prop.type === 'string') {
			if (prop.enum && !isStringArray(prop.enum)) {
				...
				errors.push(
					nls.localize('validations.stringIncorrectType', 'Incorrect type. Expected "string".'),
				)
			} else {
				errors.push(
					...stringValidations
...
function getNumericValidators(prop: IConfigurationPropertySchema): Validator<number>[] {
	...
	const isNullable = canBeType(type, 'null')
	const isIntegral =
		canBeType(type, 'integer') && (type.length === 1 || (type.length === 2 && isNullable))
	const isNumeric =
		canBeType(type, 'number', 'integer') && (type.length === 1 || (type.length === 2 && isNullable))
	if (!isNumeric) {
		return []
	}
	...
	return [
		{
			enabled:
				exclusiveMax !== undefined && (prop.maximum === undefined || exclusiveMax <= prop.maximum),
		...
		{
			enabled:
				exclusiveMin !== undefined && (prop.minimum === undefined || exclusiveMin >= prop.minimum),
		...
		{
			enabled:
				prop.maximum !== undefined && (exclusiveMax === undefined || exclusiveMax > prop.maximum),
		...
		{
			enabled:
				prop.minimum !== undefined && (exclusiveMin === undefined || exclusiveMin < prop.minimum),
...
function getObjectValidator(
	...
	if (prop.type === 'object') {
		...
		return (value) => {
			...
			if (!isObject(value)) {
				errors.push(
					nls.localize('validations.objectIncorrectType', 'Incorrect type. Expected an object.'),
				)
			} else {
				Object.keys(value).forEach((key: string) => {
					...
					if (additionalProperties === false) {
						errors.push(
							nls.localize('validations.objectPattern', 'Property {0} is not allowed.\n', key),
						)
