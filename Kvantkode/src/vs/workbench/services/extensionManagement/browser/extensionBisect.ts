...
export const IExtensionBisectService =
...
registerAction2(
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const extensions = (await extensionManagement.getInstalled(ExtensionType.User)).filter(
				(ext) => extensionEnablementService.isEnabled(ext),
			)
