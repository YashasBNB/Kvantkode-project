...
export class NativeRemoteExtensionManagementService extends RemoteExtensionManagementService {
	...
	private async downloadCompatibleAndInstall(
		...
		const location = await this.localExtensionManagementServer.extensionManagementService.download(
			compatible,
			installed.filter((i) => areSameExtensions(i.identifier, compatible.identifier))[0]
	...
	private async installUIDependenciesAndPackedExtensions(local: ILocalExtension): Promise<void> {
		...
		if (toInstall.length) {
			...
			await Promises.settled(
				toInstall.map((d) =>
					this.localExtensionManagementServer.extensionManagementService.installFromGallery(d),
				),
			)
		}
	}
