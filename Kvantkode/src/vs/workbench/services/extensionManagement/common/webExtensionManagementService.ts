	...
	implements IProfileAwareExtensionManagementService
{
	...
	async installExtensionsFromProfile(
		...
		const extensionsToInstall = (
			await this.webExtensionsScannerService.scanUserExtensions(fromProfileLocation)
	...
	private isConfiguredToExecuteOnWeb(gallery: IGalleryExtension): boolean {
		const configuredExtensionKind =
			this.extensionManifestPropertiesService.getUserConfiguredExtensionKind(gallery.identifier)
	...
	private async whenProfileChanged(e: DidChangeUserDataProfileEvent): Promise<void> {
		...
		const oldExtensions =
			await this.webExtensionsScannerService.scanUserExtensions(previousProfileLocation)
		const newExtensions =
			await this.webExtensionsScannerService.scanUserExtensions(currentProfileLocation)
