...
namespace schema {
	...
	export function isValidMenuItem(
		...
		if (item.alt && typeof item.alt !== 'string') {
			collector.error(
				localize('optstring', 'property `{0}` can be omitted or must be of type `string`', 'alt'),
			)
			return false
		}
		if (item.when && typeof item.when !== 'string') {
			collector.error(
				localize('optstring', 'property `{0}` can be omitted or must be of type `string`', 'when'),
			)
			return false
		}
		if (item.group && typeof item.group !== 'string') {
			collector.error(
				localize('optstring', 'property `{0}` can be omitted or must be of type `string`', 'group'),
			)
			return false
		}
	...
	export function isValidSubmenu(
		...
		if (typeof submenu.id !== 'string') {
			collector.error(
				localize('requirestring', 'property `{0}` is mandatory and must be of type `string`', 'id'),
			)
			return false
		}
	...
	function handleCommand(
		...
		if (existingCmd) {
			if (existingCmd.source) {
				...
				extension.collector.info(
					localize('dup0', 'Command `{0}` already registered', userFriendlyCommand.command),
				)
			}
		}
