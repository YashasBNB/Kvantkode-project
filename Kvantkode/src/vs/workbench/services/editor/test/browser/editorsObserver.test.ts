...
suite('EditorsObserver', function () {
	...
	test('basics (multi group)', async () => {
		...
		const input2 = disposables.add(
			new TestFileEditorInput(URI.parse('foo://bar2'), TEST_SERIALIZABLE_EDITOR_INPUT_ID),
		)
	...
	test('hasEditor/hasEditors - same resource, different type id', async () => {
		...
		const input1 = disposables.add(
			new TestFileEditorInput(URI.parse('foo://bar1'), TEST_SERIALIZABLE_EDITOR_INPUT_ID),
		)
	...
	test('copy group', async function () {
		...
		const input3 = disposables.add(
			new TestFileEditorInput(URI.parse('foo://bar3'), TEST_SERIALIZABLE_EDITOR_INPUT_ID),
		)
