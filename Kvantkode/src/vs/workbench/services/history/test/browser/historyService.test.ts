...
suite('HistoryService', function () {
	...
	test('back / forward: editor navigation stack - mutations', async function () {
		...
		const stack = disposables.add(
			instantiationService.createInstance(EditorNavigationStack, GoFilter.NONE, GoScope.DEFAULT),
		)
	...
	test('go to last edit location', async function () {
		const [, historyService, editorService, textFileService, instantiationService] =
	...
	test('getHistory', async () => {
		...
		const input1 = disposables.add(
			new TestFileEditorInput(URI.parse('foo://bar1/node_modules/test.txt'), TEST_EDITOR_INPUT_ID),
		)
		await part.activeGroup.openEditor(input1, { pinned: true })
		...
		const input3 = disposables.add(
			new TestFileEditorInputWithUntyped(URI.parse('foo://bar3'), TEST_EDITOR_INPUT_ID),
		)
		await part.activeGroup.openEditor(input3, { pinned: true })

		const input4 = disposables.add(
			new TestFileEditorInputWithUntyped(URI.file('bar4'), TEST_EDITOR_INPUT_ID),
		)
		...
		const input5 = disposables.add(
			new TestFileEditorInputWithUntyped(URI.parse('file://bar5'), TEST_EDITOR_INPUT_ID),
		)
	...
	test('transient editors suspends editor change tracking', async () => {
		...
		editorChangePromise = Event.toPromise(editorService.onDidActiveEditorChange).then(() =>
			Event.toPromise(editorService.onDidActiveEditorChange),
		)
