...
export class ViewsService extends Disposable implements IViewsService {
	...
	private readonly _onDidChangeViewVisibility: Emitter<{ id: string; visible: boolean }> =
	...
	isViewContainerActive(id: string): boolean {
		...
		return (
			this.viewDescriptorService.getViewContainerModel(viewContainer).activeViewDescriptors.length >
			0
		)
	}
	...
	async openView<T extends IView>(id: string, focus?: boolean): Promise<T | null> {
		...
		if (compositeDescriptor) {
			const paneComposite = (await this.openComposite(compositeDescriptor.id, location!)) as
				| IPaneComposite
				| undefined
		...
		if (viewContainer.openCommandActionDescriptor) {
						...
						constructor() {
							super({
								...
								get title(): ICommandActionTitle {
									const viewContainerLocation =
										that.viewDescriptorService.getViewContainerLocation(viewContainer)
								...
								keybinding: keybindings
									? { ...keybindings, weight: KeybindingWeight.WorkbenchContrib }
									: undefined,
								f1: true,
							})
						}
			...
			if (mnemonicTitle) {
				const defaultLocation =
					this.viewDescriptorService.getDefaultViewContainerLocation(viewContainer)
				disposables.add(
		...
		if (viewDescriptor.openCommandActionDescriptor) {
			...
			if (viewDescriptor.openCommandActionDescriptor.mnemonicTitle) {
				...
				if (defaultViewContainer) {
					const defaultLocation =
						this.viewDescriptorService.getDefaultViewContainerLocation(defaultViewContainer)
					disposables.add(
						MenuRegistry.appendMenuItem(MenuId.MenubarViewMenu, {
							command: {
								id: commandId,
			...
			class ResetViewLocationAction extends Action2 {
				...
				run(accessor: ServicesAccessor): void {
					...
					if (defaultContainer.hideIfEmpty && containerModel.visibleViewDescriptors.length === 0) {
						const defaultLocation =
							viewDescriptorService.getDefaultViewContainerLocation(defaultContainer)!
	...
	private createViewPaneContainer(
		...
		const viewPaneContainer: ViewPaneContainer = (instantiationService as any).createInstance(
