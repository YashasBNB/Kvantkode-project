...
export class NativeWindow extends BaseWindow {
	...
	protected registerListeners(): void {
		// Layout
		this._register(
			addDisposableListener(mainWindow, EventType.RESIZE, () => this.layoutService.layout()),
		)
		...
		for (const event of [EventType.DRAG_OVER, EventType.DROP]) {
			this._register(
				addDisposableListener(mainWindow.document.body, event, (e: DragEvent) => {
					EventHelper.stop(e)
				}),
			)
		}
		...
		ipcRenderer.on('vscode:addRemoveFolders', (event: unknown, request: IAddRemoveFoldersRequest) =>
		...
		ipcRenderer.on(
			...
			async (
				event: unknown,
				payload: { authInfo: AuthInfo; username?: string; password?: string; replyChannel: string },
			) => {
		...
		ipcRenderer.on('vscode:configureAllowedUNCHost', async (event: unknown, host: string) => {
			...
			const configuredAllowedUncHosts =
				this.configurationService.getValue<string[] | undefined>('security.allowedUNCHosts') ?? []
		...
		this._register(
			onDidChangeZoomLevel((targetWindowId) => this.handleOnDidChangeZoomLevel(targetWindowId)),
		)
		...
		this._register(
			this.lifecycleService.onBeforeShutdownError((e) => this.onBeforeShutdownError(e)),
		)
	...
	private handleRepresentedFilename(part: IEditorPart): void {
		...
		disposables.add(
			scopedEditorService.onDidActiveEditorChange(() =>
				this.updateRepresentedFilename(scopedEditorService, part.windowId),
			),
		)
	}
			...
			if (confirmBeforeClose) {
				...
				return veto(
					(async () => {
						...
						let confirmed = true
						if (confirmBeforeClose) {
							confirmed = await this.instantiationService.invokeFunction((accessor) =>
	...
	private async handleWarnings(): Promise<void> {
		...
		if (isMacintosh) {
			...
			const eolReleases = new Map<string, string>([['19', 'macOS Catalina']])
		...
		this.progressService.withProgress(
			...
			() => shellEnv,
			() => this.openerService.open('https://go.microsoft.com/fwlink/?linkid=2149667'),
		)
	}
	...
	private async doAddRemoveFolders(): Promise<void> {
		const foldersToAdd: IWorkspaceFolderCreationData[] = this.pendingFoldersToAdd.map((folder) => ({
			uri: folder,
		}))
			...
			if (request.filesToWait) {
				...
				if (openedEditorPanes.length) {
					return this.trackClosedWaitFiles(
						...
						coalesce(request.filesToWait.paths.map((path) => URI.revive(path.fileUri))),
					)
				} else {
	...
	private createWindowZoomStatusEntry(part: IEditorPart): void {
		...
		this.mapWindowIdToZoomStatusEntry.set(
			part.windowId,
			disposables.add(scopedInstantiationService.createInstance(ZoomStatusEntry)),
		)
	...
	private createZoomEntry(visibleOrText: string): void {
		...
		const zoomSettingsAction: Action = disposables.add(
			new Action(
				...
				true,
				() => this.commandService.executeCommand(zoomSettingsAction.id, 'window.zoom'),
			),
		)
