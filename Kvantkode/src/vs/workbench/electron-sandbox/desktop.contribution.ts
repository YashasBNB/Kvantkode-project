...
;(function registerActions(): void {
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		async handler(accessor: ServicesAccessor) {
			...
			const confirmBeforeClose = configurationService.getValue<'always' | 'never' | 'keyboardOnly'>(
	...
	registry.registerConfiguration({
		...
		properties: {
			...
			'window.zoomLevel': {
				...
				markdownDescription: localize(
					{ comment: ['{0} will be a setting name rendered as a link'], key: 'zoomLevel' },
			...
			'window.zoomPerWindow': {
				...
				default: true,
				markdownDescription: localize(
					{ comment: ['{0} will be a setting name rendered as a link'], key: 'zoomPerWindow' },
