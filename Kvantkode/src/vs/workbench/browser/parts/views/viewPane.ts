				...
				if (linkedText.nodes.length === 1 && typeof linkedText.nodes[0] !== 'string') {
					...
					if (precondition) {
						const updateEnablement = () =>
							(button.enabled = this.contextKeyService.contextMatchesRules(precondition))
						updateEnablement()
...
export abstract class ViewPane extends Pane implements IView {
	...
	private getIcon(): ThemeIcon | URI {
		return (
			this.viewDescriptorService.getViewDescriptorById(this.id)?.containerIcon || defaultViewIcon
		)
	}
