...
export class MultiRowEditorControl extends Disposable implements IEditorTabsControl {
	...
	private handleTabBarsLayoutChange(): void {
		...
		const hasTwoTabBars =
			this.groupView.count !== this.groupView.stickyCount && this.groupView.stickyCount > 0
	...
	openEditors(editors: EditorInput[]): boolean {
		...
		const didChange =
			didChangeOpenEditorsSticky || didChangeOpenEditorsUnSticky || didActiveControlChange
		if (didChange) {
			this.handleOpenedEditors()
		}
