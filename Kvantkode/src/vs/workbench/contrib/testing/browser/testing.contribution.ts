...
).registerWorkbenchContribution(TestingContentProvider, LifecyclePhase.Restored)
Registry.as<IWorkbenchContributionsRegistry>(
	WorkbenchExtensions.Workbench,
).registerWorkbenchContribution(TestingPeekOpener, LifecyclePhase.Eventually)
Registry.as<IWorkbenchContributionsRegistry>(
	WorkbenchExtensions.Workbench,
).registerWorkbenchContribution(TestingProgressTrigger, LifecyclePhase.Eventually)
...
CommandsRegistry.registerCommand({
	...
	handler: async (
		...
		const position =
			accessor.get(ITestingDecorationsService).getDecoratedTestPosition(uri, extId) ||
