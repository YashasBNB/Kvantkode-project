...
export class WatchExpressionsView extends ViewPane implements IDebugViewWithVariables {
	...
	private onContextMenu(e: ITreeContextMenuEvent<IExpression>): void {
		...
		const attributes =
			element instanceof Variable ? element.presentationHint?.attributes : undefined
		...
		this.contextMenuService.showContextMenu({
			...
			getActions: () => actions,
			getActionsContext: () =>
				element && selection.includes(element) ? selection : element ? [element] : [],
		})
	}
}
...
export class WatchExpressionsRenderer extends AbstractExpressionsRenderer {
	...
	protected renderExpression(
		...
		const showType =
			this.configurationService.getValue<IDebugConfiguration>('debug').showVariableTypes
