...
export class Repl extends FilterViewPane implements IHistoryNavigationWidget {
										...
										if (item && item.label) {
											...
											if (typeof item.selectionStart === 'number') {
												...
												const selectionLength =
													typeof item.selectionLength === 'number' ? item.selectionLength : 0
												const placeholder =
													selectionLength > 0
														? '${1:' +
	...
	private navigateHistory(previous: boolean): void {
		const historyInput =
			(previous ? (this.history.previous() ?? this.history.first()) : this.history.next()) ?? ''
	...
	private createReplTree(): void {
		...
		const wordWrap =
			this.configurationService.getValue<IDebugConfiguration>('debug').console.wordWrap
			...
			tree.onDidChangeContentHeight(() => {
				if (tree.scrollHeight !== this.previousTreeScrollHeight) {
					...
					const lastElementWasVisible =
						tree.scrollTop + tree.renderHeight >= this.previousTreeScrollHeight - 2
	...
	private createReplInput(container: HTMLElement): void {
		...
		const { historyNavigationBackwardsEnablement, historyNavigationForwardsEnablement } =
			this._register(registerAndCreateHistoryNavigationContext(this.scopedContextKeyService, this))
