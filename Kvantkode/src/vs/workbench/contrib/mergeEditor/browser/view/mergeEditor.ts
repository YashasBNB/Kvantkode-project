...
export class MergeEditor extends AbstractTextEditor<IMergeEditorViewState> {
	...
	private readonly input1View = this._register(
		this.instantiationService.createInstance(InputCodeEditorView, 1, this._viewModel),
	)
	private readonly baseView = observableValue<BaseCodeEditorView | undefined>(this, undefined)
	private readonly baseViewOptions = observableValue<Readonly<ICodeEditorOptions> | undefined>(
		this,
		undefined,
	)
	private readonly input2View = this._register(
		this.instantiationService.createInstance(InputCodeEditorView, 2, this._viewModel),
	)
	...
	private readonly _ctxIsMergeEditor: IContextKey<boolean> = ctxIsMergeEditor.bindTo(
		this.contextKeyService,
	)
	private readonly _ctxUsesColumnLayout: IContextKey<string> = ctxMergeEditorLayout.bindTo(
		this.contextKeyService,
	)
	private readonly _ctxShowBase: IContextKey<boolean> = ctxMergeEditorShowBase.bindTo(
		this.contextKeyService,
	)
	private readonly _ctxShowBaseAtTop = ctxMergeEditorShowBaseAtTop.bindTo(this.contextKeyService)
	private readonly _ctxResultUri: IContextKey<string> = ctxMergeResultUri.bindTo(
		this.contextKeyService,
	)
	...
	private readonly _inputModel = observableValue<IMergeEditorInputModel | undefined>(
		this,
		undefined,
	)
				...
				constructor() {
					for (const model of this.baseInput1Input2()) {
						this._disposable.add(
							model.onDidChangeContent(() => this._checkBaseInput1Input2AllEmpty()),
						)
					}
				}
	...
	private _applyViewState(state: IMergeEditorViewState | undefined) {
		...
		if (state.focusIndex >= 0) {
			;[this.input1View.editor, this.input2View.editor, this.inputResultView.editor][
				state.focusIndex
			].focus()
		}
	}
	...
	private readonly showNonConflictingChangesStore = this.instantiationService.createInstance(
