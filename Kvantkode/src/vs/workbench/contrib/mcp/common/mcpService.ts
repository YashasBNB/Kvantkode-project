...
export class McpService extends Disposable implements IMcpService {
	...
	public readonly servers: IObservable<readonly IMcpServer[]> = this._servers.map((servers) =>
		servers.map((s) => s.object),
	)
	...
	constructor(
		...
		this.workspaceCache = this._register(
			_instantiationService.createInstance(McpServerMetadataCache, StorageScope.WORKSPACE),
		)
			...
			autorun((reader) => {
				...
				for (const tool of server.tools.read(reader)) {
					...
					const toolData: IToolData = {
						...
						runsInWorkspace:
							collection?.scope === StorageScope.WORKSPACE || !!collection?.remoteAuthority,
						tags: ['mcp'],
					}
					...
					if (existing) {
						...
						tools.set(tool.id, {
							...
							implDispose: this._toolsService.registerToolImplementation(
								tool.id,
								this._instantiationService.createInstance(McpToolImplementation, tool, server),
							),
						})
					}
				}
...
function defsEqual(
	server: IMcpServer,
	def: { serverDefinition: McpServerDefinition; collectionDefinition: McpCollectionDefinition },
) {
	return (
		...
		return {
			confirmationMessages: {
				...
				message: new MarkdownString(
					localize('msg.msg', '{0}\n\n {1}', tool.definition.description, mcpToolWarning),
					{ supportThemeIcons: true },
				),
				allowAutoConfirm: true,
			},
			invocationMessage: new MarkdownString(
				localize('msg.run', 'Running `{0}`', tool.definition.name, server.definition.label),
			),
			pastTenseMessage: new MarkdownString(
				localize('msg.ran', 'Ran `{0}` ', tool.definition.name, server.definition.label),
			),
			toolSpecificData: {
				kind: 'input',
				rawInput: parameters,
