	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this.logger = this._register(
			loggerService.createLogger(joinPath(environmentService.logsHome, `${LOG_ID}.log`), {
				id: LOG_ID,
				name: LOGGER_NAME,
			}),
		)
		...
		this._register(
			this.remoteTunnelService.onDidChangeTunnelStatus((s) => this.handleTunnelStatusUpdate(s)),
		)
			...
			this.notificationService.notify({
				...
				actions: {
					primary: [
						...
						new Action(
							'doNotShowAgain',
							localize('action.doNotShowAgain', 'Do not show again'),
							undefined,
							true,
							() => {
	...
	private async getAuthenticationProviders(): Promise<IAuthenticationProvider[]> {
		...
		const configuredAuthenticationProviders = Object.keys(authenticationProviders).reduce<
				...
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor) {
						...
						const asService = await new Promise<boolean | undefined>((resolve) => {
							disposables.add(
								quickPick.onDidAccept(() => resolve(quickPick.selectedItems[0]?.service)),
							)
							...
							notificationService.notify({
								...
								actions: {
									primary: [
										new Action(
											'copyToClipboard',
											localize('action.copyToClipboard', 'Copy Browser Link to Clipboard'),
											undefined,
											true,
