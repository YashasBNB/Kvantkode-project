...
export class ForwardedPortsView extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		super()
		this._register(
			Registry.as<IViewsRegistry>(Extensions.ViewsRegistry).registerViewWelcomeContent(
				TUNNEL_VIEW_ID,
				{
	...
	private async enableForwardedPortsFeatures() {
		...
		if (featuresEnabled || viewEnabled) {
			...
			this.contextKeyListener.value = this.contextKeyService.onDidChangeContext((e) => {
				if (
					e.affectsSome(
						new Set([...forwardedPortsFeaturesEnabled.keys(), ...forwardedPortsViewEnabled.keys()]),
					)
				) {
					this.enableForwardedPortsFeatures()
				}
	...
	private enableBadgeAndStatusBar() {
		const disposable = Registry.as<IViewsRegistry>(Extensions.ViewsRegistry).onViewsRegistered(
			(e) => {
				if (
					e.find((view) =>
						view.views.find((viewDescriptor) => viewDescriptor.id === TUNNEL_VIEW_ID),
					)
				) {
					this._register(
						Event.debounce(
	...
	private async updateActivityBadge() {
		if (this.remoteExplorerService.tunnelModel.forwarded.size > 0) {
			this.activityBadge.value = this.activityService.showViewActivity(TUNNEL_VIEW_ID, {
				badge: new NumberBadge(this.remoteExplorerService.tunnelModel.forwarded.size, (n) =>
					n === 1
...
export class AutomaticPortForwarding extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		if (
			!this.storageService.getBoolean('processPortForwardingFallback', StorageScope.WORKSPACE, true)
		) {
		...
		if (
			...
			this.portListener = this._register(
				this.remoteExplorerService.tunnelModel.onForwardPort(async () => {
					...
					if (
						Array.from(this.remoteExplorerService.tunnelModel.forwarded.values()).filter(
						...
						this.notificationService.notify({
							...
							actions: {
								primary: [
									new Action(
										'switchBack',
										nls.localize('remote.autoForwardPortsSource.fallback.switchBack', 'Undo'),
										undefined,
										true,
										async () => {
											await this.configurationService.updateValue(
	...
	private setup(environment: IRemoteAgentEnvironment | null) {
		...
		if (environment?.os !== OperatingSystem.Linux) {
			if (
				this.configurationService.inspect<string>(PORT_AUTO_SOURCE_SETTING).default?.value !==
				PORT_AUTO_SOURCE_SETTING_OUTPUT
			) {
...
class OutputAutomaticPortForwarding extends Disposable {
	...
	constructor(
		...
		if (
			configurationService.getValue(PORT_AUTO_SOURCE_SETTING) === PORT_AUTO_SOURCE_SETTING_HYBRID
		) {
			this._register(
...
class ProcAutomaticPortForwarding extends Disposable {
	...
	private async initialize() {
		if (!this.remoteExplorerService.tunnelModel.environmentTunnelsSet) {
			await new Promise<void>((resolve) =>
				this.remoteExplorerService.tunnelModel.onEnvironmentTunnelsSet(() => resolve()),
			)
		}
	...
	private async setInitialCandidates() {
		...
		if (!startingCandidates) {
			await new Promise<void>((resolve) =>
				this.remoteExplorerService.tunnelModel.onCandidatesChanged(() => resolve()),
			)
