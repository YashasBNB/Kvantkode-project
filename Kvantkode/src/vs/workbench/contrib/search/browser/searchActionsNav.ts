	...
	class ToggleQueryDetailsAction extends Action2 {
		...
		run(accessor: ServicesAccessor, ...args: any[]) {
			...
			if (contextService.getValue(SearchEditorConstants.InSearchEditor.serialize())) {
				...
				contextService.getValue(Constants.SearchContext.SearchViewFocusedKey.serialize())
			) {
...
registerAction2(
	class OpenMatchAction extends Action2 {
		...
		run(accessor: ServicesAccessor) {
			const searchView = getSearchView(accessor.get(IViewsService))
			if (searchView) {
				const tree: WorkbenchCompressibleAsyncDataTree<ISearchResult, RenderableMatch> =
