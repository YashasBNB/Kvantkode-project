	...
	implements ICompressibleTreeRenderer<ISearchTreeFileMatch, any, IFileMatchTemplate>
{
	...
	renderElement(
		...
		const decorationConfig =
			this.configurationService.getValue<ISearchConfigurationProperties>('search').decorations
		...
		const twistieContainer =
			templateData.el.parentElement?.parentElement?.querySelector('.monaco-tl-twistie')
	...
	implements ICompressibleTreeRenderer<ISearchTreeMatch, void, IMatchTemplate>
{
	...
	private getMatchTitle(match: ISearchTreeMatch, showLineNumbers: boolean): string {
		...
		const numLinesStr =
			numLines > 0 ? '+ ' + nls.localize('numLinesStr', '{0} more lines', numLines) : ''
