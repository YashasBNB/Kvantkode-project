...
export const IQuickDiffModelService =
	createDecorator<IQuickDiffModelService>('IQuickDiffModelService')
...
export class QuickDiffModel extends Disposable {
	...
	private onDidAddRepository(repository: ISCMRepository): void {
		...
		disposables.add(
			onDidRemoveRepository(() => this._repositoryDisposables.deleteAndDispose(repository)),
		)
	...
	private triggerDiff(): void {
		...
		this._diffDelayer
			.trigger(async () => {
				const result: { changes: QuickDiffChange[]; mapChanges: Map<string, number[]> } | null =
					await this.diff()
		...
		this._quickDiffsPromise = this.getOriginalResource().then(async (quickDiffs) => {
			...
			if (
				equals(
					...
					quickDiffs,
					(a, b) =>
						a.originalResource.toString() === b.originalResource.toString() && a.label === b.label,
				)
			) {
				return quickDiffs
			}
	...
	findNextClosestChange(lineNumber: number, inclusive = true, provider?: string): number {
		...
		for (let i = 0; i < this.changes.length; i++) {
			...
			if (
				!this.quickDiffs.find((quickDiff) => quickDiff.label === this.changes[i].label)?.visible
			) {
				continue
			}
