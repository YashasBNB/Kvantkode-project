...
class TextModelPromptParserTest extends Disposable {
	...
	constructor(
		...
		this.model = this._register(
			createTextModel(initialContents.join(lineEnding), 'fooLang', undefined, uri),
		)
	 ...
	 */
	public async validateReferences(expectedReferences: readonly ExpectedReference[]) {
...
suite('TextModelPromptParser', () => {
	 ...
	 */
	const createTest = (uri: URI, initialContents: string[]): TextModelPromptParserTest => {
		return disposables.add(
			instantiationService.createInstance(TextModelPromptParserTest, uri, initialContents),
		)
	}
		...
		await test.validateReferences([
			new ExpectedReference({
				...
				childrenOrError: new OpenFailed(
					createURI('/absolute/folder/and/a/foo-bar-baz/another-file.ts'),
					'File not found.',
				),
			}),
			new ExpectedReference({
				...
				pathStartColumn: 17,
				childrenOrError: new OpenFailed(
					createURI('/absolute/c/file_name.prompt.md'),
					'File not found.',
				),
			}),
			new ExpectedReference({
			...
			new ExpectedReference({
				...
				pathStartColumn: 62,
				childrenOrError: new OpenFailed(
					createURI('/absolute/folder/and/a/samefile.jpeg'),
					'File not found.',
				),
			}),
		])
