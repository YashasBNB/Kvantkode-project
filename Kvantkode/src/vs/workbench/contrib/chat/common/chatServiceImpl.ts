...
export class ChatService extends Disposable implements IChatService {
	...
	private readonly _onDidDisposeSession = this._register(
		new Emitter<{ sessionId: string; reason: 'initializationFailed' | 'cleared' }>(),
	)
	...
	async getHistory(): Promise<IChatDetail[]> {
		if (this.useFileStorage) {
			...
			const entries = Object.values(index)
				.filter(
					(entry) =>
						!this._sessionModels.has(entry.sessionId) && !entry.isImported && !entry.isEmpty,
				)
				.map(
					(entry): IChatDetail => ({
						...entry,
		...
		const persistedSessionItems = persistedSessions
			.filter(
				(session) =>
					!session.isImported && session.initialLocation !== ChatAgentLocation.EditingSession,
			)
			.map((session) => {
	...
	private async initializeSession(model: ChatModel, token: CancellationToken): Promise<void> {
		try {
			...
			const defaultAgentData =
				this.chatAgentService.getContributedDefaultAgent(model.initialLocation) ??
	...
	async sendRequest(
		...
		const agent =
			parsedRequest.parts.find((r): r is ChatRequestAgentPart => r instanceof ChatRequestAgentPart)
				?.agent ?? defaultAgent
		...
		const sendRequestInternal = async () => {
			...
			const listener = token.onCancellationRequested(() => {
				...
				>('interactiveSessionProviderInvoked', {
					...
					agentExtensionId:
						detectedAgent?.extensionId.value ?? agentPart?.agent.extensionId.value ?? '',
