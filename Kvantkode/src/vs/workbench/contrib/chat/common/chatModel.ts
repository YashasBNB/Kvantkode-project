...
export type ChatResponseModelChangeReason = { reason: 'other' } | { reason: 'undoStop'; id: string }
...
export class Response extends AbstractResponse implements IDisposable {
	...
	updateContent(
		...
		if (progress.kind === 'markdownContent') {
			...
			let found = false
			const groupKind =
				progress.kind === 'textEdit' && !notebookUri ? 'textEditGroup' : 'notebookEditGroup'
			const edits: any =
			...
			progress.task?.().then((content) => {
				...
				if (typeof content === 'string') {
					;(this._responseParts[responsePosition] as IChatTask).content = new MarkdownString(
						content,
					)
				}
				this._updateRepr(false)
...
export class ChatResponseModel extends Disposable implements IChatResponseModel {
	 ...
	 */
	updateContent(
		responsePart: IChatProgressResponseContent | IChatTextEdit | IChatNotebookEdit,
		quiet?: boolean,
	) {
