...
class SetupChatAgentImplementation extends Disposable implements IChatAgentImplementation {
	...
	): { disposable: IDisposable; agent: SetupChatAgentImplementation } {
	...
	private static readonly SETUP_NEEDED_MESSAGE = new MarkdownString(
		localize('settingUpCopilotNeeded', 'You need to set up Copilot to use Chat.'),
	)
	...
	private createDialog(disposables: DisposableStore): HTMLElement {
		...
		element.appendChild(
			$(
				'p.setup-header',
				undefined,
				disposables.add(markdown.render(new MarkdownString(header, { isTrusted: true }))).element,
			),
		)

		// Terms
		const terms = localize(
			{ key: 'terms', comment: ['{Locked="["}', '{Locked="]({0})"}', '{Locked="]({1})"}'] },
		...
		element.appendChild(
			$(
				'p.setup-legal',
				undefined,
				disposables.add(markdown.render(new MarkdownString(terms, { isTrusted: true }))).element,
			),
		)
		...
		if (this.telemetryService.telemetryLevel !== TelemetryLevel.NONE) {
			const settings = localize(
				{ key: 'settings', comment: ['{Locked="["}', '{Locked="]({0})"}', '{Locked="]({1})"}'] },
			...
			element.appendChild(
				$(
					'p.setup-settings',
					undefined,
					disposables.add(markdown.render(new MarkdownString(settings, { isTrusted: true })))
						.element,
				),
			)
		}
...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		const controller = new Lazy(() =>
			this._register(
				this.instantiationService.createInstance(ChatSetupController, context, requests),
			),
		)
	...
	private registerSetupAgents(
		...
		const updateRegistration = () => {
			const disabled =
				context.state.hidden || !this.configurationService.getValue('chat.setupFromDialog')
			if (!disabled && !registration.value) {
				const { agent: panelAgent, disposable: panelDisposable } =
			...
			Event.runAndSubscribe(
				Event.any(
					...
					Event.filter(this.configurationService.onDidChangeConfiguration, (e) =>
						e.affectsConfiguration('chat.setupFromDialog'),
					),
				),
				() => updateRegistration(),
			),
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				if (location === ViewContainerLocation.AuxiliaryBar) {
					const activeContainers = viewsDescriptorService
						...
						.filter(
							(container) =>
								viewsDescriptorService.getViewContainerModel(container).activeViewDescriptors
									.length > 0,
						)
...
type InstallChatClassification = {
	...
	setupFromDialog: {
		classification: 'SystemMetaData'
		purpose: 'FeatureInsight'
		comment: 'Whether the setup was triggered from the dialog or not.'
	}
}
...
class ChatSetupController extends Disposable {
	...
	async setup(options?: { forceSignIn?: boolean; setupFromDialog?: boolean }): Promise<boolean> {
		...
		const badge = this.activityService.showViewContainerActivity(
			preferCopilotEditsView(this.viewsService)
	...
	private async doSetup(
		options: { forceSignIn?: boolean; setupFromDialog?: boolean },
		watch: StopWatch,
	): Promise<boolean> {
	...
	private async signIn(
		providerId: string,
		options?: { setupFromDialog?: boolean },
	): Promise<{
	...
	private create(): void {
		...
		// Header
		{
			...
			this.element.appendChild(
				$(
					'p',
					undefined,
					this._register(markdown.render(new MarkdownString(header, { isTrusted: true }))).element,
				),
			)
		...
		const freeContainer = this.element.appendChild(
			$(
				...
				undefined,
				this._register(
					markdown.render(new MarkdownString(free, { isTrusted: true, supportThemeIcons: true })),
				).element,
			),
		)
		...
		this.element.appendChild(
			$(
				'p',
				undefined,
				this._register(markdown.render(new MarkdownString(terms, { isTrusted: true }))).element,
			),
		)
		...
		const settingsContainer = this.element.appendChild(
			$(
				'p',
				undefined,
				this._register(markdown.render(new MarkdownString(settings, { isTrusted: true }))).element,
			),
		)
