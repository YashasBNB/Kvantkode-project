...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(ChatEditor, ChatEditorInput.EditorID, nls.localize('chat', 'Chat')),
...
class ChatAgentSettingContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerMaxRequestsSetting(): void {
		...
		const registerMaxRequestsSetting = () => {
			const treatmentId =
				this.entitlementService.entitlement === ChatEntitlement.Limited
					? 'chatAgentMaxRequestsFree'
					: 'chatAgentMaxRequestsPro'
			this.experimentService.getTreatment<number>(treatmentId).then((value) => {
				const defaultValue =
					value ?? (this.entitlementService.entitlement === ChatEntitlement.Limited ? 5 : 15)
						...
						await Promise.all(
							agents
								...
								.map(async (a) => {
									...
									const agentMarkdown = instantiationService.invokeFunction((accessor) =>
										agentToMarkdown(a, true, accessor),
									)
