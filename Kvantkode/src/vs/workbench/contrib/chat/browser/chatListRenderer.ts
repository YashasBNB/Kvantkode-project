				...
				if (ev.equals(KeyCode.Space) || ev.equals(KeyCode.Enter)) {
					...
					if (content) {
						this.hoverService.showInstantHover(
							{ content, target: user, trapFocus: true, actions: hoverOptions.actions },
							true,
						)
					}
	...
	private basicRenderElement(
		...
		if (isRequestVM(element) && !element.confirmation) {
			...
			if (this.rendererOptions.renderStyle === 'minimal' && !element.isComplete) {
				...
				templateData.elementDisposables.add(
					toDisposable(() => templateData.value.classList.remove('inline-progress')),
				)
				value.push({
		...
		if (isResponseVM(element) && element.errorDetails?.message) {
			if (element.errorDetails.isQuotaExceeded) {
				...
				templateData.elementDisposables.add(
					renderedError.onDidChangeHeight(() => this.updateItemHeight(templateData)),
				)
			} else {
				const level =
					element.errorDetails.level ??
...
export class ChatListDelegate implements IListVirtualDelegate<ChatTreeItem> {
	...
	getHeight(element: ChatTreeItem): number {
		...
		const height =
			('currentRenderedHeight' in element ? element.currentRenderedHeight : undefined) ??
