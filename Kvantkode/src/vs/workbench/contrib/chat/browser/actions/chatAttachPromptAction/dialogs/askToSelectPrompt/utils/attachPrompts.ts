 ...
 */
const openNewChat = async (options: ISelectPromptOptions, edits: boolean): Promise<IChatWidget> => {
	...
	const widget =
		edits === true ? await showEditsView(viewsService) : await showChatView(viewsService)
...
const showExistingChat = async (
	...
	const widget =
		edits && chatService.unifiedViewEnabled === false
