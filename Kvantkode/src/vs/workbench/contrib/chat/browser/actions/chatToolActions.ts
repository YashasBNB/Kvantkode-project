			...
			if (tool.source.type === 'mcp') {
				...
				bucket = toolBuckets.get(mcpServer.definition.id) ?? {
					...
					status: localize(
						...
						McpConnectionState.toString(mcpServer.connectionState.get()),
					),
				...
				const ext = extensionService.extensions.find((value) =>
					ExtensionIdentifier.equals(value.identifier, extensionId),
				)
				if (!ext) {
					continue
				}
