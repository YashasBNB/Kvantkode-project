		...
		class RetryChatAction extends Action2 {
			...
			async run(accessor: ServicesAccessor, ...args: any[]) {
				...
				const userSelectedTools =
					widget?.input.currentMode === ChatMode.Agent
						...
						[
							new ResourceNotebookCellEdit(notebookEditor.textModel.uri, {
								...
								cells: splitContents.map((content) => {
									...
									const mime =
										content.type === 'markdown' ? 'text/markdown' : `text/x-${content.language}`
									return {
										cellKind: kind,
										language,
										mime,
	...
	registerAction2(
		class ContinueEditingAction extends Action2 {
			...
			async run(accessor: ServicesAccessor, ...args: any[]) {
				...
				await Promise.all(
					Array.from(workingSetAdditions, async (uri) =>
						editsView.widget.attachmentModel.addFile(uri),
					),
				)
				...
				await chatService.sendRequest(editingSession.chatSessionId, '', {
					...
					confirmation:
						typeof this.desc.title === 'string' ? this.desc.title : this.desc.title.value,
				})
			}
			...
			private async _pickTurns(
				...
				const relatedRequests = requests.filter(
					(request) =>
						request.timestamp >= 0 && lastRequestTimestamp - request.timestamp <= timeThreshold,
				)
				...
				const picks: IQuickPickItem[] =
					relatedRequests.length !== 0 ? [lastPick, allPick, customPick] : [allPick, customPick]
				...
				const customPicks: (IQuickPickItem & { request: IChatRequestModel })[] = requests.map(
					(request) => ({
						picked: false,
						request: request,
