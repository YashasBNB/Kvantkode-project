	...
	registerAction2(
		class ToggleChatAction extends Action2 {
			...
			async run(accessor: ServicesAccessor) {
				...
				if (
					viewsService.isViewVisible(ChatViewId) ||
			...
			async run(accessor: ServicesAccessor) {
				...
				const showPicker = async () => {
					...
					const picker = store.add(
						quickInputService.createQuickPick<IChatPickerItem>({ useSeparators: true }),
					)
			...
			constructor() {
				super({
					...
					precondition: ContextKeyExpr.and(
						ContextKeyExpr.or(ChatContextKeys.Entitlement.limited, ChatContextKeys.Entitlement.pro),
			...
			override async run(accessor: ServicesAccessor) {
				...
				if (chatQuotaExceeded && !completionsQuotaExceeded) {
					message = localize(
						...
						dateFormatter.format(chatEntitlementService.quotas.quotaResetDate),
					)
					...
					buttons: [
						{
							...
							run: () => {
								...
								telemetryService.publicLog2<
									WorkbenchActionExecutedEvent,
									WorkbenchActionExecutedClassification
								>('workbenchActionExecuted', { id: commandId, from: 'chat-dialog' })
	...
	class ResetTrustedToolsAction extends Action2 {
		...
		override run(accessor: ServicesAccessor): void {
			...
			accessor
				...
				.info(
					localize('resetTrustedToolsSuccess', 'Tool confirmation preferences have been reset.'),
				)
		}
	},
)
