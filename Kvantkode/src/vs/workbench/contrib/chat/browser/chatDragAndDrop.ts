...
export class ChatDragAndDrop extends Themable {
	private readonly overlays: Map<HTMLElement, { overlay: HTMLElement; disposable: IDisposable }> =
		new Map()
	...
	private guessDropType(e: DragEvent): ChatDragAndDropType | undefined {
		...
		if (this.isImageDnd(e)) {
			...
			containsDragType(e, Mimes.uriList, CodeDataTransfers.FILES, DataTransfers.RESOURCES)
		) {
	...
	private async resolveHTMLAttachContext(e: DragEvent): Promise<IChatRequestVariableEntry[]> {
		...
		for (
			let appendValue = 2;
			this.attachmentModel.attachments.some((attachment) => attachment.name === finalDisplayName);
			appendValue++
		) {
	...
	private async createImageVariable(
		data: Uint8Array,
		name: string,
		uri?: URI,
		id?: string,
...
async function getResourceAttachContext(
	resource: URI,
	isDirectory: boolean,
	...
	if (/\.(png|jpg|jpeg|gif|webp)$/i.test(editor.resource.path)) {
		...
		if (readFile.size > 30 * 1024 * 1024) {
			...
			dialogService.error(
				...
				localize('imageTooLargeMessage', 'The image {0} is too large to be attached.', fileName),
			)
