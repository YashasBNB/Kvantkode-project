...
class ChatToolInvocationSubPart extends Disposable {
	...
	constructor(
		...
		if (toolInvocation.kind === 'toolInvocation' && toolInvocation.confirmationMessages) {
			...
			Array.isArray(toolInvocation.resultDetails) &&
	...
	private createConfirmationWidget(toolInvocation: IChatToolInvocation): HTMLElement {
		...
		if (typeof message === 'string') {
			confirmWidget = this._register(
				this.instantiationService.createInstance(ChatConfirmationWidget, title, message, buttons),
			)
		} else {
	...
	private createTerminalConfirmationWidget(
	...
	private createTerminalMarkdownProgressPart(
	...
	private createInputOutputMarkdownProgressPart(
