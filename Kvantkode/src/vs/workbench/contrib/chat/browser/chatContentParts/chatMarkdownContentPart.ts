...
export class ChatMarkdownContentPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		const inUndoStop = (
			findLast(context.content, (e) => e.kind === 'undoStop', context.contentIndex) as
				| IChatUndoStop
				| undefined
		)?.id
			...
			renderer.render(
				markdown.content,
				{
					...
					codeBlockRendererSync: (languageId, text, raw) => {
						...
						if (
							(!text || (text.startsWith('<vscode_codeblock_uri') && !text.includes('\n'))) &&
							!isCodeBlockComplete
						) {
						...
						if (equalsIgnoreCase(languageId, localFileLanguageId)) {
							...
							const sessionId =
								isResponseVM(element) || isRequestVM(element) ? element.sessionId : ''
						...
						if (
							...
							this._register(
								ref.object.onDidChangeContentHeight(() => this._onDidChangeHeight.fire()),
							)
							...
							if (isResponseVM(codeBlockInfo.element)) {
								...
								this.codeBlockModelCollection
									.update(
										...
										{ text, languageId: codeBlockInfo.languageId, isComplete: isCodeBlockComplete },
									)
									.then((e) => {
...
class CollapsedCodeBlock extends Disposable {
	...
	constructor(
		...
		this._register(
			dom.addDisposableListener(this.element, dom.EventType.CONTEXT_MENU, (domEvent) => {
				...
				this.contextMenuService.showContextMenu({
					...
					getActions: () => {
						const menu = this.menuService.getMenuActions(
							...
							{ arg: { sessionId, requestId, uri: this.uri, stopId: inUndoStop } },
						)
			...
			autorun((r) => {
				...
				if (!isStreaming && !isComplete) {
					...
					iconEl.classList.add(
						...getIconClasses(this.modelService, this.languageService, uri, fileKind),
					)
					labelDetail.textContent = ''
				}
