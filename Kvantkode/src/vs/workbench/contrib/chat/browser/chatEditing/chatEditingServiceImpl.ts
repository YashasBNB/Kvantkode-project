...
export class ChatEditingService extends Disposable implements IChatEditingService {
	...
	private readonly _sessionsObs = observableValueOpts<LinkedList<ChatEditingSession>>(
		{ equalsFn: (a, b) => false },
		new LinkedList(),
	)
	...
	constructor(
		...
		this._register(
			textModelService.registerTextModelContentProvider(
				...
				_instantiationService.createInstance(ChatEditingTextModelContentProvider as any, this),
			),
		)
		this._register(
			textModelService.registerTextModelContentProvider(
		...
		const readonlyEnabledContextKey =
			chatEditingAgentSupportsReadonlyReferencesContextKey.bindTo(contextKeyService)
		...
		const ensureEditorOpen = (partUri: URI) => {
			...
			editedFilesExist.set(
				...
				fileExists.then((e) => {
					...
					const inactive =
						(this._editorService.activeEditorPane?.input instanceof ChatEditorInput &&
...
class ChatEditingMultiDiffSource implements IResolvedMultiDiffSource {
	...
	constructor(private readonly _currentSession: IObservable<IChatEditingSession | undefined>) {}
}
