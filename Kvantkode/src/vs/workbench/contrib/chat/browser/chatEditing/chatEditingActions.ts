...
export function getEditingSessionContext(
	accessor: ServicesAccessor,
	args: any[],
): { editingSession?: IChatEditingSession; chatWidget: IChatWidget } | undefined {
	...
	class RemoveFileFromWorkingSet extends WorkingSetAction {
		...
		async runWorkingSetAction(
			...
			const pendingEntries = currentEditingSession.entries
				...
				.filter(
					(entry) =>
						uris.includes(entry.modifiedURI) && entry.state.get() === WorkingSetEntryState.Modified,
				)
...
registerAction2(
	class OpenWorkingSetHistoryAction extends Action2 {
		...
		override async run(accessor: ServicesAccessor, ...args: any[]): Promise<void> {
			const context:
				| { sessionId: string; requestId: string; uri: URI; stopId: string | undefined }
				| undefined = args[0]
