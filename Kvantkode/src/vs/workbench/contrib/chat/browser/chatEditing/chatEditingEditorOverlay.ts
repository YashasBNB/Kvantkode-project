...
class ChatEditorOverlayWidget {
		...
		this._toolbar = instaService.createInstance(
			...
			MenuId.ChatEditingEditorContent,
			{
				...
				actionViewItemProvider: (action, options) => {
									...
									this._store.add(
										autorun((r) => {
											...
											if (ctrl) {
												...
												listener.value = addDisposableGenericMouseMoveListener(this.element, () =>
													ctrl.cancel(),
												)
											} else {
					...
					if (
						action.id === 'inlineChat2.reveal' ||
						action.id === 'workbench.action.chat.openEditSession'
					) {
						return new (class extends ActionViewItem {
							private _requestMessage: IObservable<
								{ message: string; paused?: boolean } | undefined
							>
							...
							constructor() {
								...
								this._requestMessage = derived((r) => {
									...
									const request = observableFromEvent(this, chatModel.onDidChange, () =>
	...
	show(
		...
		indicies: { entryIndex: IObservable<number>; changeIndex: IObservable<number> },
	) {
		...
		this._showStore.add(
			autorun((r) => {
				...
				this._navigationBearings.set(
					{ changeCount: totalChangesCount, activeIdx, entriesCount: entries.length },
					undefined,
				)
			}),
		)
			...
			autorun((r) => {
				...
				if (
					...
					const changeIndex = derived((r) =>
						entry ? entry.getEditorIntegration(editorPane).currentIndex.read(r) : 0,
					)
