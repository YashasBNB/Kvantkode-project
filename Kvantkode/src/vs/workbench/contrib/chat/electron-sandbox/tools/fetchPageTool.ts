...
export class FetchWebPageTool implements IToolImpl {
	...
	async prepareToolInvocation(
		...
		const urlsNeedingConfirmation = valid.filter(
			(url) =>
				!this._trustedDomainService.isValid(url) &&
		...
		if (valid.length > 1) {
			...
			invocationMessage.appendMarkdown(
				localize('fetchWebPage.invocationMessage.plural', 'Fetching {0} web pages', valid.length),
			)
		} else {
			...
			if (url.length > 400) {
				...
				pastTenseMessage.appendMarkdown(
					localize('fetchWebPage.pastTenseMessageResult.singular', 'Fetched {0}', url),
				)
				invocationMessage.appendMarkdown(
					localize('fetchWebPage.invocationMessage.singular', 'Fetching {0}', url),
				)
			}
		}
