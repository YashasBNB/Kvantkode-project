		...
		if (missingDependencies.length) {
			...
			if (extensions.length) {
				await Promises.settled(
					extensions.map((extension) => this.extensionsWorkbenchService.install(extension)),
				)
				this.notificationService.notify({
					...
					actions: {
						primary: [
							new Action('realod', localize('reload', 'Reload Window'), '', true, () =>
			...
			this.notificationService.info(
				localize('no missing deps', 'There are no missing dependencies to install.'),
			)
		}
	}
}
