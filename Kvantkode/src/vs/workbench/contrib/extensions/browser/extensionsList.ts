...
export class Renderer implements IPagedRenderer<IExtension, ITemplateData> {
	...
	renderElement(extension: IExtension, index: number, data: ITemplateData): void {
		...
		if (extension.state !== ExtensionState.Uninstalled && !extension.server) {
			...
			extension =
				this.extensionsWorkbenchService.local.filter(
					(e) =>
						e.server === extension.server && areSameExtensions(e.identifier, extension.identifier),
				)[0] || extension
		}
		...
		data.extensionDisposables.push(
			addDisposableListener(data.icon, 'error', () => (data.icon.src = extension.iconUrlFallback), {
				once: true,
			}),
		)
