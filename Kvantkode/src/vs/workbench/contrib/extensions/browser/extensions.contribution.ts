...
CommandsRegistry.registerCommand({
	...
	handler: async (accessor, id: string) => {
		...
		if (extensionToUninstall.isBuiltin) {
			throw new Error(
				localize('builtin', "Extension '{0}' is a Built-in extension and cannot be installed", id),
			)
		}
...
const CONTEXT_GALLERY_ALL_REPOSITORY_SIGNED = new RawContextKey<boolean>(
...
class ExtensionsContributions extends Disposable implements IWorkbenchContribution {
	...
	private async registerGalleryCapabilitiesContexts(
		...
		CONTEXT_GALLERY_ALL_REPOSITORY_SIGNED.bindTo(this.contextKeyService).set(
	...
	private registerGlobalActions(): void {
		...
		this.registerExtensionAction({
			...
			run: async (accessor: ServicesAccessor, resources: URI[] | URI) => {
				...
				for (const r of result) {
					...
					requireReload =
						requireReload ||
						r.value.runtimeState?.action === ExtensionRuntimeActionType.ReloadWindow
					requireRestart =
						requireRestart ||
						r.value.runtimeState?.action === ExtensionRuntimeActionType.RestartExtensions
				}
				if (error) {
					throw error
				}
		...
		;[
			...
			this.registerExtensionAction({
				...
				precondition: ContextKeyExpr.and(
					precondition,
					ContextKeyExpr.regex(ExtensionsSearchValueContext.key, /^@feature:/).negate(),
