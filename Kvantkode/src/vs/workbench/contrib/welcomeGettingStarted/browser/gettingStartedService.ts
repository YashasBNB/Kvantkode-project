	...
	private initCompletionEventListeners() {
		...
		this.extensionManagementService.getInstalled().then((installed) => {
			installed.forEach((ext) =>
				this.progressByEvent(`extensionInstalled:${ext.identifier.id.toLowerCase()}`),
			)
		})
			...
			extension.contributes?.walkthroughs?.map(async (walkthrough, index) => {
				...
				if (
					...
					this.contextService.contextMatchesRules(
						ContextKeyExpr.deserialize(override ?? walkthrough.when) ?? ContextKeyExpr.true(),
					)
				) {
				...
				if (walkthrough.featuredFor) {
					...
					isFeatured = await this.instantiationService.invokeFunction((a) =>
						checkGlobFileExists(a, folders, walkthrough.featuredFor!, token.token),
					)
				}
				...
				const walkthoughDescriptor: IWalkthrough = {
					...
					icon: {
						type: 'image',
						path: iconStr
							? FileAccess.uriToBrowserUri(joinPath(extension.extensionLocation, iconStr)).toString(
									true,
								)
							: DefaultIconPath,
					},
...
const convertRelativeMediaPathsToWebviewURIs = (
	...
	const convertPath = (path: string) =>
		path.startsWith('https://') ? URI.parse(path, true) : asWebviewUri(joinPath(basePath, path))
