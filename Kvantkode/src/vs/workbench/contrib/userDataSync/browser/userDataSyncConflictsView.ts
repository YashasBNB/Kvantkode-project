	...
	implements IUserDataSyncConflictsView
{
	...
	private async getTreeItems(): Promise<ITreeItem[]> {
		...
		for (const previewResource of conflictResources) {
			let result =
				conflictResourcesByProfile[conflictResourcesByProfile.length - 1]?.[0].id ===
				...
				const treeItem = {
					...
					label: {
						...
						strikethrough:
							...
							(resource.localChange === Change.Deleted || resource.remoteChange === Change.Deleted),
					},
					...
					command: {
						...
						title: '',
						arguments: [
							{ $treeViewId: '', $treeItemHandle: handle } satisfies TreeViewItemHandleArg,
						],
					},
	...
	async open(conflictToOpen: IResourcePreview): Promise<void> {
		if (
			!this.userDataSyncService.conflicts.some(({ conflicts }) =>
				conflicts.some(({ localResource }) => isEqual(localResource, conflictToOpen.localResource)),
			)
		) {
			return
		}
