...
export class LanguageConfigurationFileHandler extends Disposable {
	...
	private async _readConfigFile(configFileLocation: URI): Promise<ILanguageConfiguration> {
		try {
			const contents =
				await this._extensionResourceLoaderService.readExtensionResource(configFileLocation)
	...
	public static extractValidConfig(
		...
		const autoCloseBefore =
			typeof configuration.autoCloseBefore === 'string' ? configuration.autoCloseBefore : undefined
		...
		if (configuration.folding) {
			...
			const markers: FoldingMarkers | undefined =
				startMarker && endMarker ? { start: startMarker, end: endMarker } : undefined
			folding = {
