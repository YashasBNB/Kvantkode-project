...
class InspectEditorTokensWidget extends Disposable implements IContentWidget {
	...
	private _beginCompute(position: Position): void {
		...
		this._domNode.appendChild(
			document.createTextNode(nls.localize('inspectTMScopesWidget.loading', 'Loading...')),
		)
	...
	private _compute(
		...
		tree: ITextModelTreeSitter | undefined,
		position: Position,
	) {
		const textMateTokenInfo = grammar && this._getTokensAtPosition(grammar, position)
		const semanticTokenInfo =
			semanticTokens && this._getSemanticTokenAtPosition(semanticTokens, position)
		...
		const semTokenText =
			semanticTokenInfo && renderTokenText(this._model.getValueInRange(semanticTokenInfo.range))
		const tmTokenText =
			textMateTokenInfo &&
			renderTokenText(
				this._model
	...
	private async _computeSemanticTokens(position: Position): Promise<SemanticTokensResult | null> {
		...
		const rangeTokenProviders =
			this._languageFeaturesService.documentRangeSemanticTokensProvider.ordered(this._model)
	...
	private _getSemanticTokenAtPosition(
		...
		for (let i = 0; i < tokenData.length; i += 5) {
			...
			if (posLine === line && character <= posCharacter && posCharacter < character + len) {
				...
				for (
					let modifierIndex = 0;
					modifierSet > 0 && modifierIndex < semanticTokens.legend.tokenModifiers.length;
					modifierIndex++
				) {
					if (modifierSet & 1) {
