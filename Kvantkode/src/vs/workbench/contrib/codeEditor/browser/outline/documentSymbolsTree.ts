...
export class DocumentSymbolDragAndDrop implements ITreeDragAndDrop<DocumentSymbolItem> {
	...
	onDragStart(data: IDragAndDropData, originalEvent: DragEvent): void {
		const elements = (data as ElementsDragAndDropData<DocumentSymbolItem, DocumentSymbolItem[]>)
			.elements
		...
		const outlineElements =
			item instanceof OutlineElement ? [item] : Array.from(item.children.values())
...
export class DocumentSymbolGroupRenderer
	implements ITreeRenderer<OutlineGroup, FuzzyScore, DocumentSymbolGroupTemplate>
{
...
export class DocumentSymbolComparator implements IOutlineComparator<DocumentSymbolItem> {
	...
	compareByType(a: DocumentSymbolItem, b: DocumentSymbolItem): number {
		if (a instanceof OutlineGroup && b instanceof OutlineGroup) {
			...
			return (
				a.symbol.kind - b.symbol.kind || this._collator.value.compare(a.symbol.name, b.symbol.name)
			)
		}
		return 0
	}
