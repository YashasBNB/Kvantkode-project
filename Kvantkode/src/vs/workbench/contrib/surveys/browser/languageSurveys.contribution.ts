...
class LanguageSurvey extends Disposable {
	constructor(
		...
		if (
			storageService.getNumber(EDITED_LANGUAGE_COUNT_KEY, StorageScope.APPLICATION, 0) <
			data.editCount
		) {
				...
				new RunOnceWorker<ITextFileEditorModel>((models) => {
					models.forEach((m) => {
						if (
							...
							date !== storageService.get(EDITED_LANGUAGE_DATE_KEY, StorageScope.APPLICATION)
						) {
							const editedCount =
								storageService.getNumber(EDITED_LANGUAGE_COUNT_KEY, StorageScope.APPLICATION, 0) + 1
		...
		const sessionCount =
			storageService.getNumber(SESSION_COUNT_KEY, StorageScope.APPLICATION, 0) + 1
		...
		const isCandidate =
			storageService.getBoolean(IS_CANDIDATE_KEY, StorageScope.APPLICATION, false) ||
