...
export const IExtensionTransferService = createDecorator<IExtensionTransferService>(
							...
							if (toStat.isDirectory) {
								...
								if (extensionFolder.name === 'extensions.json') {
									try {
										...
										const j2 = json.filter(
											(entry: { identifier?: { id?: string } }) =>
												!isBlacklisted(entry?.identifier?.id),
										)
...
const transferTheseFilesOfOS = (
	...
	throw new Error(
		`os '${os}' not recognized or editor type '${fromEditor}' not supported for this OS`,
	)
}
