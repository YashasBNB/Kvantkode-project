...
export class RelatedInformationRenderer
	implements
		ITreeRenderer<RelatedInformation, RelatedInformationFilterData, IRelatedInformationTemplateData>
{
	constructor(@ILabelService private readonly labelService: ILabelService) {}
...
export class Filter implements ITreeFilter<MarkerElement, FilterData> {
	...
	private filterMarker(
		...
		const codeMatches = marker.marker.code
			...
			: undefined
		const matched =
			sourceMatches || codeMatches || lineMatches.some((lineMatch) => lineMatch.length > 0)
...
export class MarkersViewModel extends Disposable {
	...
	> = new Map<string, { viewModel: MarkerViewModel; disposables: IDisposable[] }>()
