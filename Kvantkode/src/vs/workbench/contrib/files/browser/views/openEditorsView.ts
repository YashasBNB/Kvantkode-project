...
export class OpenEditorsView extends ViewPane {
					...
					elements.forEach((e) => {
						if (e instanceof OpenEditor) {
							labelChangeListeners.push(
								e.editor.onDidChangeLabel(() => this.listRefreshScheduler?.schedule()),
							)
						}
					})
				}
	...
	private openEditor(
		...
		if (element) {
			this.telemetryService.publicLog2<
				WorkbenchActionExecutedEvent,
				WorkbenchActionExecutedClassification
			>('workbenchActionExecuted', { id: 'workbench.files.openFile', from: 'openEditors' })
	...
	private updateSize(): void {
		...
		this.minimumBodySize =
			this.orientation === Orientation.VERTICAL ? this.getMinExpandedBodySize() : 170
...
class OpenEditorRenderer implements IListRenderer<OpenEditor, IOpenEditorTemplateData> {
	...
	private readonly closeEditorAction = this.instantiationService.createInstance(
		CloseEditorAction,
		CloseEditorAction.ID,
		CloseEditorAction.LABEL,
	)
	private readonly unpinEditorAction = this.instantiationService.createInstance(
		UnpinEditorAction,
	...
	renderElement(
		...
		templateData.root.setResource(
			...
			},
			{
				...
				fileDecorations:
					this.configurationService.getValue<IFilesConfiguration>().explorer.decorations,
