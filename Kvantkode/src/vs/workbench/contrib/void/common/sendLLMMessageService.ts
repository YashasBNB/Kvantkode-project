...
export class LLMMessageService extends Disposable implements ILLMMessageService {
	...
	private readonly llmMessageHooks = {
		...
		onFinalMessage: {} as {
			[eventId: string]: (params: EventLLMMessageOnFinalMessageParams) => void
		},
	...
	private readonly listHooks = {
		ollama: {
			success: {} as {
				[eventId: string]: (params: EventModelListOnSuccessParams<OllamaModelResponse>) => void
			},
			error: {} as {
				[eventId: string]: (params: EventModelListOnErrorParams<OllamaModelResponse>) => void
			},
		},
		openAICompat: {
			success: {} as {
				[eventId: string]: (
					params: EventModelListOnSuccessParams<OpenaiCompatibleModelResponse>,
				) => void
			},
			error: {} as {
				[eventId: string]: (
					params: EventModelListOnErrorParams<OpenaiCompatibleModelResponse>,
				) => void
			},
		},
	} satisfies {
	...
	constructor(
		...
		// llm
		this._register(
			(this.channel.listen('onText_sendLLMMessage') satisfies Event<EventLLMMessageOnTextParams>)(
				(e) => {
		...
		this._register(
			(
				...
				) satisfies Event<EventLLMMessageOnFinalMessageParams>
			)((e) => {
		...
		this._register(
			(this.channel.listen('onError_sendLLMMessage') satisfies Event<EventLLMMessageOnErrorParams>)(
				(e) => {
		...
		this._register(
			(
				this.channel.listen('onSuccess_list_ollama') satisfies Event<
					EventModelListOnSuccessParams<OllamaModelResponse>
				>
			)((e) => {
		...
		this._register(
			(
				this.channel.listen('onError_list_ollama') satisfies Event<
					EventModelListOnErrorParams<OllamaModelResponse>
				>
			)((e) => {
