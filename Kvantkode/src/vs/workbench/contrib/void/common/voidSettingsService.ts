...
type SetSettingOfProviderFn = <S extends SettingName>(
	...
	settingName: S,
	newVal: SettingsOfProvider[ProviderName][S extends keyof SettingsOfProvider[ProviderName]
		? S
		: never],
) => Promise<void>
...
type SetGlobalSettingFn = <T extends GlobalSettingName>(
	settingName: T,
	newVal: GlobalSettings[T],
) => void
...
const _modelsWithSwappedInNewModels = (options: {
	existingModels: VoidStatefulModelInfo[]
	models: string[]
...
export const modelFilterOfFeatureName: {
	...
	Autocomplete: {
		filter: (o, opts) =>
			getModelCapabilities(o.providerName, o.modelName, opts.overridesOfModel).supportsFIM,
...
const _validatedModelState = (
	state: Omit<VoidSettingsState, '_modelOptions'>,
): VoidSettingsState => {
	let newSettingsOfProvider = state.settingsOfProvider
	...
	for (const featureName of featureNames) {
		...
		const modelOptionsForThisFeature = newModelOptions.filter((o) =>
			filter(o.selection, filterOpts),
		)
		...
		const selnIdx =
			modelSelectionAtFeature === null
				? -1
				: modelOptionsForThisFeature.findIndex((m) =>
						modelSelectionsEqual(m.selection, modelSelectionAtFeature),
					)
		...
		newModelSelectionOfFeature = {
			...newModelSelectionOfFeature,
			[featureName]:
				modelOptionsForThisFeature.length === 0 ? null : modelOptionsForThisFeature[0].selection,
		}
	}
...
class VoidSettingsService extends Disposable implements IVoidSettingsService {
	...
	async readAndInitializeState() {
		...
		try {
			...
			if (!readS.globalSettings.autoAllToolsEnabledOnce) {
				...
				const autoApprove = { ...readS.globalSettings.autoApprove } as any
				for (const t of toolApprovalTypes) autoApprove[t] = true
		...
		try {
			...
			{
				const providerName = 'openAICompatible' as const
				const s = readS.settingsOfProvider[
					providerName
				] as (typeof readS.settingsOfProvider)[typeof providerName]
				if (!s.endpoint) s.endpoint = defaultProviderSettings.openAICompatible.endpoint
				...
				if (s.endpoint) {
					...
					if (!/\/(v1)$/.test(ep)) ep = ep + '/v1'
					s.endpoint = ep
				}
				if (!s.apiKey) s.apiKey = defaultProviderSettings.openAICompatible.apiKey
				if (!s.headersJSON) s.headersJSON = '{}'
	...
	private async _readState(): Promise<VoidSettingsState> {
		const encryptedState = this._storageService.get(
	...
	setAutodetectedModels(
		...
		if (
			!(
				oldModelNames.length === new_names.length &&
