...
export type SnakeCase<S extends string> =
	...
	S extends 'URI'
		...
		: // suffix URI: e.g. 'rootURI' -> snakeCase('root') + '_uri'
			S extends `${infer Prefix}URI`
			? `${SnakeCase<Prefix>}_uri`
...
export const availableTools = (
	...
	const builtinToolNames: BuiltinToolName[] | undefined =
		chatMode === 'normal'
			? ['read_file', 'ls_dir', 'get_dir_tree', 'search_pathnames_only', 'search_for_files', 'search_in_file', 'read_lint_errors'] as BuiltinToolName[] // Chat mode = chat + gather (all info gathering tools)
			: chatMode === 'gather'
				? (Object.keys(builtinTools) as BuiltinToolName[]).filter(
						(toolName) => !(toolName in approvalTypeOfBuiltinToolName),
					)
				: chatMode === 'agent'
					? ['read_file', 'ls_dir', 'get_dir_tree', 'search_pathnames_only', 'search_for_files', 'search_in_file', 'read_lint_errors', 'create_file_or_folder', 'delete_file_or_folder', 'edit_file', 'rewrite_file', 'run_command', 'open_persistent_terminal'] as BuiltinToolName[] // Agent mode = all powerful tools (agent + gather + editing)
					: undefined

	const effectiveBuiltinTools =
		builtinToolNames?.map((toolName) => builtinTools[toolName]).filter((tool) => tool !== undefined) ?? undefined
	const effectiveMCPTools = chatMode === 'agent' ? undefined : mcpTools // TEMP FIX: Disable MCP tools in agent mode

	const tools: InternalToolInfo[] | undefined = !(builtinToolNames || mcpTools)
		? undefined
...
- Open files:
${openedURIs.join('\n') || 'NO OPENED FILES'}${'' /* separator */}${
		mode === 'agent' && persistentTerminalIDs.length !== 0
			? `
...
${directoryStr}
	...
	if (mode === 'agent' || mode === 'gather') {
		...
		details.push(
			`You can use tools to gather information from files and directories when it helps answer the user's question.`
		)
		details.push(
			`Only call tools if they help you accomplish the user's goal. If the user simply says hi or asks you a general question that you can answer without tools, then do NOT use tools.`,
		)
		details.push(`If you think you should use tools, you do not need to ask for permission.`)
		details.push('Only use ONE tool call at a time.')
		details.push(
			`NEVER say something like "I'm going to use \`tool_name\`". Instead, describe at a high level what the tool will do, like "I'm going to read the ___ file to understand the implementation", etc.`,
		)
	}

	if (mode === 'agent') {
		details.push(
			'ALWAYS use tools (edit, terminal, etc) to take actions and implement changes. For example, if you would like to edit a file, you MUST use a tool. Do NOT show code blocks for edits; perform the edit using tools.',
		)
		details.push('Follow a Plan → Act → Verify loop:')
		details.push(
			'- Plan: Briefly outline the minimal steps to achieve the goal (bulleted, concise).',
		)
		details.push(
			"- Act: Execute the steps one-by-one using tool calls (read/search/edit/run). When exactly one staged file is present in SELECTIONS, edit that file directly using 'rewrite_file' (full overwrite) or 'edit_file' (search/replace) as appropriate.",
		)
		details.push(
			'- Verify: After edits, run commands (tests, build, dev server, lint) to check for errors or failing conditions. If issues arise, iterate with additional tool calls until the goal is achieved or a clear blocker is identified.',
		)
		details.push(
			'Prioritize completing the task end-to-end over stopping early. Use multiple assistant turns if needed to complete all steps.',
		)
		details.push(
			`You will OFTEN need to gather context before making a change. Use read_file and other information gathering tools to maximize certainty that your change is correct.`,
		)
		details.push(
			`ALWAYS have maximal certainty in a change BEFORE you make it. If you need more information about a file, variable, function, or type, you should inspect it, search it, or take all required actions to maximize certainty that your change is correct.`,
		)
		details.push(
		...
		details.push(
			'- Plan a minimal automation workflow (e.g., create a small script using a browser automation library, parameterize symbol/qty/order type).',
		)
		details.push(
			'- Ask for any missing inputs (broker site URL, credentials method, 2FA flow, symbol, quantity, order type/price). Never assume secrets; request or use environment variables/config files the user specifies.',
		)
		details.push(
			'- Implement the script using tools to create/edit files. Prefer a single entrypoint script and a small config file. Avoid hardcoding secrets in code.',
		)
		details.push(
			'- Use terminal tools to install dependencies and run the script. Confirm with the user before placing a real order; provide a dry-run mode if possible.',
		)
		details.push(
			'- After execution, verify success (output, screenshots/logs if available), and report the result. If it fails, iterate to fix and retry.',
		)
	}
...
export const messageOfSelection = async (
	...
	if (s.type === 'CodeSelection') {
		...
		const innerVal = val
		const content =
			val === null ? '' : `${tripleTick[0]}${s.language}\n${innerVal}\n${tripleTick[1]}`
...
export const rewriteCode_userMessage = ({
	originalCode,
	applyStr,
	language,
}: {
...
export const searchReplaceGivenDescription_userMessage = ({
	originalCode,
	applyStr,
}: {
	originalCode: string
...
export const ctrlKStream_systemMessage = ({
	...
	quickEditFIMTags: QuickEditFimTagsType
}) => {
	return `\
 ...
 * ...
 */
export const gitCommitMessage_userMessage = (
	stat: string,
	sampledDiffs: string,
	branch: string,
	log: string,
