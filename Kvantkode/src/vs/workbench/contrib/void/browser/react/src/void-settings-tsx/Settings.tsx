		...
		<div className="">
			...
			{showCheckmark ? (
				...
				<div className="mt-4">
					...
					{errorString && (
						<div className="text-red-500 truncate whitespace-nowrap mt-1">{errorString}</div>
					)}
				</div>
			) : (
				<div
		...
		<ErrorBoundary>
			<div className="my-1">
				<VoidSimpleInputBox
					...
					compact={true}
				/>
				{!subTextMd ? null : <div className="py-1 px-3 opacity-50 text-sm">{subTextMd}</div>}
			</div>
		</ErrorBoundary>
	)
}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return (
		<div className="prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-void-fg-3 text-sm list-decimal select-text">
			...
			<div className="pl-6 flex items-center w-fit" data-tooltip-id="void-tooltip-ollama-settings">
				<ChatMarkdownRender
		...
		<>
			...
			>
				{transferState.type === 'done' ? (
					...
					<AnimatedCheckmarkButton text="Settings Transferred" className="bg-none" />
				) : null}
			</VoidButtonBgDarken>
...
const MCPServersList = () => {
	...
	if (mcpServiceState.error) {
		content = <div className="text-void-fg-3 text-sm mt-2">{mcpServiceState.error}</div>
	} else {
		const entries = Object.entries(mcpServiceState.mcpServerOfName)
		if (entries.length === 0) {
			content = <div className="text-void-fg-3 text-sm mt-2">No servers found</div>
		} else {
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					<div className="max-w-3xl">
						<h1 className="text-2xl w-full">{`KvantKode's Settings`}</h1>
								...
								<ErrorBoundary>
									...
									<div className="flex flex-col gap-y-8 my-4">
										...
										<div className="w-full">
											...
											<div className="text-sm text-void-fg-3 mt-1">{`Settings that control the visibility of KvantKode suggestions in the code editor.`}</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className="text-void-fg-3 mb-4">{`Transfer your editor settings into KvantKode.`}</h4>
								...
								<div>
									...
									<h4 className="text-void-fg-3 mb-4">{`Transfer KvantKode's settings and chats in and out of KvantKode.`}</h4>
