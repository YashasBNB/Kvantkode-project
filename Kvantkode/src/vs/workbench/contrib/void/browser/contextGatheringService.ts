...
export const IContextGatheringService =
	createDecorator<IContextGatheringService>('contextGatheringService')
...
class ContextGatheringService extends Disposable implements IContextGatheringService {
	...
	public async updateCache(model: ITextModel, pos: Position): Promise<void> {
		...
		await this._gatherNearbySnippets(
			model,
			pos,
			this._NUM_LINES,
			3,
		...
		await this._gatherParentSnippets(
			model,
			pos,
			this._NUM_LINES,
			3,
	...
	private async _gatherParentSnippets(
		...
		const containerRange =
			container.kind === SymbolKind.Method ? container.selectionRange : container.range
	...
	private _isRangeVisited(
		...
		return visited.some(
			(interval) =>
				interval.uri === uri && !(endLine < interval.startLine || startLine > interval.endLine),
		)
	}

	private async _getSymbolsNearPosition(
		model: ITextModel,
		pos: Position,
		numLines: number,
	...
	private async _getSymbolsNearRange(
		model: ITextModel,
		range: IRange,
		numLines: number,
	): Promise<DocumentSymbol[]> {
	...
	private async _getDefinitionSymbols(
		model: ITextModel,
		symbol: DocumentSymbol,
	): Promise<(DocumentSymbol & { uri: URI })[]> {
	...
	private async _findContainerFunction(
		model: ITextModel,
		pos: Position,
	): Promise<DocumentSymbol | null> {
		const searchRange = new Range(
			Math.max(pos.lineNumber - 1, 1),
			1,
