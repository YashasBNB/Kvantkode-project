...
export const IVoidCommandBarService =
	createDecorator<IVoidCommandBarService>('VoidCommandBarService')
...
export class VoidCommandBarService extends Disposable implements IVoidCommandBarService {
	...
	constructor(
		...
		// state updaters
		this._register(
			this._editCodeService.onDidAddOrDeleteDiffZones((e) => {
				for (const uri of this._listenToTheseURIs) {
					...
					const { addedDiffZones, deletedDiffZones } = this._getDiffZoneChanges(
						oldDiffZones,
						currentDiffZones || [],
					)

					const diffZonesWithoutDeleted = oldDiffZones.filter(
						(olddiffareaid) => !deletedDiffZones.has(olddiffareaid),
					)
		...
		this._register(
			this._editCodeService.onDidChangeStreamingInDiffZone((e) => {
				for (const uri of this._listenToTheseURIs) {
					if (e.uri.fsPath !== uri.fsPath) continue
					// --- sortedURIs: no change
					// --- state:
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		this.instantiationService.invokeFunction((accessor) => {
			...
			this._register(
				editor.onWillChangeModel((model) => {
