...
class CommentingRangeDecorator {
	...
	private _doUpdate(
		...
		const rangesDifference =
			this.commentingRangeDecorations.length - commentingRangeDecorations.length
...
export function moveToNextCommentInThread(
	commentInfo: { thread: languages.CommentThread<IRange>; comment?: languages.Comment } | undefined,
	type: 'next' | 'previous',
) {
	...
	const comment =
		commentInfo.thread.comments?.[type === 'previous' ? currentIndex - 1 : currentIndex + 1]
	if (!comment) {
		return
	}
	return {
...
export class CommentController implements IEditorContribution {
	...
	private _pendingEditsCache: {
		[key: string]: { [key: string]: { [key: number]: languages.PendingComment } }
	...
	private async handleCommentAdded(
		...
		const pendingEdits =
			this._pendingEditsCache[uniqueOwner] && this._pendingEditsCache[uniqueOwner][thread.threadId]
		const shouldReveal =
			thread.canReply &&
			thread.isTemplate &&
	...
	private async openCommentsView(thread: languages.CommentThread) {
		if (thread.comments && thread.comments.length > 0 && threadHasMeaningfulComments(thread)) {
			const openViewState =
				this.configurationService.getValue<ICommentsConfiguration>(COMMENTS_SECTION).openView
			if (openViewState === 'file') {
				...
				const hasShownView =
					this.viewsService.getViewWithId<CommentsPanel>(COMMENTS_VIEW_ID)?.hasRendered
				if (!hasShownView) {
					return this.viewsService.openView(COMMENTS_VIEW_ID)
				}
			}
	...
	private getContextMenuActions(
		...
		commentInfos.forEach((commentInfo) => {
			...
			actions.push(
				new Action('addCommentThread', `${label || extensionId}`, undefined, true, () => {
