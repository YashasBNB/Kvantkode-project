...
class TimelineAggregate {
	...
	add(timeline: Timeline, options: TimelineOptions) {
		...
		if (timeline.items.length !== 0 && this.items.length !== 0) {
			...
			if (
				(timeline.items[timeline.items.length - 1]?.timestamp ?? 0) >= (this.newest?.timestamp ?? 0)
			) {
...
export class TimelinePane extends ViewPane {
	...
	get pageOnScroll() {
		if (this._pageOnScroll === undefined) {
			this._pageOnScroll =
				this.configurationService.getValue<boolean | null | undefined>('timeline.pageOnScroll') ??
				false
		}
	...
	get pageSize() {
		...
		if (pageSize === undefined || pageSize === null) {
			...
			pageSize = Math.max(
				20,
				Math.floor((this.tree?.renderHeight ?? 0 / ItemHeight) + (this.pageOnScroll ? 1 : -1)),
			)
		}
		return pageSize
	}
	...
	private onActiveEditorChanged() {
		...
		if (
			(this.uriIdentityService.extUri.isEqual(uri, this.uri) && uri !== undefined) ||
	...
	private loadTimelineForSource(
		...
		disposables.add(
			tokenSource.token.onCancellationRequested(() => this.pendingRequests.delete(source)),
		)
		...
		if (this.uri === undefined) {
			...
			if (this.pendingRequests.size !== 0) {
				...
				const scmProviderCount =
					this.contextKeyService.getContextKeyValue<number>('scm.providerCount')
				if (
					this.timelineService.getSources().filter(({ id }) => !this.excludedSources.has(id))
						.length === 0
				) {
					this.message = localize(
					...
					if (
						this.configurationService.getValue('workbench.localHistory.enabled') &&
