...
suite('NotebookDiff Diff Service', () => {
	...
	async function mapCells(original: MockNotebookCell[], modified: MockNotebookCell[]) {
		...
		).map((mapping) => ({ modified: mapping.modified, original: mapping.original }))
		...
		worker.$acceptNewModel(
			...
			{},
			{},
			original.map((cell, index) => cellToMainCellTdo(originalUri, cell, index)),
		)
		worker.$acceptNewModel(
			modifiedUri.toString(),
			{},
			{},
			modified.map((cell, index) => cellToMainCellTdo(modifiedUri, cell, index)),
		)
