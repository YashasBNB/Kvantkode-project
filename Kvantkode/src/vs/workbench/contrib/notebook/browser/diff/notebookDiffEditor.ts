		...
		IEditorPaneWithScrolling
{
	...
	protected createEditor(parent: HTMLElement): void {
		...
		this._register(
			this._list.onDidChangeFocus(() =>
				this._onDidChangeSelection.fire({ reason: EditorPaneSelectionChangeReason.USER }),
			),
		)
		...
		this._register(
			DOM.addStandardDisposableGenericMouseUpListener(this._overflowContainer, () => {
	...
	firstChange(): void {
		...
		const index = currentViewModels.findIndex(
			(vm) =>
				vm.type !== 'unchanged' && vm.type !== 'unchangedMetadata' && vm.type !== 'placeholder',
		)
		if (index >= 0) {
	...
	createOutput(
		...
		this._insetModifyQueueByOutputId.queue(
			...
			async () => {
				const activeWebview =
					diffSide === DiffSide.Modified ? this._modifiedWebview : this._originalWebview
				if (!activeWebview) {
					return
				}
	...
	layout(dimension: DOM.Dimension, position: DOM.IDomPosition): void {
		...
		this._dimension = dimension.with(
			dimension.width -
				(overviewRulerEnabled ? NotebookTextDiffEditor.ENTIRE_DIFF_OVERVIEW_WIDTH : 0),
		)
