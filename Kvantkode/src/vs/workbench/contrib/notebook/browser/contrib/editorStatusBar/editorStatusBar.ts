...
class KernelStatus extends Disposable implements IWorkbenchContribution {
	...
	private _showKernelStatus(notebook: NotebookTextModel) {
		...
		const suggested =
			((suggestions.length === 1 ? suggestions[0] : undefined) ?? all.length === 1)
				? all[0]
				: undefined
		...
		if (all.length === 0) {
			...
			if (!kernel) {
				...
				this._kernelInfoElement.add(
					this._instantiationService.createInstance(ImplictKernelSelector, notebook, kernel),
				)
			}
...
class NotebookIndentationStatus extends Disposable {
	...
	private _show(editor: INotebookEditor) {
		...
		const insertSpaces =
			cellEditorOverridesRaw?.['editor.insertSpaces'] ?? cellOptions?.insertSpaces
...
class NotebookEditorStatusContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(@IEditorGroupsService private readonly editorGroupService: IEditorGroupsService) {
		...
		this._register(
			editorGroupService.onDidCreateAuxiliaryEditorPart((part) => this.createNotebookStatus(part)),
		)
	}
