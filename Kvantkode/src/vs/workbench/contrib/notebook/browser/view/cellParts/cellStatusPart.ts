...
export class CellEditorStatusBar extends CellContentPart {
	...
	constructor(
		...
		this._register(
			DOM.addDisposableListener(this.statusBarContainer, DOM.EventType.CLICK, (e) => {
				if (
	...
	updateContext(context: INotebookCellActionContext) {
		...
		this.itemsDisposable.add(
			this.currentContext.cell.onDidChangeCellStatusBarItems(() => this.updateRenderedItems()),
		)
		this.itemsDisposable.add(
			this.currentContext.notebookEditor.onDidChangeActiveCell(() => this.updateActiveCell()),
		)
		this.updateInternalLayoutNow(this.currentContext.cell)
	...
	private updateActiveCell(): void {
		const isActiveCell =
			this.currentContext!.notebookEditor.getActiveCell() === this.currentContext?.cell
	...
	updateItem(item: INotebookCellStatusBarItem, maxWidth: number | undefined) {
		...
		if (item.tooltip) {
			...
			this._itemDisposables.add(
				this._hoverService.setupManagedHover(this._hoverDelegate, this.container, hoverContent),
			)
		}
