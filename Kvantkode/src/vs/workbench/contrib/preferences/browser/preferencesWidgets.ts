...
export class FolderSettingsActionViewItem extends BaseActionViewItem {
	...
	override render(container: HTMLElement): void {
		...
		this._register(
			DOM.addDisposableListener(this.anchorElement, DOM.EventType.MOUSE_DOWN, (e) =>
				DOM.EventHelper.stop(e),
			),
		)
		this._register(
			DOM.addDisposableListener(this.anchorElement, DOM.EventType.CLICK, (e) => this.onClick(e)),
		)
		this._register(
			DOM.addDisposableListener(this.container, DOM.EventType.KEY_UP, (e) => this.onKeyUp(e)),
		)
...
export class SettingsTargetsWidget extends Widget {
	...
	private resetLabels() {
		...
		const hostLabel =
			remoteAuthority && this.labelService.getHostLabel(Schemas.vscodeRemote, remoteAuthority)
	...
	private create(parent: HTMLElement): void {
		...
		this.userLocalSettings = new Action('userSettings', '', '.settings-tab', true, () =>
			this.updateTarget(ConfigurationTarget.USER_LOCAL),
		)
		this.userLocalSettings.tooltip = localize('userSettings', 'User')

		this.userRemoteSettings = new Action('userSettingsRemote', '', '.settings-tab', true, () =>
		...
		this.workspaceSettings = new Action('workspaceSettings', '', '.settings-tab', false, () =>
	...
	private async update(): Promise<void> {
		...
		this.userRemoteSettings.enabled = !!(
			this.options.enableRemoteSettings && this.environmentService.remoteAuthority
		)
