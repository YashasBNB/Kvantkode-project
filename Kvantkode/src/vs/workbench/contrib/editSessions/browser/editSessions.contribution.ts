...
export class EditSessionsContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this._register(
			this.editSessionsStorageService.onDidSignOut(() => this.updateAccountsMenuBadge()),
		)
	}

	private async autoResumeEditSession() {
		const shouldAutoResumeOnReload =
			this.configurationService.getValue('workbench.cloudChanges.autoResume') === 'onReload'
		...
		if (this.environmentService.editSessionId !== undefined) {
			...
			await this.progressService.withProgress(
				resumeProgressOptions,
				async (progress) =>
					await this.resumeEditSession(undefined, true, undefined, undefined, progress),
			)
	...
	private updateAccountsMenuBadge() {
		...
		const badge = new NumberBadge(1, () =>
			localize('check for pending cloud changes', 'Check for pending cloud changes'),
		)
					...
					async run(
						...
						if (!destination && !uri) {
							...
							if (!destination) {
								...
								>('continueOn.editSessions.pick.outcome', { outcome: 'noSelection' })
								return
							}
						}
									...
									async () => {
										...
										if (ref !== undefined) {
											...
											that.telemetryService.publicLog2<
												ContinueOnEventOutcome,
												ContinueOnClassificationOutcome
											>('continueOn.editSessions.store.outcome', { outcome: 'storeSkipped' })
										}
										return ref
									},
									() => {
										...
										that.telemetryService.publicLog2<
											ContinueOnEventOutcome,
											ContinueOnClassificationOutcome
										>('continueOn.editSessions.store.outcome', { outcome: 'storeCancelledByUser' })
									},
								)
							} catch (ex) {
								that.telemetryService.publicLog2<
									ContinueOnEventOutcome,
									ContinueOnClassificationOutcome
								>('continueOn.editSessions.store.outcome', { outcome: 'storeFailed' })
								throw ex
							}
						}
				...
				class ResumeLatestEditSessionAction extends Action2 {
					...
					async run(
						...
						await that.progressService.withProgress(
							...
							async () =>
								await that.resumeEditSession(editSessionId, undefined, forceApplyUnrelatedChange),
						)
					}
				},
			),
					...
					async run(accessor: ServicesAccessor): Promise<void> {
						...
						await that.progressService.withProgress(
							...
							async () => {
								...
								that.telemetryService.publicLog2<StoreEvent, StoreClassification>(
									'editSessions.store',
								)
	...
	async resumeEditSession(
		...
		if (!data) {
			if (ref === undefined && !silent) {
				this.notificationService.info(
					localize('no cloud changes', 'There are no changes to resume from the cloud.'),
				)
		...
		try {
			...
			this.notificationService.error(
				localize('resume failed', 'Failed to resume your working changes from the cloud.'),
			)
		}
	...
	async storeEditSession(
		...
		const data: EditSession = {
			...
			version: 2,
			workspaceStateId:
				this.editSessionsStorageService.lastWrittenResources.get('workspaceState')?.ref,
		}
			...
			if (ex instanceof UserDataSyncStoreError) {
				switch (ex.code) {
					...
					default:
						...
						this.notificationService.error(
							localize('payload failed', 'Your working changes cannot be stored.'),
						)
						break
				}
			}
	...
	private async shouldContinueOnWithEditSession(): Promise<boolean> {
		...
		if (this.configurationService.getValue(useEditSessionsWithContinueOn) === 'off') {
			...
			>('continueOn.editSessions.canStore.outcome', { outcome: 'disabledEditSessionsViaSetting' })
			return false
		}
	...
	private async pickContinueEditSessionDestination(): Promise<string | undefined> {
		...
		const quickPick = disposables.add(
			this.quickInputService.createQuickPick<ContinueEditSessionItem>({ useSeparators: true }),
		)
	...
	private async resolveDestination(command: string): Promise<URI | 'noDestinationUri' | undefined> {
		...
		try {
			...
			if (uri === undefined) {
				...
				>('continueOn.openDestination.outcome', { selection: command, outcome: 'noDestinationUri' })
			...
			if (URI.isUri(uri)) {
				this.telemetryService.publicLog2<
					EvaluateContinueOnDestinationEvent,
					EvaluateContinueOnDestinationClassification
				>('continueOn.openDestination.outcome', { selection: command, outcome: 'resolvedUri' })
				return uri
			}
			...
			this.telemetryService.publicLog2<
				EvaluateContinueOnDestinationEvent,
				EvaluateContinueOnDestinationClassification
			>('continueOn.openDestination.outcome', { selection: command, outcome: 'invalidDestination' })
			return undefined
		} catch (ex) {
			if (ex instanceof CancellationError) {
				this.telemetryService.publicLog2<
					EvaluateContinueOnDestinationEvent,
					EvaluateContinueOnDestinationClassification
				>('continueOn.openDestination.outcome', { selection: command, outcome: 'cancelled' })
			} else {
				this.telemetryService.publicLog2<
					EvaluateContinueOnDestinationEvent,
					EvaluateContinueOnDestinationClassification
				>('continueOn.openDestination.outcome', { selection: command, outcome: 'unknownError' })
			}
			return undefined
		}
	}
	...
	private createPickItems(): (ContinueEditSessionItem | IQuickPickSeparator)[] {
		const items = [...this.continueEditSessionOptions].filter(
			(option) =>
				option.when === undefined || this.contextKeyService.contextMatchesRules(option.when),
		)
		...
		if (getVirtualWorkspaceLocation(this.contextService.getWorkspace()) !== undefined && isNative) {
			items.push(
				new ContinueEditSessionItem(
					'$(folder) ' +
						localize('continueEditSessionItem.openInLocalFolder.v2', 'Open in Local Folder'),
					openLocalFolderCommand.id,
