...
export class InlineChatSessionServiceImpl implements IInlineChatSessionService {
	...
	async createSession(
		...
		const chatModel =
			options.session?.chatModel ?? this._chatService.startSession(ChatAgentLocation.Editor, token)
		if (!chatModel) {
	...
	private _releaseSession(session: Session, byServer: boolean): void {
		...
		this._telemetryService.publicLog2<TelemetryData, TelemetryDataClassification>(
			'interactiveEditor/session',
			session.asTelemetryData(),
		)
		...
		store.add(
			autorun((r) => {
				...
				const allSettled = entries.every((entry) => {
					...
					return (
						(state === WorkingSetEntryState.Accepted || state === WorkingSetEntryState.Rejected) &&
