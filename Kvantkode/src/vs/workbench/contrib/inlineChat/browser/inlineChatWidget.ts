...
export class InlineChatWidget {
	...
	constructor(
		...
		const statusMenuId =
			_options.statusMenuId instanceof MenuId ? _options.statusMenuId : _options.statusMenuId.menu

		// BUTTON bar
		const statusMenuOptions =
			_options.statusMenuId instanceof MenuId ? undefined : _options.statusMenuId.options
		...
		this._store.add(
			this._chatService.onDidPerformUserAction((e) => {
				if (
					e.sessionId === this._chatWidget.viewModel?.model.sessionId &&
					e.action.kind === 'vote'
				) {
	...
	private _updateAriaLabel(): void {
		...
		if (this._accessibilityService.isScreenReaderOptimized()) {
			...
			if (
				this._configurationService.getValue<boolean>(AccessibilityVerbositySettingId.InlineChat)
			) {
	...
	updateStatus(
		message: string,
		ops: { classes?: string[]; resetAfter?: number; keepMessage?: boolean; title?: string } = {},
	) {
