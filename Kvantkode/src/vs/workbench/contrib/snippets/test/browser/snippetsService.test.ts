...
suite('SnippetsService', function () {
...

		{
			await provider
				...
				.then((result) => {
					...
					assert.strictEqual(
						(result.suggestions[0].range as CompletionItemRanges).insert.startColumn,
						1,
					)
					...
					assert.strictEqual(
						(result.suggestions[1].range as CompletionItemRanges).insert.startColumn,
						1,
					)
				})
			...
			assert.strictEqual(
				(completions.items[0].completion.range as CompletionItemRanges).insert.startColumn,
				1,
			)
			...
			assert.strictEqual(
				(completions.items[1].completion.range as CompletionItemRanges).insert.startColumn,
				1,
			)
		}
	...
	test('No user snippets in suggestions, when inside the code, #30508', async function () {
		...
		const model = disposables.add(
			instantiateTextModel(instantiationService, '<head>\n\t\n>/head>', 'fooLang'),
		)
		await provider
			.provideCompletionItems(model, new Position(1, 1), defaultCompletionContext)!
			.then((result) => {
