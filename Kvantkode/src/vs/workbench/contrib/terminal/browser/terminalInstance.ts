...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	constructor(
		...
		this._shellLaunchConfig.shellIntegrationEnvironmentReporting =
			this._configurationService.getValue(TerminalSettingId.ShellIntegrationEnvironmentReporting)
	...
	private _open(): void {
		...
		this._register(
			dom.addDisposableListener(xterm.raw.textarea, 'focus', () => this._setFocus(true)),
		)
		this._register(
			dom.addDisposableListener(xterm.raw.textarea, 'blur', () => this._setFocus(false)),
		)
		this._register(
			dom.addDisposableListener(xterm.raw.textarea, 'focusout', () => this._setFocus(false)),
		)
			...
			processManager.onProcessReady(async (e) => {
				...
				if (!this._labelComputer) {
					this._labelComputer = this._register(
						this._scopedInstantiationService.createInstance(TerminalLabelComputer),
					)
					this._register(
				...
				if (this._shellLaunchConfig.name) {
					...
					setTimeout(() => {
						this._xtermReadyPromise.then((xterm) => {
							if (xterm) {
								this._messageTitleDisposable.value = xterm.raw.onTitleChange((e) =>
...
export class TerminalLabelComputer extends Disposable {
	...
	computeLabel(
		...
		const type =
			instance.shellLaunchConfig.attachPersistentProcess?.type || instance.shellLaunchConfig.type
