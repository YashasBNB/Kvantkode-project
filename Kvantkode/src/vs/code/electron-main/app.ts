...
export class CodeApplication extends Disposable {
	...
	private configureSession(): void {
		...
		const isUrlFromWindow = (requestingUrl?: string | undefined) =>
			requestingUrl?.startsWith(`${Schemas.vscodeFileResource}://${VSCODE_AUTHORITY}`)
	...
	private registerListeners(): void {
		...
		app.on('web-contents-created', (event, contents) => {
			...
			if (
				contents?.opener?.url.startsWith(`${Schemas.vscodeFileResource}://${VSCODE_AUTHORITY}/`)
			) {
	...
	async startup(): Promise<void> {
		...
		appInstantiationService.invokeFunction((accessor) =>
			this._register(
				new ErrorTelemetry(accessor.get(ILogService), accessor.get(ITelemetryService)),
			),
		)
		...
		const initialProtocolUrls = await appInstantiationService.invokeFunction((accessor) =>
	...
	private setupManagedRemoteResourceUrlHandler(mainProcessElectronServer: ElectronIPCServer) {
		...
		protocol.registerBufferProtocol(Schemas.vscodeManagedRemoteResource, (request, callback) => {
			...
			remoteResourceChannel.value
				.call<NodeRemoteResourceResponse>(NODE_REMOTE_RESOURCE_IPC_METHOD_NAME, [url])
				.then(
	...
	private setupSharedProcess(
		...
		const sharedProcess = this._register(
			this.mainInstantiationService.createInstance(SharedProcess, machineId, sqmId, devDeviceId),
		)
	...
	private async initServices(
		...
		services.set(
			IWindowsMainService,
			new SyncDescriptor(WindowsMainService, [machineId, sqmId, devDeviceId, this.userEnv], false),
		)
		services.set(
		...
		services.set(
			ILaunchMainService,
			new SyncDescriptor(LaunchMainService, undefined, false /* proxied to other processes */),
		)

		// Diagnostics
		services.set(
			IDiagnosticsMainService,
			new SyncDescriptor(DiagnosticsMainService, undefined, false /* proxied to other processes */),
		)
		services.set(
			IDiagnosticsService,
			ProxyChannel.toService(
				getDelayedChannel(sharedProcessReady.then((client) => client.getChannel('diagnostics'))),
			),
		)
		...
		services.set(
			INativeHostMainService,
			new SyncDescriptor(NativeHostMainService, undefined, false /* proxied to other processes */),
		)
		...
		services.set(
			IWorkspacesService,
			new SyncDescriptor(WorkspacesMainService, undefined, false /* proxied to other processes */),
		)
		services.set(
			IWorkspacesHistoryMainService,
		...
		services.set(
			IURLService,
			new SyncDescriptor(NativeURLService, undefined, false /* proxied to other processes */),
		)
		...
		if (supportsTelemetry(this.productService, this.environmentMainService)) {
			...
			const channel = getDelayedChannel(
				sharedProcessReady.then((client) => client.getChannel('telemetryAppender')),
			)
	...
	private initChannels(
		...
		sharedProcessClient.then((client) =>
			client.registerChannel(LOCAL_FILE_SYSTEM_CHANNEL_NAME, fileSystemProviderChannel),
		)
		...
		const storageChannel = disposables.add(
			new StorageDatabaseChannel(this.logService, accessor.get(IStorageMainService)),
		)
		...
		const voidUpdatesChannel = ProxyChannel.fromService(
			accessor.get(IVoidUpdateService),
			disposables,
		)
		...
		const electronExtensionHostDebugBroadcastChannel =
			new ElectronExtensionHostDebugBroadcastChannel(accessor.get(IWindowsMainService))
		mainProcessElectronServer.registerChannel(
