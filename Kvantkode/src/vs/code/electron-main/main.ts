...
class CodeMain {
	...
	private async startup(): Promise<void> {
		...
		try {
			...
			await instantiationService.invokeFunction(async (accessor) => {
				...
				FSPromises.writeFile(environmentMainService.mainLockfile, String(process.pid)).catch(
					(err) => {
	...
	private createServices(): [
		...
		const logService = disposables.add(
			new LogService(bufferLogger, [new ConsoleMainLogger(loggerService.getLogLevel())]),
		)
		...
		if (isWindows && productService.win32RegValueName) {
			...
			policyService = disposables.add(
				new NativePolicyService(logService, productService.darwinBundleIdentifier),
			)
		} else if (environmentMainService.policyFile) {
			policyService = disposables.add(
				new FilePolicyService(environmentMainService.policyFile, fileService, logService),
			)
		} else {
		...
		services.set(
			...
			new ProtocolMainService(environmentMainService, userDataProfilesMainService, logService),
		)
	...
	private handleStartupDataDirError(
		...
		if (error.code === 'EACCES' || error.code === 'EPERM') {
			const directories = coalesce([
				...
				getPathLabel(URI.file(folder), { os: OS, tildify: environmentMainService }),
			)
