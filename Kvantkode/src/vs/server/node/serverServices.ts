...
export async function setupServerServices(
	...
	logService.onDidChangeLogLevel((logLevel) =>
		log(logService, logLevel, `Log level changed to ${LogLevelToString(logService.getLevel())}`),
	)
	...
	if (supportsTelemetry(productService, environmentService)) {
		...
		services.set(
			IServerTelemetryService,
			new SyncDescriptor(ServerTelemetryService, [config, injectedTelemetryLevel]),
		)
	} else {
	...
	services.set(
		...
		new DownloadServiceChannelClient(
			downloadChannel,
			() =>
				getUriTransformer('renderer') /* TODO: @Sandy @Joao need dynamic context based router */,
		),
	)
