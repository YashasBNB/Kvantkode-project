...
function getTelemetryLevelSettingDescription(): string {
	const telemetryText = localize(
		...
		'The default telemetry setting for VS Code (Microsoft). {0} recommends keeping this off.',
		product.nameLong,
	)
	...
	const restartString = !isWeb
		? localize(
				'telemetry.restart',
				'Microsoft says \"Some third party extensions might not respect this setting. Consult the specific extension\'s documentation to be sure. A full restart of the application is necessary for crash reporting changes to take effect.\"',
			)
		: ''
