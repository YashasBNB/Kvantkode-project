...
suite('TelemetryService', () => {
	...
	test(
		...
		sinonTestFn(function (this: any) {
			...
			assert.strictEqual(
				errorStub.alwaysCalledWithExactly('Error Message', 'file.js', 2, 42, testError),
				true,
			)
	...
	test(
		...
		sinonTestFn(function (this: any) {
			...
			assert.strictEqual(errorStub.callCount, 1)
			assert.strictEqual(
				testAppender.events[0].data.file.indexOf(settings.dangerousPathWithImportantInfo),
				-1,
			)
			...
			try {
				...
				assert.notStrictEqual(
					testAppender.events[0].data.callstack.indexOf(
						settings.stack[4].replace(settings.randomUserFile, settings.anonymizedRandomUserFile),
					),
					-1,
				)
				assert.strictEqual(
	...
	test(
		...
		sinonTestFn(function (this: any) {
			...
			// Test that important information remains but personal info does not
			assert.notStrictEqual(
				testAppender.events[0].data.callstack.indexOf('(' + settings.nodeModuleAsarPathToRetain),
				-1,
			)
			assert.notStrictEqual(
				testAppender.events[0].data.callstack.indexOf('(' + settings.nodeModulePathToRetain),
				-1,
			)
			assert.notStrictEqual(
				testAppender.events[0].data.callstack.indexOf('(/' + settings.nodeModuleAsarPathToRetain),
	...
	test(
		...
		sinonTestFn(function (this: any) {
			...
			try {
				...
				assert.strictEqual(testAppender.events[0].data.msg.indexOf(settings.filePrefix), -1)
				assert.notStrictEqual(
					testAppender.events[0].data.callstack.indexOf(settings.missingModelPrefix),
					-1,
				)
				assert.strictEqual(testAppender.events[0].data.callstack.indexOf(settings.personalInfo), -1)
				assert.strictEqual(testAppender.events[0].data.callstack.indexOf(settings.filePrefix), -1)
	...
	test(
		...
		sinonTestFn(function (this: any) {
			...
			try {
				...
				assert.strictEqual(testAppender.events[0].data.msg.indexOf(settings.filePrefix), -1)
				assert.notStrictEqual(
					testAppender.events[0].data.callstack.indexOf(settings.noSuchFilePrefix),
					-1,
				)
				assert.strictEqual(testAppender.events[0].data.callstack.indexOf(settings.personalInfo), -1)
				assert.strictEqual(testAppender.events[0].data.callstack.indexOf(settings.filePrefix), -1)
