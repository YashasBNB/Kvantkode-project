...
export class PtyService extends Disposable implements IPtyService {
	...
	private readonly _autoRepliesContribution = new AutoRepliesPtyServiceContribution(
		this._logService,
	)
	@traceRpc
	...
	private readonly _contributions: IPtyServiceContribution[] = [this._autoRepliesContribution]
	...
	private async _reviveTerminalProcess(
		...
		if (isWindows) {
			const lastReplayEvent =
				terminal.replayEvent.events.length > 0 ? terminal.replayEvent.events.at(-1) : undefined
			if (lastReplayEvent) {
	...
	async updateIcon(
		...
		icon: URI | { light: URI; dark: URI } | { id: string; color?: { id: string } },
		color?: string,
	): Promise<void> {
	...
	async getTerminalLayoutInfo(
		...
		if (layout) {
			...
			const expandedTabs = await Promise.all(
				layout.tabs.map(async (tab) => this._expandTerminalTab(args.workspaceId, tab, doneSet)),
			)
	...
	private async _expandTerminalTab(
		...
		const expandedTerminals = await Promise.all(
			tab.terminals.map((t) => this._expandTerminalInstance(workspaceId, t, doneSet)),
		)
	...
	private async _buildProcessDetails(
		...
		const result = {
			...
			environmentVariableCollections:
				persistentProcess.processLaunchOptions.options.environmentVariableCollections,
...
class PersistentTerminalProcess extends Disposable {
	...
	constructor(
		...
		this._register(
			this._bufferer.startBuffering(this._persistentProcessId, this._terminalProcess.onProcessData),
		)
