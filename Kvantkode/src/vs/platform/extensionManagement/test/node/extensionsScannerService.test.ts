...
suite('NativeExtensionsScanerService Test', () => {
	...
	test('scan all user extensions exclude invalid extensions', async () => {
		...
		await aUserExtension(
			anExtensionManifest({ name: 'name2', publisher: 'pub', engines: { vscode: '^1.67.0' } }),
		)
		const testObject = disposables.add(
			instantiationService.createInstance(ExtensionsScannerService),
		)
	...
	test('scan system extensions include additional builtin extensions', async () => {
		...
		await instantiationService
			...
			.writeFile(
				...
				VSBuffer.fromString(
					JSON.stringify({ 'pub.name2': 'disabled', 'pub.name': extensionLocation.fsPath }),
				),
			)
		const testObject: IExtensionsScannerService = disposables.add(
			instantiationService.createInstance(ExtensionsScannerService),
	...
	test('scan all user extensions with default nls replacements', async () => {
		const extensionLocation = await aUserExtension(
			anExtensionManifest({ name: 'name', publisher: 'pub', displayName: '%displayName%' }),
		)
		await instantiationService
			.get(IFileService)
			.writeFile(
	...
	test('scan extension with en nls replacements', async () => {
		...
		await instantiationService
			...
			.writeFile(
				nlsLocation,
				VSBuffer.fromString(
					JSON.stringify({ contents: { package: { displayName: 'Hello World EN' } } }),
				),
			)
		const testObject: IExtensionsScannerService = disposables.add(
			instantiationService.createInstance(ExtensionsScannerService),
...
suite('ExtensionScannerInput', () => {
	...
	test('compare inputs - location', () => {
		...
		assert.strictEqual(
			ExtensionScannerInput.equals(anInput(ROOT, undefined), anInput(ROOT, undefined)),
			true,
		)
	...
	test('compare inputs - extension type', () => {
		...
		assert.strictEqual(
			ExtensionScannerInput.equals(anInput(ExtensionType.System), anInput(ExtensionType.System)),
			true,
		)
		assert.strictEqual(
			ExtensionScannerInput.equals(anInput(ExtensionType.User), anInput(ExtensionType.User)),
			true,
		)
		assert.strictEqual(
			ExtensionScannerInput.equals(anInput(ExtensionType.User), anInput(ExtensionType.System)),
			false,
		)
	})
})
