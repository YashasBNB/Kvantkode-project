...
export abstract class AbstractExtensionGalleryService implements IExtensionGalleryService {
	...
	private async getResourceApi(
		...
		const value =
			(await this.assignmentService?.getTreatment<'unpkg' | 'marketplace' | 'none'>(
				'extensions.gallery.useResourceApi',
			)) ?? 'unpkg'
	...
	private areApiProposalsCompatible(
		...
		if (
			!this.extensionsEnabledWithApiProposalVersion.includes(extensionIdentifier.id.toLowerCase())
		) {
			return true
		}
	...
	private async isValidVersion(
		...
		if (
			!isTargetPlatformCompatible(targetPlatformForExtension, allTargetPlatforms, targetPlatform)
		) {
			return false
		}
	...
	private async queryRawGalleryExtensions(
		...
		const data = JSON.stringify({
			filters: [
				{
					criteria: query.criteria.reduce<{ filterType: number; value?: string }[]>(
						(criteria, c) => {
							const criterium =
		...
		try {
			...
			if (result) {
				...
				const resultCount =
					r.resultMetadata && r.resultMetadata.filter((m) => m.metadataType === 'ResultCount')[0]
				total =
					(resultCount &&
	...
	async download(
		...
		const headers: IHeaders | undefined = extension.queryContext?.[SEARCH_ACTIVITY_HEADER_NAME]
