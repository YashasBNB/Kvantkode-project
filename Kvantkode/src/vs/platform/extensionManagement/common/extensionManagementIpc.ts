...
function transformOutgoingExtension(
	...
	return transformer
		? cloneAndChange(extension, (value) =>
				value instanceof URI ? transformer.transformOutgoingURI(value) : undefined,
			)
		: extension
}
	...
	listen(context: any, event: string): Event<any> {
		...
		switch (event) {
			...
			case 'onDidUninstallExtension': {
				return Event.map<DidUninstallExtensionEvent, DidUninstallExtensionEvent>(
	...
	implements IExtensionManagementService
{
	...
	zip(extension: ILocalExtension): Promise<URI> {
		return Promise.resolve(
			this.channel.call<UriComponents>('zip', [extension]).then((result) => URI.revive(result)),
		)
	}
	...
	installFromLocation(location: URI, profileLocation: URI): Promise<ILocalExtension> {
		return Promise.resolve(
			this.channel.call<ILocalExtension>('installFromLocation', [location, profileLocation]),
	...
	installFromGallery(
		...
		return Promise.resolve(
			this.channel.call<ILocalExtension>('installFromGallery', [extension, installOptions]),
		).then((local) => transformIncomingExtension(local, null))
	}
	...
	async installGalleryExtensions(
		...
		const results = await this.channel.call<InstallExtensionResult[]>('installGalleryExtensions', [
			extensions,
		])
	...
	toggleAppliationScope(
		...
		return this.channel
			.call<ILocalExtension>('toggleAppliationScope', [local, fromProfileLocation])
			.then((extension) => transformIncomingExtension(extension, null))
	}
	...
	getExtensionsControlManifest(): Promise<IExtensionsControlManifest> {
		return Promise.resolve(
			this.channel.call<IExtensionsControlManifest>('getExtensionsControlManifest'),
		)
	}
