...
export class ProfileStorageChangesListenerChannel extends Disposable implements IServerChannel {
	...
	private registerStorageChangeListeners(): IDisposable {
		...
		disposables.add(
			Event.debounce(
				...
				(
					changes:
						| Map<string, { profile: IUserDataProfile; keys: string[]; storage: IStorageMain }>
						| undefined,
					e,
				) => {
					if (!changes) {
					...
					if (!profileChanges) {
						changes.set(
							e.profile.id,
							(profileChanges = { profile: e.profile, keys: [], storage: e.storage }),
						)
					}
	...
	private onDidChangeProfileStorage(
		changes: Map<string, { profile: IUserDataProfile; keys: string[]; storage: IStorageMain }>,
	): void {
