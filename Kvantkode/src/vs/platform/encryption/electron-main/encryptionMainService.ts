...
const safeStorage: typeof import('electron').safeStorage & Partial<ISafeStorageAdditionalAPIs> =
	safeStorageElectron
	...
	constructor(@ILogService private readonly logService: ILogService) {
		...
		if (isLinux && !app.commandLine.getSwitchValue('password-store')) {
			this.logService.trace(
				'[EncryptionMainService] No password-store switch, defaulting to basic...',
			)
