...
export class ViewLinesGpu extends ViewPart implements IViewLines {
	...
	async initWebgpu() {
		...
		this._glyphRasterizer.value = this._register(
			new GlyphRasterizer(fontSize, fontFamily, this._viewGpuContext.devicePixelRatio.get()),
		)
		this._register(
		...
		this._glyphStorageBuffer = this._register(
			GPULifecycle.createBuffer(this._device, {
				...
				size:
					...
					(TextureAtlasPage.maximumGlyphCount * GlyphStorageBufferInfo.BytesPerEntry),
	...
	private _refreshRenderStrategy(viewportData: ViewportData) {
		...
		this._register(
			viewportRenderStrategy.onDidChangeBindGroupEntries(() => this._rebuildBindGroup?.()),
		)
		this._rebuildBindGroup?.()
	}
	...
	getLineWidth(lineNumber: number): number | undefined {
		...
		if (
			!this._viewGpuContext.canRender(this._lastViewLineOptions, this._lastViewportData, lineNumber)
		) {
			return undefined
		}
