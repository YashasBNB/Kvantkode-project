...
export class MovedBlocksLinesFeature extends Disposable {
	...
	private readonly _modifiedViewZonesChangedSignal = observableSignalFromEvent(
		'modified.onDidChangeViewZones',
		this._editors.modified.onDidChangeViewZones,
	)
	private readonly _originalViewZonesChangedSignal = observableSignalFromEvent(
	...
	private readonly _state = derivedWithStore(this, (reader, store) => {
		...
		lines.sort(
			tieBreakComparators(
				...
				compareBy(
					(l) =>
						l.fromWithoutScroll > l.toWithoutScroll ? l.fromWithoutScroll : -l.toWithoutScroll,
					numberComparator,
				),
			),
		)
...
class MovedBlockOverlayWidget extends ViewZoneOverlayWidget {
	...
	constructor(
		...
		const editorLayout = observableFromEvent(this._editor.onDidLayoutChange, () =>
