...
export class DiffEditorWidget extends DelegatingEditor implements IDiffEditor {
	...
	private readonly _movedBlocksLinesPart = observableValue<MovedBlocksLinesFeature | undefined>(
		this,
		undefined,
	)
	...
	constructor(
		...
		const visibility = this._accessibleDiffViewerVisible.map<CSSStyle['visibility']>((v) =>
			v ? 'hidden' : 'visible',
		)
		...
		if (this._diffModel.get() !== viewModel?.object) {
			subtransaction(tx, (tx) => {
				...
				this._diffModelSrc.set(
					viewModel?.createNewRef(this) as RefCounted<DiffEditorViewModel> | undefined,
					tx,
				)
				setTimeout(() => {
