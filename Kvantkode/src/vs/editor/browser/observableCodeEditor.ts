...
export class ObservableCodeEditor extends Disposable {
	...
	public readonly isReadonly = observableFromEvent(this, this.editor.onDidChangeConfiguration, () =>
	...
	>({ owner: this, lazy: true }, this.editor.getModel()?.getVersionId() ?? null)
	...
	public readonly scrollTop = observableFromEvent(this.editor.onDidScrollChange, () =>
		this.editor.getScrollTop(),
	)
	public readonly scrollLeft = observableFromEvent(this.editor.onDidScrollChange, () =>
		this.editor.getScrollLeft(),
	)

	public readonly layoutInfo = observableFromEvent(this.editor.onDidLayoutChange, () =>
	...
	public readonly layoutInfoVerticalScrollbarWidth = this.layoutInfo.map(
		(l) => l.verticalScrollbarWidth,
	)

	public readonly contentWidth = observableFromEvent(this.editor.onDidContentSizeChange, () =>
	...
	public setDecorations(decorations: IObservable<IModelDeltaDecoration[]>): IDisposable {
		...
		d.add(
			autorunOpts(
				{ owner: this, debugName: () => `Apply decorations from ${decorations.debugName}` },
				(reader) => {
	...
	public observePosition(
		...
		const w: IContentWidget = {
			...
			getPosition: () => {
				return pos
					? { preference: [ContentWidgetPositionPreference.EXACT], position: position.get() }
					: null
			},
			getId: () => contentWidgetId,
