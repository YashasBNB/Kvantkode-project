...
suite('TokensStore', () => {
	...
	test('issue #86303 - color shifting between different tokens', () => {
		testTokensAdjustment(
			...
			[`import { |URI| } from 'vs/base/common/uri';`, `const fo = |URI|.parse('hey');`],
		)
	})
	...
	test('partial tokens 1', () => {
		...
		store.setPartial(new Range(18, 1, 42, 1), [
			SparseMultilineTokens.create(
				20,
				new Uint32Array([0, 5, 10, 4, 5, 5, 10, 5, 10, 5, 10, 6, 15, 5, 10, 7, 20, 5, 10, 8]),
			),
		])
	...
	test('partial tokens 2', () => {
		...
		store.setPartial(new Range(6, 1, 36, 2), [
			SparseMultilineTokens.create(
				10,
				new Uint32Array([0, 5, 10, 2, 5, 5, 10, 3, 10, 5, 10, 4, 15, 5, 10, 5, 20, 5, 10, 6]),
			),
		])
