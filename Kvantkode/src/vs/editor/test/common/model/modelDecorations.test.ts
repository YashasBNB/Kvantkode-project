...
suite('Editor Model - Model Decorations', () => {
	...
	test('removeAllDecorationsWithOwnerId works', () => {
		thisModel.deltaDecorations(
			[],
			[{ range: new Range(1, 2, 4, 1), options: { description: 'test', className: 'myType1' } }],
			1,
		)
...
suite('Decorations and editing', () => {
	...
	function _runTest(
		...
		const id = model.deltaDecorations(
			[],
			[{ range: decRange, options: { description: 'test', stickiness: stickiness } }],
		)[0]
		model.applyEdits([
			{
				range: editRange,
...
suite('deltaDecorations', () => {
	...
	test('deltaDecorations 1', () => {
		testDeltaDecorations(
			['This is a text', 'That has multiple lines', 'And is very friendly', 'Towards testing'],
			[
