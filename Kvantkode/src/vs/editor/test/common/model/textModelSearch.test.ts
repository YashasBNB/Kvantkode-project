...
suite('TextModelSearch', () => {
	...
	test('multiline find 1', () => {
		assertFindMatches(
			['Just some text text', 'Just some text text', 'some text again', 'again some text'].join(
				'\n',
			),
			'text\\n',
			true,
	...
	test('issue #4836 - ^.*$', () => {
		assertFindMatches(
			['Just some text text', '', 'some text again', '', 'again some text'].join('\n'),
			'^.*$',
			true,
			false,
			null,
	...
	test('\\r can never be found', () => {
		...
		assert.deepStrictEqual(
			TextModelSearch.findMatches(model, searchParams, model.getFullModelRange(), true, 1000),
			[],
		)
