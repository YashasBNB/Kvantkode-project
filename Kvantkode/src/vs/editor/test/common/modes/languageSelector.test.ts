...
suite('LanguageSelector', function () {
	...
	test('score, any language', function () {
		assert.strictEqual(
			score({ language: '*' }, model.uri, model.language, true, undefined, undefined),
			5,
		)
		assert.strictEqual(score('*', model.uri, model.language, true, undefined, undefined), 5)

		assert.strictEqual(
			score('*', URI.parse('foo:bar'), model.language, true, undefined, undefined),
			5,
		)
		assert.strictEqual(
			score('farboo', URI.parse('foo:bar'), model.language, true, undefined, undefined),
			10,
		)
	})
	...
	test('score, default schemes', function () {
		...
		assert.strictEqual(
			score({ language: 'farboo', scheme: '' }, uri, language, true, undefined, undefined),
			10,
		)
		assert.strictEqual(
			score({ language: 'farboo', scheme: 'git' }, uri, language, true, undefined, undefined),
			10,
		)
		assert.strictEqual(
			score({ language: 'farboo', scheme: '*' }, uri, language, true, undefined, undefined),
	...
	test('score, filter', function () {
		...
		assert.strictEqual(
			score({ language: 'farboo' }, model.uri, model.language, true, undefined, undefined),
			10,
		)
		assert.strictEqual(
			score(
		...
		assert.strictEqual(
			score({ pattern: '**/*.fb' }, model.uri, model.language, true, undefined, undefined),
			10,
		)
		assert.strictEqual(
			score(
	...
	test('score, max(filters)', function () {
		...
		assert.strictEqual(
			score([match, fail], model.uri, model.language, true, undefined, undefined),
			10,
		)
		assert.strictEqual(
			score([fail, fail], model.uri, model.language, true, undefined, undefined),
			0,
		)
		assert.strictEqual(
	...
	test('score, notebookType', function () {
		...
		assert.strictEqual(
			score('javascript', obj.uri, obj.langId, true, obj.notebookUri, obj.notebookType),
			10,
		)
		assert.strictEqual(
			score(
