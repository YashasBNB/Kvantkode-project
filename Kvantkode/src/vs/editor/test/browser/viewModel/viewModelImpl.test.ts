...
suite('ViewModel', () => {
	...
	test('normalizePosition with multiple touching injected text', () => {
		testViewModel(['just some text'], {}, (viewModel, model) => {
			...
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 11), PositionAffinity.Right),
				new Position(1, 13),
			)
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 12), PositionAffinity.Right),
				new Position(1, 13),
			)
			assert.deepStrictEqual(
				viewModel.normalizePosition(new Position(1, 13), PositionAffinity.Right),
				new Position(1, 13),
			)
		})
	})
