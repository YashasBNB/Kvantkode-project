...
class GenericGoToLocationAction extends SymbolNavigationAction {
	...
	protected _getGoToPreference(editor: IActiveCodeEditor): GoToLocationValues {
		return (
			this._gotoMultipleBehaviour ?? editor.getOption(EditorOption.gotoLocation).multipleReferences
		)
	}
			...
			.then((control) => {
				...
				const references = createCancelablePromise((token) =>
					getReferencesAtPosition(
						...
						(references) =>
							new ReferencesModel(references, nls.localize('ref.title', 'References')),
					),
				)
