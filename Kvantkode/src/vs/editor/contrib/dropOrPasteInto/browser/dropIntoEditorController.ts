		...
		const p = createCancelablePromise(async (token) => {
			...
			try {
				...
				if (editSession.edits.length) {
					...
					const canShowWidget =
						editor.getOption(EditorOption.dropIntoEditor).showDropSelector === 'afterDrop'
			...
			if (Array.isArray(data)) {
				for (const id of data) {
					const treeDataTransfer =
						await this._treeViewsDragAndDropService.removeDragOperationTransfer(id.identifier)
					if (treeDataTransfer) {
