...
export class SectionHeaderDetector extends Disposable implements IEditorContribution {
	...
	private createOptions(
		...
		const commentsConfiguration =
			this.languageConfigurationService.getLanguageConfiguration(languageId).comments
		const foldingRules =
			this.languageConfigurationService.getLanguageConfiguration(languageId).foldingRules
