...
export class InlineCompletionsController extends Disposable {
	...
	private readonly _positions = derived(
		this,
		(reader) =>
			this._editorObs.selections.read(reader)?.map((s) => s.getEndPosition()) ?? [
				new Position(1, 1),
			],
	)
	...
	constructor(
		...
		this._register(
			autorun((reader) => {
				...
				if (
					model.state.get()?.inlineCompletion?.request.isExplicitRequest &&
		...
		const currentInlineCompletionBySemanticId = derivedObservableWithCache<string | undefined>(
			this,
			(reader, last) => {
				const model = this.model.read(reader)
		...
		this._register(
			contextKeySvcObs.bind(
				...
				this.model.map((m, reader) => m?.inlineEditState?.read(reader)?.cursorAtInlineEdit),
			),
		)
		this._register(
			contextKeySvcObs.bind(
		...
		this._register(
			contextKeySvcObs.bind(
				...
				(reader) =>
					this.model.read(reader)?.getIndentationInfo(reader)?.startsWithIndentationLessThanTabSize,
			),
		)
		this._register(
