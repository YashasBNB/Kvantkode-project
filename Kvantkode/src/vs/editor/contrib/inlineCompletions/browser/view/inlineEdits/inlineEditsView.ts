...
export class InlineEditsView extends Disposable {
	...
	protected readonly _indicator = derivedWithStore<InlineEditsGutterIndicator | undefined>(
		this,
		(reader, store) => {
	...
	protected readonly _deletion = this._register(
		this._instantiationService.createInstance(
	...
	protected readonly _insertion = this._register(
		this._instantiationService.createInstance(
	...
	private readonly _inlineDiffViewState = derived<IOriginalEditorInlineDiffViewState | undefined>(
		this,
		(reader) => {
	...
	protected readonly _inlineCollapsedView = this._register(
		this._instantiationService.createInstance(
	...
	protected readonly _wordReplacementViews = mapObservableArrayCached(
		this,
	...
	protected readonly _lineReplacementView = this._register(
		this._instantiationService.createInstance(
	...
	private determineRenderState(
		...
		const replacements = inner.map(
			(m) => new SingleTextEdit(m.originalRange, newText.getValueOfRange(m.modifiedRange)),
		)
