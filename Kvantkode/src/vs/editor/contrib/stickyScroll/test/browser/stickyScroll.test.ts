		...
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			await withAsyncTestCodeEditor(
				...
				async (editor, _viewModel, instantiationService) => {
					...
					const provider: StickyLineCandidateProvider = new StickyLineCandidateProvider(
						editor,
						languageService,
					...
					assert.deepStrictEqual(
						provider.getCandidateStickyLinesIntersecting({ startLineNumber: 1, endLineNumber: 4 }),
						[new StickyLineCandidate(1, 2, 0, 19)],
					)
					assert.deepStrictEqual(
						provider.getCandidateStickyLinesIntersecting({ startLineNumber: 8, endLineNumber: 10 }),
						[
