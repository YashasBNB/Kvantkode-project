...
export class StickyScrollWidget extends Disposable implements IOverlayWidget {
	...
	constructor(editor: ICodeEditor) {
		...
		const updateScrollLeftPosition = () => {
			this._linesDomNode.style.left = this._editor.getOption(EditorOption.stickyScroll)
				.scrollWithEditor
	...
	private _isWidgetHeightZero(state: StickyScrollWidgetState | undefined): boolean {
		...
		const futureWidgetHeight =
			state.startLineNumbers.length * this._lineHeight + state.lastLineRelativePosition
	...
	private async _renderRootNode(
		...
		const widgetHeight =
			this._lineNumbers.length * this._lineHeight + this._lastLineRelativePosition
	...
	private _renderChildNode(
		...
		if (
			...
			(lineNumberOption.renderType === RenderLineNumbersType.Interval && line % 10 === 0)
		) {
