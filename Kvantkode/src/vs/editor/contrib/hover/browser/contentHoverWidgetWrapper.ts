...
export class ContentHoverWidgetWrapper extends Disposable implements IHoverWidget {
	...
	private _findHoverAnchorCandidates(mouseEvent: IEditorMouseEvent): HoverAnchor[] {
		...
		switch (target.type) {
			...
			case MouseTargetType.CONTENT_EMPTY: {
				const epsilon =
					this._editor.getOption(EditorOption.fontInfo).typicalHalfwidthCharacterWidth / 2
	...
	public doesHoverAtIndexSupportVerbosityAction(
		...
		return (
			this._renderedContentHover.value?.doesHoverAtIndexSupportVerbosityAction(index, action) ??
			false
		)
	}
