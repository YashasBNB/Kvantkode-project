...
class EditorStringEnumOption<K extends EditorOption, V extends string> extends SimpleEditorOption<
	K,
	V
> {
...
class EditorEnumOption<K extends EditorOption, T extends string, V> extends BaseEditorOption<
	K,
	T,
	V
> {
...
> {
	...
	public validate(_input: any): EditorFindOptions {
		...
		return {
			...
			history: stringSet<'never' | 'workspace'>(input.history, this.defaultValue.history, [
				'never',
				'workspace',
			]),
...
> {
	...
	public validate(_input: any): GoToLocationOptions {
		...
		return {
			...
			multipleTests:
				input.multipleTests ??
				stringSet<GoToLocationValues>(input.multipleTests, 'peek', ['peek', 'gotoAndPeek', 'goto']),
...
> {
	...
	public static computeLayout(
		...
		const showLineNumbers =
			options.get(EditorOption.lineNumbers).renderType !== RenderLineNumbersType.Off
...
export const EditorOptions = {
	...
	ariaRequired: register(
		new EditorBooleanOption(EditorOption.ariaRequired, 'ariaRequired', false, undefined),
	),
	...
	automaticLayout: register(
		new EditorBooleanOption(EditorOption.automaticLayout, 'automaticLayout', false),
	),
	...
	extraEditorClassName: register(
		new EditorStringOption(EditorOption.extraEditorClassName, 'extraEditorClassName', ''),
	),
	...
	overviewRulerLanes: register(
		new EditorIntOption(EditorOption.overviewRulerLanes, 'overviewRulerLanes', 3, 0, 3),
	),
	...
	selectOnLineNumbers: register(
		new EditorBooleanOption(EditorOption.selectOnLineNumbers, 'selectOnLineNumbers', true),
	),
	...
	tabIndex: register(
		new EditorIntOption(EditorOption.tabIndex, 'tabIndex', 0, -1, Constants.MAX_SAFE_SMALL_INTEGER),
	),
