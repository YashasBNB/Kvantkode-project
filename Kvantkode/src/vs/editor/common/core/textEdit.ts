...
export class TextEdit {
	public static fromOffsetEdit(edit: OffsetEdit, initialState: AbstractText): TextEdit {
		const edits = edit.edits.map(
			(e) => new SingleTextEdit(initialState.getTransformer().getRange(e.replaceRange), e.newText),
		)
	...
	inverse(doc: AbstractText): TextEdit {
		...
		return new TextEdit(
			this.edits.map((e, idx) => new SingleTextEdit(ranges[idx], doc.getValueOfRange(e.range))),
		)
	}
