...
export class DefaultLinesDiffComputer implements ILinesDiffComputer {
	...
	private refineDiff(
		...
		diffs = extendDiffsToEntireWordIfAppropriate(slice1, slice2, diffs, (seq, idx) =>
			seq.findWordContaining(idx),
		)
		if (check) {
			SequenceDiff.assertSorted(diffs)
		}
		...
		const result = diffs.map(
			(d) =>
				new RangeMapping(slice1.translateRange(d.seq1Range), slice2.translateRange(d.seq2Range)),
		)
