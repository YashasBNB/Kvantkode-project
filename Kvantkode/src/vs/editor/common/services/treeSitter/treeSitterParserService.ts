...
export class TreeSitterTextModelService extends Disposable implements ITreeSitterParserService {
	...
	private _textModelTreeSitters: DisposableMap<ITextModel, TextModelTreeSitterItem> =
	...
	private _getSetting(languageId: string): boolean {
		const setting = this._configurationService.getValue<boolean>(
	...
	public async getTextModelTreeSitter(
		model: ITextModel,
		parseImmediately: boolean = false,
	): Promise<ITextModelTreeSitter> {
	...
	private _createTextModelTreeSitter(
		...
		disposables.add(
			textModelTreeSitter.onDidChangeParseResult((e) =>
				this._handleOnDidChangeParseResult(e, model),
			),
		)
