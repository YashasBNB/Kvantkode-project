...
suite('Workbench editor utils', () => {
	...
	test('untyped check functions', () => {
		...
		assert.ok(
			isUntitledResourceEditorInput({ resource: URI.file('/').with({ scheme: Schemas.untitled }) }),
		)
	...
	test('EditorResourceAccessor - untyped inputs', () => {
		...
		for (const untypedInput of [diffInput, sideBySideInput]) {
			...
			assert.ok(
				!EditorResourceAccessor.getCanonicalUri(untypedInput, { filterByScheme: Schemas.file }),
			)
			...
			assert.ok(
				!EditorResourceAccessor.getOriginalUri(untypedInput, { filterByScheme: Schemas.file }),
			)
