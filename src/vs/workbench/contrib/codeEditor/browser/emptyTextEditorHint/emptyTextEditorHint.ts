...
export class EmptyTextEditorHintContribution implements IEditorContribution {
	...
	protected _shouldRenderHint() {
		...
		const shouldRenderDefaultHint =
			model?.uri.scheme === Schemas.untitled && languageId === PLAINTEXT_LANGUAGE_ID
...
class EmptyTextEditorHintContentWidget implements IContentWidget {
	...
	private _getHintInlineChat(providers: IChatAgent[]) {
		const providerName =
			(providers.length === 1 ? providers[0].fullName : undefined) ?? this.productService.nameShort
		...
		if (keybindingHint && keybindingHintLabel) {
			...
			const [before, after] = actionPart.split(keybindingHintLabel).map((fragment) => {
				if (this.options.clickable) {
					...
					this.toDispose.add(
						dom.addDisposableListener(hintPart, dom.EventType.CONTEXT_MENU, (e) =>
							this._disableHint(e),
						),
					)
			...
			if (this.options.clickable) {
				...
				this.toDispose.add(
					dom.addDisposableListener(label.element, dom.EventType.CONTEXT_MENU, (e) =>
						this._disableHint(e),
					),
				)
				this.toDispose.add(
	...
	private _getHintDefault() {
		...
		for (const anchor of hintElement.querySelectorAll('a')) {
			...
			hintHandler.disposables.add(
				this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), anchor, title ?? ''),
			)
		}
