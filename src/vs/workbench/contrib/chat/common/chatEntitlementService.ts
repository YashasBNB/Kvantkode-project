...
export const IChatEntitlementService =
	createDecorator<IChatEntitlementService>('chatEntitlementService')
...
export class ChatEntitlementService extends Disposable implements IChatEntitlementService {
	...
	constructor(
		...
		this.chatQuotaExceededContextKey = ChatContextKeys.chatQuotaExceeded.bindTo(
		...
		this.onDidChangeSentiment = Event.map(
			Event.filter(
				...
				(e) =>
					e.affectsSome(
						new Set([ChatContextKeys.Setup.hidden.key, ChatContextKeys.Setup.installed.key]),
					),
				this._store,
			),
			() => {},
	...
	get entitlement(): ChatEntitlement {
		if (
			this.contextKeyService.getContextKeyValue<boolean>(ChatContextKeys.Entitlement.pro.key) ===
			true
		) {
		...
		this._register(
			this.contextKeyService.onDidChangeContext((e) => {
				...
				if (e.affectsSome(chatQuotaExceededSet)) {
					const newChatQuotaExceeded = this.contextKeyService.getContextKeyValue<boolean>(
				...
				if (e.affectsSome(completionsQuotaExceededSet)) {
					const newCompletionsQuotaExceeded = this.contextKeyService.getContextKeyValue<boolean>(
						this.ExtensionQuotaContextKeys.completionsQuotaExceeded,
					)
					if (
	...
	get sentiment(): ChatSentiment {
		if (
			this.contextKeyService.getContextKeyValue<boolean>(ChatContextKeys.Setup.installed.key) ===
			true
		) {
			return ChatSentiment.Installed
		} else if (
			this.contextKeyService.getContextKeyValue<boolean>(ChatContextKeys.Setup.hidden.key) === true
		) {
...
export class ChatEntitlementRequests extends Disposable {
		...
		const entitlements: IEntitlements = {
			...
			quotas: {
				...
				completionsRemaining:
					typeof completionsRemaining === 'number' ? Math.max(0, completionsRemaining) : undefined,
	...
	private update(state: IEntitlements): void {
		...
		if (state.quotas) {
			this.chatQuotasAccessor.acceptQuotas({
				chatQuotaExceeded:
					typeof state.quotas.chatRemaining === 'number' ? state.quotas.chatRemaining <= 0 : false,
				completionsQuotaExceeded:
					typeof state.quotas.completionsRemaining === 'number'
	...
	async signUpLimited(
		...
		true /* signed up */ | false /* already signed up */ | { errorCode: number } /* error */
	> {
		const body = {
			restricted_telemetry:
				this.telemetryService.telemetryLevel === TelemetryLevel.NONE ? 'disabled' : 'enabled',
			...
			Event.runAndSubscribe<IExtension | undefined>(
				...
				(e) => {
					...
					const defaultChatExtension = this.extensionsWorkbenchService.local.find((value) =>
						ExtensionIdentifier.equals(value.identifier.id, defaultChat.extensionId),
					)
					this.update({
						installed:
