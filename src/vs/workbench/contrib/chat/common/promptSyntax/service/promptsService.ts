...
export class PromptsService extends Disposable implements IPromptsService {
	 ...
	 */
	public getSyntaxParserFor(model: ITextModel): TextModelPromptParser & { disposed: false } {
	...
	public getSourceFolders(type: IPromptPath['type']): readonly IPromptPath[] {
 ...
 */
const addType = (type: 'local' | 'user'): ((uri: URI) => IPromptPath) => {
	return (uri) => {
 ...
 */
const withType = (type: 'local' | 'user'): ((uris: readonly URI[]) => readonly IPromptPath[]) => {
	return (uris) => {
