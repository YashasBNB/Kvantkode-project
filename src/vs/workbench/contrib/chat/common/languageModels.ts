...
export const ILanguageModelsService =
	createDecorator<ILanguageModelsService>('ILanguageModelsService')
...
export class LanguageModelsService implements ILanguageModelsService {
	...
	async selectLanguageModels(selector: ILanguageModelChatSelector): Promise<string[]> {
		if (selector.vendor) {
			...
			const all = Array.from(this._vendors).map((vendor) =>
				this._extensionService.activateByEvent(`onLanguageModelChat:${vendor}`),
			)
		...
		for (const [identifier, model] of this._providers) {
			if (
				(selector.vendor === undefined || model.metadata.vendor === selector.vendor) &&
				...
				(!model.metadata.targetExtensions ||
					model.metadata.targetExtensions.some((candidate) =>
						ExtensionIdentifier.equals(candidate, selector.extension),
					))
			) {
	...
	private updateUserSelectableModelsContext() {
		...
		const hasUserSelectableModels = Array.from(this._providers.values()).some(
			(p) => p.metadata.isUserSelectable && !p.metadata.isDefault,
		)
