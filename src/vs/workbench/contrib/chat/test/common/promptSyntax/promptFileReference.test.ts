...
class TestPromptFileReference extends Disposable {
	...
	public async run() {
		...
		const rootReference = this._register(
			this.initService.createInstance(FilePromptParser, this.rootFileUri, []),
		).start()
		...
		const resolvedReferences: readonly (IPromptReference | undefined)[] =
		...
		for (let i = 0; i < this.expectedReferences.length; i++) {
			...
			assert(
				resolvedReference && resolvedReference.uri.toString() === expectedReference.uri.toString(),
				[
			...
			instantiationService.createInstance(
				...
				[
					...
					new ExpectedReference(
						...
						createTestFileReference(`./some-other-folder/non-existing-folder`, 2, 1),
						new OpenFailed(
					...
					new ExpectedReference(
						...
						new FolderReference(URI.joinPath(rootUri, './folder1'), 'Uggh ohh!'),
					),
				],
			),
		)
