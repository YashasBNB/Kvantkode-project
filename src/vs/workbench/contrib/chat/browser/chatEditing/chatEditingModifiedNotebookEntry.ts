...
export class ChatEditingModifiedNotebookEntry extends AbstractChatEditingModifiedFileEntry {
	...
	public static canHandleSnapshot(snapshot: ISnapshotEntry): boolean {
		if (
			...
			ChatEditingModifiedNotebookEntry.canHandleSnapshotContent(snapshot.current)
		) {
			return true
		}
		return false
	...
	protected override _createUndoRedoElement(
		...
		const outputSizeLimit =
			this.configurationService.getValue<number>(NotebookSetting.outputBackupSizeLimit) * 1024
	...
	acceptNotebookEdit(edit: ICellEditOperation): void {
		...
		edit.cells.forEach((_, i) => {
			...
			const internalId = generateCellHash(cell.uri)
			const edits: ICellEditOperation[] = [
				{ editType: CellEditType.PartialInternalMetadata, index, internalMetadata: { internalId } },
			]
