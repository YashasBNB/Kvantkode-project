...
export class ChatEditingSession extends Disposable implements IChatEditingSession {
	...
	private readonly _linearHistory = observableValue<readonly IChatEditingSessionSnapshot[]>(
		this,
		[],
	)
	...
	private readonly _ignoreTrimWhitespaceObservable = observableConfigValue(
	...
	private _createDiffBetweenStopsObservable(
		...
		const entries = derivedOpts<undefined | { before: ISnapshotEntry; after: ISnapshotEntry }>(
			{
				equalsFn: (a, b) =>
					snapshotsEqualForDiff(a?.before, b?.before) && snapshotsEqualForDiff(a?.after, b?.after),
			},
			(reader) => {
	...
	private async _restoreSnapshot(
		{ workingSet, entries }: IChatEditingSessionStop,
		...
		for (const snapshotEntry of entries.values()) {
			...
			const restoreToDisk =
				snapshotEntry.state === WorkingSetEntryState.Modified || restoreResolvedToDisk
		...
		if (next) {
			if (stopIndex === snap.stops.length - 1) {
				const postEdit = new ResourceMap(
					snap.postEdit || this._createSnapshot(undefined, undefined).entries,
				)
	...
	public async storeState(state: StoredSessionState): Promise<void> {
		...
		try {
			const data: IChatEditingSessionDTO = {
				...
				initialFileContents: serializeResourceMap(state.initialFileContents, (value) =>
