...
export function createSnapshot(
	...
	const outputSizeLimit =
		(typeof outputSizeConfig === 'number'
			? outputSizeConfig
			: outputSizeConfig.getValue<number>(NotebookSetting.outputBackupSizeLimit)) * 1024
	return serializeSnapshot(
		notebook.createSnapshot({ context: SnapshotContext.Backup, outputSizeLimit, transientOptions }),
		transientOptions,
	)
}
	...
	isEqual(notebook: NotebookData | NotebookTextModel): boolean {
		...
		for (let i = 0; i < notebook.cells.length; i++) {
			...
			if (notebookCell instanceof NotebookCellTextModel) {
				...
				// When comparing ignore transient items.
				if (
					JSON.stringify(sanitizeCellDto2(notebookCell, true, this.transientOptions)) !==
