	...
	implements IModifiedFileEntryEditorIntegration
{
	...
	private readonly _currentChange = observableValue<
		{ change: ICellDiffInfo; index: number } | undefined
	>(this, undefined)
	readonly currentChange: IObservable<{ change: ICellDiffInfo; index: number } | undefined> =
		this._currentChange
	...
	private _revealFirstOrLast(change: ICellDiffInfo, firstOrLast: boolean = true) {
		switch (change.type) {
			...
			case 'modified': {
				const index =
					firstOrLast || change.type === 'insert' ? 0 : change.diff.get().changes.length - 1
	...
	private async revealChangeInView(
		...
		await this.notebookEditor.revealRangeInCenterAsync(
			cell,
			new Range(targetLines.startLineNumber, 0, targetLines.endLineNumberExclusive, 0),
		)
	}
					...
					if (cellIntegration) {
						if (cellIntegration.next(false)) {
							this._currentChange.set(
								{ change: currentChange.change, index: cellIntegration.currentIndex.get() },
								undefined,
							)
							return true
						}
