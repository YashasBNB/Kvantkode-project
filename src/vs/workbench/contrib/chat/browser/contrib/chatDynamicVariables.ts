...
export async function createFolderQuickPick(accessor: ServicesAccessor): Promise<URI | undefined> {
	...
	const topLevelFolderItems = (await getTopLevelFolders(workspaces, fileService)).map(
		createQuickPickItem,
	)
...
export async function createMarkersQuickPick(
	accessor: ServicesAccessor,
	level: 'problem' | 'file',
	onBackgroundAccept?: (item: IDiagnosticVariableEntryFilterData[]) => void,
): Promise<IDiagnosticVariableEntryFilterData | undefined> {
	...
	const grouped = groupBy(markers, (a, b) =>
		uriIdentityService.extUri.compare(a.resource, b.resource),
	)

	const severities = new Set<MarkerSeverity>()
	type MarkerPickItem = IQuickPickItem & {
		resource?: URI
		entry: IDiagnosticVariableEntryFilterData
	}
	...
	const quickPick = store.add(
		quickInputService.createQuickPick<MarkerPickItem>({ useSeparators: true }),
	)
