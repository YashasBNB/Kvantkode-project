...
class InputEditorDecorations extends Disposable {
	...
	constructor(
		...
		this._register(
			this.themeService.onDidColorThemeChange(() => this.updateRegisteredDecorationTypes()),
		)
		this.updateRegisteredDecorationTypes()

		this.updateInputEditorDecorations()
		this._register(
			this.widget.inputEditor.onDidChangeModelContent(() => this.updateInputEditorDecorations()),
		)
		...
		this._register(
			this.chatAgentService.onDidChangeAgents(() => this.updateInputEditorDecorations()),
		)
	...
	private async updateInputEditorDecorations() {
		...
		if (onlyAgentAndAgentCommandAndWhitespace) {
			...
			const shouldRenderFollowupPlaceholder =
				isFollowupSlashCommand && agentSubcommandPart.command.followupPlaceholder
			if (
			...
			this.widget.inputEditor.onDidChangeModelContent((e) => {
				...
				if (!change.text && this.widget.viewModel) {
					const previousParsedValue = parser.parseChatRequest(
						...
						previousInputValue,
						widget.location,
						{ selectedAgent: previousSelectedAgent, mode: this.widget.input.currentMode },
					)
