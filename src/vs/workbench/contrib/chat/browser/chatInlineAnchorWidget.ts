...
export class InlineAnchorWidget extends Disposable {
	...
	constructor(
		...
		if (this.data.kind === 'symbol') {
			...
			const recomputeIconClasses = () =>
				getIconClasses(
					...
					location.uri,
					fileKind,
					fileKind === FileKind.FOLDER && !themeService.getFileIconTheme().hasFolderIcons
						? FolderThemeIcon
						: undefined,
				)
			...
			// Context menu
			this._register(
				dom.addDisposableListener(element, dom.EventType.CONTEXT_MENU, async (domEvent) => {
		...
		const resourceContextKey = this._register(
			new ResourceContextKey(contextKeyService, fileService, languageService, modelService),
		)
...
registerAction2(
	class CopyResourceAction extends Action2 {
		...
		override async run(accessor: ServicesAccessor): Promise<void> {
			...
			const resource =
				anchor.data.kind === 'symbol' ? anchor.data.symbol.location.uri : anchor.data.uri
	...
	class OpenToSideResourceAction extends Action2 {
		...
		constructor() {
			super({
				...
				menu: [MenuId.ChatInlineSymbolAnchorContext, MenuId.ChatInputSymbolAttachmentContext].map(
					(id) => ({
						id: id,
						group: 'navigation',
						order: 1,
		...
		constructor() {
			super({
				...
				title: {
					...
					mnemonicTitle: nls.localize(
						{ key: 'miGotoImplementations', comment: ['&& denotes a mnemonic'] },
