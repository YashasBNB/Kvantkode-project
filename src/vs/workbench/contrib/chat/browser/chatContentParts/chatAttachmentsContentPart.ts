...
export class ChatAttachmentsContentPart extends Disposable {
	...
	private readonly _contextResourceLabels = this._register(
		...
		this.variables.forEach(async (attachment) => {
			...
			if (resource && (attachment.isFile || attachment.isDirectory)) {
				...
				this.instantiationService.invokeFunction((accessor) => {
					if (resource) {
						this.attachedContextDisposables.add(
							hookUpResourceAttachmentDragAndContextMenu(accessor, widget, resource),
						)
					}
				})
				...
				if (!isAttachmentPartialOrOmitted) {
					...
					this.attachedContextDisposables.add(
						this.hoverService.setupManagedHover(hoverDelegate, widget, hoverElement, {
							trapFocus: false,
						}),
					)
				}
				...
				if (!this.attachedContextDisposables.isDisposed) {
					this.attachedContextDisposables.add(
						this.hoverService.setupManagedHover(hoverDelegate, widget, hoverContent, {
							trapFocus: true,
						}),
					)
			...
			if (resource) {
				...
				if (!this.attachedContextDisposables.isDisposed) {
					this.attachedContextDisposables.add(
						dom.addDisposableListener(widget, dom.EventType.CLICK, async (e: MouseEvent) => {
	...
	store.add(
		dom.addDisposableListener(widget, 'dragstart', (e) => {
			instantiationService.invokeFunction((accessor) =>
				fillEditorsDragData(
					accessor,
					[{ resource: attachment.value.uri, selection: attachment.value.range }],
					e,
				),
			)
