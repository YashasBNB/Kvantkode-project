...
type IAttachmentQuickPickItem =
	...
	| IReusablePromptQuickPickItem
	| IFolderQuickPickItem
	| IDiagnosticsQuickPickItem
...
function isIFolderSearchResultQuickPickItem(obj: unknown): obj is IFolderResultQuickPickItem {
	return (
		typeof obj === 'object' && (obj as IFolderResultQuickPickItem).kind === 'folder-search-result'
	)
}

function isIDiagnosticsQuickPickItemWithFilter(
	obj: unknown,
): obj is IDiagnosticsQuickPickItemWithFilter {
	return (
		typeof obj === 'object' &&
...
function isIOpenEditorsQuickPickItem(obj: unknown): obj is IOpenEditorsQuickPickItem {
	return typeof obj === 'object' && (obj as IOpenEditorsQuickPickItem).id === 'open-editors'
}

function isISearchResultsQuickPickItem(obj: unknown): obj is ISearchResultsQuickPickItem {
	return typeof obj === 'object' && (obj as ISearchResultsQuickPickItem).kind === 'search-results'
}

function isScreenshotQuickPickItem(obj: unknown): obj is IScreenShotQuickPickItem {
	return typeof obj === 'object' && (obj as IScreenShotQuickPickItem).kind === 'screenshot'
}

function isRelatedFileQuickPickItem(obj: unknown): obj is IRelatedFilesQuickPickItem {
	return typeof obj === 'object' && (obj as IRelatedFilesQuickPickItem).kind === 'related-files'
}
...
export class AttachSearchResultAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...args: any[]) {
		...
		const originalRange =
			editor.getSelection() ?? editor.getModel()?.getFullModelRange().collapseToEnd()
...
export class AttachContextAction extends Action2 {
		...
		for (const pick of picks) {
			if (isISymbolQuickPickItem(pick) && pick.symbol) {
				...
				const itemsPromise = chatEditingService
					...
					.then((files) =>
						(files ?? []).reduce<(WithUriValue<IQuickPickItem> | IQuickPickSeparator)[]>(
							(acc, cur) => {
	...
	override async run(accessor: ServicesAccessor, ...args: any[]): Promise<void> {
		...
		const context:
			| { widget?: IChatWidget; showFilesOnly?: boolean; placeholder?: string }
			| undefined = args[0]
		...
		const chatEditingService =
			widget.location === ChatAgentLocation.EditingSession || widget.isUnifiedPanelWidget
		...
		if (widget.viewModel?.sessionId) {
			const agentPart = widget.parsedInput.parts.find(
				(part): part is ChatRequestAgentPart => part instanceof ChatRequestAgentPart,
			)
			if (agentPart) {
	...
	private async _showDiagnosticsPick(
		instantiationService: IInstantiationService,
		onBackgroundAccept: (item: IChatContextQuickPickItem[]) => void,
	): Promise<IDiagnosticsQuickPickItemWithFilter | undefined> {
		const convert = (
			item: IDiagnosticVariableEntryFilterData,
		): IDiagnosticsQuickPickItemWithFilter => ({
			kind: 'diagnostic-filter',
		...
		const filter = await instantiationService.invokeFunction((accessor) =>
			createMarkersQuickPick(accessor, 'problem', (items) =>
				onBackgroundAccept(items.map(convert)),
			),
		)
		...
		const providerOptions: AnythingQuickAccessProviderRunOptions = {
			...
			filter: (item: IChatContextQuickPickItem | IQuickPickSeparator) => {
				...
				if (item && typeof item === 'object' && 'resource' in item && URI.isUri(item.resource)) {
					return (
						[Schemas.file, Schemas.vscodeRemote, Schemas.untitled].includes(item.resource.scheme) &&
				...
				if (item && typeof item === 'object' && 'uri' in item && item.uri && item.range) {
					return !attachedContext.has(
						this._getFileContextId({ uri: item.uri, range: item.range.decoration }),
					)
				}
	...
	private async _showFolders(
		instantiationService: IInstantiationService,
	): Promise<IFolderResultQuickPickItem | undefined> {
		const folder = await instantiationService.invokeFunction((accessor) =>
			createFolderQuickPick(accessor),
		)
		if (!folder) {
			return undefined
		}
