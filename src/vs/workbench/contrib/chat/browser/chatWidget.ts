...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	get isUnifiedPanelWidget(): boolean {
		return (
			this._location.location === ChatAgentLocation.Panel &&
							...
							if (input.options?.selection) {
								...
								editor.setSelection({
									...
									endLineNumber:
										...
										input.options.selection.startLineNumber,
									endColumn:
										input.options.selection.endColumn ?? input.options.selection.startColumn,
								})
							}
	...
	private renderWelcomeViewContentIfNeeded() {
		...
		const welcomeContent =
			defaultAgent?.metadata.welcomeMessageContent ?? this.persistedWelcomeMessage
		if (
			welcomeContent &&
			!numItems &&
	...
	private renderSampleQuestions() {
		if (this.viewModel?.getItems().length === 0) {
			...
			this.renderFollowups(
				this.input.currentMode === ChatMode.Ask ? this.viewModel.model.sampleQuestions : undefined,
			)
		}
	}
			...
			scopedInstantiationService.createInstance(
				...
				[this.renderer],
				{
					...
					accessibilityProvider:
						this.instantiationService.createInstance(ChatAccessibilityProvider),
		...
		this._register(
			this.tree.onDidScroll(() => {
				...
				const isScrolledDown =
					this.tree.scrollTop >= this.tree.scrollHeight - this.tree.renderHeight - 2
		...
		if (this.tree.scrollHeight !== this.previousTreeScrollHeight) {
			...
			if (!lastResponseIsRendering || this.scrollLock) {
				...
				const lastElementWasVisible =
					this.tree.scrollTop + this.tree.renderHeight >= this.previousTreeScrollHeight - 2
	...
	layout(height: number, width: number): void {
		...
		const lastElementVisible =
			this.tree.scrollTop + this.tree.renderHeight >= this.tree.scrollHeight - 2
