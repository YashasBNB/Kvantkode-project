...
class QuickDiffDecorator extends Disposable {
	...
	constructor(
		...
		this._register(
			Event.runAndSubscribe(this.quickDiffModelRef.object.onDidChange, () => this.onDidChange()),
		)
	}
	...
	private onDidChange(): void {
		...
		const decorations = this.quickDiffModelRef.object.changes
			.filter((labeledChange) =>
				visibleQuickDiffs.some((quickDiff) => quickDiff.label === labeledChange.label),
			)
			.map((labeledChange) => {
...
export class QuickDiffWorkbenchController extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this._register(
			onDidChangeDiffVisibilityConfiguration(this.onDidChangeDiffVisibilityConfiguration, this),
		)
