	...
	class extends Action2 {
		...
		override async run(
			...
			const historyItemParentId =
				historyItemLast.parentIds.length > 0 ? historyItemLast.parentIds[0] : undefined
			...
			const multiDiffSourceUri = URI.from(
				{ scheme: 'scm-history-item', path: `${path}/${historyItemParentId}..${historyItem.id}` },
				true,
			)
...
class HistoryItemRenderer
	implements ITreeRenderer<SCMHistoryItemViewModelTreeElement, LabelFuzzyScore, HistoryItemTemplate>
{
...
class HistoryItemLoadMoreRenderer
	implements ITreeRenderer<SCMHistoryItemLoadMoreTreeElement, void, LoadMoreTemplate>
{
	...
	renderElement(
		...
		const pageOnScroll =
			this._configurationService.getValue<boolean>('scm.graph.pageOnScroll') === true
...
class SCMHistoryViewModel extends Disposable {
	...
	getHistoryItemsFilter(): 'all' | 'auto' | ISCMHistoryItemRef[] | undefined {
		...
		const filterState =
			this._repositoryFilterState.get(getProviderKey(repository.provider)) ?? 'auto'
...
export class SCMHistoryViewPane extends ViewPane {
	...
	protected override renderHeaderTitle(container: HTMLElement): void {
		...
		this._register(
			this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), element.root, {
				markdown: {
					value: localize(
	...
	private async _onDidOpen(e: IOpenEvent<TreeElement | undefined>): Promise<void> {
		if (!e.element) {
			...
			const pageOnScroll =
				this.configurationService.getValue<boolean>('scm.graph.pageOnScroll') === true
			if (!pageOnScroll) {
				this._loadMore()
