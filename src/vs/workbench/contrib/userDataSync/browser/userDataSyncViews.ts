					...
					constructor() {
						super({
							...
							menu: {
								...
								when: ContextKeyExpr.and(
									ContextKeyExpr.equals('view', viewId),
									ContextKeyExpr.regex('viewItem', /sync-associatedResource-.*/i),
								),
							},
						})
					}
				...
				class extends Action2 {
						...
						super({
							...
							menu: {
								...
								when: ContextKeyExpr.and(
									ContextKeyExpr.equals('view', viewId),
									ContextKeyExpr.regex('viewItem', /sync-resource-.*/i),
									ContextKeyExpr.notEquals('viewItem', `sync-resource-${SyncResource.Profiles}`),
								),
								group: 'inline',
							},
						})
					...
					async run(accessor: ServicesAccessor, handle: TreeViewItemHandleArg): Promise<void> {
						...
						const { syncResourceHandle, syncResource } = <
							{ syncResourceHandle: UriDto<ISyncResourceHandle>; syncResource: SyncResource }
						>JSON.parse(handle.$treeItemHandle)
...
abstract class UserDataSyncActivityViewDataProvider<T = Profile> implements ITreeViewDataProvider {
	...
	protected async getChildrenForSyncResourceTreeItem(
		...
		const associatedResources =
			await this.userDataSyncResourceProviderService.getAssociatedResources(syncResourceHandle)
