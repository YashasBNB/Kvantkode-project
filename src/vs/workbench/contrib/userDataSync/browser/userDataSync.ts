	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		if (userDataSyncWorkbenchService.enabled) {
			...
			this._register(
				userDataSyncService.onSyncErrors((errors) => this.onSynchronizerErrors(errors)),
			)
	...
	private async turnOn(): Promise<void> {
		try {
			if (!this.userDataSyncWorkbenchService.authenticationProviders.length) {
				throw new Error(
					localize('no authentication providers', 'No authentication providers are available.'),
				)
			}
						...
						super({
							...
							menu: {
								...
								when: ContextKeyExpr.and(
									CONTEXT_TURNING_ON_STATE,
									ContextKeyExpr.equals('viewContainer', SYNC_VIEW_CONTAINER_ID),
								),
								group: 'navigation',
								order: 1,
							},
			...
			registerAction2(
				class AcceptMergesAction extends Action2 {
					...
					private getSyncResource(previewResource: URI): IUserDataSyncResource {
						const conflict = that.userDataSyncService.conflicts.find(({ conflicts }) =>
							conflicts.some((conflict) => isEqual(conflict.previewResource, previewResource)),
						)
						if (conflict) {
							return conflict
						}
