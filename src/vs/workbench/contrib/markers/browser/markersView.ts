...
export class MarkersView extends FilterViewPane implements IMarkersView {
	...
	constructor(
		...
		this._register(
			this.onDidChangeVisibility((visible) => this.onDidChangeMarkersViewVisibility(visible)),
		)
	...
	private createWidget(parent: HTMLElement): void {
		...
		const relatedInformationFocusContextKey =
			MarkersContextKeys.RelatedInformationFocusContextKey.bindTo(this.widget.contextKeyService)
	...
	private reInitialize(): IDisposable[] {
		...
		disposables.push(
			this.markersViewModel.onDidChange((marker) => this.onDidChangeViewState(marker)),
		)
		disposables.push(
			toDisposable(() =>
				this.markersModel.resourceMarkers.forEach((resourceMarker) =>
	...
	private renderMessage(): void {
		...
		if (filtered === 0) {
			...
			if (filtered === total) {
				...
				this.setAriaLabel(
					localize('problems filtered', 'Showing {0} of {1} problems', filtered, total),
				)
			}
