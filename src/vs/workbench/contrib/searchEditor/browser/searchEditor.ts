...
export class SearchEditor extends AbstractTextCodeEditor<SearchEditorViewState> {
	...
	private createQueryEditor(
		...
		;[
			...
			this._register(
				tracker.onDidFocus(() => setTimeout(() => inputBoxFocusedContextKey.set(true), 0)),
			)
			...
			this.searchResultEditor.onMouseUp((e) => {
				if (e.event.detail === 1) {
					...
					if (position && behaviour === 'peekDefinition') {
						const line =
							this.searchResultEditor.getModel()?.getLineContent(position.lineNumber) ?? ''
	...
	private iterateThroughMatches(reverse: boolean) {
		...
		const currentPosition =
			this.searchResultEditor.getSelection()?.getStartPosition() ?? fallbackStart
