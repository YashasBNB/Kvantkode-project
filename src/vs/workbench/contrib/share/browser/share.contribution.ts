...
class ShareWorkbenchContribution extends Disposable {
	...
	constructor(
		...
		if (
			this.configurationService.getValue<boolean>(ShareWorkbenchContribution.SHARE_ENABLED_SETTING)
		) {
			...
			registerAction2(
				class ShareAction extends Action2 {
					...
					override async run(accessor: ServicesAccessor, ...args: any[]): Promise<void> {
						...
						const selection =
							accessor.get(ICodeEditorService).getActiveCodeEditor()?.getSelection() ?? undefined
						...
						const result = await progressService.withProgress(
							...
							async () =>
								shareService.provideShare({ resourceUri, selection }, CancellationToken.None),
						)
