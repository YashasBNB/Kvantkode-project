...
export function threadHasMeaningfulComments(thread: CommentThread): boolean {
	return (
		...
		thread.comments.some((comment) =>
			isMarkdownString(comment.body) ? comment.body.value.length > 0 : comment.body.length > 0,
		)
	)
}
...
export interface ICommentsModel {
	...
	readonly commentThreadsMap: Map<
		string,
		{ resourceWithCommentThreads: ResourceWithCommentThreads[]; ownerLabel?: string }
	>
}
	...
	public updateCommentThreads(event: ICommentThreadChangedEvent): boolean {
		...
		removed.forEach((thread) => {
			...
			const matchingResourceData =
				matchingResourceIndex >= 0 ? threadsForOwner[matchingResourceIndex] : undefined
		...
		added.forEach((thread) => {
			...
			if (existingResource.length) {
				...
				threadsForOwner.push(
					new ResourceWithCommentThreads(uniqueOwner, owner, URI.parse(thread.resource!), [thread]),
				)
			}
		})
	...
	private groupByResource(
		...
		for (const group of groupBy(commentThreads, CommentsModel._compareURIs)) {
			commentThreadsByResource.set(
				group[0].resource!,
				new ResourceWithCommentThreads(uniqueOwner, owner, URI.parse(group[0].resource!), group),
			)
		}
