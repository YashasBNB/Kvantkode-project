...
export class CommentsPanel extends FilterViewPane implements ICommentsView {
	...
	private getScreenReaderInfoForNode(element: CommentNode, forAriaLabel?: boolean): string {
		...
		if (
			forAriaLabel &&
			this.configurationService.getValue(AccessibilityVerbositySettingId.Comments)
		) {
	...
	private openFile(
		...
		const commentToReveal =
			element instanceof ResourceWithCommentThreads ? element.commentThreads[0].comment : undefined
	...
	private async refresh(): Promise<void> {
		...
		if (this.isVisible()) {
			...
			if (
				...
				const firstComment =
					this.commentService.commentsModel.resourceCommentThreads[0].commentThreads[0]
				if (firstComment) {
