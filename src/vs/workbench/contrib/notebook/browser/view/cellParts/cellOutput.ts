...
class CellOutputElement extends Disposable {
	...
	private shouldEnableCopy(mimeTypes: readonly IOrderedMimeType[]) {
		if (
			!mimeTypes.find(
				(mimeType) =>
					TEXT_BASED_MIMETYPES.indexOf(mimeType.mimeType) || mimeType.mimeType.startsWith('image/'),
			)
		) {
			return false
		}
	...
	private async _attachToolbar(
		...
		const pickAction = this.toolbarDisposables.add(
			new Action(
				...
				undefined,
				async (_context) =>
					this._pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, this.output),
			),
		)
...
export class CellOutputContainer extends CellContentPart {
	...
	private _doRender() {
		if (this.viewCell.outputsViewModels.length > 0) {
			...
			for (
				let index = 0;
				index < Math.min(this.options.limit, this.viewCell.outputsViewModels.length);
				index++
			) {
	...
	private _renderNow(splice: NotebookCellOutputsSplice, context: CellOutputUpdateContext) {
		...
		if (
			firstGroupEntries.length + newlyInserted.length + secondGroupEntries.length >
			this.options.limit
		) {
			// exceeds limit again
			...
			for (let i = 0; i < outputsNewlyAvailable.length; i++) {
				this._outputEntries[
					firstGroupEntries.length + newlyInserted.length + secondGroupEntries.length + i
				...
				callback: (content) => {
					if (content === 'command:workbench.action.openLargeOutput') {
						this.openerService.open(
							CellUri.generateCellOutputUriWithId(this.notebookEditor.textModel!.uri),
						)
					}
