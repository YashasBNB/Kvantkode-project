...
export class NotebookDocumentMetadataViewModel extends DiffElementViewModelBase {
	...
	protected _layout(delta: ILayoutInfoDelta) {
		...
		const editorHeight =
			delta.editorHeight !== undefined ? delta.editorHeight : this._layoutInfo.editorHeight
		const editorMargin =
			delta.editorMargin !== undefined ? delta.editorMargin : this._layoutInfo.editorMargin
		const cellStatusHeight =
			delta.cellStatusHeight !== undefined
				? delta.cellStatusHeight
				: this._layoutInfo.cellStatusHeight
		const bodyMargin =
			delta.bodyMargin !== undefined ? delta.bodyMargin : this._layoutInfo.bodyMargin
	...
	saveSpirceEditorViewState(
		viewState: editorCommon.ICodeEditorViewState | editorCommon.IDiffEditorViewState | null,
	) {
...
export abstract class DiffElementCellViewModelBase extends DiffElementViewModelBase {
	...
	protected _layout(delta: ILayoutInfoDelta) {
		...
		const metadataHeight =
			delta.metadataHeight !== undefined ? delta.metadataHeight : this._layoutInfo.metadataHeight
		const cellStatusHeight =
			delta.cellStatusHeight !== undefined
				? delta.cellStatusHeight
				: this._layoutInfo.cellStatusHeight
...
export class SideBySideDiffElementViewModel extends DiffElementCellViewModelBase {
	...
	override checkIfInputModified(): false | { reason: string | undefined } {
		if (
			this.original.textModel.getTextBufferHash() === this.modified.textModel.getTextBufferHash()
		) {
			return false
		}
		return {
