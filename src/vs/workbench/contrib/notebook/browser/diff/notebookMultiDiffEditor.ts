...
export class NotebookMultiTextDiffEditor extends EditorPane {
	...
	private readonly ctxAllCollapsed = this._parentContextKeyService.createKey<boolean>(
		NOTEBOOK_DIFF_CELLS_COLLAPSED.key,
		false,
	)
	private readonly ctxHasUnchangedCells = this._parentContextKeyService.createKey<boolean>(
		NOTEBOOK_DIFF_HAS_UNCHANGED_CELLS.key,
		false,
	)
	private readonly ctxHiddenUnchangedCells = this._parentContextKeyService.createKey<boolean>(
	...
	override async setInput(
		...
		const widgetInput = this.modelSpecificResources.add(
			NotebookMultiDiffEditorWidgetInput.createInput(this.viewModel, this.instantiationService),
		)
					...
					if (uri.scheme === Schemas.vscodeNotebookCell) {
						...
						if (notebookDocument && cellIndex !== undefined) {
							...
							const nb = notebookDocument
								...
								: undefined
							const cellLanguage =
								nb && cellIndex !== undefined ? nb.cells[cellIndex].language : undefined
