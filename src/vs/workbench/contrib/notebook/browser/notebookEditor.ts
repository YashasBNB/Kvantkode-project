	...
	override async setInput(
		...
		try {
			...
			this._widgetDisposableStore.add(
				this._widget.value!.onDidChangeModel(() => this._onDidChangeModel.fire()),
			)
			this._widgetDisposableStore.add(
								...
								try {
									if (extensionInfo) {
										await this._extensionsWorkbenchService.setEnablement(
											extensionInfo,
											extensionInfo.enablementState === EnablementState.DisabledWorkspace
			...
			for (const cell of notebook.cells) {
				if (cell.cellKind === CellKind.Code) {
					...
					outputBytes =
						...
						cell.outputs.reduce(
							(prev, cur) =>
								prev + cur.outputs.reduce((size, item) => size + item.data.byteLength, 0),
							0,
						)
				} else {
