...
export function runCutCells(
	...
	if (targetCell) {
		...
		if (!containingSelection) {
			...
			const newFocus =
				focus.end <= targetCellIndex ? focus : { start: focus.start - 1, end: focus.end - 1 }
			...
			textModel.applyEdits(
				...
				true,
				{ kind: SelectionStateType.Index, focus: editor.getFocus(), selections: selections },
	...
	if (!containingSelection) {
		...
		clipboardService.writeText(targetCell.getText())
		const newFocus =
			focus.end === editor.getLength() ? { start: focus.start - 1, end: focus.end - 1 } : focus
		const newSelections = selections.map((selection) =>
	...
	textModel.applyEdits(
		edits,
		true,
		{ kind: SelectionStateType.Index, focus: editor.getFocus(), selections: selectionRanges },
		() => {
			return {
				kind: SelectionStateType.Index,
...
export class NotebookClipboardContribution extends Disposable {
	...
	constructor(@IEditorService private readonly _editorService: IEditorService) {
		...
		if (CopyAction) {
			this._register(
				CopyAction.addImplementation(PRIORITY, 'notebook-clipboard', (accessor) => {
