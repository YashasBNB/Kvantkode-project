...
export class FoldingController extends Disposable implements INotebookEditorContribution {
	...
	setFoldingStateDown(index: number, state: CellFoldingState, levels: number) {
		...
		regions.forEach((r) =>
			this._foldingModel!.setCollapsed(r.regionIndex, state === CellFoldingState.Collapsed),
		)
		this._updateEditorFoldingRanges()
	}
	...
	setFoldingStateUp(index: number, state: CellFoldingState, levels: number) {
		...
		const regions = this._foldingModel.getAllRegionsAtLine(
			index + 1,
			(region, level) =>
				region.isCollapsed !== (state === CellFoldingState.Collapsed) && level <= levels,
		)
		regions.forEach((r) =>
			this._foldingModel!.setCollapsed(r.regionIndex, state === CellFoldingState.Collapsed),
		)
...
registerAction2(
	class extends Action2 {
		...
		async run(
			...
			if (index !== undefined) {
				const targetCell =
					index < 0 || index >= editor.getLength() ? undefined : editor.cellAt(index)
