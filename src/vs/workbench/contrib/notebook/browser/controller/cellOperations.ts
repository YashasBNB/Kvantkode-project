...
export function runDeleteAction(editor: IActiveNotebookEditor, cell: ICellViewModel) {
	...
	if (containingSelection) {
		...
		if (editor.cellAt(focus.start) === cell) {
			...
			const newFocus =
				focus.end === textModel.length ? { start: focus.start - 1, end: focus.end - 1 } : focus
			...
			const newFocus =
				focus.start > targetCellIndex ? { start: focus.start - 1, end: focus.end - 1 } : focus
...
export function insertCell(
	...
	if (type === CellKind.Code) {
		const supportedLanguages =
			activeKernel?.supportedLanguages ?? languageService.getRegisteredLanguageIds()
