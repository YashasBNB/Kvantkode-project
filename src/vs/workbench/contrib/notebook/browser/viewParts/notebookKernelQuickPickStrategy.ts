...
abstract class KernelPickerStrategyBase implements IKernelPickerStrategy {
	...
	async showQuickPick(
		...
		const quickPick = localDisposableStore.add(
			this._quickInputService.createQuickPick<KernelQuickPickItem>({ useSeparators: true }),
		)
	...
	protected async _showKernelExtension(
		...
		for (const extId of extIds) {
			const extension = (
				await extensionWorkbenchService.getExtensions([{ id: extId }], CancellationToken.None)
			)[0]
			if (
	...
	private getSuggestedLanguage(notebookTextModel: NotebookTextModel): string | undefined {
		...
		let suggestedKernelLanguage: string | undefined = (metaData as any)?.metadata?.language_info
			?.name
...
export class KernelPickerMRUStrategy extends KernelPickerStrategyBase {
	...
	private async displaySelectAnotherQuickPick(
		...
		const quickPickItem = await new Promise<KernelQuickPickItem | IQuickInputButton | undefined>(
			(resolve) => {
