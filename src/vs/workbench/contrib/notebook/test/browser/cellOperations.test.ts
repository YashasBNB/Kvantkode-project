...
suite('CellOperations', () => {
	...
	test('split cell', async function () {
		await withTestNotebook(
			...
			(editor, viewModel) => {
				assert.deepStrictEqual(
					computeCellLinesContents(viewModel.cellAt(0)!, [{ lineNumber: 1, column: 4 }]),
					['var', ' b = 1;'],
				)
				...
				assert.deepStrictEqual(
					computeCellLinesContents(viewModel.cellAt(0)!, [{ lineNumber: 1, column: 1 }]),
					['', 'var b = 1;'],
				)

				assert.deepStrictEqual(
					computeCellLinesContents(viewModel.cellAt(0)!, [{ lineNumber: 1, column: 11 }]),
					['var b = 1;', ''],
				)
			},
		)
