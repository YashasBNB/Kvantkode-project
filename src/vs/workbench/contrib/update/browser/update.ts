	...
	constructor(
		...
		hostService.hadLastFocus().then(async (hadLastFocus) => {
			...
			const lastVersion = parseVersion(
				storageService.get(ProductContribution.KEY, StorageScope.APPLICATION, ''),
			)
			const currentVersion = parseVersion(productService.version)
			const shouldShowReleaseNotes =
				configurationService.getValue<boolean>('update.showReleaseNotes')
			...
			if (
				...
				showReleaseNotesInEditor(instantiationService, productService.version, false).then(
					undefined,
					() => {
...
export class UpdateContribution extends Disposable implements IWorkbenchContribution {
	...
	private async onUpdateStateChange(state: UpdateState): Promise<void> {
		...
		if (
			...
			badge = new NumberBadge(1, () =>
				nls.localize('updateIsReady', 'New {0} update available.', this.productService.nameShort),
			)
			...
			badge = new ProgressBadge(() =>
				nls.localize('downloading', 'Downloading {0} update...', this.productService.nameShort),
			)
		} else if (state.type === StateType.Updating) {
			badge = new ProgressBadge(() =>
				nls.localize('updating', 'Updating {0}...', this.productService.nameShort),
			)
		}
	...
	private onUpdateDownloaded(update: IUpdate): void {
		...
		if (
			this.configurationService.getValue('update.enableWindowsBackgroundUpdates') &&
