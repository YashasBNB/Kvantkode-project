...
export class NotebookSearchService implements INotebookSearchService {
	...
	notebookSearch(
		...
		const getAllResults = (): {
			...
			let closedResultsPromise: Promise<IClosedNotebookSearchResults | undefined> =
	...
	private async getLocalNotebookResults(
		...
		for (const widget of widgets) {
			...
			const askMax =
				(isNumber(query.maxResults) ? query.maxResults : DEFAULT_MAX_SEARCH_RESULTS) + 1
			...
			let matches = await widget.find(
				query.contentPattern.pattern,
				{
					...
					includeMarkupPreview:
						query.contentPattern.notebookInfo?.isInNotebookMarkdownPreview ?? true,
