		...
		.catch((error) => {
			if (error instanceof OpenAI.APIError) {
				if (error.status === 401) {
					...
					const resetAtHeader =
						anyErr?.headers?.['x-quota-reset-at'] || anyErr?.response?.headers?.['x-quota-reset-at']
...
const rawToolCallObjOfAnthropicParams = (
...
const _sendOpenAICompatibleChat = async ({
	...
	const { modelName, specialToolFormat, reasoningCapabilities, additionalOpenAIPayload } =
...
const sendAnthropicChat = async ({
	...
	const { modelName, specialToolFormat } = getModelCapabilities(
		providerName,
		modelName_,
		overridesOfModel,
	)
	...
	stream.on('finalMessage', (response) => {
		const anthropicReasoning = response.content.filter(
...
const sendMistralFIM = ({
	...
	const { modelName, supportsFIM } = getModelCapabilities(
		providerName,
		modelName_,
		overridesOfModel,
	)
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
	...
	if (!endpoint)
		throw new Error(
			`Ollama Endpoint was empty (please enter ${defaultProviderSettings.ollama.endpoint} in Void if you want the default url).`,
		)
...
const sendGeminiChat = async ({
	...
	const toolConfig =
		potentialTools && specialToolFormat === 'gemini-style' ? potentialTools : undefined
