		...
		public run(
			...
			if (walkthroughID) {
				const selectedCategory =
					typeof walkthroughID === 'string' ? walkthroughID : walkthroughID.category
				...
				for (const group of editorGroupsService.groups) {
					if (group.activeEditor instanceof GettingStartedInput) {
						...
						;(group.activeEditorPane as GettingStartedPage).makeCategoryVisibleWhenAvailable(
							selectedCategory,
							selectedStep,
						)
						return
				...
				if (activeEditor instanceof ExtensionsInput) {
					...
					editorService
						...
						.then((editor) => {
							;(editor as GettingStartedPage)?.makeCategoryVisibleWhenAvailable(
								selectedCategory,
								selectedStep,
							)
						})
