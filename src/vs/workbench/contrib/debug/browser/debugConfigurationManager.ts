...
export class ConfigurationManager implements IConfigurationManager {
	...
	async getDynamicConfigurationsByType(
		...
		this.getLaunches().forEach((launch) => {
			if (provider) {
				picks.push(
					provider.provideDebugConfigurations!(launch.workspace?.uri, token).then(
						(configurations) =>
	...
	private initLaunches(): void {
		this.launches = this.contextService
			.getWorkspace()
			.folders.map((folder) =>
				this.instantiationService.createInstance(Launch, this, this.adapterManager, folder),
			)
	...
	private setCompoundSchemaValues(): void {
		const compoundConfigurationsSchema = (<IJSONSchema>launchSchema.properties!['compounds'].items)
		...
		;(<IJSONSchema>compoundConfigurationsSchema.items).oneOf![1].properties!.folder.enum =
			folderNames
	...
	private getDeduplicatedConfig(): IGlobalConfig | undefined {
		...
		return (
			original && {
				...
				configurations:
					original.configurations && distinguishConfigsByName(original.configurations),
			}
		)
	}
}
