...
export class DebugToolBar extends Themable implements IWorkbenchContribution {
		...
		this.updateScheduler = this._register(
			new RunOnceScheduler(() => {
				...
				const toolBarLocation =
					this.configurationService.getValue<IDebugConfiguration>('debug').toolBarLocation
				if (
		...
		this._register(
			this.actionBar.actionRunner.onDidRun((e: IRunEvent) => {
				...
				this.telemetryService.publicLog2<
					WorkbenchActionExecutedEvent,
					WorkbenchActionExecutedClassification
				>('workbenchActionExecuted', { id: e.action.id, from: 'debugActionsWidget' })
			}),
		)
	...
	private get yRange(): [number, number] {
		if (!this._yRange) {
			...
			if (
				this.configurationService.getValue(LayoutSettings.EDITOR_TABS_MODE) !== EditorTabsMode.NONE
			) {
				yMax += 35
			}
