...
export class DebugStorage extends Disposable {
	...
	loadDebugUxState(): 'simple' | 'default' {
		return this.storageService.get(DEBUG_UX_STATE_KEY, StorageScope.WORKSPACE, 'default') as
			| 'simple'
			| 'default'
	}
	...
	private loadBreakpoints(): Breakpoint[] {
		...
		try {
			result = JSON.parse(
				this.storageService.get(DEBUG_BREAKPOINTS_KEY, StorageScope.WORKSPACE, '[]'),
	...
	private loadFunctionBreakpoints(): FunctionBreakpoint[] {
		...
		try {
			result = JSON.parse(
				this.storageService.get(DEBUG_FUNCTION_BREAKPOINTS_KEY, StorageScope.WORKSPACE, '[]'),
	...
	private loadExceptionBreakpoints(): ExceptionBreakpoint[] {
		...
		try {
			result = JSON.parse(
				this.storageService.get(DEBUG_EXCEPTION_BREAKPOINTS_KEY, StorageScope.WORKSPACE, '[]'),
	...
	private loadDataBreakpoints(): DataBreakpoint[] {
		...
		try {
			result = JSON.parse(
				this.storageService.get(DEBUG_DATA_BREAKPOINTS_KEY, StorageScope.WORKSPACE, '[]'),
	...
	private loadWatchExpressions(): Expression[] {
		...
		try {
			result = JSON.parse(
				this.storageService.get(DEBUG_WATCH_EXPRESSIONS_KEY, StorageScope.WORKSPACE, '[]'),
	...
	loadChosenEnvironments(): Record<string, IChosenEnvironment> {
		const obj = JSON.parse(
			this.storageService.get(DEBUG_CHOSEN_ENVIRONMENTS_KEY, StorageScope.WORKSPACE, '{}'),
		)
