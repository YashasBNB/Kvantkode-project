...
namespace ShellConfiguration {
	...
	export function is(value: any): value is IShellConfiguration {
		...
		return (
			candidate && (Types.isString(candidate.executable) || Types.isStringArray(candidate.args))
		)
	}
...
namespace ConfiguringTask {
	...
	export function from(
		...
		const taskIdentifier: Tasks.KeyedTaskIdentifier | undefined =
			Tasks.TaskDefinition.createTaskIdentifier(identifier, context.problemReporter)
		...
		for (let index = 0; index < externals.length; index++) {
			...
			if (isCustomTask(external)) {
				...
				if (customTask) {
					...
					if (
						customTask.configurationProperties.group === Tasks.TaskGroup.Build &&
	...
	private createTaskRunnerConfiguration(
		...
		if ((!result.custom || result.custom.length === 0) && globals.command && globals.command.name) {
			const matchers: ProblemMatcher[] =
				ProblemMatcherConverter.from(fileConfig.problemMatcher, context).value ?? []
