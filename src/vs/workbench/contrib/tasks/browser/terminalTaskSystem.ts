...
export class TerminalTaskSystem extends Disposable implements ITaskSystem {
	...
	public run(
		...
		const instances =
			InMemoryTask.is(task) || this._isTaskEmpty(task) ? [] : this._getInstances(task)
		const validInstance =
			instances.length < ((task.runOptions && task.runOptions.instanceLimit) ?? 1)
	...
	private _showDependencyCycleMessage(task: Task) {
		this._log(
			nls.localize('dependencyCycle', 'There is a dependency cycle. See task "{0}".', task._label),
		)
		this._showOutput()
	}
		...
		if (task.configurationProperties.isBackground) {
			...
			toDispose.add(
				watchingProblemMatcher.onDidStateChange((event) => {
					if (event.kind === ProblemCollectorEventKind.BackgroundProcessingBegins) {
						...
						this._fireTaskEvent(
							TaskEvent.general(TaskEventKind.Inactive, task, terminal?.instanceId),
						)
						if (eventCounter === 0) {
							if (
								...
								this._fireTaskEvent(
									TaskEvent.general(TaskEventKind.ProblemMatcherEnded, task, terminal?.instanceId),
								)
							}
						}
					}
			...
			terminal.processReady.then(
				() => {
					if (!processStartedSignaled) {
						this._fireTaskEvent(
							TaskEvent.processStarted(task, terminal!.instanceId, terminal!.processId!),
						)
			...
			promise = new Promise<ITaskSummary>((resolve, reject) => {
				const onExit = terminal!.onExit((terminalLaunchResult) => {
					...
					this._fireTaskEvent(
						TaskEvent.processEnded(task, terminal?.instanceId, exitCode ?? undefined),
					)
					if (this._busyTasks[mapKey]) {
						delete this._busyTasks[mapKey]
					}
	...
	private async _createShellLaunchConfig(
		...
		if (isShellCommand) {
			...
			if (task.command.presentation && task.command.presentation.echo) {
				...
				shellLaunchConfig.initialText = {
					text:
						this.taskShellIntegrationStartSequence(cwd) + this.taskShellIntegrationOutputSequence,
	...
	private async _resolveMatchers(
		...
		for (const value of values) {
			...
			const hasUriProvider =
				taskSystemInfo !== undefined && taskSystemInfo.uriProvider !== undefined
					...
					if (Types.isString(filePrefix)) {
						...
						if (filePrefix.include) {
							filePrefix.include = Array.isArray(filePrefix.include)
								? await Promise.all(
										filePrefix.include.map((x) => this._resolveVariable(resolver, x)),
									)
						...
						if (filePrefix.exclude) {
							filePrefix.exclude = Array.isArray(filePrefix.exclude)
								? await Promise.all(
										filePrefix.exclude.map((x) => this._resolveVariable(resolver, x)),
									)
