...
export abstract class AbstractTaskService extends Disposable implements ITaskService {
	...
	private async _reconnectTasks(): Promise<boolean> {
		...
		this._log(
			nls.localize('TaskService.reconnectingTasks', 'Reconnecting to {0} tasks...', taskLabels),
			true,
		)
	...
	public async getSavedTasks(
		...
		for (const key of storedTasks.keys()) {
			if (readTasksMap.has(key)) {
				...
				this._log(
					nls.localize('taskService.getSavedTasks.resolved', 'Resolved task {0}', key),
					true,
				)
			} else {
				this._log(
					nls.localize('taskService.getSavedTasks.unresolved', 'Unable to resolve task {0} ', key),
					true,
				)
			}
		}
	...
	public removePersistentTask(key: string) {
		this._log(
			nls.localize('taskService.removePersistentTask', 'Removing persistent task {0}', key),
			true,
		)
	...
	private async _setPersistentTask(task: Task): Promise<void> {
		...
		if (!InMemoryTask.is(task) && key) {
			...
			this._log(
				nls.localize('taskService.setPersistentTask', 'Setting persistent task {0}', key),
				true,
			)
	...
	private _savePersistentTasks(): void {
		...
		this._log(
			nls.localize('savePersistentTask', 'Saving persistent tasks: {0}', keys.join(', ')),
			true,
		)
		this._storageService.store(
			AbstractTaskService.PersistentTasks_Key,
	...
	private _openDocumentation(): void {
		this._openerService.open(
			URI.parse('https://code.visualstudio.com/docs/editor/tasks#_defining-a-problem-matcher'),
		)
	}
	...
	private async _openEditorAtTask(
		...
		if (configIndex !== -1) {
			const json: TaskConfig.IExternalTaskRunnerConfiguration =
				this._configurationService.getValue<TaskConfig.IExternalTaskRunnerConfiguration>('tasks', {
					resource,
				})
	...
	private _createCustomizableTask(
		...
		const taskConfig =
			CustomTask.is(task) || ConfiguringTask.is(task) ? task._source.config : undefined
		...
		async function quickResolve(
			...
			const foundTasks = await that._findWorkspaceTasks((task: Task | ConfiguringTask): boolean => {
				...
				if (Types.isString(identifier)) {
					return (
						task._label === identifier || task.configurationProperties.identifier === identifier
					)
				} else {
		...
		if (executeResult.kind === TaskExecuteKind.Active) {
			...
			if (active && active.same) {
				if (this._taskSystem?.isTaskVisible(executeResult.task)) {
					...
					const lastInstance =
						this._getTaskSystem().getLastInstance(executeResult.task) ?? executeResult.task
					this._notificationService.prompt(
						Severity.Warning,
						message,
						[
	...
	private async _getGroupedTasks(
		...
		try {
			...
			await Promise.all(
				this._getCustomTaskPromises(tasks, filter, result, contributedTasks, waitToActivate),
			)
	...
	protected _getConfiguration(
		...
		if (
			...
			const wholeConfig =
				this._configurationService.inspect<TaskConfig.IExternalTaskRunnerConfiguration>('tasks', {
					resource: workspaceFolder.uri,
				})
			switch (source) {
				case TaskSourceKind.User: {
	...
	private _needsRecentTasksMigration(): boolean {
		return (
			this.getRecentlyUsedTasksV1().size > 0 && this._getTasksFromStorage('historical').size === 0
		)
	}
	...
	private _doRunTaskCommand(tasks?: Task[], type?: string, name?: string): void {
		...
		this._showIgnoredFoldersMessage().then(() => {
			if (this._configurationService.getValue(USE_SLOW_PICKER)) {
				let taskResult: { tasks: Promise<Task[]>; grouped: Promise<TaskMap> } | undefined =
					undefined
				if (!tasks) {
	...
	private _getDefaultTasks(tasks: Task[], taskGlobsInList: boolean = false): Task[] {
		...
		for (const task of tasks.filter((t) => !!t.configurationProperties.group)) {
			...
			if (
				taskGlobsInList &&
				typeof (task.configurationProperties.group as TaskGroup).isDefault === 'string'
			) {
				defaults.push(task)
			} else if (
				!taskGlobsInList &&
	...
	private async _upgrade(): Promise<void> {
		...
		for (const folder of this.workspaceFolders) {
			...
			const globalConfig = {
				...
				osx: <ICommandUpgrade>(
					this._configurationService.getValue(TasksSchemaProperties.Osx, { resource: folder.uri })
				),
			...
			if (
				this._configurationService.getValue(TasksSchemaProperties.IsShellCommand, {
					resource: folder.uri,
				})
			) {
			...
			if (
				this._configurationService.getValue(TasksSchemaProperties.SuppressTaskName, {
					resource: folder.uri,
				})
			) {
				await this._configurationService.updateValue(
