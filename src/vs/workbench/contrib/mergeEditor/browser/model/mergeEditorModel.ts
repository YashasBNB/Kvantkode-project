...
export class MergeEditorModel extends EditorModel {
	...
	private computeAutoMergedResult(): string {
		...
		for (const baseRange of baseRanges) {
			appendLinesToResult(
				baseLines,
				LineRange.fromLineNumbers(baseStartLineNumber, baseRange.baseRange.startLineNumber),
			)
	...
	public readonly inputDiffComputingState = derived(this, (reader) => {
		const states = [this.input1TextModelDiffs, this.input2TextModelDiffs].map((s) =>
			s.state.read(reader),
		)
	...
	public async getResultValueWithConflictMarkers(): Promise<string> {
		...
		appendLinesToResult(
			resultLines,
			LineRange.fromLineNumbers(resultStartLineNumber, resultLines.length + 1),
		)
	...
	public get manuallySolvedConflictCountThatEqualNone(): number {
		return arrayCount(
			this.modifiedBaseRangeResultStates.get().entries(),
			([r, s]) =>
				r.isConflicting && s.accepted.get().kind === ModifiedBaseRangeStateKind.unrecognized,
		)
	}
...
class ModifiedBaseRangeData {
	...
	public accepted: ISettableObservable<ModifiedBaseRangeState> = observableValue(
