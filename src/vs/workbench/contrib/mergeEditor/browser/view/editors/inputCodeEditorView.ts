		...
		this._register(
			autorunOpts(
				...
				(reader) => {
					...
					const description =
						this.inputNumber === 1 ? vm.model.input1.description : vm.model.input2.description
	...
	private readonly modifiedBaseRangeGutterItemInfos = derivedOpts(
		...
		(reader) => {
			...
			return model.modifiedBaseRanges
				...
				.map(
					(baseRange, idx) =>
						new ModifiedBaseRangeGutterItemModel(idx.toString(), baseRange, inputNumber, viewModel),
				)
		},
	)
...
export class ModifiedBaseRangeGutterItemModel implements IGutterItemInfo {
	...
	public readonly state: IObservable<{ handled: boolean; focused: boolean }> = derived(
		this,
		(reader) => {
	...
	public getContextMenuActions(): readonly IAction[] {
		...
		return [
			...
			this.baseRange.isConflicting
				? setFields(
						action('mergeEditor.swap', localize('mergeEditor.swap', 'Swap'), state.swap(), false),
