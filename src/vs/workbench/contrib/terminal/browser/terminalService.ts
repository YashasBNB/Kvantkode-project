...
export class TerminalService extends Disposable implements ITerminalService {
	...
	private _hostActiveTerminals: Map<ITerminalInstanceHost, ITerminalInstance | undefined> =
		new Map()
	...
	@memoize get onAnyInstanceDataInput() {
		return this._register(
			this.createOnInstanceEvent((e) => Event.map(e.onDidInputData, () => e, e.store)),
		).event
	}
	...
	@memoize get onAnyInstanceMaximumDimensionsChange() {
		return this._register(
			this.createOnInstanceEvent((e) => Event.map(e.onMaximumDimensionsChanged, () => e, e.store)),
		).event
	}
	...
	constructor(
		...
		timeout(0).then(() =>
			this._register(
				this._instantiationService.createInstance(TerminalEditorStyle, mainWindow.document.head),
			),
		)
	}
	...
	private async _initializePrimaryBackend() {
		...
		reconnectedPromise.then(async () => {
			...
			await Promise.all(
				instances.map((e) => new Promise<void>((r) => Event.once(e.onProcessReplayComplete)(r))),
			)
	...
	async createTerminal(options?: ICreateTerminalOptions): Promise<ITerminalInstance> {
		...
		const splitActiveTerminal =
			typeof options?.location === 'object' && 'splitActiveTerminal' in options.location
...
class TerminalEditorStyle extends Themable {
	...
	private _registerListeners(): void {
		...
		this._register(
			this._terminalProfileService.onDidChangeAvailableProfiles(() => this.updateStyles()),
		)
	}
