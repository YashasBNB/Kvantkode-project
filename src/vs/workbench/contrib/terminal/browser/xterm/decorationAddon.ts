...
export class DecorationAddon extends Disposable implements ITerminalAddon, IDecorationAddon {
	...
	constructor(
		...
		this._updateDecorationVisibility()
		this._register(
			this._capabilities.onDidAddCapabilityType((c) => this._createCapabilityDisposables(c)),
		)
		this._register(
			this._capabilities.onDidRemoveCapabilityType((c) => this._removeCapabilityDisposables(c)),
		)
	...
	private async _getCommandActions(command: ITerminalCommand): Promise<IAction[]> {
		...
		actions.push({
			...
			enabled: true,
			run: () =>
				this._commandService.executeCommand('workbench.action.terminal.goToRecentDirectory'),
		})
