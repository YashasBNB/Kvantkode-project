...
export class McpServerMetadataCache extends Disposable {
	...
	constructor(scope: StorageScope, @IStorageService storageService: IStorageService) {
		super()
...
export class McpServer extends Disposable implements IMcpServer {
	...
	private readonly toolsFromServerPromise = observableValue<
		ObservablePromise<readonly IValidatedMcpTool[]> | undefined
	>(this, undefined)
	private readonly toolsFromServer = derived(
	...
	public readonly toolsState = derived((reader) => {
		...
		if (!fromServerResult) {
			return this.toolsFromCache
				? McpServerToolsState.RefreshingFromCached
				: McpServerToolsState.RefreshingFromUnknown
		}

		return fromServerResult.error
			? this.toolsFromCache
				? McpServerToolsState.Cached
				: McpServerToolsState.Unknown
			: McpServerToolsState.Live
	})
	...
	public start(isFromInteraction?: boolean): Promise<McpConnectionState> {
		return this._connectionSequencer.queue(async () => {
			...
			if (
				...
				await Promise.all(
					this._mcpRegistry.delegates.map((r) => r.waitForInitialProviderPromises()),
				)
			...
			this._telemetryService.publicLog2<ServerBootState, ServerBootStateClassification>(
				'mcp/serverBootState',
				{
			...
			d = autorun((reader) => {
				...
				if (!handler) {
					...
					if (state.state === McpConnectionState.Kind.Error) {
						reject(
							new McpConnectionFailedError(`MCP server could not be started: ${state.message}`),
						)
						return
