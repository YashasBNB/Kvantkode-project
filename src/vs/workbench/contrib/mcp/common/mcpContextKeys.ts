...
export namespace McpContextKeys {
	export const serverCount = new RawContextKey<number>('mcp.serverCount', undefined, {
		type: 'number',
	...
	export const hasUnknownTools = new RawContextKey<boolean>('mcp.hasUnknownTools', undefined, {
		type: 'boolean',
		description: localize(
	...
	export const toolsCount = new RawContextKey<number>('mcp.toolsCount', undefined, {
		type: 'number',
		description: localize(
...
export class McpContextKeysController extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this._store.add(
			bindContextKey(McpContextKeys.hasServersWithErrors, contextKeyService, (r) =>
				mcpService.servers
					.read(r)
					.some((c) => c.connectionState.read(r).state === McpConnectionState.Kind.Error),
			),
		)
		...
		this._store.add(
			autorun((r) => {
				...
				ctxHasUnknownTools.set(
					mcpService.lazyCollectionState.read(r) !== LazyCollectionState.AllKnown ||
						servers.some((s) => {
							...
							return (
								toolState === McpServerToolsState.Unknown ||
								toolState === McpServerToolsState.RefreshingFromUnknown
							)
						}),
				)
			}),
