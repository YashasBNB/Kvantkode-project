...
export class ExtensionsListView extends ViewPane {
	...
	private async queryByIds(
		...
		const result = (await this.extensionsWorkbenchService.queryLocal(this.options.server)).filter(
			(e) => idsSet.has(e.identifier.id.toLowerCase()),
		)
			...
			.filter(
				(e) =>
					runningExtensions.some((r) =>
						areSameExtensions({ id: r.identifier.value, uuid: r.uuid }, e.identifier),
					) &&
					(e.name.toLowerCase().indexOf(value) > -1 ||
						e.displayName.toLowerCase().indexOf(value) > -1) &&
					this.filterExtensionByCategory(e, includedCategories, excludedCategories),
	...
	private async getOtherRecommendationsModel(
		...
		const result = coalesce(
			otherRecommendations.map((id) =>
				installableRecommendations.find((i) => areSameExtensions(i.identifier, { id })),
			),
		)
		return new PagedModel(result)
	}
	...
	implements IWorkspaceRecommendedExtensionsView
{
	...
	override async show(query: string): Promise<IPagedModel<IExtension>> {
		const shouldShowEmptyView =
			query && query.trim() !== '@recommended' && query.trim() !== '@recommended:workspace'
...
export class PreferredExtensionsPagedModel implements IPagedModel<IExtension> {
	...
	async resolve(index: number, cancellationToken: CancellationToken): Promise<IExtension> {
		...
		const indexInPagedModel =
			index - this.preferredExtensions.length + this.resolvedGalleryExtensionsFromQuery.length
