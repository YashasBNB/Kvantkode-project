...
class Extensions extends Disposable {
	...
	private async onDidUpdateExtensionMetadata(local: ILocalExtension): Promise<void> {
		...
		if (extension?.local) {
			const hasChanged =
				extension.local.pinned !== local.pinned || extension.local.preRelease !== local.preRelease
			extension.local = local
			if (hasChanged) {
				this._onChange.fire({ extension })
			}
	...
	private onUninstallExtension(identifier: IExtensionIdentifier): void {
		...
		if (extension) {
			const uninstalling =
				this.uninstalling.filter((e) => areSameExtensions(e.identifier, identifier))[0] || extension
			this.uninstalling = [
				uninstalling,
	...
	getExtensionState(extension: Extension): ExtensionState {
		if (
			...
			this.installing.some(
				(e) =>
					!!e.gallery && areSameExtensions(e.gallery.identifier, extension.gallery!.identifier),
			)
		) {
	...
	implements IExtensionsWorkbenchService, IURLHandler
{
	...
	> {
		...
		const invalidExtensions = this.local.filter(
			(e) =>
				e.enablementState === EnablementState.DisabledByInvalidExtension && !e.isWorkspaceScoped,
		)
	...
	private getInstalledExtensionMatchingLocation(location: URI): IExtension | null {
		return (
			this.local.find(
				(e) =>
					e.local && this.uriIdentityService.extUri.isEqualOrParent(location, e.local?.location),
			) ?? null
		)
	}
	...
	async updateRunningExtensions(auto: boolean = false): Promise<void> {
		...
		if (toAdd.length || toRemove.length) {
			if (
				...
				>('extensions:autorestart', { count: toAdd.length + toRemove.length, auto })
			}
		}
	}
	...
	private getRuntimeState(extension: IExtension): ExtensionRuntimeState | undefined {
		...
		if (isUninstalled) {
			const canRemoveRunningExtension =
				runningExtension && this.extensionService.canRemoveExtension(runningExtension)
			...
			// Extension is not running
				...
				if (otherServer && extension.enablementState === EnablementState.DisabledByExtensionKind) {
					const extensionInOtherServer = this.local.filter(
						(e) =>
							areSameExtensions(e.identifier, extension.identifier) && e.server === otherServer,
					)[0]
	...
	async downloadVSIX(extensionId: string, preRelease: boolean): Promise<void> {
		...
		if (!galleryExtension) {
			throw new Error(
				nls.localize('extension not found', "Extension '{0}' not found.", extensionId),
			)
		}
	...
	private async syncInstalledExtensionsWithGallery(gallery: IGalleryExtension[]): Promise<void> {
		...
		await Promise.allSettled(
			extensions.map((extensions) =>
				extensions.syncInstalledExtensionsWithGallery(gallery, this.getProductVersion()),
			),
		)
	...
	private async autoUpdateBuiltinExtensions(): Promise<void> {
		...
		await Promises.settled(
			toUpdate.map((e) =>
				this.install(e, e.local?.preRelease ? { installPreReleaseVersion: true } : undefined),
			),
		)
	}
			...
			if (gallery) {
				...
				if (installOptions.installEverywhere) {
					...
					const installableServers =
						await this.extensionManagementService.getInstallableServers(gallery)
				...
				buttons.push({
					label:
						isString(installOptions.justification) || !installOptions.justification.action
							? nls.localize(
									{ key: 'installButtonLabel', comment: ['&& denotes a mnemonic'] },
									'&&Install Extension',
								)
							: nls.localize(
				...
				if (installOptions.justification) {
					const result = await this.dialogService.confirm({
						...
						primaryButton: isString(installOptions.justification)
							? nls.localize(
									{ key: 'enableButtonLabel', comment: ['&& denotes a mnemonic'] },
									'&&Enable Extension',
								)
							: nls.localize(
	...
	async installInServer(
		...
		await this.doInstall(extension, async () => {
			...
			if (
				!isTargetPlatformCompatible(local.targetPlatform, [local.targetPlatform], targetPlatform)
			) {
				throw new Error(
					nls.localize(
						'incompatible',
	...
	async uninstall(e: IExtension): Promise<void> {
		...
		for (const { extension } of extensionsToUninstall) {
			for (const local of this.local) {
				...
				if (
					dependents.some((d) =>
						d.extensionPack.some((id) => areSameExtensions({ id }, local.identifier)),
					)
				) {
					continue
				}
				if (
					local.dependencies.some((dep) => areSameExtensions(extension.identifier, { id: dep }))
				) {
	...
	async toggleExtensionIgnoredToSync(extension: IExtension): Promise<void> {
		...
		if (extension.local && isIgnored) {
			;(<Extension>extension).local = await this.updateSynchronizingInstalledExtension(
				extension.local,
				true,
			)
			this._onChange.fire(extension)
	...
	private onError(err: any): void {
		...
		if (
			/getaddrinfo ENOTFOUND|getaddrinfo ENOENT|connect EACCES|connect ECONNREFUSED/.test(message)
		) {
			return
		}
