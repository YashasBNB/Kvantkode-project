							...
							[
								{
									...
									run: () =>
										this.commandService.executeCommand(SELECT_INSTALL_VSIX_EXTENSION_COMMAND_ID),
								},
							],
						)
					}),
...
export class ButtonWithDropDownExtensionAction extends ExtensionAction {
	...
	constructor(
		...
		this.update()
		this._register(
			Event.any(...this.extensionActions.map((a) => a.onDidChange))(() => this.update(true)),
		)
	...
	update(): void {
		...
		if (this.canInstall()) {
			const extensionInOtherServer = this.extensionsWorkbenchService.installed.filter(
				(e) =>
					areSameExtensions(e.identifier, this.extension!.identifier) && e.server === this.server,
			)[0]
...
export class MigrateDeprecatedExtensionAction extends ExtensionAction {
	...
	update(): void {
		...
		if (
			this.extensionsWorkbenchService.local.some((e) => areSameExtensions(e.identifier, { id }))
		) {
			return
		}
		this.enabled = true
...
export class ExtensionEditorManageExtensionAction extends DropDownExtensionAction {
	...
	override async run(): Promise<any> {
		...
		;(
			await getContextMenuActions(this.extension, this.contextKeyService, this.instantiationService)
...
export class SetColorThemeAction extends ExtensionAction {
	...
	override async run(
		...
		const pickedTheme = await this.quickInputService.pick(picks, {
			...
			onDidFocus: (item) =>
				delayer.trigger(() => this.workbenchThemeService.setColorTheme(item.id, undefined)),
			ignoreFocusLost,
		})
...
export class SetFileIconThemeAction extends ExtensionAction {
	...
	override async run(
		...
		const pickedTheme = await this.quickInputService.pick(picks, {
			...
			onDidFocus: (item) =>
				delayer.trigger(() => this.workbenchThemeService.setFileIconTheme(item.id, undefined)),
			ignoreFocusLost,
		})
...
export class SetProductIconThemeAction extends ExtensionAction {
	...
	override async run(
		...
		const pickedTheme = await this.quickInputService.pick(picks, {
			...
			onDidFocus: (item) =>
				delayer.trigger(() => this.workbenchThemeService.setProductIconTheme(item.id, undefined)),
			ignoreFocusLost,
		})
...
export abstract class AbstractConfigureRecommendedExtensionsAction extends Action {
	...
	protected openExtensionsFile(extensionsFileResource: URI): Promise<any> {
		return this.getOrCreateExtensionsFile(extensionsFileResource).then(
			({ created, content }) =>
				this.getSelectionPosition(content, extensionsFileResource, ['recommendations']).then(
					(selection) =>
	...
	private getOrUpdateWorkspaceConfigurationFile(
		...
		return Promise.resolve(this.fileService.readFile(workspaceConfigurationFile)).then(
			(content) => {
	...
	private getSelectionPosition(
		...
		if (node && node.parent && node.parent.children) {
			...
			return Promise.resolve(this.textModelResolverService.createModelReference(resource)).then(
				(reference) => {
	...
	private computeLabel(): string | null {
		...
		if (currentEnablementState !== null) {
			const currentlyEnabled =
				this.extensionEnablementService.isEnabledEnablementState(currentEnablementState)
	...
	private async computeAndUpdateStatus(): Promise<void> {
		...
		if (!this.extension.local.isValid) {
			const errors = this.extension.local.validations
				...
				.map(([, message]) => message)
			this.updateStatus(
				{ icon: warningIcon, message: new MarkdownString(errors.join(' ').trim()) },
				true,
			)
			return
		}
