...
export class RemoteStatusIndicator extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this.legacyIndicatorMenu = this._register(
			this.menuService.createMenu(MenuId.StatusBarWindowIndicatorMenu, this.contextKeyService),
		) // to be removed once migration completed
		this.remoteIndicatorMenu = this._register(
			this.menuService.createMenu(MenuId.StatusBarRemoteIndicatorMenu, this.contextKeyService),
		)
	...
	private registerListeners(): void {
		...
		this._register(
			this.labelService.onDidChangeFormatters(() => this.updateRemoteStatusIndicator()),
		)
		...
		this._register(
			this.extensionManagementService.onDidUninstallExtension(async (result) => {
				const index = this.remoteExtensionMetadata.findIndex((value) =>
	...
	private async initializeRemoteMetadata(): Promise<void> {
		...
		for (let i = 0; i < this.remoteExtensionMetadata.length; i++) {
			...
			const isInstalled = (await this.extensionManagementService.getInstalled()).find((value) =>
	...
	private updateRemoteStatusIndicator(): void {
		...
		if (remoteIndicator) {
			...
			this.renderRemoteStatusIndicator(
				truncate(remoteIndicatorLabel, RemoteStatusIndicator.REMOTE_STATUS_LABEL_MAX_LENGTH),
		...
		if (this.remoteAuthority) {
			const hostLabel =
				this.labelService.getHostLabel(Schemas.vscodeRemote, this.remoteAuthority) ||
	...
	private withNetworkStatus(
		...
		initialTooltip?: string | MarkdownString,
		showProgress?: boolean,
	): { text: string; tooltip: string | IMarkdownString | undefined; ariaLabel: string } {
