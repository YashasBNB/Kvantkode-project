...
class TunnelItem implements ITunnelItem {
	...
	get privacy(): TunnelPrivacy {
		if (this.tunnelService?.privacyOptions) {
			return (
				this.tunnelService?.privacyOptions.find((element) => element.id === this._privacy) ?? {
					id: '',
	...
	private onSelectionChanged(event: ITableEvent<ITunnelItem>) {
		...
		if (elements.length > 1) {
			this.tunnelViewMultiSelectionContext.set(
				elements.map((element) => makeAddress(element.remoteHost, element.remotePort)),
			)
		} else {
				...
				onFinish: async (value, success) => {
					...
					if (success && (parsed = parseAddress(value))) {
						remoteExplorerService
							...
							.then((tunnelOrError) =>
								error(notificationService, tunnelOrError, parsed!.host, parsed!.port),
							)
					}
				},
