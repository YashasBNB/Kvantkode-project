	...
	private async doUploadEntry(
		...
		const reportProgress = (fileSize: number, bytesUploaded: number): void => {
			...
			const bytesUploadedPerSecond =
				operation.totalBytesUploaded / ((Date.now() - operation.startTime) / 1000)
...
export class ExternalFileImport {
	...
	private async doImport(
		...
		await Promise.all(
			candidateFiles.map((resource) => this.fileService.activateProvider(resource.scheme)),
		)
	...
	private async importResources(
		...
		if (resources && resources.length > 0) {
			...
			const resourcesFiltered = coalesce(
				await Promises.settled(
					resources.map(async (resource) => {
						...
						if (
							targetNames.has(caseSensitive ? basename(resource) : basename(resource).toLowerCase())
						) {
			...
			await this.explorerService.applyBulkEdit(resourceFileEdits, {
				...
				confirmBeforeUndo:
					undoLevel === UndoConfirmLevel.Verbose || undoLevel === UndoConfirmLevel.Default,
			})

			// if we only add one file, just open it directly
			const autoOpen =
				this.configurationService.getValue<IFilesConfiguration>().explorer.autoOpenDroppedFile
...
export class FileDownload {
	...
	private reportProgress(
		...
		const bytesDownloadedPerSecond =
			operation.totalBytesDownloaded / ((Date.now() - operation.startTime) / 1000)
