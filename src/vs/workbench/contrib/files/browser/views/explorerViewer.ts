...
export class ExplorerFindProvider implements IAsyncFindProvider<ExplorerItem> {
	...
	private createPhantomItems(
		...
		for (const stat of pathSegments) {
			...
			if (!child) {
				const isDirectory =
					pathSegments[pathSegments.length - 1] === stat ? resourceIsDirectory : true
	...
	private async getSearchResults(
		...
		{ explorerRoot: ExplorerItem; files: URI[]; directories: URI[]; hitMaxResults: boolean }[]
	> {
		const patternLowercase = pattern.toLowerCase()
		const isFuzzyMatch = matchType === TreeFindMatchType.Fuzzy
		return await Promise.all(
			roots.map((root, index) =>
				this.searchInWorkspace(patternLowercase, root, index, isFuzzyMatch, token),
			),
		)
	}
	...
	private async searchInWorkspace(
		...
		const searchExcludePattern =
			getExcludes(
				this.configurationService.getValue<ISearchConfiguration>({ resource: root.resource }),
			) || {}
		...
		IDisposable
{
	...
	renderCompressedElements(
		...
		const editableData =
			editable.length === 0 ? undefined : this.explorerService.getEditableData(editable[0])
	...
	private renderStat(
		...
		const twistieContainer =
			templateData.container.parentElement?.parentElement?.querySelector('.monaco-tl-twistie')
		...
		const themeIsUnhappyWithNesting =
			theme.hasFileIcons && (theme.hidesExplorerArrows || !theme.hasFolderIcons)
	...
	private isVisible(stat: ExplorerItem, parentVisibility: TreeVisibility): boolean {
		...
		if (isHiddenResource || stat.parent?.isExcluded) {
			...
			const editor = editors.find(
				(e) =>
					e.resource && this.uriIdentityService.extUri.isEqualOrParent(e.resource, stat.resource),
			)
...
export class FileDragAndDrop implements ITreeDragAndDrop<ExplorerItem> {
	...
	private handleDragOver(
		...
		const effectType =
			isNative || isCopy ? ListDragOverEffectType.Copy : ListDragOverEffectType.Move
	...
	private async doHandleExplorerDropOnMove(
		...
		const options = {
			confirmBeforeUndo:
				this.configurationService.getValue<IFilesConfiguration>().explorer.confirmUndo ===
		...
		try {
			...
			// Conflict
			if (
				(<FileOperationError>error).fileOperationResult === FileOperationResult.FILE_MOVE_CONFLICT
			) {
