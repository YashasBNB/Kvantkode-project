			...
			},
			{
				createEditorInput: ({ resource }) => {
					const editorInput = editorService
						...
						.find(
							(editor) =>
								editor.editor instanceof InteractiveEditorInput &&
...
function createEditor(resource: URI, instantiationService: IInstantiationService): EditorInput {
	...
	const inputBoxPath =
		counter && counter[1] ? `/InteractiveInput-${counter[1]}` : 'InteractiveInput'
	...
	implements IWorkbenchContribution, IWorkingCopyEditorHandler
{
	...
	isOpen(workingCopy: IWorkingCopyIdentifier, editor: EditorInput): boolean {
		...
		return (
			editor instanceof InteractiveEditorInput && isEqual(workingCopy.resource, editor.resource)
		)
	}
	...
	class extends Action2 {
		...
		async run(
			...
			const editorOptions = {
				...
				preserveFocus:
					typeof showOptions !== 'number' ? (showOptions?.preserveFocus ?? false) : false,
			}
			...
			if (editorControl && isReplEditorControl(editorControl) && editorControl.notebookEditor) {
				...
				if (notebookDocument && textModel && editorControl.activeCodeEditor) {
					...
					await editorControl.notebookEditor.executeNotebookCells(
						editorControl.notebookEditor.getCellsInRange({ start: index, end: index + 1 }),
					)
