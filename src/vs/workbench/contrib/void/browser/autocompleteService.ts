...
const postprocessAutocompletion = ({
	...
	const numStartingNewlines =
		generatedMiddle.slice(startIdx).match(new RegExp(`^${_ln}+`))?.[0].length || 0
	if (
...
const toInlineCompletions = ({
	...
	let rangeToReplace: Range = new Range(
		position.lineNumber,
		position.column,
		position.lineNumber,
		position.column,
...
const getAutocompletionMatchup = ({
	...
	const lineStart =
		trimmedCurrentPrefix.split(_ln).length - trimmedCompletionPrefix.split(_ln).length
...
export class AutocompleteService extends Disposable implements IAutocompleteService {
	...
	async _provideInlineCompletionItems(
		...
		try {
			...
			const autocompletionMatchup: AutocompletionMatchupBounds = {
				startIdx: 0,
				startLine: 0,
				startCharacter: 0,
			}
