...
import {
	IConsistentEditorItemService,
	IConsistentItemService,
} from './helperServices/consistentItemService.js'
import {
	voidPrefixAndSuffix,
...
type StreamLocationMutable = {
	line: number
	col: number
	addedSplitYet: boolean
	originalCodeStartLine: number
}
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	private readonly _onDidChangeDiffsInDiffZoneNotStreaming = new Emitter<{
		uri: URI
		diffareaid: number
	}>()
	...
	private readonly _onDidChangeStreamingInCtrlKZone = new Emitter<{
		uri: URI
		diffareaid: number
	}>()
	...
	constructor(
		...
		@IConsistentEditorItemService
		private readonly _consistentEditorItemService: IConsistentEditorItemService,
		...
		private readonly _convertToLLMMessageService: IConvertToLLMMessageService,
	) {
		super()
	...
	private _onUserChangeContent(uri: URI, e: IModelContentChangedEvent) {
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] ?? []) {
			...
			const shouldDelete =
				diffArea?.type === 'DiffZone' && Object.keys(diffArea._diffOfId).length === 0
			if (shouldDelete) {
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					...
					const fn2 =
						diffArea._streamState.line + 1 <= diffArea.endLine
							? this._addLineDecoration(
									model,
									diffArea._streamState.line + 1,
									diffArea.endLine,
									'void-sweepBG',
								)
							: null
				...
				const fn = this._addLineDecoration(
					model,
					diffArea.startLine,
					diffArea.endLine,
					'void-highlightBG',
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		...
		const elt: IUndoRedoElement = {
			...
			redo: async () => {
				if (afterSnapshot) await this._restoreVoidFileSnapshot(uri, afterSnapshot)
			},
		}
	...
	private _realignAllDiffAreasLines(
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			// if the change contains only the diffArea's top
				...
				const numRemainingLinesInDA =
					diffArea.endLine - diffArea.startLine + 1 - numOverlappingLines
	...
	public instantlyApplySearchReplaceBlocks({
		uri,
		searchReplaceBlocks,
	}: {
		uri: URI
	...
	private _initializeWriteoverStream(
		...
		if (from === 'ClickApply') {
			const { messages: a, separateSystemMessage: b } =
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onText: (params) => {
						...
						const [croppedText, deltaCroppedText, croppedSuffix] = extractText(
							fullTextSoFar,
							newText.length,
						)
	...
	private _initializeSearchAndReplaceStream(
		...
		const convertOriginalRangeToFinalRange = (
					...
					for (let blockNum = currStreamingBlockNum; blockNum < blocks.length; blockNum += 1) {
						...
						if (!(blockNum in addedTrackingZoneOfBlockNum)) {
							...
							if (typeof originalBounds === 'string' || hasOverlap) {
								const errorMessage =
									typeof originalBounds === 'string' ? originalBounds : ('Has overlap' as const)
						...
						if (!diffZone._streamState.isStreaming) {
							console.error(
								'DiffZone was not in streaming state in _initializeSearchAndReplaceStream',
							)
							continue
						}
						...
						if (block.state === 'done') {
							const { startLine: finalStartLine, endLine: finalEndLine } =
								addedTrackingZoneOfBlockNum[blockNum]
							this._writeURIText(
								uri,
								block.final,
								{
	...
	public acceptOrRejectAllDiffAreas: IEditCodeService['acceptOrRejectAllDiffAreas'] = async ({
		...
		const { onFinishEdit } =
			_addToHistory === false ? { onFinishEdit: () => {} } : this._addToHistory(uri)
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		this._register(
			this._voidCommandBarService.onDidChangeState((e) => {
