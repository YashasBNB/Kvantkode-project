...
let directoryHistory: ITerminalPersistedHistory<{ remoteAuthority?: string }> | undefined =
	undefined
...
let commandHistory: ITerminalPersistedHistory<{ shellType: TerminalShellType }> | undefined =
	undefined
...
const shellFileHistory: Map<TerminalShellType | undefined, IShellFileHistoryEntry | null> =
	new Map()
...
export async function fetchPwshHistory(
	...
	const remoteEnvironment = await remoteAgentService.getEnvironment()
	const isFileWindows =
		remoteEnvironment?.os === OperatingSystem.Windows || (!remoteEnvironment && isWindows)
