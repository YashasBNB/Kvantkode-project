...
export class TerminalLinkManager extends DisposableStore {
	...
	constructor(
		...
		this._openers.set(
			...
			this._instantiationService.createInstance(TerminalLocalFolderOutsideWorkspaceLinkOpener),
		)
		this._openers.set(
	...
	private _setupLinkDetector(
		...
		const detectorAdapter = this.add(
			this._instantiationService.createInstance(TerminalLinkDetectorAdapter, detector),
		)
		this.add(
	...
	async getLinks(): Promise<{ viewport: IDetectedLinks; all: Promise<IDetectedLinks> }> {
		...
		const viewportLinks: Required<
			Pick<IDetectedLinks, 'wordLinks' | 'webLinks' | 'fileLinks' | 'folderLinks'>
		> = {
			wordLinks: [],
			webLinks: [],
			fileLinks: [],
		...
		const allLinks: Promise<
			Required<Pick<IDetectedLinks, 'wordLinks' | 'webLinks' | 'fileLinks' | 'folderLinks'>>
	...
	private _registerStandardLinkProviders(): void {
		 ...
		 */
		const proxyLinkProvider: OmitFirstArg<ITerminalExternalLinkProvider['provideLinks']> = async (
			bufferLineNumber,
		) => {
