...
registerAction2(
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor, item: ITimelineCommandArgument): Promise<void> {
			...
			const selectedEntry = (
				await findLocalHistoryEntry(workingCopyHistoryService, itemSelectedForCompare)
			).entry
			if (!selectedEntry) {
				return
			}
	...
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const entryPicker = entryPickerDisposables.add(
				quickInputService.createQuickPick<IQuickPickItem & { entry: IWorkingCopyHistoryEntry }>(),
			)
