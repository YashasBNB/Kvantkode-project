	...
	implements IWorkspaceExtensionsConfigService
{
	...
	async getUnwantedRecommendations(): Promise<string[]> {
		...
		return distinct(
			configs.flatMap((c) =>
				c.unwantedRecommendations ? c.unwantedRecommendations.map((c) => c.toLowerCase()) : [],
			),
		)
	}
	...
	private async pickWorkspaceOrFolders(
		...
		const result =
			(await this.quickInputService.pick(folderPicks, { placeHolder, canPickMany: true })) || []
	...
	private parseExtensionConfig(
		...
		return {
			recommendations: distinct(
				(extensionsConfigContent.recommendations || []).map((e) => e.toLowerCase()),
			),
