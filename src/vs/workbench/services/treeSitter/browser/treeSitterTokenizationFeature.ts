...
export const ITreeSitterTokenizationFeature = createDecorator<ITreeSitterTokenizationFeature>(
...
export class TreeSitterTokenizationFeature
	extends Disposable
	implements ITreeSitterTokenizationFeature
{
	public _serviceBrand: undefined
	private readonly _tokenizersRegistrations: DisposableMap<string, DisposableStore> =
	...
	private _handleGrammarsExtPoint(): void {
		...
		for (const languageId of TREESITTER_ALLOWED_SUPPORT) {
			...
			if (setting && !this._tokenizersRegistrations.has(languageId)) {
				...
				disposableStore.add(
					TreeSitterTokenizationRegistry.registerFactory(languageId, lazyTokenizationSupport),
				)
				this._tokenizersRegistrations.set(languageId, disposableStore)
				TreeSitterTokenizationRegistry.getOrCreate(languageId)
			}
		}
		const languagesToUnregister = [...this._tokenizersRegistrations.keys()].filter(
			(languageId) => !this._getSetting(languageId),
		)
...
export class TreeSitterTokenizationSupport
	extends Disposable
	implements ITreeSitterTokenizationSupport
{
	...
	private readonly _onDidCompleteBackgroundTokenization: Emitter<{ textModel: ITextModel }> =
		this._register(new Emitter())
	public readonly onDidChangeBackgroundTokenization: Event<{ textModel: ITextModel }> =
	...
	constructor(
		...
		private readonly _tokenizationStoreService: ITreeSitterTokenizationStoreService,
		@IInstantiationService private readonly _instantiationService: IInstantiationService,
	) {
		super()
		...
		this._register(
			Event.runAndSubscribe(this._themeService.onDidColorThemeChange, (e) => this._updateTheme(e)),
		)
		this._register(
	...
	private async _parseAndTokenizeViewPort(model: ITextModel, viewportRanges: Range[]) {
		...
		for (const range of viewportRanges) {
			...
			const oldRangeLength =
				lastToken.startOffsetInclusive + lastToken.length - tokenUpdates[0].startOffsetInclusive
	...
	private _handleTreeUpdate(
		...
		textModel: ITextModel,
		versionId: number,
		textModelTreeSitter: ITextModelTreeSitter,
	) {
		...
		const captures = rangeChanges.map((range) =>
			this._getCaptures(range.range, textModelTreeSitter, tree),
		)
		// Don't block
	...
	captureAtRangeTree(
		range: Range,
		tree: Parser.Tree,
		textModelTreeSitter: ITextModelTreeSitter | undefined,
	): QueryCapture[] {
	...
	public tokenizeEncoded(lineNumber: number, textModel: ITextModel) {
		...
		if (tokens.versionId === textModel.getVersionId()) {
			this._tokenizationStoreService.updateTokens(
				textModel,
				tokens.versionId,
				[{ newTokens: updates, oldRangeLength: textModel.getLineLength(lineNumber) }],
				TokenQuality.Accurate,
			)
		}
	}
	...
	private _createTokensFromCaptures(
		...
		for (let captureIndex = 0; captureIndex < captures.length; captureIndex++) {
			...
			const tokenStartIndex =
				capture.node.startIndex < rangeStartOffset ? rangeStartOffset : capture.node.startIndex
			...
			if (previousEndOffset >= endOffset) {
				...
				let previousTokenStartOffset =
					withinTokenIndex >= 2 ? endOffsetsAndScopes[withinTokenIndex - 1].endOffset : 0
				do {
	...
	private _tokenizeCapturesWithMetadata(
		...
		| { endOffsetsAndMetadata: EndOffsetToken[]; captureTime: number; metadataTime: number }
		| undefined {
		const stopwatch = StopWatch.create()
	...
	private _tokenizeEncoded(
		...
		| { result: EndOffsetToken[]; captureTime: number; metadataTime: number; versionId: number }
		| undefined {
