...
class ViewDescriptorsState extends Disposable {
	...
	private getStoredWorkspaceState(): IStringDictionary<IStoredWorkspaceViewState> {
		return JSON.parse(
			this.storageService.get(this.workspaceViewsStateStorageId, StorageScope.WORKSPACE, '{}'),
		)
	}
...
export class ViewContainerModel extends Disposable implements IViewContainerModel {
	...
	constructor(
		...
		this._register(
			this.viewDescriptorsState.onDidChangeStoredState((items) => this.updateVisibility(items)),
		)
	...
	private updateContainerInfo(): void {
		...
		const useDefaultContainerInfo =
			...
			this.visibleViewDescriptors.some(
				(v) =>
					Registry.as<IViewsRegistry>(ViewExtensions.ViewsRegistry).getViewContainer(v.id) ===
					this.viewContainer,
			)
