...
suite('EditorService', () => {
	...
	setup(() => {
		disposables.add(
			registerTestEditor(
				TEST_EDITOR_ID,
				[new SyncDescriptor(TestFileEditorInput), new SyncDescriptor(TestSingletonFileEditorInput)],
	...
	async function testOpenBasics(
		...
		assert.strictEqual(
			editorService.getVisibleTextEditorControls(EditorsOrder.MOST_RECENTLY_ACTIVE).length,
			0,
		)
		...
		const sequentialEditorsExcludingSticky = editorService.getEditors(EditorsOrder.SEQUENTIAL, {
			excludeSticky: true,
		})
		...
		const mruEditorsExcludingSticky = editorService.getEditors(EditorsOrder.MOST_RECENTLY_ACTIVE, {
			excludeSticky: true,
		})
	...
	test('openEditor() - singleton typed editors reveal instead of split', async () => {
		...
		const input1 = disposables.add(
			new TestSingletonFileEditorInput(URI.parse('my://resource-basics1'), TEST_EDITOR_INPUT_ID),
		)
		const input2 = disposables.add(
			new TestSingletonFileEditorInput(URI.parse('my://resource-basics2'), TEST_EDITOR_INPUT_ID),
		)
	...
	async function testOpenEditors(useOpenEditors: boolean) {
						...
						return {
							editor: createTestFileEditorInput(
								untitledEditor.resource ??
									URI.parse(`untitled://my-untitled-editor-${untitledEditorFactoryCalled}`),
								TEST_EDITOR_INPUT_ID,
							),
						}
					},
		...
		// untyped
		{
			...
			// untyped resource editor, options (sticky: true, preserveFocus: true), no group
			{
				...
				assert.strictEqual(
					(lastEditorFactoryEditor as IResourceEditorInput).options?.preserveFocus,
					true,
				)
				assert.ok(!lastUntitledEditorFactoryEditor)
				assert.ok(!lastDiffEditorFactoryEditor)
		...
		// Untyped untitled
		{
			...
			// untyped untitled editor, options (sticky: true, preserveFocus: true), no group
			{
				...
				assert.strictEqual(lastUntitledEditorFactoryEditor, untypedEditor)
				assert.strictEqual(
					(lastUntitledEditorFactoryEditor as IUntitledTextResourceEditorInput).options
						?.preserveFocus,
					true,
				)
				assert.strictEqual(
					(lastUntitledEditorFactoryEditor as IUntitledTextResourceEditorInput).options?.sticky,
					true,
				)
				assert.ok(!lastDiffEditorFactoryEditor)
		...
		// Untyped diff
		{
			...
			// untyped diff editor, options (sticky: true, preserveFocus: true), no group
			{
				...
				assert.strictEqual(lastDiffEditorFactoryEditor, untypedEditor)
				assert.strictEqual(
					(lastDiffEditorFactoryEditor as IUntitledTextResourceEditorInput).options?.preserveFocus,
					true,
				)
				assert.strictEqual(
					(lastDiffEditorFactoryEditor as IUntitledTextResourceEditorInput).options?.sticky,
					true,
				)
	...
	test('openEditor() applies options if editor already opened', async () => {
		...
		let pane = await service.openEditor(
			createTestFileEditorInput(URI.parse('my://resource-openEditors'), TEST_EDITOR_INPUT_ID),
		)
	...
	test('openEditors() handles workspace trust (typed editors)', async () => {
		...
		try {
			...
			assert.strictEqual(
				trustEditorUris.some((uri) => uri.toString() === input1.resource.toString()),
				true,
			)
			assert.strictEqual(
				trustEditorUris.some((uri) => uri.toString() === input2.resource.toString()),
				true,
			)
			assert.strictEqual(
				trustEditorUris.some((uri) => uri.toString() === input3.resource.toString()),
				true,
			)
			assert.strictEqual(
				trustEditorUris.some((uri) => uri.toString() === input4.resource.toString()),
				true,
			)
	...
	test('openEditors() extracts proper resources from untyped editors for workspace trust', async () => {
		...
		try {
			...
			assert.strictEqual(
				trustEditorUris.some((uri) => uri.toString() === input.resource.toString()),
				true,
			)
			assert.strictEqual(
				trustEditorUris.some((uri) => uri.toString() === otherInput.original.resource?.toString()),
				true,
			)
			assert.strictEqual(
				trustEditorUris.some((uri) => uri.toString() === otherInput.modified.resource?.toString()),
				true,
			)
		} finally {
			accessor.workspaceTrustRequestService.requestOpenUrisHandler = oldHandler
	...
	test('editor group activation', async () => {
		...
		let editor = await service.openEditor(
			input2,
			{ pinned: true, preserveFocus: true, activation: EditorActivation.ACTIVATE },
			SIDE_GROUP,
		)
		...
		editor = await service.openEditor(
			input1,
			{ pinned: true, preserveFocus: true, activation: EditorActivation.PRESERVE },
			rootGroup,
		)
		assert.strictEqual(part.activeGroup, sideGroup)
		...
		editor = await service.openEditor(
			input1,
			{ pinned: true, preserveFocus: true, activation: EditorActivation.RESTORE },
			rootGroup,
		)
		assert.strictEqual(part.activeGroup, sideGroup)
	})
	...
	test('findEditors (in group)', async () => {
		...
		// Make sure we don't find editors across groups
		{
			const newEditor = await service.openEditor(
				createTestFileEditorInput(URI.parse('my://other-group-resource'), TEST_EDITOR_INPUT_ID),
