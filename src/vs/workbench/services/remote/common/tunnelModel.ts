...
export class TunnelModel extends Disposable {
	...
	public onCandidatesChanged: Event<Map<string, { host: string; port: number }>> =
	...
	private async doForward(
		...
		attributes =
			...
			(attributes !== null
				...
				: undefined)
		const localPort =
			tunnelProperties.local !== undefined ? tunnelProperties.local : tunnelProperties.remote.port
	...
	private async mergeAttributesIntoExistingTunnel(
		...
		if (
			(attributes?.protocol || existingTunnel.protocol !== TunnelProtocol.Http) &&
	...
	setCandidateFilter(
		filter: ((candidates: CandidatePort[]) => Promise<CandidatePort[]>) | undefined,
	): void {
	...
	async setCandidates(candidates: CandidatePort[]) {
		...
		this.logService.trace(
			`ForwardedPorts: (TunnelModel) removed candidates ${Array.from(removedCandidates.values())
