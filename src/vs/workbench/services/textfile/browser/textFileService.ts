...
export abstract class AbstractTextFileService extends Disposable implements ITextFileService {
		...
		const provider = this._register(
			new (class extends Disposable implements IDecorationsProvider {
				...
				private registerListeners(): void {
					...
					// Changes
					this._register(
						this.files.onDidChangeReadonly((model) => this._onDidChange.fire([model.resource])),
					)
					this._register(
						this.files.onDidChangeOrphaned((model) => this._onDidChange.fire([model.resource])),
					)
				}
	...
	private async doRead(
		...
		try {
			...
			if (
				(<DecodeStreamError>error).decodeStreamErrorKind === DecodeStreamErrorKind.STREAM_IS_BINARY
			) {
	...
	getEncoding(resource: URI): string {
		const model =
			resource.scheme === Schemas.untitled ? this.untitled.get(resource) : this.files.get(resource)
	...
	private async suggestSavePath(resource: URI): Promise<URI> {
		...
		if (resource.scheme === Schemas.untitled) {
			...
			if (model) {
				...
				if (
					await this.pathService.hasValidBasename(joinPath(defaultFilePath, model.name), model.name)
				) {
...
export class EncodingOracle extends Disposable implements IResourceEncodings {
	...
	async getPreferredReadEncoding(
		...
		// Encoding configured
			...
			this.textResourceConfigurationService.getValue(resource, 'files.encoding') === UTF8_with_bom
		) {
