	...
	implements IWorkbenchConfigurationService, IWorkspaceContextService
{
	...
	constructor(
		...
		this.defaultConfiguration = this._register(
			new DefaultConfiguration(configurationCache, environmentService, logService),
		)
		this.policyConfiguration =
			policyService instanceof NullPolicyService
				? new NullPolicyConfiguration()
				: this._register(
						new PolicyConfiguration(this.defaultConfiguration, policyService, logService),
					)
		...
		this._register(
			userDataProfileService.onDidChangeCurrentProfile((e) => this.onUserDataProfileChanged(e)),
		)
	...
	public isCurrentWorkspace(
		...
		switch (this.getWorkbenchState()) {
			...
			case WorkbenchState.WORKSPACE:
				return (
					isWorkspaceIdentifier(workspaceIdOrFolder) && this.workspace.id === workspaceIdOrFolder.id
				)
		}
		return false
	}
	...
	async updateValue(key: string, value: any, arg3?: any, arg4?: any, options?: any): Promise<void> {
		...
		await Promises.settled(
			targets.map((target) => this.writeConfigurationValue(key, value, target, overrides, options)),
		)
	}
	...
	private onApplicationConfigurationChanged(applicationConfiguration: ConfigurationModel): void {
		...
		const previousAllProfilesSettings =
			...
			[]
		const change =
			this._configuration.compareAndUpdateApplicationConfiguration(applicationConfiguration)
	...
	private updateRestrictedSettings(): void {
		...
		const userLocal = this.localUserConfiguration
			...
			.sort((a, b) => a.localeCompare(b))
		const userLocalDelta = delta(userLocal, this._restrictedSettings.userLocal || [], (a, b) =>
			a.localeCompare(b),
		)
		changed.push(...userLocalDelta.added, ...userLocalDelta.removed)

		const userRemote = (this.remoteUserConfiguration?.getRestrictedSettings() || []).sort((a, b) =>
			a.localeCompare(b),
		)
		const userRemoteDelta = delta(userRemote, this._restrictedSettings.userRemote || [], (a, b) =>
			a.localeCompare(b),
		)
		...
		for (const workspaceFolder of this.workspace.folders) {
			...
			const workspaceFolderDelta = delta(folderRestrictedSettings, previous, (a, b) =>
				a.localeCompare(b),
			)
		...
		const workspace =
			this.getWorkbenchState() === WorkbenchState.WORKSPACE
				...
				: this.workspace.folders[0]
					...
					: []
		const workspaceDelta = delta(workspace, this._restrictedSettings.workspace || [], (a, b) =>
			a.localeCompare(b),
		)
		...
		switch (editableConfigurationTarget) {
			...
			case EditableConfigurationTarget.WORKSPACE_FOLDER: {
				const workspaceFolder =
					overrides && overrides.resource ? this.workspace.getFolder(overrides.resource) : null
				if (workspaceFolder) {
	...
	implements IWorkbenchContribution
{
	...
	private registerConfigurationSchemas(): void {
		...
		const defaultSettingsSchema = {
			...
			patternProperties: Object.keys(allSettings.patternProperties).reduce<IJSONSchemaMap>(
				(result, key) => {
