...
suite('ConfigurationEditing', () => {
	...
	test('errors cases - invalid global tasks configuration', async () => {
		...
		try {
			await testObject.writeConfiguration(
				EditableConfigurationTarget.USER_LOCAL,
				{ key: 'tasks.configurationEditing.service.testSetting', value: 'value' },
				{ donotNotifyError: true },
			)
		} catch (error) {
			assert.strictEqual(error.code, ConfigurationEditingErrorCode.ERROR_INVALID_CONFIGURATION)
	...
	test('errors cases - ERROR_POLICY_CONFIGURATION', async () => {
		await runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				...
				VSBuffer.fromString('{ "configurationEditing.service.policySetting": "policyValue" }'),
			)
			await promise
		})
		try {
	...
	test('write one setting - empty file', async () => {
		await testObject.writeConfiguration(EditableConfigurationTarget.USER_LOCAL, {
	...
	test('write overridable settings to workspace settings', async () => {
		...
		const contents = await fileService.readFile(
			joinPath(workspaceService.getWorkspace().folders[0].uri, FOLDER_SETTINGS_PATH),
		)
		const parsed = json.parse(contents.value.toString())
		assert.deepStrictEqual(parsed[key], value)
	})
