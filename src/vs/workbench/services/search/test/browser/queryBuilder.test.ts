...
suite('QueryBuilder', () => {
	...
	test('does not split include pattern when expandPatterns disabled', () => {
		assertEqualQueries(
			queryBuilder.file([ROOT_1_NAMED_FOLDER], { includePattern: '**/foo, **/bar' }),
			{
				folderQueries: [
					{
						folder: ROOT_1_URI,
	...
	test('multiroot exclude settings', () => {
		...
		mockWorkspace.folders = toWorkspaceFolders(
			[{ path: ROOT_1_URI.fsPath }, { path: ROOT_2_URI.fsPath }, { path: ROOT_3_URI.fsPath }],
			WS_CONFIG_PATH,
	...
	suite('parseSearchPaths 1', () => {
		...
		test('include ./foldername', () => {
			...
			mockWorkspace.folders = toWorkspaceFolders(
				[{ path: ROOT_1_URI.fsPath, name: ROOT_1_FOLDERNAME }, { path: getUri(ROOT_2).fsPath }],
				WS_CONFIG_PATH,
				extUriBiasedIgnorePathCase,
			)
			mockWorkspace.configuration = uri.file(fixPath('config'))
