...
export class SearchService implements IRawSearchService {
	...
	private doSortedSearch(
		...
		return allResultsPromise.then(([result, results]) => {
			...
			const sortSW =
				(typeof config.maxResults !== 'number' || config.maxResults > 0) && StopWatch.create(false)
	...
	private sortResults(
		...
		const compare = (matchA: IRawFileMatch, matchB: IRawFileMatch) =>
			compareItemsByFuzzyScore(matchA, matchB, query, true, FileMatchItemAccessor, scorerCache)

		const maxResults =
			typeof config.maxResults === 'number' ? config.maxResults : DEFAULT_MAX_SEARCH_RESULTS
	...
	private getResultsFromCache(
		...
		progressCallback: IFileProgressCallback,
		token?: CancellationToken,
	): Promise<[ISearchEngineSuccess, IRawFileMatch[], ICachedSearchStats]> | null {
		...
		return cachedRow.promise.then<[ISearchEngineSuccess, IRawFileMatch[], ICachedSearchStats]>(
	...
	private preventCancellation<C>(promise: CancelablePromise<C>): CancelablePromise<C> {
		return new (class implements CancelablePromise<C> {
			...
			then<TResult1 = C, TResult2 = never>(
				resolve?: ((value: C) => TResult1 | Promise<TResult1>) | undefined | null,
