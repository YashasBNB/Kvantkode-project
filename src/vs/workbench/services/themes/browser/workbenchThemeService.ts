...
export class WorkbenchThemeService extends Disposable implements IWorkbenchThemeService {
	...
	constructor(
		...
		if (!themeData) {
			const colorScheme =
				this.settings.getPreferredColorScheme() ?? (isWeb ? ColorScheme.LIGHT : ColorScheme.DARK)
				...
				this.fileIconThemeRegistry.onDidChange(async (event) => {
					...
					if (await this.restoreFileIconTheme()) {
						...
						if (
							...
							event.added.some((t) => t.settingsId === this.currentFileIconTheme.settingsId)
						) {
							await this.reloadCurrentFileIconTheme()
						}
					} else if (
						event.removed.some((t) => t.settingsId === this.currentFileIconTheme.settingsId)
					) {
						// current theme is no longer available
			...
			this.productIconThemeRegistry.onDidChange(async (event) => {
				...
				if (await this.restoreProductIconTheme()) {
					...
					if (
						...
						event.added.some((t) => t.settingsId === this.currentProductIconTheme.settingsId)
					) {
						await this.reloadCurrentProductIconTheme()
					}
				} else if (
					event.removed.some((t) => t.settingsId === this.currentProductIconTheme.settingsId)
				) {
					// current theme is no longer available
	...
	private reloadCurrentColorTheme() {
		return this.colorThemeSequencer.queue(async () => {
			try {
				const theme =
					this.colorThemeRegistry.findThemeBySettingsId(this.currentColorTheme.settingsId) ||
