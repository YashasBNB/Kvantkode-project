...
export class WorkbenchLanguageService extends LanguageService {
	...
	constructor(
		...
		languagesExtPoint.setHandler(
			(extensions: readonly IExtensionPointUser<IRawLanguageExtensionPoint[]>[]) => {
...
function isValidLanguageExtensionPoint(
	...
	if (!value) {
		collector?.error(
			localize('invalid.empty', 'Empty value for `contributes.{0}`', languagesExtPoint.name),
		)
		return false
	}
	if (typeof value.id !== 'string') {
		collector?.error(
			localize('require.id', 'property `{0}` is mandatory and must be of type `string`', 'id'),
		)
		return false
	}
