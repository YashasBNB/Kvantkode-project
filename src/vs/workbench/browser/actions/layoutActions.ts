...
class MoveFocusedViewAction extends Action2 {
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		if (focusedViewId === undefined || focusedViewId.trim() === '') {
			dialogService.error(
				localize('moveFocusedView.error.noFocusedView', 'There is no view currently focused.'),
			)
			return
		}
		...
		const isViewSolo =
			viewDescriptorService.getViewContainerModel(currentContainer).allViewDescriptors.length === 1
...
registerAction2(
	class extends Action2 {
		...
		run(accessor: ServicesAccessor): void {
			...
			if (!viewDescriptor) {
				dialogService.error(
					localize('resetFocusedView.error.noFocusedView', 'There is no view currently focused.'),
				)
				return
			}
...
ToggleVisibilityActions.push(
	...[
		...
		CreateToggleLayoutItem(
			...
			AuxiliaryBarVisibleContext,
			localize('secondarySideBar', 'KvantKode Side Bar'),
			{
				whenA: ContextKeyExpr.equals('config.workbench.sideBar.location', 'left'),
