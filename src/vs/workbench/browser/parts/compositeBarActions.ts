...
export class CompositeBarActionViewItem extends BaseActionViewItem {
	...
	protected updateStyles(): void {
		...
		if (this.badgeContent) {
			...
			const badgeFg =
				badgeStyles?.badgeForeground ?? colors.badgeForeground ?? theme.getColor(badgeForeground)
			const badgeBg =
				badgeStyles?.badgeBackground ?? colors.badgeBackground ?? theme.getColor(badgeBackground)
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		for (let i = 0; i < visibleCompositeIds.length; i++) {
			if (visibleCompositeIds[i] === activeComposite.getId()) {
				targetCompositeId =
					visibleCompositeIds[
						(i + visibleCompositeIds.length + this.offset) % visibleCompositeIds.length
					]
				break
			}
		}
