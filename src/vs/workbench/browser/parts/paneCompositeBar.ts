		...
		return this._register(
			this.instantiationService.createInstance(CompositeBar, cachedItems, {
				...
				openComposite: async (compositeId, preserveFocus) => {
					return (
						(await this.paneCompositePart.openPaneComposite(compositeId, !preserveFocus)) ?? null
					)
				},
		...
		if (!compositeActions) {
			...
			if (viewContainer) {
				...
				compositeActions = {
					...
					pinnedAction: this._register(
						new PlaceHolderToggleCompositePinnedAction(compositeId, this.compositeBar),
					),
					badgeAction: this._register(
						new PlaceHolderToggleCompositeBadgeAction(compositeId, this.compositeBar),
					),
				}
			}
			...
			cachedViewContainers.map(
				({ id, pinned, order }) =>
					({
						...
						pinned,
						visible: Boolean(
							pinnedViewContainers.find(({ id: pinnedId }) => pinnedId === id)?.visible,
						),
						order,
