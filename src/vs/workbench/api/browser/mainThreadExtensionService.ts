...
export class MainThreadExtensionService implements MainThreadExtensionServiceShape {
	...
	async $onExtensionActivationError(
		...
		const isDev =
			!this._environmentService.isBuilt || this._environmentService.isExtensionDevelopment
		if (isDev) {
	...
	private async _handleMissingInstalledDependency(
		...
		if (this._extensionEnablementService.isEnabled(missingInstalledDependency)) {
			this._notificationService.notify({
				...
				actions: {
					primary: [
						new Action('reload', localize('reload', 'Reload Window'), '', true, () =>
			...
			if (enablementState === EnablementState.DisabledByVirtualWorkspace) {
				...
				this._notificationService.notify({
					...
					actions: {
						primary: [
							new Action('enable', localize('enable dep', 'Enable and Reload'), '', true, () =>
