			...
			invalidateTestResults: (items) => {
				if (items === undefined) {
					...
					this.proxy.$markTestRetired(
						itemsArr.map((i) => TestId.fromExtHostTestItem(i!, controllerId).toString()),
					)
				}
			},
				...
				targets: [
					{
						testIds: req.include?.map((t) =>
							TestId.fromExtHostTestItem(t, controller.collection.root.id).toString(),
...
class TestRunTracker extends Disposable {
	...
	private ensureTestIsKnown(test: vscode.TestItem) {
		...
		if (
			this.sharedTestIds.has(TestId.fromExtHostTestItem(test, this.dto.controllerId).toString())
		) {
			return
		}
...
export class TestRunCoordinator {
	...
	public createTestRun(
		...
		this.proxy.$startedExtensionTestRun({
			...
			exclude:
				request.exclude?.map((t) => TestId.fromExtHostTestItem(t, collection.root.id).toString()) ??
				[],
			id: dto.id,
