...
const newCommands: ApiCommand[] = [
	...
	new ApiCommand(
		...
		>('A promise that resolves to an array of Command-instances.', (value, _args, converter) => {
			return tryMapWith<CustomCodeAction, vscode.CodeAction | vscode.Command | undefined>(
				(codeAction) => {
	...
	new ApiCommand(
		...
		[
			new ApiCommandArgument<URI | string>(
				...
				(v) =>
					URI.isUri(v) ||
					(typeof v === 'string' && matchesSomeScheme(v, Schemas.http, Schemas.https)),
				(v) => v,
			),
			new ApiCommandArgument<
	...
	new ApiCommand(
		...
		[
			new ApiCommandArgument<InlineChatEditorApiArg | undefined, InlineChatRunOptions | undefined>(
				'Run arguments',
				'',
				(_v) => true,
				(v) => {
