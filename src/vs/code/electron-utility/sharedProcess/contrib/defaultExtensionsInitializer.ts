...
export class DefaultExtensionsInitializer extends Disposable {
	constructor(
		...
		if (
			isWindows &&
			storageService.getBoolean(defaultExtensionsInitStatusKey, StorageScope.APPLICATION, true)
		) {
	...
	private getDefaultExtensionVSIXsLocation(): URI {
		...
		return URI.file(
			join(dirname(dirname(this.environmentService.appRoot)), 'bootstrap', 'extensions'),
		)
	}
}
