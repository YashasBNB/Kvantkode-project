...
class SharedProcessMain extends Disposable implements IClientConnectionFilter {
	...
	private async initServices(): Promise<IInstantiationService> {
		...
		services.set(
			IChecksumService,
			new SyncDescriptor(ChecksumService, undefined, false /* proxied to other processes */),
		)
		...
		services.set(
			...
			new ExtensionRecommendationNotificationServiceChannelClient(
				this.server.getChannel('extensionRecommendationNotification', activeWindowRouter),
			),
		)
		...
		services.set(
			IDiagnosticsService,
			new SyncDescriptor(DiagnosticsService, undefined, false /* proxied to other processes */),
		)
		...
		services.set(
			...
			ProxyChannel.toService(
				this.server.getChannel('userDataSyncUtil', (client) => client.ctx !== 'main'),
			),
		)
		services.set(
			IGlobalExtensionEnablementService,
		...
		services.set(
			IUserDataSyncService,
			new SyncDescriptor(UserDataSyncService, undefined, false /* Initializes the Sync State */),
		)
		services.set(
			IUserDataProfileStorageService,
			new SyncDescriptor(SharedProcessUserDataProfileStorageService, undefined, true),
		)
		services.set(
			IUserDataSyncResourceProviderService,
		...
		services.set(
			ISignService,
			new SyncDescriptor(SignService, undefined, false /* proxied to other processes */),
		)
	...
	private initChannels(accessor: ServicesAccessor): void {
		...
		this.server.registerChannel(
			'IUserDataSyncResourceProviderService',
			ProxyChannel.fromService(accessor.get(IUserDataSyncResourceProviderService), this._store),
		)
