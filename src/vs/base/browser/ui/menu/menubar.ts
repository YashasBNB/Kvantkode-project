...
export class MenuBar extends Disposable {
	...
	constructor(
		...
		this._register(
			DOM.addDisposableListener(this.container, DOM.EventType.KEY_DOWN, (e) => {
				...
				if (
					!this.isCompact &&
					(event.equals(KeyCode.Tab | KeyMod.Shift) || event.equals(KeyCode.Tab))
				) {
	...
	private updateOverflowAction(): void {
		...
		if (this.isCompact) {
			...
			while (
				currentSize + this.overflowMenu.buttonElement.offsetWidth > sizeAvailable &&
				this.numMenusShown > 0
			) {
				this.numMenusShown--
		...
		if (this.options.enableMnemonics) {
			...
			if (escMatch) {
				...
				titleElement.append(
					...
					strings.rtrim(
						replaceDoubleEscapes(cleanLabel.substr(escMatch.index + escMatch[0].length)),
						' ',
					),
				)
			} else {
	...
	private showCustomMenu(menuIndex: number, selectFirst = true): void {
		...
		const customMenu =
			actualMenuIndex === MenuBar.OVERFLOW_INDEX ? this.overflowMenu : this.menus[actualMenuIndex]
		...
		const menuOptions: IMenuOptions = {
			...
			enableMnemonics:
				this.options.alwaysOnMnemonics || (this.mnemonicsInUse && this.options.enableMnemonics),
