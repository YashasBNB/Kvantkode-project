...
function asListOptions<T, TFilterData, TRef>(
	...
	return (
		options && {
			...
			dnd:
				options.dnd && disposableStore.add(new TreeNodeListDragAndDrop(modelProvider, options.dnd)),
...
class FindWidget<T, TFilterData> extends Disposable {
	...
	constructor(
		...
		this.toggles = toggleContributions.map((contribution) =>
			this._register(new TreeFindToggle(contribution, styles.toggleStyles, toggleHoverDelegate)),
		)
		this.onDidToggleChange = Event.any(
			...this.toggles.map((toggle) =>
				Event.map(toggle.onChange, () => ({ id: toggle.id, isChecked: toggle.checked })),
			),
		)
			...
			model.onDidSpliceRenderedNodes((e) => {
				...
				const hasRemovedStickyNode =
					...
					state.stickyNodes.some(
						(stickyNode) => !this.model.has(this.model.getNodeLocation(stickyNode.node)),
					)
