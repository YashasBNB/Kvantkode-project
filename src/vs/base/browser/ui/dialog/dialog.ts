		...
		return new Promise<IDialogResult>((resolve) => {
			...
			buttonMap.forEach((_, index) => {
				...
				if (primary && this.options?.primaryButtonDropdown) {
					...
					button = this._register(
						buttonBar.addButtonWithDescription({ secondary: !primary, ...this.buttonStyles }),
					)
				} else {
					button = this._register(
					...
					(e) => {
						...
						if (evt.equals(KeyCode.Enter)) {
							...
							if (this.inputs.some((input) => input.hasFocus())) {
								...
								resolve({
									button:
										buttonMap.find((button) => button.index !== this.options.cancelId)?.index ?? 0,
									checkboxChecked: this.checkbox ? this.checkbox.checked : undefined,
									values:
										this.inputs.length > 0 ? this.inputs.map((input) => input.value) : undefined,
								})
							}
