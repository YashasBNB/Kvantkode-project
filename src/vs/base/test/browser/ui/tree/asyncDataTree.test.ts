...
suite('AsyncDataTree', function () {
	...
	test("issue #192422 - resolved collapsed nodes with changed children don't show old children", async () => {
		...
		const bChild = container.querySelector('.monaco-list-row:nth-child(2)') as
			| HTMLElement
			| undefined
		...
		const event = tree.onDidChangeCollapseState((e) => {
			const child = container.querySelector('.monaco-list-row:nth-child(2)') as
				| HTMLElement
				| undefined
	...
	test('issues #84569, #82629 - rerender', async () => {
		...
		await tree.expand(model.get('a'))
		assert.deepStrictEqual(
			Array.from(container.querySelectorAll('.monaco-list-row')).map((e) => e.textContent),
			['a', 'b1'],
		)
	...
	test('Tree Navigation: AsyncDataTree', async () => {
		...
		assert.deepStrictEqual(
			Array.from(container.querySelectorAll('.monaco-list-row')).map((e) => e.textContent),
			['a', 'aa', 'aa.txt', 'ab', 'ab.txt', 'b', 'ba', 'ba.txt', 'bb', 'bb.txt'],
		)
