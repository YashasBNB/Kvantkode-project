...
suite('ConfigurationService.test.ts', () => {
	...
	test('model defaults', () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			await fileService.writeFile(
				settingsResource,
				VSBuffer.fromString('{ "configuration.service.testSetting": "isChanged" }'),
			)
	...
	test('update configuration throws error for policy setting', async () => {
		...
		await fileService.writeFile(
			policyFile,
			VSBuffer.fromString('{ "configurationService.policySetting": "policyValue" }'),
		)
		const policyService = disposables.add(
			new FilePolicyService(policyFile, fileService, logService),
		)
		const testObject = disposables.add(
			new ConfigurationService(settingsResource, fileService, policyService, logService),
		)
		await testObject.initialize()
