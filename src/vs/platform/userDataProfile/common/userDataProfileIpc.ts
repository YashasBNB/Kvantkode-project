...
export class UserDataProfilesService extends Disposable implements IUserDataProfilesService {
	...
	async createNamedProfile(
		...
		const result = await this.channel.call<UriDto<IUserDataProfile>>('createNamedProfile', [
			name,
			options,
			workspaceIdentifier,
		])
	...
	async createProfile(
		...
		const result = await this.channel.call<UriDto<IUserDataProfile>>('createProfile', [
			id,
			name,
			options,
			workspaceIdentifier,
	...
	async createTransientProfile(
		...
		const result = await this.channel.call<UriDto<IUserDataProfile>>('createTransientProfile', [
			workspaceIdentifier,
		])
		return reviveProfile(result, this.profilesHome.scheme)
	}
	...
	async setProfileForWorkspace(
		...
		await this.channel.call<UriDto<IUserDataProfile>>('setProfileForWorkspace', [
			workspaceIdentifier,
			profile,
		])
	}
	...
	async updateProfile(
		...
		const result = await this.channel.call<UriDto<IUserDataProfile>>('updateProfile', [
			profile,
			updateOptions,
		])
		return reviveProfile(result, this.profilesHome.scheme)
