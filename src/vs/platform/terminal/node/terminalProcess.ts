...
export class TerminalProcess extends Disposable implements ITerminalChildProcess {
			...
			this.onProcessReady((e) => {
				...
				this._register(
					this._windowsShellHelper.onShellTypeChanged((e) =>
						this._onDidChangeProperty.fire({ type: ProcessPropertyType.ShellType, value: e }),
					),
				)
				this._register(
					this._windowsShellHelper.onShellNameChanged((e) =>
						this._onDidChangeProperty.fire({ type: ProcessPropertyType.Title, value: e }),
					),
				)
			})
		}
	...
	async start(): Promise<ITerminalLaunchError | { injectedArgs: string[] } | undefined> {
		...
		if (this._options.shellIntegration.enabled) {
			injection = await getShellIntegrationInjection(
	...
	private async setupPtyProcess(
		...
		this._childProcessMonitor.onDidChangeHasChildProcesses((value) =>
			this._onDidChangeProperty.fire({ type: ProcessPropertyType.HasChildProcesses, value }),
		)
	...
	private _sendProcessTitle(ptyProcess: IPty): void {
		...
		if (sanitizedTitle.toLowerCase().startsWith('python')) {
			...
			const shellTypeValue =
				posixShellTypeMap.get(sanitizedTitle) || generalShellTypeMap.get(sanitizedTitle)
	...
	getWindowsPty(): IProcessReadyWindowsPty | undefined {
		return isWindows
			? {
					backend:
						'useConpty' in this._ptyOptions && this._ptyOptions.useConpty ? 'conpty' : 'winpty',
