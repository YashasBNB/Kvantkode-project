...
export interface IThemeMainService {
	...
	saveWindowSplash(
		...
		workspace: IBaseWorkspaceIdentifier | undefined,
		splash: IPartsSplash,
	): void
...
export class ThemeMainService extends Disposable implements IThemeMainService {
	...
	private updateSystemColorTheme(): void {
		if (isLinux || this.configurationService.getValue(ThemeSettings.DETECT_COLOR_SCHEME)) {
			...
			switch (
				this.configurationService.getValue<'default' | 'auto' | 'light' | 'dark'>(
	...
	private updateWindowSplashOverride(
		workspace: IBaseWorkspaceIdentifier | undefined,
		splash: IPartsSplash,
	): IPartsSplashWorkspaceOverride | undefined {
	...
	getWindowSplash(workspace: IBaseWorkspaceIdentifier | undefined): IPartsSplash | undefined {
		...
		if (workspace) {
			const [auxiliarySideBarWidth, workspaceIds] =
				this.getWindowSplashOverride().layoutInfo.auxiliarySideBarWidth
			if (workspaceIds.includes(workspace.id)) {
		...
		return {
			...
			layoutInfo: {
				...
				auxiliarySideBarWidth:
					typeof auxiliarySideBarWidthOverride === 'number' ? auxiliarySideBarWidthOverride : 0,
			},
		}
	}

	private getWindowSplashOverride(): IPartsSplashWorkspaceOverride {
		return this.stateService.getItem<IPartsSplashWorkspaceOverride>(
