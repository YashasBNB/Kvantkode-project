...
export class ExtensionsSynchroniser extends AbstractSynchroniser implements IUserDataSynchroniser {
	...
	*/
	...
	constructor(
		...
		this._register(
			Event.any<any>(
				Event.filter(this.extensionManagementService.onDidInstallExtensions, (e) =>
					e.some(({ local }) => !!local),
				),
	...
	protected async generateSyncPreview(
		...
		const { localExtensions, ignoredExtensions } =
			await this.localExtensionsProvider.getLocalExtensions(this.syncResource.profile)
	...
	private async acceptLocal(
		...
		const ignoredExtensions =
			this.ignoredExtensionsManagementService.getIgnoredExtensions(installedExtensions)
...
export class LocalExtensionsProvider {
		...
		const localExtensions = await this.withProfileScopedServices(
			...
			async (extensionEnablementService, extensionStorageService) => {
				...
				return installedExtensions.map((extension) => {
					const { identifier, isBuiltin, manifest, preRelease, pinned, isApplicationScoped } =
						extension
					...
					try {
						...
						if (keys) {
							...
							syncExntesion.state = Object.keys(extensionStorageState).reduce(
	...
	async updateLocalExtensions(
		...
		const results =
			await this.extensionManagementService.installGalleryExtensions(extensionsToInstall)
...
export abstract class AbstractExtensionsInitializer extends AbstractInitializer {
	...
	protected generatePreview(
		...
		for (const extension of remoteExtensions) {
			if (
				this.ignoredExtensionsManagementService.hasToNeverSyncExtension(extension.identifier.id)
			) {
