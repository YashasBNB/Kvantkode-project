	...
	implements IExtensionsScannerService
{
	...
	private readonly systemExtensionsCachedScanner = this._register(
		this.instantiationService.createInstance(CachedExtensionsScanner, this.currentProfile),
	)
	...
	async scanOneOrMultipleExtensions(
		...
		const extensions =
			await this.extensionsScanner.scanOneOrMultipleExtensions(extensionsScannerInput)
	...
	private async scanDevSystemExtensions(
		...
		const devSystemExtensionsLocation = URI.file(
			path.normalize(
				path.join(FileAccess.asFileUri('').fsPath, '..', '.build', 'builtInExtensions'),
			),
		)
...
class CachedExtensionsScanner extends ExtensionsScanner {
	...
	private getProfile(input: ExtensionScannerInput): IUserDataProfile {
		...
		if (
			this.uriIdentityService.extUri.isEqual(input.location, this.currentProfile.extensionsResource)
		) {
