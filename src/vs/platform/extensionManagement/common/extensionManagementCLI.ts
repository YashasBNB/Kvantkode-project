...
export class ExtensionManagementCLI {
	...
	public async installExtensions(
				...
				installExtensionInfos.push({
					...
					installOptions: {
						...
						installPreReleaseVersion:
							version === 'prerelease' || installOptions.installPreReleaseVersion,
					},
				})
			}
			for (const extension of extensions) {
		...
		if (failed.length) {
			throw new Error(
				localize('installation failed', 'Failed Installing Extensions: {0}', failed.join(', ')),
			)
		}
	}
			...
			installExtensionInfos.map(async ({ id, version, installOptions }) => {
				...
				extensionsToInstall.push({
					...
					options: {
						...
						isApplicationScoped:
							installOptions.isApplicationScoped || installedExtension?.isApplicationScoped,
					},
				})
			}),
		)
		...
		if (valid) {
			try {
				...
				if (isCancellationError(error)) {
					this.logger.info(
						localize('cancelVsixInstall', "Cancelled installing extension '{0}'.", basename(vsix)),
					)
				} else {
					throw error
				}
	...
	public async uninstallExtensions(
		...
		for (const extension of extensions) {
			...
			if (this.location) {
				...
				this.logger.info(
					localize('successUninstall', "Extension '{0}' was successfully uninstalled!", id),
				)
			}
		}
	}
