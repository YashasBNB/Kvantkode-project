	...
	implements IExtensionManagementService
{
	...
	async toggleAppliationScope(
		...
		if (extension.isApplicationScoped) {
			...
			for (const profile of this.userDataProfilesService.profiles) {
				const existing = (
					await this.getInstalled(ExtensionType.User, profile.extensionsResource)
	...
	protected async installExtensions(
		...
		const installingExtensionsMap = new Map<
			string,
			{ task: IInstallExtensionTask; root: IInstallExtensionTask | undefined }
		>()
		...
		try {
			...
			await this.joinAllSettled(
				[...installingExtensionsMap.entries()].map(async ([key, { task }]) => {
			...
			for (const { task } of installingExtensionsMap.values()) {
				if (task.source && !URI.isUri(task.source)) {
					this.installingExtensions.delete(
						getInstallExtensionTaskKey(task.source, task.options.profileLocation),
					)
				}
			}
		}
	...
	private async checkAndGetCompatibleVersion(
		...
		const deprecationInfo =
			extensionsControlManifest.deprecated[extension.identifier.id.toLowerCase()]
		if (deprecationInfo?.extension?.autoMigrate) {
			...
			if (!compatibleExtension) {
				...
				if (
					...
					(
						await this.galleryService.getExtensions([extension.identifier], CancellationToken.None)
					)[0]
				) {
					throw new ExtensionManagementError(
						nls.localize(
	...
	async uninstallExtensions(extensions: UninstallExtensionInfo[]): Promise<void> {
		...
		const createUninstallExtensionTask = (
			...
			this.uninstallingExtensions.set(
				getUninstallExtensionTaskKey(uninstallExtensionTask.extension, uninstallOptions),
		...
		try {
			...
			if (extensionsToRemove.length) {
				await this.joinAllSettled(
					extensionsToRemove.map((extension) => this.removeExtension(extension)),
				)
			}
		} catch (e) {
			const error = toExtensionManagementError(e)
