...
suite('AllowedExtensionsService', () => {
	...
	test('should allow all extensions if no allowed extensions are configured', () => {
		...
		assert.strictEqual(
			testObject.isAllowed({ id: 'test.extension', publisherDisplayName: undefined }) === true,
			true,
		)
	})
	...
	test('should allow extension if publisher is in allowed list', () => {
		...
		assert.strictEqual(
			testObject.isAllowed({ id: 'test.extension', publisherDisplayName: undefined }),
			true,
		)
	})
	...
	test('should allow extension if publisher is not in allowed list and has publisher mapping', () => {
		...
		const testObject = disposables.add(
			new AllowedExtensionsService(aProductService(['hello']), configurationService),
		)
		assert.strictEqual(
			testObject.isAllowed({ id: 'test.extension', publisherDisplayName: 'Hello' }),
			true,
		)
	})
	...
	test('should allow extension if publisher is not in allowed list and has different publisher mapping', () => {
		configurationService.setUserConfiguration(AllowedExtensionsConfigKey, { hello: true })
		const testObject = disposables.add(
			new AllowedExtensionsService(aProductService(['bar']), configurationService),
		)
		assert.strictEqual(
			testObject.isAllowed({ id: 'test.extension', publisherDisplayName: 'Hello' }) === true,
			false,
		)
	})
