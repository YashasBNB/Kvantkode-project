...
suite('ExtensionsProfileScannerService', () => {
	...
	test('add extension trigger events', async () => {
		...
		assert.deepStrictEqual(
			(<ProfileExtensionsEvent>target1.args[0][0]).extensions[0].location.toString(),
		...
		assert.deepStrictEqual(
			(<ProfileExtensionsEvent>target2.args[0][0]).extensions[0].location.toString(),
			extension.location.toString(),
		)
	})
	...
	test('remove extensions trigger events', async () => {
		...
		assert.deepStrictEqual(
			(<ProfileExtensionsEvent>target1.args[0][0]).extensions[1].location.toString(),
		...
		assert.deepStrictEqual(
			(<ProfileExtensionsEvent>target2.args[0][0]).extensions[1].location.toString(),
			extension2.location.toString(),
		)
	})
	...
	test('add same extension with different metadata', async () => {
		...
		const actual = await testObject.scanProfileExtensions(extensionsManifest)
		assert.deepStrictEqual(
			actual.map((a) => ({ ...a, location: a.location.toJSON(), metadata: a.metadata })),
			[
				{
					identifier: extension.identifier,
					location: extension.location.toJSON(),
