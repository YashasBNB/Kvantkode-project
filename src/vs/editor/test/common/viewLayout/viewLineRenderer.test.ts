...
suite('viewLineRenderer.renderLine', () => {
	...
	function assertCharacterReplacement(
		...
		const info = expectedCharOffsetInPart.map<CharacterMappingInfo>((absoluteOffset) => [
			absoluteOffset,
			[0, absoluteOffset],
		])
...
suite('viewLineRenderer.renderLine 2', () => {
	...
	test('createLineParts simple', () => {
		assert.deepStrictEqual(
			testCreateLineParts(false, 'Hello world!', [createPart(12, 1)], 0, 'none', null),
			{
	...
	test('createLineParts render whitespace for trailing with line containing only whitespaces', () => {
		assert.deepStrictEqual(
			testCreateLineParts(false, ' \t ', [createPart(2, 0), createPart(3, 1)], 0, 'trailing', null),
			{
				html: [
					'<span class="mtkz" style="width:40px">\u00b7\u200c\u2192\u00a0\u00a0</span>',
					'<span class="mtkz" style="width:10px">\u00b7\u200c</span>',
	...
	): (partIndex: number, partLength: number, offset: number, expected: number) => void {
