...
class MockParser implements Parser.Parser {
	...
	delete(): void {}
	setLanguage(language: Parser.Language | null) {
		return this
	}
	parse(
		callback: string | Parser.ParseCallback,
...
suite('TreeSitterParserService', function () {
	...
	test('TextModelTreeSitter race condition: first language is slow to load', async function () {
		...
		await timeout(300)
		assert.strictEqual(
			(textModelTreeSitter.parseResult?.language as MockLanguage).languageId,
			'typescript',
		)
	})
})
