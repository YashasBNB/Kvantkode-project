...
export class TextAreaEditContext extends AbstractEditContext {
			...
			getScreenReaderContent: (): TextAreaState => {
				if (this._accessibilitySupport === AccessibilitySupport.Disabled) {
					...
					if (
						...
						!selection.isEmpty() &&
						simpleModel.getValueLengthInRange(selection, EndOfLinePreference.TextDefined) <
							LIMIT_CHARS
					) {
	...
	private _ensureReadOnlyAttribute(): void {
		...
		const useReadOnly =
			!IME.enabled || (options.get(EditorOption.domReadOnly) && options.get(EditorOption.readOnly))
		if (useReadOnly) {
