...
export class PagedScreenReaderStrategy {
	...
	public static fromEditorSelection(
		...
		if (
			trimLongText &&
			model.getValueLengthInRange(pretextRange, EndOfLinePreference.LF) > LIMIT_CHARS
		) {
		...
		if (
			trimLongText &&
			model.getValueLengthInRange(posttextRange, EndOfLinePreference.LF) > LIMIT_CHARS
		) {
