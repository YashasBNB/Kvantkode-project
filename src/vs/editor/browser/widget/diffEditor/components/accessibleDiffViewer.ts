		...
		this._register(
			autorun((reader) => {
				...
				if (currentViewItem && currentViewItem.type !== LineType.Header) {
					const lineNumber =
						currentViewItem.modifiedLineNumber ?? currentViewItem.diff.modified.startLineNumber
...
function computeViewElementGroups(
	...
	for (const g of groupAdjacentBy(
		...
		result.push(
			new ViewElementGroup(new LineRangeMapping(modifiedRange, originalRange), viewElements),
		)
	}
	return result
}
