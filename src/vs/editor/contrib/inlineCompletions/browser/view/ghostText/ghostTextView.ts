...
export class GhostTextView extends Disposable {
	...
	private readonly uiState = derived(this, (reader) => {
		...
		const edit = new OffsetEdit(
			inlineTexts.map((t) => SingleOffsetEdit.insert(t.column - 1, t.text)),
		)
	...
	public readonly isHovered = derived(this, (reader) => {
		...
		return (
			this._isInlineTextHovered.read(reader) || this._additionalLinesWidget.isHovered.read(reader)
		)
	})
...
function computeGhostTextViewData(
	...
	if (hiddenTextStartColumn !== undefined) {
		addToAdditionalLines(
			[{ line: textBufferLine.substring(lastIdx), lineDecorations: [] }],
			undefined,
		)
	}
