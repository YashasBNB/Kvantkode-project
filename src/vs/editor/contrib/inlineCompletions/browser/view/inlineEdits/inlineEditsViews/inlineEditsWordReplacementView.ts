...
export class InlineEditsWordReplacementView extends Disposable implements IInlineEditsView {
	...
	private readonly _start = this._editor.observePosition(
		constObservable(this._edit.range.getStartPosition()),
		this._store,
	)
	private readonly _end = this._editor.observePosition(
	...
	readonly isHovered = this._hoverableElement.map(
	...
	private readonly _renderTextEffect = derived((_reader) => {
		...
		const edit = SingleOffsetEdit.replace(
			new OffsetRange(this._edit.range.startColumn - 1, this._edit.range.endColumn - 1),
			this._edit.text,
		)
								...
								n.svg(
									{
										...
										style: {
											...
											top: layout.map(
												(l) => l.modifiedLine.top + Math.round((l.lineHeight - 14 - 5) / 2),
											),
										},
									},
									[
