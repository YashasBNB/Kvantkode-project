...
export class InlineEditsSideBySideView extends Disposable implements IInlineEditsView {
		...
		this._register(
			autorun((reader) => {
				...
				this.previewEditor.layout({
					...
					width:
						layoutInfo.previewEditorWidth + 15 /* Make sure editor does not scroll horizontally */,
				})
	...
	private readonly _updatePreviewEditor = derived((reader) => {
		...
		const reducedLinesCount =
			range.endLineNumberExclusive - range.startLineNumber - uiState.newTextLineCount
	...
	private readonly _previewEditorLayoutInfo = derived(this, (reader) => {
		...
		const maxContentWidth =
			editorContentMaxWidthInRange + ORIGINAL_END_PADDING + previewContentWidth + 70
		...
		const editHeight =
			this._editor.getOption(EditorOption.lineHeight) * inlineEdit.modifiedLineRange.length
	...
	private readonly _originalBackgroundColor = observableFromEvent(
		this,
		this._themeService.onDidColorThemeChange,
		() => {
			return (
				this._themeService.getColorTheme().getColor(originalBackgroundColor) ?? Color.transparent
			)
		},
	)
