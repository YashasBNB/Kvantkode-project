	...
	private _createHoverContent() {
					...
					typeof options.commandId === 'string'
						...
						: derived((reader) =>
								typeof options.commandId === 'string'
									? undefined
									: this._getKeybinding(
											options.commandArgs ? undefined : options.commandId.read(reader),
										).read(reader),
							),
		...
		const gotoAndAccept = option(
			createOptionArgs({
				...
				icon: this._model.tabAction.map((action) =>
					action === InlineEditTabAction.Accept ? Codicon.check : Codicon.arrowRight,
				),
				commandId: this._model.tabAction.map((action) =>
					action === InlineEditTabAction.Accept ? inlineSuggestCommitId : jumpToNextInlineEditId,
				),
			}),
		)
			...
			actions.length > 0
				? actionBar(
						actions.map((action) => ({
							...
							enabled: true,
							run: () =>
								this._commandService.executeCommand(action.id, ...(action.arguments ?? [])),
							class: undefined,
...
function option(props: {
	...
	return derivedWithStore((_reader, store) =>
		n.div(
			{
