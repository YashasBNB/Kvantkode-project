...
export class InlineEditsGutterIndicator extends Disposable {
	...
	private readonly _layout = derived(this, (reader) => {
		...
		const fullViewPort = Rect.fromLeftTopRightBottom(
			0,
			0,
			layout.width,
			layout.height - bottomPadding,
		)
		...
		const targetRect = Rect.fromRanges(
			OffsetRange.fromTo(
				leftPadding + layout.glyphMarginLeft,
				layout.decorationsLeft + layout.decorationsWidth - rightPadding,
			),
			targetVertRange,
		)
		...
		pillRect = targetRect.containsRect(pillRectMoved)
			? pillRectMoved
			: pillRectMoved.moveToBeContainedIn(
					fullViewPort.intersect(targetRect.union(fullViewPort.withHeight(lineHeight)))!,
				) //viewPortWithStickyScroll.intersect(rect)!;
		...
		if (docked && pillRect.top === targetRect.top + pillOffset) {
			...
			lineNumberRect = pillRect.intersectHorizontal(
				new OffsetRange(
					0,
					Math.max(layout.lineNumbersLeft + layout.lineNumbersWidth - leftPadding - 1, 0),
				),
			)
							...
							n.div(
								{
									...
									style: {
										...
										['--vscodeIconForeground' as any]: this._gutterIndicatorStyles.map(
											(v) => v.foreground,
										),
