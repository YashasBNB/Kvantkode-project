...
export class InlineEditsLineReplacementView extends Disposable implements IInlineEditsView {
	...
	private readonly _maxPrefixTrim = this._edit.map((e) =>
		e
			? getPrefixTrim(
	...
	private readonly _layout = derived(this, (reader) => {
		...
		const originalLineWidths = edit.originalRange.mapToLineArray(
			(line) =>
				this._editor.editor.getOffsetForColumn(line, textModel.getLineMaxColumn(line)) -
				prefixLeftOffset,
		)
	...
	private readonly _viewZoneInfo = derived<{ height: number; lineNumber: number } | undefined>(
		(reader) => {
								...
								n.div(
									{
										...
										style: {
											position: 'absolute',
											...rectToProps((reader) =>
												layout.read(reader).lowerBackground.translateX(-contentLeft),
											),
											borderRadius: '4px',
	...
	constructor(
		...
		this._register(
			toDisposable(() =>
				this._editor.editor.changeViewZones((accessor) => this.removePreviousViewZone(accessor)),
			),
		)
			...
			autorun((reader) => {
				...
				if (edit) {
					originalBubbles.push(
						...rangesToBubbleRanges(edit.replacements.map((r) => r.originalRange)),
					)
				}
