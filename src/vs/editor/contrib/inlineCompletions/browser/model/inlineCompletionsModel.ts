...
export class InlineCompletionsModel extends Disposable {
	...
	private readonly _inlineEditsShowCollapsedEnabled = this._editorObs
		.getOption(EditorOption.inlineSuggest)
	...
	private _lastShownInlineCompletionInfo:
		| {
				...
				/* already freed! */ inlineCompletion: InlineCompletionItem
		  }
		| undefined = undefined
	...
	private readonly _fetchInlineCompletionsPromise = derivedHandleChanges(
		...
		(reader, changeSummary) => {
			...
			const shouldUpdate =
				(this._enabled.read(reader) && this._selectedSuggestItem.read(reader)) ||
			...
			const itemToPreserveCandidate =
				this.selectedInlineCompletion.get() ?? this._inlineCompletionItems.get()?.inlineEdit
			...
			const userJumpedToActiveCompletion = this._jumpedToId.map(
				(jumpedTo) =>
					!!jumpedTo && jumpedTo === this._inlineCompletionItems.get()?.inlineEdit?.semanticId,
			)
	...
	public readonly selectedInlineCompletion = derived<InlineCompletionWithUpdatedRange | undefined>(
		this,
		(reader) => {
			const filteredCompletions = this._filteredInlineCompletionItems.read(reader)
		...
		(reader) => {
			...
			if (suggestItem) {
				...
				const ghostTexts = edits
					.map((edit, idx) =>
						computeGhostText(edit, model, mode, positions[idx], fullEditPreviewLength),
					)
					.filter(isDefined)
	...
	public readonly showCollapsed = derived<boolean>(this, (reader) => {
		...
		const isCurrentModelVersion =
			state.inlineCompletion.updatedEditModelVersion === this._textModelVersionId.read(reader)
		return (
	...
	private async _deltaSelectedInlineCompletionIndex(delta: 1 | -1): Promise<void> {
		...
		if (completions.length > 0) {
			const newIdx =
				(this.selectedInlineCompletionIndex.get() + delta + completions.length) % completions.length
	...
	public jump(): void {
		...
		transaction((tx) => {
			...
			const isSingleLineChange =
				edit.range.startLineNumber === edit.range.endLineNumber && !edit.text.includes('\n')
