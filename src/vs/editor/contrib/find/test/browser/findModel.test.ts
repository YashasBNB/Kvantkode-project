...
suite('FindModel', () => {
	...
	function findTest(testName: string, callback: (editor: IActiveCodeEditor) => void): void {
		test(testName, () => {
			...
			withTestCodeEditor(factory, {}, (editor) => callback(editor as IActiveCodeEditor))
		})
	}
	...
	findTest('find model next stays in scope', (editor) => {
		const findState = disposables.add(new FindReplaceState())
		findState.change(
			{ searchString: 'hello', wholeWord: true, searchScope: [new Range(7, 1, 9, 1)] },
			false,
		)
		const findModel = disposables.add(new FindModelBoundToEditorModel(editor, findState))
	...
	findTest(
		...
		(editor) => {
			const findState = disposables.add(new FindReplaceState())
			findState.change(
				{ searchString: 'hel(lo)(?=\\sworld)', replaceString: 'hi$1', isRegex: true },
				false,
			)
			const findModel = disposables.add(new FindModelBoundToEditorModel(editor, findState))
	...
	findTest(
		...
		(editor) => {
			const findState = disposables.add(new FindReplaceState())
			findState.change(
				{ searchString: 'wo(rl)d(?=.*;$)', replaceString: 'gi$1', isRegex: true },
				false,
			)
			const findModel = disposables.add(new FindModelBoundToEditorModel(editor, findState))
	...
	findTest(
		...
		(editor) => {
			const findState = disposables.add(new FindReplaceState())
			findState.change(
				{ searchString: 'hello(z)?', replaceString: 'hi$1', isRegex: true, matchCase: true },
				false,
			)
			const findModel = disposables.add(new FindModelBoundToEditorModel(editor, findState))
	...
	findTest('issue #27083. search scope works even if it is a single line', (editor) => {
		const findState = disposables.add(new FindReplaceState())
		findState.change(
			{ searchString: 'hello', wholeWord: true, searchScope: [new Range(7, 1, 8, 1)] },
			false,
		)
		const findModel = disposables.add(new FindModelBoundToEditorModel(editor, findState))
