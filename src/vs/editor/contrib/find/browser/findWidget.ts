...
export class FindWidget extends Widget implements IOverlayWidget, IVerticalSashLayoutProvider {
	...
	private _reveal(): void {
		...
		if (!this._isVisible) {
			...
			if (
				this._codeEditor.getOption(EditorOption.find).seedSearchStringFromSelection &&
				selection
			) {
	...
	private _buildDomNode(): void {
		...
		this._findInput = this._register(
			new ContextScopedFindInput(
				...
				this._contextViewProvider,
				{
					...
					history:
						findSearchHistoryConfig === 'workspace' ? this._findWidgetSearchHistory : new Set([]),
				},
		...
		this._resizeSash = this._register(
			new Sash(this._domNode, this, { orientation: Orientation.VERTICAL, size: 2 }),
		)
		this._resized = false
