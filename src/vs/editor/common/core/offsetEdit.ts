...
function joinEdits(edits1: OffsetEdit, edits2: OffsetEdit): OffsetEdit {
	...
	for (const edit2 of edits2.edits) {
		...
		if (!firstIntersecting) {
			...
			let prefix = ''
			const prefixLength =
				edit2.replaceRange.start - (firstIntersecting.replaceRange.start + firstEdit1ToEdit2)
			if (prefixLength > 0) {
				prefix = firstIntersecting.newText.slice(0, prefixLength)
			}
			const suffixLength =
				lastIntersecting!.replaceRange.endExclusive + edit1ToEdit2 - edit2.replaceRange.endExclusive
			if (suffixLength > 0) {
