...
export class TextModelTreeSitter extends Disposable implements ITextModelTreeSitter {
	private _onDidChangeParseResult: Emitter<ModelTreeUpdateEvent> = this._register(
		new Emitter<ModelTreeUpdateEvent>(),
	)
	public readonly onDidChangeParseResult: Event<ModelTreeUpdateEvent> =
	...
	constructor(
		...
		if (parseImmediately) {
			this._register(
				Event.runAndSubscribe(this.textModel.onDidChangeLanguage, (e) =>
					this._onDidChangeLanguage(e ? e.newLanguage : this.textModel.getLanguageId()),
				),
			)
		} else {
			this._register(
	 ...
	 */
	public async parse(
		languageId: string = this.textModel.getLanguageId(),
		...
		this._parseSessionDisposables.add(
			this.textModel.onDidChangeContent((e) => this._onDidChangeContent(treeSitterTree, [e])),
		)
	...
	private async _handleTreeUpdate(
		e: TreeParseUpdateEvent,
		parentTreeResult?: ITreeSitterParseResult,
	...
	private async _collectInjections(
		tree: Parser.Tree,
	): Promise<Map<string, Parser.Range[]> | undefined> {
	...
	private _mergeAdjacentRanges(
		injections: Map<string, Parser.Range[]>,
	): Map<string, Parser.Range[]> {
	...
	private _mergeRanges(current: Parser.Range, next: Parser.Range): Parser.Range {
		return {
			...
			endPosition:
				next.endPosition.row > current.endPosition.row ? next.endPosition : current.endPosition,
		}
	}
	...
	private async _getOrCreateInjectedTree(
		...
		if (!treeSitterTree) {
			...
			this._parseSessionDisposables.add(
				treeSitterTree.onDidUpdate((e) => this._handleTreeUpdate(e, parentTree, parentLanguage)),
			)
	...
	private _onDidChangeContent(
		...
		change: IModelContentChangedEvent[] | undefined,
		ranges?: Parser.Range[],
	) {
...
export class TreeSitterParseResult implements IDisposable, ITreeSitterParseResult {
	...
	private findTreeChanges(
		newTree: Parser.Tree,
		changedNodes: Parser.Range[],
		newRanges: Parser.Range[],
	): RangeChange[] {
		let newRangeIndex = 0
	...
	public onDidChangeContent(
		model: ITextModel,
		changes: IModelContentChangedEvent[] | undefined,
		ranges?: Parser.Range[],
	): void {
	...
	private _applyEdits(changes: IModelContentChange[], version: number) {
		for (const change of changes) {
			...
			const summedTextLengths =
				change.text.length === 0 ? newTextLength : originalTextLength.add(newTextLength)
			const edit = {
		...
		do {
			...
			try {
				newTree = this.parser.parse(
					(index: number, position?: Parser.Point) => this._parseCallback(model, index),
					this._tree,
					{
	...
	private sendParseTimeTelemetry(
		...
		if (parseType === TelemetryParseType.Full) {
			...
			this._telemetryService.publicLog2<
				{ languageId: string; time: number; passes: number },
				ParseTimeClassification
			>(`treeSitter.incrementalParse`, { languageId: this.languageId, time, passes })
		}
	}
}
