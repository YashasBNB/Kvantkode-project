...
export const AcceptAllButtonWrapper = ({
  text,
  onClick,
  className,
  ...props
...
}: {text: string;onClick: () => void;className?: string;} & React.ButtonHTMLAttributes<HTMLButtonElement>) =>
<button
...
export const RejectAllButtonWrapper = ({
  text,
  onClick,
  className,
  ...props
...
export const VoidCommandBar = ({ uri, editor }: VoidCommandBarProps) => {
  ...
  const uriIdxInStepper =
  currFileIdx !== null ?
  currFileIdx // use currFileIdx if it exists, else use latestNotNullUriIdxRef
  : _latestValidUriIdxRef.current === null ?
  null :
  _latestValidUriIdxRef.current < sortedCommandBarURIs.length ?
  _latestValidUriIdxRef.current :
  null;
  ...
  const downKeybindLabel = editCodeService.processRawKeybindingText(
    _downKeybinding?.getLabel() || ''
  );
  const leftKeybindLabel = editCodeService.processRawKeybindingText(
    _leftKeybinding?.getLabel() || ''
  );
  const rightKeybindLabel = editCodeService.processRawKeybindingText(
    _rightKeybinding?.getLabel() || ''
  );
  const acceptFileKeybindLabel = editCodeService.processRawKeybindingText(
    _acceptFileKeybinding?.getAriaLabel() || ''
  );
  const rejectFileKeybindLabel = editCodeService.processRawKeybindingText(
    _rejectFileKeybinding?.getAriaLabel() || ''
  );
  const acceptAllKeybindLabel = editCodeService.processRawKeybindingText(
    _acceptAllKeybinding?.getAriaLabel() || ''
  );
  const rejectAllKeybindLabel = editCodeService.processRawKeybindingText(
    _rejectAllKeybinding?.getAriaLabel() || ''
  );
    ...
    <div className="void-pointer-events-auto">
      ...
      <div className="void-flex void-justify-end void-mb-1">
            ...
            data-tooltip-delay-show={500}>
					...
					<span
            className={`void-text-xs void-whitespace-nowrap void-px-1 ${!isADiffInThisFile ? "void-opacity-70" : ""}`}>
        ...
        <div className="void-flex void-self-stretch void-gap-0 !void-px-0 !void-py-0">
          ...
          onClick={onRejectFile} />
				...
				{showAcceptRejectAll &&
        <div className="!void-px-0 !void-py-0 void-self-stretch void-flex void-justify-center void-items-center">
						<div
            ...
            onClick={() => setShowAcceptRejectAllButtons(!showAcceptRejectAllButtons)}>
							...
							<EllipsisVertical className="void-size-3" />
						</div>
					</div>
        }
			</div>
